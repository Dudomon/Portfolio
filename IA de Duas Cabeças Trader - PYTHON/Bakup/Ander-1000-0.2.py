...
# Caminhos exclusivos para ANDER
ANDER_MODEL_DIR = "Otimizacao/treino_principal/models/ANDER"
ANDER_CHECKPOINT_DIR = "Otimizacao/treino_principal/checkpoints/ANDER"
ANDER_ENVSTATE_DIR = "trading_framework/training/checkpoints/ANDER"

os.makedirs(ANDER_MODEL_DIR, exist_ok=True)
os.makedirs(ANDER_CHECKPOINT_DIR, exist_ok=True)
os.makedirs(ANDER_ENVSTATE_DIR, exist_ok=True)
...
# 🔄 SISTEMA ROBUSTO DE VECNORMALIZE - ANTI-ACHATAMENTO GARANTIDO
import pickle

def create_robust_vecnormalize(env, vec_normalize_file=None):
    """🎯 CRIAR VecNormalize ROBUSTO que não achata observações"""
    print("🔄 CRIANDO VecNormalize ROBUSTO...")
    
    # Configurações conservadoras para evitar achatamento
    vec_env = VecNormalize(
        env, 
        norm_obs=True, 
        norm_reward=True, 
        clip_obs=5.0,  # REDUZIDO: 10.0 -> 5.0 para evitar achatamento extremo
        clip_reward=10.0,  # REDUZIDO: 50.0 -> 10.0 para evitar saturação
        gamma=0.99,
        epsilon=1e-8,  # Precisão numérica adequada
        training=True
    )
    
    # 🔥 CORREÇÃO CRÍTICA: DESABILITAR carregamento para evitar métricas congeladas
    if False:  # Desabilitado: vec_normalize_file and os.path.exists(vec_normalize_file):
        print(f"🔄 Carregando VecNormalize existente: {vec_normalize_file}")
        try:
            # Carregar VecNormalize existente
            vec_env = VecNormalize.load(vec_normalize_file, env)
            vec_env.training = True
            vec_env.norm_reward = True
            print("✅ VecNormalize carregado com sucesso")
        except Exception as e:
            print(f"⚠️ Erro ao carregar VecNormalize: {e}")
            print("🔄 Criando novo VecNormalize...")
    
    # Calibração inteligente com 1000 steps (suficiente para estabilizar)
    print("🔄 Calibrando VecNormalize com 1000 steps...")
    obs = vec_env.reset()
    for i in range(1000):
    ...
    print("✅ VecNormalize robusto criado e calibrado")
    return vec_env

def save_production_ready_vecnormalize(vec_env, filepath):
    """🎯 SALVAR VecNormalize PRONTO PARA PRODUÇÃO"""
    print(f"💾 Salvando VecNormalize pronto para produção: {filepath}")
    
    # Configurar para produção
    vec_env.training = False  # Modo produção
    vec_env.norm_reward = False  # Não normalizar rewards na produção
    ...
    print(f"✅ VecNormalize salvo pronto para produção: {filepath}")

def monitor_vecnormalize_health(vec_env, obs):
    """🔍 MONITORAR SAÚDE DO VecNormalize"""
    # Verificar se observações estão sendo achatadas
    ...
    # Detectar achatamento (muitos valores próximos de zero)
    near_zero = np.sum(np.abs(obs_flat) < 0.01) / len(obs_flat)
    
    # Alertar se >70% das observações estão próximas de zero
    if near_zero > 0.7:
        print(f"⚠️ ALERTA VecNormalize: {near_zero*100:.1f}% das observações próximas de zero!")
...
def save_good_vecnormalize_state(vec_env, filepath="vecnormalize_good_state.pkl"):
    """💾 Salva estado 'bom' do VecNormalize para restaurar depois"""
    try:
        good_state = {
        ...
        print(f"✅ Estado bom do VecNormalize salvo em {filepath}")
        return True
    except Exception as e:
        print(f"❌ Erro ao salvar estado VecNormalize: {e}")
        return False

def restore_good_vecnormalize_state(vec_env, filepath="vecnormalize_good_state.pkl"):
    """🔄 Restaura estado 'bom' do VecNormalize - RECUPERAÇÃO AUTOMÁTICA"""
    try:
        if not os.path.exists(filepath):
            print(f"⚠️ Arquivo {filepath} não encontrado - usando pré-calibração")
        ...
        print(f"✅ Estado bom do VecNormalize restaurado de {filepath}")
        return vec_env
    except Exception as e:
        print(f"❌ Erro ao restaurar estado VecNormalize: {e}")
        print(f"🔄 Fazendo pré-calibração como fallback...")
        return precalibrate_vecnormalize(vec_env, 3000)

def smart_vecnormalize_wrapper(vec_env):
    """🧠 Wrapper inteligente que monitora e corrige VecNormalize automaticamente"""
    ...
    class SmartVecNormalize:
        ...
        def __getattr__(self, name):
            # Delegar todos os outros atributos/métodos para vec_env
        ...
        def step(self, actions):
            ...
            if self.health_check_counter % 100 == 0:
                if not monitor_vecnormalize_health(self.vec_env, obs):
                    print("🔄 AUTO-CORREÇÃO: Restaurando estado bom do VecNormalize...")
                    restore_good_vecnormalize_state(self.vec_env)
                elif not self.good_state_saved:
                    # Salvar estado bom na primeira vez que detectamos saúde boa
        ...
        if self.pbar is not None:
            ...
            if self.num_timesteps % 1000 == 0:
                ...
                try:
                    ...
                    postfix_info = {
                        'Portfolio': f"${1000 + self.num_timesteps * 0.01:.0f}",  # Valor dinâmico baseado em steps
...
class MetricsCallback(BaseCallback):
    ...
    def __init__(self, env, log_freq=2000, verbose=0):
        ...
        self.global_metrics = {
            ...
            'peak_portfolio': 1000.0,       # Pico de portfolio global
    ...
    def _on_step(self) -> bool:
        ...
        if self.num_timesteps - self.last_step >= self.log_freq:
            try:
                ...
                if env is None and hasattr(self, 'training_env'):
                    try:
                        ...
                        if portfolio_values and len(portfolio_values) > 0:
                            ...
                            # 🔥 CORREÇÃO: NÃO ACUMULAR AQUI - JÁ É FEITO EM _update_global_metrics()
                            # Métricas já são acumuladas na função _update_global_metrics()
                            self.global_metrics['peak_portfolio'] = max(self.global_metrics.get('peak_portfolio', 1000), portfolio)  # Atualizar pico portfolio
                            self.global_metrics['peak_drawdown'] = max(self.global_metrics.get('peak_drawdown', 0), episode_drawdown)  # Só o drawdown usa max
                            ...
                            drawdown = episode_drawdown  # Drawdown atual do episódio (já em %)
                            peak_drawdown = self.global_metrics['peak_drawdown']  # Pico DD global (já em %)
                ...
                # 🔥 USAR MÉTRICAS GLOBAIS PERSISTENTES (JÁ EM PERCENTUAL)
                drawdown = episode_drawdown  # Drawdown atual do episódio (já em %)
                peak_drawdown = self.global_metrics['peak_drawdown']  # Pico DD global (já em %)
    ...
    def _on_training_end(self) -> None:
        ...
        if self.global_metrics['total_trades'] > 0:
            ...
            final_return_pct = ((self.global_metrics['peak_portfolio'] - 1000) / 1000) * 100
    ...
    def _update_global_metrics(self, env):
        ...
        try:
            ...
            # 🔥 CORREÇÃO CRÍTICA: Acumular métricas globais entre episódios
            if current_trades_count > 0:
                ...
                # 🔥 CORREÇÃO: Acumular métricas globais (não substituir)
                self.global_metrics['total_trades'] += episode_total_trades  # ACUMULAR
                self.global_metrics['total_pnl'] += episode_pnl  # ACUMULAR
                self.global_metrics['profitable_trades'] += episode_profitable_trades  # ACUMULAR
...
class TradingEnv(gym.Env):
    ...
    def __init__(self, df, window_size=20, is_training=True, initial_balance=1000, trading_params=None):
        ...
        # 🔥 FEATURES OTIMIZADAS ALINHADAS COM PPOV1.PY
        ...
        # 🚀 POSITION SIZING OTIMIZADO PARA BANCA $1000
        self.base_lot_size = 0.2  # Tamanho base otimizado para $1000
        self.max_lot_size = 0.3   # Tamanho máximo otimizado para $1000
    ...
    def _calculate_missing_features(self, missing_features):
        ...
        # Calcular apenas features ausentes para 5m e 15m (não mais 4h)
        for tf in ['5m', '15m']:
        ...
        # 🎯 FEATURES DE ALTA QUALIDADE (substituem 4h inúteis)
        close_5m = self.df.get('close_5m', self.df.get('close', 0))
        high_5m = self.df.get('high_5m', close_5m)
        low_5m = self.df.get('low_5m', close_5m)
        volume_5m = self.df.get('volume_5m', self.df.get('volume', 1))
        
        if 'volume_momentum' in missing_features:
            # Momentum de volume (taxa de mudança do volume)
            volume_momentum = volume_5m.pct_change(periods=5).fillna(0)
            self.df.loc[:, 'volume_momentum'] = volume_momentum.clip(-5, 5)
            
        if 'price_position' in missing_features:
            # Posição do preço no range intraday (0-1)
            intraday_range = high_5m - low_5m
            price_position = ((close_5m - low_5m) / intraday_range.replace(0, 1)).fillna(0.5)
            self.df.loc[:, 'price_position'] = price_position.clip(0, 1)
            
        if 'volatility_ratio' in missing_features:
            # Razão entre volatilidade atual e média histórica
            current_vol = (high_5m - low_5m) / close_5m.replace(0, 1)
            avg_vol = current_vol.rolling(window=20).mean()
            volatility_ratio = (current_vol / avg_vol.replace(0, 1)).fillna(1.0)
            self.df.loc[:, 'volatility_ratio'] = volatility_ratio.clip(0.1, 5.0)
            
        if 'intraday_range' in missing_features:
            # Range intraday normalizado pelo preço
            intraday_range = ((high_5m - low_5m) / close_5m.replace(0, 1)).fillna(0)
            self.df.loc[:, 'intraday_range'] = intraday_range.clip(0, 0.1)
            
        if 'market_regime' in missing_features:
            # Regime de mercado (-1: baixa, 0: lateral, 1: alta)
            sma_20 = close_5m.rolling(window=20).mean()
            sma_50 = close_5m.rolling(window=50).mean()
            regime = np.where(sma_20 > sma_50 * 1.001, 1, 
                            np.where(sma_20 < sma_50 * 0.999, -1, 0))
            self.df.loc[:, 'market_regime'] = regime
            
        if 'spread_pressure' in missing_features:
            # Pressão de spread (baseada em volatilidade intraday vs preço)
            intraday_range = high_5m - low_5m
        ...
        if 'session_momentum' in missing_features:
            # Momentum da sessão (diferença entre abertura e fechamento)
            open_5m = self.df.get('open_5m', close_5m)
            session_momentum = ((close_5m - open_5m) / open_5m.replace(0, 1)).fillna(0)
            self.df.loc[:, 'session_momentum'] = session_momentum.clip(-0.1, 0.1)
            
        if 'time_of_day' in missing_features:
            # Hora do dia normalizada (0-1)
            if hasattr(self.df.index, 'hour'):
                time_of_day = self.df.index.hour / 24.0
            else:
                time_of_day = 0.5  # Valor padrão se não há informação de hora
            self.df.loc[:, 'time_of_day'] = time_of_day
            
        if 'tick_momentum' in missing_features:
            # Momentum de tick (mudança de preço por período)
            tick_momentum = close_5m.diff().fillna(0)
            tick_momentum_norm = tick_momentum / close_5m.replace(0, 1)
            self.df.loc[:, 'tick_momentum'] = tick_momentum_norm.clip(-0.01, 0.01)
        
        print(f"[FALLBACK] ✅ Features ausentes calculadas (5m+15m + alta qualidade)")
    ...
    def _calculate_reward_and_info(self, action, old_state):
        ...
        # Garantir que action é um array
        ...
        if len(action) >= 4:
            ...
            if entry_decision > 0 and len(self.positions) < self.max_positions:
                ...
                if len(action) >= 6:
                    ...
                    sl_points = abs(sl_adjust) * 100  # Max 300 pontos (3.0 × 100 = 300) - Liberdade total
                    tp_points = abs(tp_adjust) * 150  # Max 450 pontos (3.0 × 150 = 450) - Liberdade total
                    
                    # 🔥 CORREÇÃO CRÍTICA: Converter pontos para diferença de preço (multiplicar por 0.01)
                    # Para OURO: 1 ponto = 0.01 diferença de preço (ex: 3304.00 -> 3304.01 = 1 ponto)
                    sl_price_diff = sl_points * 1.0  # Converter pontos para preços (300 pontos = $300.00 diferença)
                    tp_price_diff = tp_points * 1.0  # Converter pontos para preços (450 pontos = $450.00 diferença)
...
class AdvancedTrainingSystem:
    def __init__(self, base_dir: str = ANDER_MODEL_DIR):
    ...
    def train(self):
        ...
        try:
            ...
            checkpoint_path = ANDER_MODEL_DIR
            ...
            if checkpoint_path_found and os.path.exists(checkpoint_path_found):
                ...
                env_state_pattern = f"{ANDER_ENVSTATE_DIR}/ANDER_env_state_{resume_steps}_steps_*.json"
                ...
                if env_state_files:
                    ...
                    try:
                        ...
                        if hasattr(env, 'env_method'):
                            ...
                            base_env.portfolio_value = env_state.get('portfolio_value', 1000)
                            base_env.realized_balance = env_state.get('realized_balance', 1000)
                            base_env.peak_portfolio = env_state.get('peak_portfolio', 1000)
                            ...
                            setattr(env, 'portfolio_value', env_state.get('portfolio_value', 1000))
                            setattr(env, 'realized_balance', env_state.get('realized_balance', 1000))
                            setattr(env, 'peak_portfolio', env_state.get('peak_portfolio', 1000))
            ...
            class RobustSaveCallback(BaseCallback):
                def __init__(self, save_freq=50000, save_path=ANDER_MODEL_DIR, name_prefix="ANDER", total_steps_offset=0, training_env=None):
                    super().__init__()
                ...
                def _on_step(self) -> bool:
                    ...
                    if real_timesteps % self.save_freq == 0:
                        try:
                            ...
                            framework_dir = ANDER_ENVSTATE_DIR
                            ...
                            env_state_path = f"{framework_dir}/ANDER_env_state_{real_timesteps}_steps_{timestamp}.json"
                            ...
                            try:
                                ...
                                for vec_path in vec_normalize_paths:
                                    try:
                                        ...
                                        # Usar função robusta para salvar VecNormalize pronto para produção
                                        save_production_ready_vecnormalize(self.training_env, vec_path)
                                        print(f"✅ VecNormalize pronto para produção salvo: {vec_path}")
                            ...
                            try:
                                emergency_path = f"{ANDER_ENVSTATE_DIR}/EMERGENCY_SAVE_{real_timesteps}.zip"
            ...
            for phase_idx in range(self.current_phase_idx, total_phases):
                ...
                robust_callback = RobustSaveCallback(
                    save_freq=50000,
                    save_path=checkpoint_path,
                                                name_prefix=f"ANDER_{phase_name}",
                    total_steps_offset=self.total_steps_completed  # 🔥 PASSAR OFFSET CORRETO
                )
                    ...
                    try:
                        ...
                        for final_vec_path in final_vec_paths:
                            try:
                                ...
                                # Usar função robusta para salvar VecNormalize pronto para produção
                                save_production_ready_vecnormalize(env, final_vec_path)
                                print(f"✅ VecNormalize FINAL pronto para produção salvo: {final_vec_path}")
    ...
    def _create_phase_environment(self, df: pd.DataFrame, phase: TrainingPhase):
        ...
        if USE_VECNORM:
            # 🔥 CORREÇÃO CRÍTICA: DESABILITAR carregamento de VecNormalize para evitar métricas congeladas
            # vec_normalize_file = f'{ANDER_MODEL_DIR}/vec_normalize.pkl'
            self.logger.info("🔄 Aplicando VecNormalize NOVO (sem carregar estados antigos)...")
            env = create_robust_vecnormalize(env, None)  # None = não carregar arquivo antigo
            self.logger.info("✅ VecNormalize ROBUSTO ativado!")
            
            # 🔥 CONFIRMAÇÃO VECNORMALIZE
            self.logger.info("=" * 60)
            self.logger.info("📊 VECNORMALIZE INTELIGENTE + ANDER ATIVADO:")
            self.logger.info("=" * 60)
            ...
            self.logger.info(f"🧠 Sistema Inteligente: AUTO-CALIBRAÇÃO + MONITORAMENTO")
            self.logger.info(f"🎯 Política: AnderV1 (POLÍTICA OTIMIZADA)")
            self.logger.info("=" * 60)
        else:
        ...
        try:
            ...
            if USE_VECNORM and hasattr(self, '_current_env') and hasattr(self._current_env, 'save'):
                try:
                    ...
                    vec_normalize_path = f'{ANDER_MODEL_DIR}/vec_normalize.pkl'
                    save_production_ready_vecnormalize(self._current_env, vec_normalize_path)
                    self.logger.info(f"✅ VecNormalize compartilhado pronto para produção salvo: {vec_normalize_path}")
                    
                    # 🚀 SALVAR VECNORMALIZE_FINAL.PKL PARA LEGION V1
                    final_vec_path = f'{ANDER_MODEL_DIR}/vecnormalize_final.pkl'
                    save_production_ready_vecnormalize(self._current_env, final_vec_path)
                    self.logger.info(f"✅ VecNormalize FINAL pronto para produção salvo: {final_vec_path}")
                    ...
                    save_production_ready_vecnormalize(self._current_env, legion_vec_path)
                    self.logger.info(f"✅ VecNormalize Legion V1 pronto para produção salvo: {legion_vec_path}")
