#!/usr/bin/env python3
"""
🚀 AVALIAÇÃO 1MIN - DATASET V4 REALISTA
Configuração EXATA do daytrader para avaliação 1min V4:
- Portfolio: $500 
- Base lot: 0.02
- Max lot: 0.03
- Modo inferência (deterministic=False + pesos congelados)
- Action Space: 4D (entry[0] + confidence[1] + pos1_mgmt[2] + pos2_mgmt[3])
- Architecture: V11 Sigmoid - SILU Activation + Simplicidade Focada
- Dataset: V4 Realista 1min (interpolado de dados reais 5min)
- Episodes: 7200 steps (5 dias de trading)
"""
...
DEFAULT_CHECKPOINT_PATH = "D:/Projeto/Otimizacao/treino_principal/models/SILUS/FINAL_ABSOLUTE_5000000_steps_20250825_230347.zip"
INITIAL_PORTFOLIO = 500.0  # $500 conforme solicitado
BASE_LOT_SIZE = 0.02
MAX_LOT_SIZE = 0.03
TEST_STEPS = 7200  # 5 dias de trading (1440 min/dia × 5 dias)
NUM_EPISODES = 3  # Número de episódios para testar
EPISODE_SPACING = 5000  # Espaçamento entre episódios no dataset

def find_v11_checkpoint():
    """🔍 Encontra o checkpoint mais recente do daytrader V11 Sigmoid"""
    import glob
    
    print("🔍 Procurando checkpoints V11 Sigmoid...")
...
def test_v8_elegance_trading():
    """🚀 Teste V8Elegance com configurações exatas do daytrader"""
    
    print(f"🚀 TESTE V8ELEGANCE - MODELOS DAYTRADER")
    print("=" * 60)
    ...
    try:
        ...
        if checkpoint_path is None or not os.path.exists(checkpoint_path):
            ...
            checkpoint_path = find_v11_checkpoint()
        ...
        total_len = len(df)
        print(f"✅ Dataset V4 (1min) carregado: {total_len:,} barras totais")
        ...
        for episode_num in range(NUM_EPISODES):
            ...
            # Variáveis de tracking do episódio (apenas o mínimo necessário)
            ...
            while not done and step < TEST_STEPS:
                ...
                if (step + 1) % 1000 == 0:  # Log básico de progresso
                    print(f"  Step {step+1}/{TEST_STEPS} - Portfolio: ${env.portfolio_value:.2f}")
                    print(f"    Positions: {len(getattr(env, 'positions', []))} | Trades: {len(getattr(env, 'trades', []))}")
                
                step += 1
            
            # Análise do episódio - usar dados do TradingEnv
            ...
            print(f"   Trades executados: {trades_count}")
            
            # Win rate usando dados do TradingEnv
            env_trades = getattr(env, 'trades', [])
            if env_trades:
                profitable_trades = [t for t in env_trades if t.get('pnl_usd', 0) > 0]
                win_rate = (len(profitable_trades) / len(env_trades)) * 100
