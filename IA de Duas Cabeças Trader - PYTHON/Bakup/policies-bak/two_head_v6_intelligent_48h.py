...
class TwoHeadV6Intelligent48h(RecurrentActorCriticPolicy):
    """
    ...
    def _init_components(self):
        ...
        self.final_projector = nn.Sequential(
            nn.Linear(64 + 32, 128),  # entry + management
            nn.ReLU(),
            nn.LayerNorm(128)
        )
    ...
    def _init_weights(self):
        """🔧 CORREÇÃO: Inicialização adequada para evitar zeros absolutos"""
        for module in self.modules():
            if isinstance(module, nn.Linear):
                nn.init.xavier_uniform_(module.weight, gain=0.5)
                if module.bias is not None:
                    # 🎯 CORREÇÃO: Bias pequeno mas não zero
                    nn.init.uniform_(module.bias, -0.01, 0.01)
            elif isinstance(module, nn.LayerNorm):
                # 🎯 CORREÇÃO: LayerNorm bias pequeno mas não zero para melhor aprendizado
                nn.init.uniform_(module.bias, -0.001, 0.001)
                # 🎯 CORREÇÃO CRÍTICA: LayerNorm weight deve ter pequena variação, não constante!
                nn.init.normal_(module.weight, mean=1.0, std=0.02)
            elif isinstance(module, nn.MultiheadAttention):
                # 🎯 CORREÇÃO CRÍTICA: MultiheadAttention bias não pode ser zero!
                ...
                for name, param in module.named_parameters():
                    if 'weight' in name:
                        ...
                        # 🎯 CORREÇÃO CRÍTICA: Bias GRU/LSTM não pode ser zero!
                        # Forget gate bias deve ser positivo para estabilidade
                        if 'bias_hh' in name or 'bias_ih' in name:
                            param.data.uniform_(-0.1, 0.1)
                            # Para GRU: forget gate bias ligeiramente positivo
                            if isinstance(module, nn.GRU):
                                hidden_size = param.data.size(0) // 3
                                param.data[hidden_size:2*hidden_size].uniform_(0.5, 1.0)  # Reset gate
                        else:
    ...
    def _apply_v6_processing(self, input_features):
        ...
        # 6. Entry Head V6 (LIMPA E FUNCIONAL)
...
def get_v6_kwargs() -> Dict[str, Any]:
    """Retorna kwargs para TwoHeadV6Intelligent48h"""
    return {
