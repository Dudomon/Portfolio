...
# 🚀 NUMBA JIT PARA OTIMIZAÇÃO DE PERFORMANCE
try:
    from numba import jit, float32
    NUMBA_AVAILABLE = True
    print("✅ Numba JIT disponível - Otimizações ativadas")
except ImportError:
    NUMBA_AVAILABLE = False
    print("⚠️ Numba não encontrado - Rodando sem otimizações JIT")
...
# 🚀 FUNÇÕES JIT COMPILADAS PARA OTIMIZAÇÃO MÁXIMA (10-100x mais rápidas)

@jit(nopython=True, cache=True)
def _calculate_position_size_jit(
    initial_balance: float, 
    current_portfolio: float, 
    base_lot: float, 
    max_lot: float, 
    growth_factor_cap: float
) -> float:
    """🚀 Versão JIT-compiled da função de position sizing - 10-100x mais rápida"""
    
    # Se o portfólio não cresceu, usa o lote base
    if current_portfolio <= initial_balance:
        return base_lot
    
    # Calcular o fator de crescimento
    growth_factor = current_portfolio / initial_balance
    ...
    # Arredondar para 2 casas decimais (usando multiplicação/divisão para evitar round() em nopython)
    return int(final_lot * 100.0) / 100.0

@jit(nopython=True, cache=True)
def _calculate_pnl_jit(entry_price: float, current_price: float, lot_size: float, trade_type: int) -> float:
    """🚀 Cálculo JIT-compiled de PnL - usado frequentemente durante trading"""
    price_diff = current_price - entry_price if trade_type == 1 else entry_price - current_price
    return price_diff * lot_size

@jit(nopython=True, cache=True) 
def _normalize_value_jit(value: float, min_val: float, max_val: float) -> float:
    """🚀 Normalização JIT-compiled - usada nas observações"""
    if max_val == min_val:
        return 0.0
    return (value - min_val) / (max_val - min_val)

# ====================================================================
# 🎯 SISTEMA DE TAGS UNIFICADO - CONFIGURAÇÃO ÚNICA
# ====================================================================
...
# 🚀 LOG DE STATUS DAS OTIMIZAÇÕES
if NUMBA_AVAILABLE:
    print("🚀 ANDERDAY OTIMIZADO: Numba JIT ativo para position sizing (speedup 10-100x)")
    print("   ⚡ Funções otimizadas: position_size, pnl_calculation, normalization")
    
    # 🔥 WARM-UP JIT (primeira compilação para cache)
    try:
        _ = _calculate_position_size_jit(1000.0, 1200.0, 0.2, 0.3, 1.6)
        _ = _calculate_pnl_jit(2000.0, 2005.0, 0.2, 1)
        _ = _normalize_value_jit(0.5, 0.0, 1.0)
        print("   ✅ JIT warm-up concluído - funções prontas para velocidade máxima")
    except Exception as e:
        print(f"   ⚠️ JIT warm-up falhou: {e}")
else:
    print("⚠️ ANDERDAY PADRÃO: Rodando sem otimizações JIT (instale: pip install numba)")
...
def load_optimized_data(phase_name=None):
    ...
    """
    # 🚀 CURRICULUM LEARNING CORRETO
    if phase_name == "Phase_0_Bootstrap_1m":
        print("🎓 [CURRICULUM] Carregando dataset 1m para fase bootstrap...")
        return load_1m_dataset()
    else:
        print("🎓 [CURRICULUM] Carregando dataset massivo 15 anos para treino principal...")
                    ...
                    try:
                        ...
                        if portfolio_values and len(portfolio_values) > 0:
                            ...
                            try:
                                ...
                                portfolio = getattr(env, 'portfolio_value', 1000.0)
...
class TradingEnv(gym.Env):
    ...
    def __init__(self, df, window_size=20, is_training=True, initial_balance=1000, trading_params=None):
        ...
        self.max_lot_size = 0.3   # Corrigido para 0.3
        ...
        #  POSITION SIZING PARA BANCA $1000 ANDER
        self.base_lot_size = 0.2   # Tamanho base para $1000
        self.max_lot_size = 0.3    # Tamanho máximo para $1000
    ...
    def _prepare_data(self):
        ...
        if hasattr(self.df, 'processed_data_cache'):
            ...
            # CORREÇÃO DIRETA BRUTAL - substituir todos os NaN
    ...
    def _calculate_adaptive_position_size(self, action_confidence=1.0):
        """
        🚀 POSITION SIZING DINÂMICO V2 OTIMIZADO: JIT-compiled para 10-100x speedup
        """
        try:
            # Parâmetros para a função JIT
            ...
            # 🚀 CHAMADA PARA FUNÇÃO JIT-COMPILED (10-100x mais rápida)
            if NUMBA_AVAILABLE:
                return _calculate_position_size_jit(
                ...
                # Fallback Python puro se Numba não disponível
...
class AdvancedTrainingSystem:
    ...
    def _create_training_phases(self) -> List[TrainingPhase]:
        """🎓 CURRICULUM LEARNING CORRETO: 100k steps dataset 1m → dataset massivo 15 anos"""
        return [
            TrainingPhase(
                name="Phase_0_Bootstrap_1m",
                phase_type=PhaseType.FUNDAMENTALS,
                timesteps=300000,  # 🚀 CURRICULUM: Aprender básicos em dataset 1m (300k steps para 5.6M barras)
                description="Aprender padrões básicos em dataset 1m de alta qualidade",
                data_filter="1m_bootstrap",
                success_criteria={
                    "win_rate": 0.45,  # Realista para dataset 1m
                    "trades_per_hour": 2  # Controlado
                },
                reset_criteria={
                    "win_rate": 0.20,  # Reset se muito ruim
                    "max_drawdown": 0.40
                }
            ),
            TrainingPhase(
    ...
    def _determine_phase_from_steps(self, steps):
        ...
        phase_thresholds = [
            300000,   # Phase_0_Bootstrap_1m: 0 - 300k (dataset 1m)
            816000,   # Phase_1_Fundamentals: 300k - 816k (dataset massivo)
            1461000,  # Phase_2_Risk_Management: 816k - 1.461M (dataset massivo)
