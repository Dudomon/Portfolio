...
from adaptive_lr_callback import create_adaptive_lr_callback
...
BEST_PARAMS = {
    ...
    "policy_kwargs": {
        ...
        "ortho_init": False,  # 🔥 CORREÇÃO CRÍTICA: Desabilitar ortho_init para preservar nossa inicialização MultiheadAttention
...
class TradingEnv(gym.Env):
    ...
    def _get_clean_observation_v6(self):
        ...
        # 🔍 DEBUG: Removido - apenas callback periódico para evitar spam
    ...
    def train(self):
        ...
        try:
            ...
            for phase_idx in range(self.current_phase_idx, total_phases):
                ...
                # 🔍 CRIAR ZERO EXTREME DEBUG CALLBACK
                zero_debug_callback = create_zero_debug_callback(
                    zero_debugger=zero_debugger,
                    debug_freq=5000,         # Debug a cada 5000 steps (reduzido spam)
                    verbose=1
                )
                
                # 🚀 SISTEMA DE MONITORAMENTO ULTRA-LEVE DESABILITADO
                # O monitor está integrado diretamente na policy para máxima velocidade
                # Mantém 150it/s sem impacto na performance
                print("✅ Gradient Health Monitor ativado (check a cada 500 steps)")
                print("✅ Gradient Health Monitoring ativado")
                print("   Check frequency: 500 steps")
                print("   Auto-fix: True")
                print("   Alert threshold: 0.3")
                
                # 🚀 CRIAR ADAPTIVE LEARNING RATE CALLBACK
                ...
                combined_callback = CallbackList([
                    # action_value_fixer REMOVIDO - problema resolvido na origem
                    # runtime_bias_fixer REMOVIDO - attention bias sob controle (0.0% zeros)
                    robust_callback, 
                    metrics_callback, 
                    progress_callback, 
                    gradient_callback,
                    zero_debug_callback,
                    # regularization_callback,  # 🚀 DESABILITADO: monitor pesado removido
