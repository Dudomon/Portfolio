...
def calculate_v6_observation_space():
    ...
    print("=" * 60)
    print(f"CALCULANDO OBSERVATION SPACE DAYTRADER V11 SIGMOID ({EXPERIMENT_TAG})")
    print("=" * 60)
...
class TradingEnv(gym.Env):
    ...
    def _generate_basic_v7_components(self, step):
        """
        🚀 FALLBACK: V7 components básicos calculados rapidamente
        """
        try:
            ...
            # Momentum simples
            if step >= 10:
                momentum = (close_price - self.df['close_1m'].iloc[step-10]) / self.df['close_1m'].iloc[step-10] if 'close_1m' in self.df.columns else 0.01
            else:
                momentum = 0.01

            # Features básicas reais (não estáticas!)
            basic_features = np.array([
                # Market regime (3 features)
                volatility,  # volatility_regime
                momentum,    # trend_strength
                np.clip(volatility + momentum + 0.2, 0.1, 0.9),  # regime_confidence dinâmica

                # Volatility context (3 features)
                volatility * 0.8,  # vol_percentile
                volatility * 1.2,  # vol_expanding
                volatility,        # vol_context

                # Momentum confluence (3 features)
                momentum,          # momentum_1
                momentum * 0.9,    # momentum_2
                momentum * 1.1,    # momentum_3

                # Risk assessment (3 features)
                min(volatility * 2, 1.0),  # risk_score
                np.clip(momentum * 5 + 0.3, 0.1, 0.9),  # risk_level dinâmico
                volatility * 0.5,          # risk_adjusted

                # V7 embeddings expandidas (25 features)
                *[volatility + i*0.01 for i in range(25)]  # Features dinâmicas baseadas em dados reais
            ], dtype=np.float32)

            # Garantir 37 features exatas
            if len(basic_features) < 37:
                padding = np.full(37 - len(basic_features), 0.1, dtype=np.float32)
                basic_features = np.concatenate([basic_features, padding])
            elif len(basic_features) > 37:
                basic_features = basic_features[:37]

            return basic_features

        except Exception as e:
            # Last resort: valores seguros mas não estáticos
            return np.random.uniform(0.05, 0.15, 37).astype(np.float32)
    ...
    def _process_dynamic_trailing_stop(self, pos, sl_adjust, tp_adjust, current_price, pos_index):
        ...
        if abs(tp_adjust) >= 0.5:  # Modelo quer modificar TP
            ...
            if current_tp > 0:
                # 🎯 UNIFICADO: Usar função unificada para ajustes
                adjustment_result = convert_model_adjustments_to_points(sl_adjust, tp_adjust, "adjustment")
                tp_change_points = adjustment_result['tp_change']  # Agora usa ±0.5 direto
                ...
                if pos['type'] == 'long':
                    ...
                    if new_tp > current_price + 2.0:  # Mínimo 2 pontos acima do preço
                        ...
                        pos['tp'] = new_tp  # Atualizar posição diretamente
            ...
            for i, pos in enumerate(self.positions):
                if i < 2:  # 🔬 CIRÚRGICO: Máximo 2 posições
                    ...
                    if trailing_result['action_taken']:
                        ...
                        if trailing_result['tp_adjusted']:
