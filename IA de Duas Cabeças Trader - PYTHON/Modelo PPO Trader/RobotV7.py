...
class TradingRobotV7(gym.Env):
        ...
        try:
            ...
            if rates is None or len(rates) == 0:
                ...
                for col in self.feature_columns:
                    self.historical_df[col] = [0.0] * 50  # 🚀 OTIMIZAÇÃO MEMÓRIA: Reduzir de 100 para 50 linhas
                return
    ...
    def _process_legion_action(self, action):
        ...
        try:
            ...
            if cooldown_check[0]:  # Em cooldown
                ...
                # ACTION_THRESHOLD_LONG = 0.33, ACTION_THRESHOLD_SHORT = 0.67
            ...
            if entry_type == "HOLD":
                ...
                # Obter preço atual
                tick = mt5.symbol_info_tick(self.symbol) if self.mt5_connected else None
                ...
                # Para primeira posição, usar pos1_mgmt (alinhado com silus.py linha 5952-5954)
                sl_adjust = pos1_sl_adjust
                tp_adjust = pos1_tp_adjust

                # Converter ajustes para pontos realistas usando mesma função do silus.py
                realistic_sltp = self._convert_action_to_realistic_sltp([sl_adjust, tp_adjust], current_price)
                sl_points = abs(realistic_sltp[0])  # [2.0-8.0] pontos
                tp_points = abs(realistic_sltp[1])  # [3.0-15.0] pontos
                
                # 🚀 UNIFICADO: Redirecionar para sistema V7 unificado
                if self.mt5_connected:
                    # Usar sistema V7 unificado com SL/TP corretos (1.0x)
                    result = self._execute_v7_unified_trade(entry_type, entry_confidence, sl_points, tp_points)
                    return result
                else:
    ...
    def _execute_v7_unified_trade(self, trade_type, confidence, sl_points, tp_points):
        """🚀 UNIFICADO: Execução de trade com SL/TP corretos (1.0x)"""
        try:
            # Obter preço atual
            tick = mt5.symbol_info_tick(self.symbol)
            if not tick:
            ...
            # 🚀 FIX: SL/TP com multiplicador 1.0 (CORRETO)
            if trade_type == "LONG":
                sl_price = current_price - (sl_points * 1.0)  # Multiplicador correto
                tp_price = current_price + (tp_points * 1.0)  # Multiplicador correto
            ...
            request = {
                ...
                "comment": f"V7 Unified - Conf: {confidence:.2f}",
    ...
    def _process_v7_action_legacy(self, action):
        """🚀 OBSOLETO: Redirecionar para sistema V7 unificado"""
            ...
            for i, pos in enumerate(positions[:3]):  # Máximo 3 posições
                if i < len(sl_adjusts) and i < len(tp_adjusts):
                    ...
                    if significant_sl:
                        ...
                        self.sl_tp_adjustments['significant_sl_adjustments'] += 1

                    if significant_tp:
                        self.sl_tp_adjustments['tp_adjustments'] += 1
                        self.sl_tp_adjustments['significant_tp_adjustments'] += 1
                    ...
                    if significant_sl or significant_tp:
                        ...
                        self._log(f"   ⏭️  Ajustes não significativos - mantendo SL/TP atuais")

            # Log resumo dos contadores
            stats = self.sl_tp_adjustments
            self._log(f"📈 [STATS SL/TP] Total: {stats['total_adjustments']}, "
    ...
    def _modify_position_v7(self, position, sl_adjust, tp_adjust):
        ...
        try:
            ...
            self._log(f"🔧 [CALC] Pos #{position.ticket} ({pos_type}): Ajustes SL={sl_adjust:.2f}→{sl_points:.1f}pts, TP={tp_adjust:.2f}→{tp_points:.1f}pts")
            ...
            if abs(sl_adjust) > 0.5:  # Aplicar SL apenas se ajuste significativo
                if position.type == mt5.POSITION_TYPE_BUY:
                    ...
                    new_sl = current_price + sl_points
                sl_changed = True
                self._log(f"🛡️  [SL NOVO] ${position.sl:.2f} → ${new_sl:.2f} (Δ{sl_points:.1f}pts)")
            else:
                new_sl = position.sl  # Manter SL atual
                self._log(f"🛡️  [SL MANTIDO] ${position.sl:.2f} (ajuste não significativo)")
            ...
            if abs(tp_adjust) > 0.5:  # Aplicar TP apenas se ajuste significativo
                if position.type == mt5.POSITION_TYPE_BUY:
                    ...
                    new_tp = current_price - tp_points
                tp_changed = True
                self._log(f"🎯 [TP NOVO] ${position.tp:.2f} → ${new_tp:.2f} (Δ{tp_points:.1f}pts)")
            else:
                new_tp = position.tp  # Manter TP atual
                self._log(f"🎯 [TP MANTIDO] ${position.tp:.2f} (ajuste não significativo)")
...
# Funções de adaptação removidas - RobotV7 agora usa formato Legion V1 nativo
    ...
    def get_model_prediction(self, observation):
        ...
        try:
            ...
            # 🕐 FILTRO DE 3 MINUTOS: Evitar consultas nos primeiros 3min do candle M5
            current_time = time.time()
            candle_start_time = (current_time // 300) * 300  # M5 = 300 segundos
            seconds_into_candle = current_time - candle_start_time
            
            if seconds_into_candle < 180:  # Primeiros 3 minutos (180 segundos)
                remaining = 180 - seconds_into_candle
                if int(current_time) % 30 == 0:  # Log a cada 30 segundos para reduzir spam
                    self._log(f"🕐 [CANDLE-FILTER] Aguardando {remaining:.0f}s para consultar modelo (primeiros 3min do candle)")
                return np.array([0.0, 0.0, 0.0, 0.0])  # HOLD forçado
    ...
    def _process_v7_action(self, action):
        ...
        try:
            ...
            if raw_decision < 0.33:  # ACTION_THRESHOLD_LONG
                ...
                entry_decision = 1  # LONG/BUY
            else:
                entry_decision = 2  # SHORT/SELL
            ...
            for i in range(2):  # Para cada posição (2 ativas máximo)
                # Usar mesma função de conversão do silus.py
                realistic_sltp = self._convert_action_to_realistic_sltp([sl_adjusts[i], tp_adjusts[i]], current_price)
                sl_points.append(abs(realistic_sltp[0]))  # [2.0-8.0] pontos
                tp_points.append(abs(realistic_sltp[1]))  # [3.0-15.0] pontos
            ...
            if entry_decision >= 0.67 and entry_confidence < 0.8:
                self._log(f"😫 [CONFIDENCE FILTER] Entry REJEITADA: decision={entry_decision:.2f}, confidence={entry_confidence:.2f} < 0.8 - Forçando HOLD")
