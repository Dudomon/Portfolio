...
# Exemplos: "DAYV5", "DAYV5_V2", "DAYV5_SCALPER", "DAYV5_SWING", etc.
EXPERIMENT_TAG = "DAYV5"

# 💰 CONFIGURAÇÕES DE TRADING: Mude APENAS aqui para diferentes setups
TRADING_CONFIG = {
    "portfolio_inicial": 1000,   # USD - Portfolio inicial
    "base_lot": 0.02,           # Lot base para trades
    "max_lot": 0.03,            # Lot máximo permitido (padronizado)
    "drawdown_limit": 0.20,     # 20% - Limite de drawdown
...
class MetricsCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        try:
            ...
            episode_length = getattr(env, 'MAX_STEPS', 3000)  # 🚀 HÍBRIDO: Duração padrão do episódio
...
TRIAL_2_TRADING_PARAMS = {
    "sl_range_min": 8,                       # ALINHADO: RobotV3 compatibility
    "sl_range_max": 25,                      # ALINHADO: RobotV3 compatibility
    "tp_range_min": 12,                      # ALINHADO: RobotV3 compatibility
    "tp_range_max": 40,                      # ALINHADO: RobotV3 compatibility
...
class TradingEnv(gym.Env):
    MAX_STEPS = 3000  # 🚀 HÍBRIDO: 3000 steps por episódio (10.4 dias) para treinar relações R:R
    ...
    def __init__(self, df, window_size=20, is_training=True, initial_balance=500, trading_params=None):
        ...
        # 🎯 SISTEMA DIFERENCIADO: Usar reward_system do daytrade
...
REALISTIC_SLTP_CONFIG = {
    # 🎯 RANGES DAYTRADE - ALINHADO COM DAYTRADER.PY
    'sl_min_points': 10,    # SL mínimo: 10 pontos (daytrade)
    'sl_max_points': 45,    # SL máximo: 45 pontos (daytrade)
...
class AdvancedTrainingSystem:
    ...
    def train(self):
        ...
        try:
            ...
            for phase_idx in range(self.current_phase_idx, total_phases):
                ...
                # 🔍 ZERO DEBUG CALLBACK REMOVIDO (conforme limpeza planejada)
                # zero_debug_callback removido - sistema limpo
                
                # Combinar callbacks
                from stable_baselines3.common.callbacks import CallbackList
                combined_callback = CallbackList([robust_callback, metrics_callback, progress_callback, gradient_callback])
        ...
        for checkpoint_dir in checkpoint_dirs:
            if os.path.exists(checkpoint_dir):
                for file in os.listdir(checkpoint_dir):
                    if file.endswith('.zip') and (EXPERIMENT_TAG.lower() in file.lower()):
    ...
    def _create_model(self, env):
        ...
        self.logger.info(f" TwoHeadV5Intelligent48h: 6 Gates Especializados + Hard Threshold (RESTAURADA)")
        ...
        self.logger.info(f" Melhorias V5: Entry Head ESPECIALIZADA, 6 Gates ESPECÍFICOS, Hard Threshold BINÁRIO, Market Fatigue DETECTOR")
        self.logger.info("=" * 60)
        ...
        # 🔍 SISTEMA DE DEBUG DE ZEROS EXTREMOS REMOVIDO (conforme limpeza planejada)
        print(f"🔍 ZERO EXTREME DEBUGGER DESABILITADO - {EXPERIMENT_TAG} (sistema limpo)")
    ...
    def _comprehensive_evaluation(self, df: pd.DataFrame, is_training: bool, eval_name: str) -> Dict:
        ...
        try:
            ...
            MAX_STEPS = 3000  # 🚀 HÍBRIDO: Episódios ajustados para melhor avaliação
            max_episodes = 10
