...
def create_enhanced_normalizer_wrapper(env, obs_size=None, normalizer_file=None):
    ...
    enhanced_env = create_enhanced_normalizer(
        ...
        norm_reward=False,  # 🚨 CRITIC FIX: Desabilitar - matando reward signal 
    ...
    try:
        ...
        if hasattr(enhanced_env, 'normalizer'):
            ...
            dummy_normalizer = EnhancedVecNormalize(
                ...
                norm_obs=True,
                norm_reward=False,  # 🚨 CRITIC FIX: Desabilitar normalização
...
class MetricsCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        #  🎯 EXECUTAR AVALIAR_V7.PY A CADA 500K STEPS
        if self.num_timesteps % 500000 == 0 and self.num_timesteps > 0:
            print(f"\n🎯 [AVALIAR_V7] Executando avaliação automática aos {self.num_timesteps:,} steps")
            self._run_avaliar_v7_evaluation()
    ...
    def _run_avaliar_v7_evaluation(self):
        """🎯 Executa avaliar_v7.py automaticamente com checkpoint atual"""
        import subprocess
        import os
        import threading
        from datetime import datetime
        ...
        def run_evaluation_async():
            try:
                # Salvar checkpoint atual primeiro
                ...
                # Atualizar avaliar_v7.py com novo checkpoint
                avaliar_path = "D:/Projeto/avaliacao/avaliar_v7.py"
                ...
                print(f"✅ Checkpoint path atualizado no avaliar_v7.py")
                
                # Executar avaliar_v7.py
                print(f"🚀 Executando avaliar_v7.py...")
                result = subprocess.run([
                    'python', 'avaliacao/avaliar_v7.py'
                ], 
                cwd='D:/Projeto',
                capture_output=True, 
                text=True, 
...
CONTINUATION_PARAMS = {
    ...
    "max_grad_norm": 0.2,             # Clipping severo: 0.3 → 0.2
...
BEST_PARAMS = {
    "learning_rate": 1.5e-05,                # 🔧 OTIMIZADO: Reduzido para maior estabilidade
    "critic_learning_rate": 2.0e-05,        # 🔧 AJUSTE: Igualar com actor (evitar conflitos)
    "n_steps": 2048,                         # 🏆 GOLD SPEC: Good trajectory length  
    "batch_size": 64,                        # 🔧 CORRIGIDO: 32→64 (batch size adequado)
    "n_epochs": 6,                           # 🔧 OTIMIZADO: 4→6 (melhor uso dos dados coletados)
    ...
    "vf_coef": 0.5,                          # 🏆 GOLD SPEC: Value function weight
    "max_grad_norm": 0.1,                    # 🔧 FIX KL: Redução 0.2→0.1 (gradients mais conservadores)
...
class TradingEnv(gym.Env):
    MAX_STEPS = 10000  # 🔧 CRITIC FIX: 10000 steps para sequências longas e aprendizado adequado
    ...
    def __init__(self, df, window_size=20, is_training=True, initial_balance=None, trading_params=None):
        ...
        # 
        # 🔧 ACTION SPACE OTIMIZADO 8D - REMOÇÃO DAS AÇÕES DESPERDIÇADAS
        # NOVO ACTION SPACE 8D OTIMIZADO:
        # [0] entry_decision: [0,2] Discrete (0=hold, 1=long, 2=short)
        # [1] entry_confidence: [0,1] Fusão entry_quality + risk_appetite
        # [2] sl_position_3: [-3,3] SL específico posição 3 (era global)
        # [3] tp_position_3: [-3,3] TP específico posição 3 (era global)
        # [4] sl_position_1: [-3,3] SL específico posição 1
        # [5] tp_position_1: [-3,3] TP específico posição 1
        # [6] sl_position_2: [-3,3] SL específico posição 2
        # [7] tp_position_2: [-3,3] TP específico posição 2
        ...
        self.reward_system = BrutalMoneyReward(initial_balance)
    ...
    def reset(self, **kwargs):
        ...
        # 🔧 NOISE FIX: Step inicial determinístico baseado em hash do episódio para consistência
        min_step = self.window_size  # 20
        max_step = len(self.df) - self.MAX_STEPS - 1  # Considera MAX_STEPS=6000
        if max_step > min_step:
            # Usar hash determinístico baseado no número do episódio para consistência
            episode_hash = hash(str(getattr(self, 'episode_count', 0))) % (max_step - min_step)
            self.current_step = min_step + episode_hash
        else:
    ...
    def step(self, action):
        ...
        """
        # Action deve ser array de 8 dimensões - NOVO ACTION SPACE 8D
        if not isinstance(action, np.ndarray) or action.shape != (8,):
            raise ValueError(f"Action deve ser numpy array (8,), recebido: {type(action)} shape={getattr(action, 'shape', 'N/A')}")
        ...
        if done:
            ...
            for pos in self.positions[:]:
                ...
                if pos['type'] == 'long' and 'sl' in pos and final_price < pos['sl']:
                    ...
                    exit_reason = 'sl_forced_episode_end'
                ...
                # Verificar se TP seria atingido (aplicar limitação de lucro)
                    ...
                    exit_reason = 'tp_forced_episode_end'
                ...
                if pnl < -max_loss_usd:
                    print(f"🚨 [EPISODE END] PnL ${pnl:.2f} excede perda máxima ${max_loss_usd:.2f}, limitando...")
                    pnl = -max_loss_usd
                    actual_exit_price = pos.get('sl', final_price)
    ...
    def _prepare_data(self):
        ...
        if hasattr(self.df, 'processed_data_cache'):
            ...
            if np.any(zero_mask):
                ...
                # 🔧 NOISE FIX: Valor determinístico ao invés de random
                self.processed_data[zero_mask] = 0.001
        ...
        for feature_name in required_hq_features:
            if feature_name in missing_features or feature_name not in self.df.columns:
                ...
                if feature_name == 'volume_momentum':
                    ...
                    # 🔧 NOISE FIX: Remover componente random - usar apenas seno determinístico
                    feature_values = 0.4 + 0.1 * np.sin(np.arange(len(close_5m)) * 0.05)
    ...
    def _calculate_reward_and_info(self, action, old_state):
        ...
        if len(action) >= 8:
            # 🚀 VALIDAÇÃO DO NOVO ACTION SPACE 8D
            if len(action) != 8:
                raise ValueError(f"Action space expects 8 dimensions, got {len(action)}")
            
            # 🔧 NOVO ACTION SPACE 8D - OTIMIZAÇÃO COMPLETA
            raw_decision = float(action[0])
            if raw_decision < ACTION_THRESHOLD_LONG:
                entry_decision = 0  # HOLD
            elif raw_decision < ACTION_THRESHOLD_SHORT:
            ...
            # [1] entry_confidence: FUSÃO quality + risk (filtro + position sizing)
            entry_confidence = float(action[1])  # [0,1] Confiança unificada
            
            # [2-7] SL/TP system: específico para cada posição 1, 2, 3
            sl_pos3 = float(action[2])       # [-3,3] SL específico pos 3 (era global)
            tp_pos3 = float(action[3])       # [-3,3] TP específico pos 3 (era global)
            sl_pos1 = float(action[4])       # [-3,3] SL específico pos 1
            tp_pos1 = float(action[5])       # [-3,3] TP específico pos 1
            sl_pos2 = float(action[6])       # [-3,3] SL específico pos 2
            tp_pos2 = float(action[7])       # [-3,3] TP específico pos 2
            
            # 🔧 LISTAS DIRETAS: cada posição tem seu próprio controle
            sl_adjusts = [sl_pos1, sl_pos2, sl_pos3]
            tp_adjusts = [tp_pos1, tp_pos2, tp_pos3]
            
            # 🚨 SISTEMA DE COOLDOWN ANTI-OVERTRADING
            if self.cooldown_counter > 0:
                entry_decision = 0  # FORÇA HOLD durante cooldown
                self.cooldown_counter -= 1
                if self.current_step % 100 == 0:  # Log esporádico
                    print(f"[COOLDOWN] Forçando HOLD - {self.cooldown_counter} steps restantes")
        ...
        if entry_decision > 0 and len(self.positions) < self.max_positions:
            ...
            if entry_confidence < MIN_CONFIDENCE_THRESHOLD:
                ...
                # 🔧 NOVO SISTEMA SL/TP 8D: Global + Específico
                pos_index = len(self.positions)  # Índice da nova posição
                
                # Escolher SL/TP baseado na posição
                if pos_index == 0:  # Primeira posição - usar específicos pos1
                    sl_adjust = sl_pos1
                    tp_adjust = tp_pos1
                elif pos_index == 1:  # Segunda posição - usar específicos pos2  
                    sl_adjust = sl_pos2
                    tp_adjust = tp_pos2
                else:  # Terceira posição - usar específicos pos3
                    sl_adjust = sl_pos3
                    tp_adjust = tp_pos3
                
                # Converter ajustes [-3,3] para pontos realistas
                ...
                # 🚨 DEBUG: Log detalhado da criação de posição
                sl_distance = abs(position['entry_price'] - position['sl'])
                tp_distance = abs(position['entry_price'] - position['tp']) 
                max_loss_usd = sl_distance * position['lot_size'] * 100
                
                print(f"🔍 NOVA POSIÇÃO: {position['type']} @ {position['entry_price']:.1f}")
                print(f"   SL: {position['sl']:.1f} ({sl_distance:.1f}pts) | TP: {position['tp']:.1f} ({tp_distance:.1f}pts)")
                print(f"   Lote: {position['lot_size']:.3f} | Perda máx: ${max_loss_usd:.2f}")
                
                # Validação de segurança
                if max_loss_usd > 100:
                    print(f"🚨 ALERTA: Perda máxima muito alta! ${max_loss_usd:.2f}")
    ...
    def _process_v5_specialized_action(self, action):
        ...
        # 🔧 CRITIC FIX: Usar constantes globais padronizadas
        if len(action) > 0:
            raw_decision = float(action[0])
            if raw_decision < ACTION_THRESHOLD_LONG:
                entry_decision = 0  # HOLD
...
def convert_action_to_realistic_sltp(sltp_action_values, current_price):
    ...
    # 🚨 CRITICAL FIX: Validar inputs contra NaN/Inf
    if not np.isfinite(sl_adjust):
        print(f"🚨 SL_adjust NaN/Inf detectado: {sl_adjust} → usando default 0.0")
        sl_adjust = 0.0
    if not np.isfinite(tp_adjust):
        print(f"🚨 TP_adjust NaN/Inf detectado: {tp_adjust} → usando default 0.0")  
        tp_adjust = 0.0
    
    # Clamping adicional de segurança
    sl_adjust = np.clip(sl_adjust, -3.0, 3.0)
    tp_adjust = np.clip(tp_adjust, -3.0, 3.0)
    
    # 🚀 CORREÇÃO: Converter para pontos realistas separadamente
    # SL: 2-8 pontos (normalizar [-3,3] para [2,8])
    try:
        ...
        # TP: 3-15 pontos (normalizar [-3,3] para [3,15])
        ...
        # 🚀 ARREDONDAR PARA MÚLTIPLOS DE 0.5 PONTOS (com proteção)
        if np.isfinite(sl_points):
            sl_points = round(sl_points * 2) / 2
        else:
            sl_points = REALISTIC_SLTP_CONFIG['sl_min_points']
            
        if np.isfinite(tp_points):
        ...
        print(f"🚨 ERRO na conversão SL/TP: {e} → usando valores padrão")
        sl_points = 5.0  # SL padrão seguro
        tp_points = 10.0  # TP padrão seguro
    
    # 🚨 FINAL VALIDATION: Garantir que nunca retornamos valores inválidos
    if not (0 < sl_points <= 20):
        print(f"🚨 SL inválido {sl_points} → usando 5.0")
        sl_points = 5.0
    if not (0 < tp_points <= 50):
        print(f"🚨 TP inválido {tp_points} → usando 10.0")
        tp_points = 10.0
...
class AdvancedTrainingSystem:
    ...
    def _create_model(self, env):
        ...
        model_config = {
            ...
            "env": env,
            "learning_rate": BEST_PARAMS["learning_rate"],  # 🎯 ACTOR LR: 3e-05
            ...
            "vf_coef": BEST_PARAMS["vf_coef"],           # 🔧 FIX CONTRADIRÇÃO: usar BEST_PARAMS (0.5)
            ...
            "policy_kwargs": {
                **get_v7_intuition_kwargs(),
                "critic_learning_rate": BEST_PARAMS["critic_learning_rate"]  # 🚀 CRITIC LR: 1e-04
            }
    ...
    def _comprehensive_evaluation(self, df: pd.DataFrame, is_training: bool, eval_name: str) -> Dict:
        ...
        try:
            ...
            MAX_STEPS = 10000  # 🔧 CRITIC FIX: Consistente - episódios longos para critic
            max_episodes = 10
...
def print_gold_spec_banner():
    ...
    print("\n" + "🏆" * 60)
    print("🏆 GOLD TRADING SYSTEM - V7 INTUITION OPTIMIZED")
    print("🏆" * 60)
    ...
    print("⚡ ARCHITECTURE: V7 Intuition com backbone unificado")
    print("🏆" * 60)
