#!/usr/bin/env python3
"""
🚀 AVALIAÇÃO V9OPTIMUS - MODELOS 4DIM
Configuração EXATA do 4dim para avaliação V9:
- Portfolio: $500 
- Base lot: 0.02
- Max lot: 0.03
- Modo inferência (deterministic=False)
- Action Space: 4D REVOLUTION (entry[0,1] + management[2,3] bidirecional)
- Architecture: V9 Optimus - 4D Action Space Revolution
"""
...
# Configuração específica do usuário - Checkpoint V9 Optimus (AUTO-UPDATED)
CHECKPOINT_PATH = "D:/Projeto/Otimizacao/treino_principal/models/Optimus/AUTO_EVAL_500000_steps_placeholder.zip"
...
def find_v9_checkpoint():
    """🔍 Encontra o checkpoint mais recente do 4dim V9 Optimus"""
    import glob
    
    print("🔍 Procurando checkpoints V9 Optimus...")
    
    # 🔧 V9: Usar EXPERIMENT_TAG correto (Optimus)
...
def test_v9_optimus_trading():
    """🚀 Teste V9Optimus com configurações exatas do 4dim"""
    
    print(f"🚀 TESTE V9OPTIMUS - MODELOS 4DIM")
    print("=" * 60)
    ...
    try:
        ...
        import sys
        sys.path.append("D:/Projeto")
        from daytrader import TradingEnv  # 🔥 USANDO DAYTRADER.PY
        
        print("✅ Imports carregados")
        
        # 🔧 FIX: SEMPRE usar busca automática para checkpoint mais recente
        checkpoint_path = find_v9_checkpoint()
        if not checkpoint_path:
            print("❌ Nenhum checkpoint encontrado!")
            print(f"   📝 Fallback: Tentando usar CHECKPOINT_PATH hardcodado...")
            checkpoint_path = CHECKPOINT_PATH
            if not os.path.exists(checkpoint_path):
                print(f"❌ CHECKPOINT_PATH também não existe: {checkpoint_path}")
                return False
        ...
        # Criar ambiente de trading com configurações EXATAS (4D ACTION SPACE V9)
        # 🔥 TradingEnv do 4dim.py usa 4D: [entry_decision, entry_confidence, pos1_mgmt, pos2_mgmt]
        trading_params = {
        ...
        # Carregar modelo
        print("🤖 Carregando modelo V9 OPTIMUS (4D REVOLUTION)...")
        ...
        try:
            ...
            try:
                # Tentar com policy_kwargs V9Optimus
                from trading_framework.policies.two_head_v9_optimus import get_v9_optimus_kwargs
                optimus_kwargs = get_v9_optimus_kwargs()
                model = RecurrentPPO.load(checkpoint_path, policy_kwargs=optimus_kwargs, device=device)
                ...
                try:
                    ...
                    with tempfile.TemporaryDirectory() as temp_dir:
                        ...
                        # Criar modelo novo da arquitetura V9Optimus
                        from trading_framework.policies.two_head_v9_optimus import get_v9_optimus_kwargs
                        optimus_kwargs = get_v9_optimus_kwargs()
                        
                        # Criar ambiente temporário para carregamento (4D ACTION SPACE V9)
                        temp_env = TradingEnv(
                            df.head(100), 
                            window_size=20, 
                            is_training=False,
                        ...
                        model = RecurrentPPO("MlpLstmPolicy", temp_env, policy_kwargs=optimus_kwargs, device=device)
        ...
        for episode_num in range(NUM_EPISODES):
            ...
            # Criar ambiente específico para este episódio (4D ACTION SPACE V9)
            env = TradingEnv(
                episode_df,
                window_size=20,
            ...
            while not done and step < TEST_STEPS:
                ...
                # 🚨 DEBUG: Log da decisão antes do step (4D ACTION SPACE V9)
                if len(action) >= 2:
                ...
                # Log da ação (4D action space V9 OPTIMUS)
                actions_log.append({
                    'step': step,
                    'entry_decision': entry_decision,
                    'entry_confidence': float(action[1]),  # Entrada
                    'pos1_management': float(action[2]),  # Gestão Posição 1
                    'pos2_management': float(action[3]),  # Gestão Posição 2
        ...
        print("\n" + "=" * 80)
        print(f"📊 RESULTADOS CONSOLIDADOS - {NUM_EPISODES} EPISÓDIOS V9OPTIMUS")
        print("=" * 80)
        ...
        # Análise de ações - V9Optimus (4D REVOLUTION)
        if actions_log:
            entry_decisions = [a['entry_decision'] for a in actions_log]
            entry_confidences = [a['entry_confidence'] for a in actions_log]
            pos1_managements = [a['pos1_management'] for a in actions_log]
            pos2_managements = [a['pos2_management'] for a in actions_log]
            ...
            avg_pos1_mgmt = np.mean(pos1_managements)
            avg_pos2_mgmt = np.mean(pos2_managements)
            
            print(f"\n🎮 ANÁLISE DAS AÇÕES V9OPTIMUS (4D REVOLUTION):")
            ...
            print(f"   📊 SISTEMA GESTÃO 4D:")
            print(f"   🎯 [2] Posição 1 Mgmt: {avg_pos1_mgmt:+.3f}")
            print(f"   🎯 [3] Posição 2 Mgmt: {avg_pos2_mgmt:+.3f}")
        ...
        report_filename = f"D:/Projeto/avaliacoes/avaliacao_v9_{steps_from_name}_{timestamp}.txt"
        
        print(f"\n💾 Salvando relatório: {report_filename}")
        
        with open(report_filename, 'w', encoding='utf-8') as f:
            f.write(f"AVALIAÇÃO V9 OPTIMUS CHECKPOINT {steps_from_name.upper()} - {timestamp}\n")
            f.write("=" * 60 + "\n\n")
...
if __name__ == "__main__":
    print(f"🚀 INICIANDO TESTE V9OPTIMUS - {datetime.now().strftime('%H:%M:%S')}")
    
    success = test_v9_optimus_trading()
    
    if success:
        print(f"\n✅ TESTE V9OPTIMUS CONCLUÍDO - {datetime.now().strftime('%H:%M:%S')}")
    else:
        print(f"\n❌ TESTE V9OPTIMUS FALHOU - {datetime.now().strftime('%H:%M:%S')}")