...
#  SISTEMA DE REWARDS ESPECÍFICO PARA DAY TRADING
from trading_framework.rewards.reward_daytrade import create_daytrading_reward_system
...
CONVERGENCE_OPTIMIZATION_CONFIG = {
    ...
    "max_grad_norm": 0.5,     # 🔧 REDUZIDO: 1.0→0.5 (mais estável)
    ...
    "noise_injection_prob": 0.15,  # 🎯 SUAVE: Diversidade sem bagunçar KL
...
def calculate_v6_observation_space():
    """Calcula e valida o observation space para TwoHeadV7Intuition"""
    print("=" * 60)
    print(f"CALCULANDO OBSERVATION SPACE DAYTRADER ({EXPERIMENT_TAG})")
    print("=" * 60)
    ...
    intelligent_v7_count = 0  # V7 Intuition não usa componentes hardcoded
    ...
    total_features_per_step = market_features + position_features + intelligent_v7_count
    ...
    print(f"🎯 OBSERVATION SPACE: {total_features_per_step} x {window_size} = {observation_space_size} dimensões")
    print("=" * 60)
    print(f"✅ DAYTRADER CONFIGURADO PARA: {observation_space_size} DIMENSÕES")
    print("=" * 60)
...
def load_optimized_data_original():
    """
     FUNÇÃO ORIGINAL: CARREGAR DATASET MASSIVO YAHOO (1.1M BARRAS) OU FALLBACK PARA GOLD_final_nostatic.pkl
    """
...
BEST_PARAMS = {
    "learning_rate": 2.0e-04,                #  🎯 ANTI-CONVERGÊNCIA: LR mais alto sem otimizador
    "n_steps": 1792,                         #  ORIGINAL: Batch size otimizado
    "batch_size": 32,                        #  🔧 MENOR: 64→32 para updates mais frequentes
    "n_epochs": 6,                           #  🎯 MAIOR: 4→6 para mais aprendizado por batch
    "gamma": 0.99,                           #  ORIGINAL: Discount factor padrão
    "gae_lambda": 0.95,                      #  ORIGINAL: GAE lambda padrão
    "clip_range": 0.3,                       #  🎯 MAIOR: 0.25→0.3 para menos clipping
    "ent_coef": 0.05,                        #  🎯 MAIOR: 0.03→0.05 para mais exploração
    "vf_coef": 0.6017559963200034,           #  ORIGINAL: VF coefficient que converge
    "max_grad_norm": 1.0,                    #  🚨 EMERGÊNCIA: Menos clipping para preservar gradientes
    "policy_kwargs": {
...
class TradingEnv(gym.Env):
    ...
    def __init__(self, df, window_size=20, is_training=True, initial_balance=None, trading_params=None):
        ...
        print(f"✅ DAYTRADER OBSERVATION SPACE VALIDADO: {calculated_obs_size} dimensões")
        ...
        # 🎯 SISTEMA ESPECIALIZADO: Reward system específico para day trading
        self.reward_system = create_daytrading_reward_system(initial_balance)
        ...
        print(f"[REWARD SYSTEM] Sistema especializado day trading ativado (scalping + velocidade + precisão)")
    ...
    def _get_observation(self):
        ...
        # ✅ V7 CLEAN: Gerar observation space limpa (compatible com V6)
        return self._get_clean_observation_v7()
    
    def _get_clean_observation_v7(self):
        """
        ✅ OBSERVATION SPACE LIMPA PARA TWOHEADV7SIMPLE
        Compatible com V6 - observation space mantido
        """
        global zero_debugger
        
        # 🎯 DADOS BÁSICOS - USAR VALORES PEQUENOS AO INVÉS DE ZEROS EXTREMOS
        positions_obs = np.full((self.max_positions, 9), 0.001, dtype=np.float32)  # 🔧 CORREÇÃO: valores pequenos
        
        # 🔍 DEBUG: Desabilitado - apenas callback periódico ativo
        
        for i in range(min(len(self.positions), self.max_positions)):
            pos = self.positions[i]
            # 🔧 GARANTIR QUE NENHUM VALOR SEJA ZERO
            entry_price = max(pos.get('entry_price', 0.01), 0.01) / 10000.0
            current_price = max(pos.get('current_price', 0.01), 0.01) / 10000.0
            unrealized_pnl = pos.get('unrealized_pnl', 0.01) if pos.get('unrealized_pnl', 0) != 0 else 0.01
            ...
            duration = max(pos.get('duration_minutes', 0.01), 0.01) / 1440.0
        ...
        # Posições vazias com valores padrão - NUNCA ZERO
        for i in range(len(self.positions), self.max_positions):
            positions_obs[i, :] = [0.01, 0.5, 0.5, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01]
        
        # 🎯 DADOS DE MERCADO BÁSICOS - CORRIGIR ZEROS
        obs_market = self.processed_data[self.current_step - self.window_size:self.current_step]
        
        # 🔧 CORREÇÃO: Eliminar zeros dos dados de mercado
        obs_market = np.where(np.abs(obs_market) < 1e-8, 
                             np.random.normal(0.01, 0.005, obs_market.shape), 
                             obs_market)
        ...
        # 🔍 MAPEAR E CORRIGIR ZEROS
        zeros_mask = np.abs(flat_obs) < 1e-8
        zeros_count = np.sum(zeros_mask)
        zeros_percentage = zeros_count / len(flat_obs) * 100
        
        if self.current_step % 5000 == 0 and zeros_count > 0:
            print(f"🔍 [ZEROS MAPPING] Step {self.current_step}: {zeros_percentage:.1f}% zeros ({zeros_count}/{len(flat_obs)})")
            
            # Mapear zeros por região
            window_size = 20  # Como definido na observação
            features_per_step = len(flat_obs) // window_size  # 1480/20 = 74 features por step
            
            zero_indices = np.where(zeros_mask)[0]
            zero_regions = {}
            
            for idx in zero_indices:
                region = idx // features_per_step
                feature_in_region = idx % features_per_step
                
                if region not in zero_regions:
                    zero_regions[region] = []
                zero_regions[region].append(feature_in_region)
            
            print(f"   📍 Zeros por step temporal (0-19): {dict(sorted(zero_regions.items()))}")
            
            # Identificar padrões de features
            feature_analysis = {}
            for idx in zero_indices[:50]:  # Analisar primeiros 50 zeros
                feature_in_step = idx % features_per_step
                if feature_in_step not in feature_analysis:
                    feature_analysis[feature_in_step] = 0
                feature_analysis[feature_in_step] += 1
            
            print(f"   🎯 Features mais afetadas: {dict(sorted(feature_analysis.items(), key=lambda x: x[1], reverse=True)[:10])}")
        
        # 🔧 CORREÇÃO AUTOMÁTICA DE ZEROS
        if zeros_count > 0:
            # Substituir zeros por valores pequenos mas não zero
            flat_obs[zeros_mask] = np.random.normal(0.01, 0.005, zeros_count)
            
            # Verificar se correção funcionou
            new_zeros = np.sum(np.abs(flat_obs) < 1e-8)
            if new_zeros == 0 and self.current_step % 5000 == 0:
                print(f"   ✅ CORRIGIDO: {zeros_count} zeros substituídos por valores pequenos")
        
        return flat_obs
    
    def _get_intelligent_observation_v7(self):
        """
        🧠 OBSERVATION SPACE PARA TWOHEADV7 INTUITION
        Observação limpa para V7 - Unified Backbone processa internamente
        """
        ...
        if not hasattr(self, '_price_min_max_cache'):
            print(f"[V7-CACHE] Calculando min/max inicial do dataset...")
            start_time = time.time()
            ...
            print(f"[V7-CACHE] OK Min/max calculado em {cache_time:.3f}s - cache permanente criado")
        ...
        # 🧠 COMPONENTES PARA V7 INTUITION (processados pelo Unified Backbone)
        ...
        if np.any(np.isnan(flat_obs)) or np.any(np.isinf(flat_obs)):
            print(f"[V7-CRITICAL] Observação contém NaN/Inf - corrigindo...")
        ...
        # 🔍 DEBUG AVANÇADO DOS ZEROS - RASTREAMENTO NA ORIGEM (FORÇADO PRIMEIROS STEPS)
        should_debug = (
            self.current_step % 500 == 0 or  # A cada 500 steps
            self.current_step < 5000  # Primeiros 5000 steps sempre
        )
        ...
        if should_debug:
            ...
            # Log básico sempre nos primeiros steps
            if self.current_step < 5000 or self.current_step % 500 == 0:
                print(f"📊 [ZEROS CHECK] Step {self.current_step}: {zeros_percentage:.1%} zeros extremos ({zeros_extreme}/{len(flat_obs)})")
            
            if zeros_percentage > 0.15:  # >15% zeros extremos (threshold menor)
                print(f"⚠️ [ZEROS ORIGEM] Step {self.current_step}: *** {zeros_percentage:.1%} zeros extremos detectados! ***")
                
                # DEBUG: Rastrear origem dos zeros ANTES da normalização
                zero_indices = np.where(np.abs(flat_obs) < 1e-8)[0]
                print(f"🔍 [ZEROS ORIGEM] {len(zero_indices)} zeros encontrados na observação RAW")
                
                # Mapear para features específicas
                obs_size = len(flat_obs)
                print(f"📊 [ZEROS ORIGEM] Observação: {obs_size} dimensões")
                
                # Verificar se vem do histórico ou features atuais
                if hasattr(self, 'historical_window_size'):
                    window_size = self.historical_window_size
                    current_features = obs_size // window_size if window_size > 0 else obs_size
                    
                    # Analisar por janela temporal
                    zeros_por_janela = {}
                    for idx in zero_indices[:100]:  # Limitar para performance
                        janela = idx // current_features if current_features > 0 else 0
                        feature_idx = idx % current_features if current_features > 0 else idx
                        
                        if janela not in zeros_por_janela:
                            zeros_por_janela[janela] = []
                        zeros_por_janela[janela].append(feature_idx)
                    
                    print(f"🗓️ [ZEROS ORIGEM] Zeros por janela temporal: {dict(list(zeros_por_janela.items())[:5])}")
                
                # Verificar features específicas conhecidas
                feature_names = ['price', 'volume', 'indicators', 'positions', 'temporal']
                if obs_size >= 74 * 20:  # Formato esperado
                    features_per_step = 74
                    zeros_por_feature = {}
                    
                    for idx in zero_indices[:100]:
                        feature_idx = idx % features_per_step
                        feature_group = self._identify_feature_group(feature_idx)
                        
                        if feature_group not in zeros_por_feature:
                            zeros_por_feature[feature_group] = 0
                        zeros_por_feature[feature_group] += 1
                    
                    print(f"🏷️ [ZEROS ORIGEM] Zeros por tipo de feature: {zeros_por_feature}")
                
                # Sample de valores não-zeros para contexto
                non_zeros = flat_obs[np.abs(flat_obs) >= 1e-8]
                if len(non_zeros) > 0:
                    print(f"📈 [ZEROS ORIGEM] Amostra não-zeros: min={np.min(non_zeros):.6f}, max={np.max(non_zeros):.6f}")
        
        # Adicionar método auxiliar para identificar grupos de features
        if not hasattr(self, '_identify_feature_group'):
            def _identify_feature_group(feature_idx):
                if feature_idx < 19:  # Base features
                    return 'base_market'
                elif feature_idx < 38:  # High quality
                    return 'high_quality'  
                elif feature_idx < 47:  # Market features
                    return 'market_total'
                elif feature_idx < 74:  # Position features
                    return 'positions'
                else:
                    return 'unknown'
            
            self._identify_feature_group = _identify_feature_group
        
        # 🚀 V7: Armazenar observação atual para uso nos filtros
        self.last_observation_v7 = flat_obs
        
        # 🔍 DEBUG ZEROS - APENAS A CADA 5000 STEPS
        if self.current_step % 5000 == 0:
            zeros_count = np.sum(np.abs(flat_obs) < 1e-8)
            zeros_percentage = zeros_count / len(flat_obs) * 100
            print(f"🔍 [ZEROS] Step {self.current_step}: {zeros_percentage:.1f}% zeros ({zeros_count}/{len(flat_obs)})")
            
            if zeros_percentage > 15:
                print(f"⚠️ ALERTA: Muitos zeros detectados!")
                # Amostra dos valores para debug
                sample_vals = flat_obs[np.abs(flat_obs) >= 1e-8][:10]
                print(f"   Sample valores: {sample_vals[:5]}")
        
        # 🔍 SEMPRE verificar zeros críticos sem spam
        zeros_critical = np.sum(np.abs(flat_obs) < 1e-8) / len(flat_obs)
        if zeros_critical > 0.25:  # >25% crítico
            print(f"🚨 CRÍTICO Step {self.current_step}: {zeros_critical*100:.1f}% zeros!")
        ...
        if entry_decision > 0 and len(self.positions) < self.max_positions:
            ...
            if entry_allowed:
                ...
                if pos_index < 3:  # Garantir que não exceda max_positions
                    ...
                    default_sl_points = 3.0  # 3 pontos (média entre 1-5)
                    default_tp_points = 4.5  # 4.5 pontos (média entre 1-8)
    ...
    def _apply_v7_intuition_filters(self, action_type, v7_outputs):
        ...
        try:
            ...
            # 1. Entry Confidence Filter (SpecializedEntryHead) - 🔥 RELAXADO!
            entry_conf = gates.get('entry_conf', 0.5)
            if entry_conf < 0.3:  # 🔥 REDUZIDO: era 0.4, agora 0.3 (mais trades!)
                print(f"[🚫 V7 FILTER] Entry confidence baixa: {entry_conf:.3f}")
                return False, f"Entry confidence baixa: {entry_conf:.3f}"
            
            # 2. Management Confidence Filter (TwoHeadDecisionMaker) - 🔥 RELAXADO!
            mgmt_conf = gates.get('mgmt_conf', 0.5)
            if mgmt_conf < 0.2:  # 🔥 REDUZIDO: era 0.3, agora 0.2 (mais trades!)
                print(f"[🚫 V7 FILTER] Management confidence baixa: {mgmt_conf:.3f}")
                return False, f"Management confidence baixa: {mgmt_conf:.3f}"
            
            # 3. Regime-Based Filter (UnifiedBackbone) - 🔥 RELAXADO PARA VOLATILIDADE!
            regime_id = gates.get('regime_id', 2)  # Default: sideways
            # 🔥 FILTRO REMOVIDO: Mercado volátil agora é OPORTUNIDADE, não risco!
            # NOVA FILOSOFIA: VOLATILIDADE = LUCRO!
            # if regime_id == 3:  # 🔥 COMENTADO: Não bloquear mais mercados voláteis
            #     print(f"[🚫 V7 FILTER] Mercado volátil (regime {regime_id}) - reduzindo exposição")
            #     return False, f"Mercado volátil (regime {regime_id})"
            
            # 🔥 NOVO: Mercado volátil é PREFERIDO!
            if regime_id == 3:  # Volatile market - OPORTUNIDADE!
                print(f"[🔥 V7 BOOST] Mercado volátil (regime {regime_id}) - OPORTUNIDADE DE LUCRO!")
                # Não bloquear - deixar passar para aproveitar volatilidade
            
            # 4. Backbone Specialization Filter - 🔥 RELAXADO!
            specialization_div = gates.get('specialization_divergence', 0.0)
            if specialization_div > 0.95:  # 🔥 RELAXADO: era 0.9, agora 0.95 (mais permissivo)
...
class AdvancedTrainingSystem:
        ...
        try:
                ...
                def _on_step(self) -> bool:
                    ...
                    if real_timesteps % self.save_freq == 0:
                        try:
                            ...
                            # Salvar no framework - CUSTOMIZADO
                            print(f"💾 Salvando (CustomRecurrentPolicy-compatible): {framework_path}")
                            self._save_custom_checkpoint(self.model, framework_path, real_timesteps)
                            
                            # Salvar no path original - CUSTOMIZADO  
                            print(f"💾 Salvando (CustomRecurrentPolicy-compatible): {model_path}")
                            self._save_custom_checkpoint(self.model, model_path, real_timesteps)
                                ...
                                if os.path.exists(path):
                                    ...
                                    if path_name == "Framework":
                                        try:
                                            ...
                                            # Verificar se é checkpoint customizado
                                            ...
                                            if isinstance(checkpoint_data, dict) and 'policy' in checkpoint_data:
                                                ...
                                                # Checkpoint padrão - testar carregamento normal
                                                print("📝 Checkpoint padrão detectado, testando...")
                                                test_model = RecurrentPPO.load(path, env=None)
                                                if test_model is not None:
                                                    print("✅ Checkpoint padrão pode ser carregado!")
                                                    del test_model
                                                else:
                                                    print("❌ ERRO: Checkpoint padrão não pode ser carregado!")
            ...
            for phase_idx in range(self.current_phase_idx, total_phases):
                ...
                try:
                    ...
                    self._save_custom_checkpoint(self.current_model, final_phase_path, total_steps_after_phase)
            ...
            try:
                ...
                self._save_custom_checkpoint(self.current_model, final_absolute_path, self.total_steps_completed)
    ...
    def _create_model(self, env):
        ...
        def lr_schedule_lstm_warmup(progress):
            """LR schedule otimizado para LSTM com warmup suave"""
            base_lr = BEST_PARAMS["learning_rate"]  # 2.678385767462569e-05
        ...
        #  CONFIGURAÇÕES ESPECIALIZADAS PARA TWOHEADV7INTUITION - ARQUITETURA EXPERIMENTAL
        model_config = {
            "policy": TwoHeadV7Intuition,
            "env": env,
            "learning_rate": BEST_PARAMS["learning_rate"],  # 🚀 LR com warmup (adaptado para V7)
            "n_steps": 2048,                               # 🚨 FIX CRÍTICO: 2048 steps para sequências MUITO longas (weight_hh_l0 repair)
            "batch_size": BEST_PARAMS["batch_size"],        #  64: Batch size refinado
            "n_epochs": 4,                                  # 🚀 AUMENTADO: 3 → 4 (mais iterações por batch para critic learning)
            ...
            "ent_coef": 0.05,                               # 🚀 AUMENTADO: 0.0171 → 0.05 (mais exploração contra convergência precoce)
            "vf_coef": 0.8,                                # 🔥 FIX: 1.2 → 0.8 (reduzido para evitar gradient domination)
            "max_grad_norm": 0.3,                           # 🚀 REDUZIDO: 0.5 → 0.3 (clipping mais agressivo para preservar gradientes)
            "verbose": 1,             #  VERBOSE ATIVADO para debug
            "device": device_policy,
            "seed": 42,
            "use_sde": False,         #  SDE DESABILITADO PARA V7
            "policy_kwargs": get_v7_intuition_kwargs()  #  CONFIGURAÇÕES ESPECIALIZADAS V7 INTUIÇÃO
        ...
        self.logger.info(f"🧠 TwoHeadV7Intuition: Backbone Unificado + Gradient Mixing + Intelligence (ARQUITETURA EXPERIMENTAL)")
        self.logger.info(f"✅ Intuition Features: Unified Backbone + Gradient Mixing + Interference Monitor")
        self.logger.info(f"✅ Intuition Philosophy: Shared Market Vision + Adaptive Intelligence + Cross-Pollination")
        self.logger.info("=" * 60)
    ...
    def _save_phase_checkpoint(self, phase: TrainingPhase):
        ...
        self._save_custom_checkpoint(self.current_model, model_path, phase.total_steps)
    ...
    def _determine_phase_from_steps(self, steps):
        """🎓 CURRICULUM LEARNING: Determinar fase baseado nos steps (100k dataset 1m + fases dataset massivo)"""
        # Fases do curriculum learning:
        # Phase_0_Bootstrap_1m: 0 - 300k (dataset 1m - 5.6M barras)
        # Phase_1_Fundamentals: 300k - 816k (dataset massivo)
        # Phase_2_Risk_Management: 816k - 1.461M (dataset massivo)
        # Phase_3_Noise_Handling: 1.461M - 2.106M (dataset massivo)
        # Phase_4_Stress_Testing: 2.106M - 2.622M (dataset massivo)
        # Phase_5_Integration: 2.622M - 2.88M (dataset massivo)
        
        phase_thresholds = [
            # Phase_0 REMOVIDA - começar direto no multi-timeframe
            1032000,  # Phase_1_Fundamentals: 0 - 1.032M (dataset multi-timeframe)
            2322000,  # Phase_2_Risk_Management: 1.032M - 2.322M (dataset multi-timeframe)
            2106000,  # Phase_3_Noise_Handling: 1.461M - 2.106M (dataset massivo)
            2622000,  # Phase_4_Stress_Testing: 2.106M - 2.622M (dataset massivo)
            2880000   # Phase_5_Integration: 2.622M - 2.88M (dataset massivo)
        ]
...
def main():
    ...
    try:
        ...
        if CONVERGENCE_OPTIMIZATION_AVAILABLE and CONVERGENCE_OPTIMIZATION_CONFIG["enabled"]:
            ...
            print(f"   - Entry Confidence: {CONVERGENCE_OPTIMIZATION_CONFIG['entry_conf_threshold']} (era 0.4)")
            print(f"   - Mgmt Confidence: {CONVERGENCE_OPTIMIZATION_CONFIG['mgmt_conf_threshold']} (era 0.3)")
