...
class ConvergenceLogger:
    ...
    def __init__(self, log_dir=DIFF_MODEL_DIR):
        ...
        if JSONL_AVAILABLE:
            try:
                # Criar pasta do experiment se não existir
                experiment_path = f"D:/Projeto/avaliacoes/{EXPERIMENT_TAG.lower()}"
...
class MetricsCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if self.num_timesteps % 100000 == 0 and self.num_timesteps > 0:
            print(f"🔍 [METRICS_DEBUG] Step {self.num_timesteps:,} - Próxima avaliação em {500000 - (self.num_timesteps % 500000):,} steps")
...
BEST_PARAMS = {
    "learning_rate": 3.0e-05,                # 🔧 OTIMIZADO: Conservador para treino longo + rede pequena
    "critic_learning_rate": 3.0e-05,        # 🔧 OTIMIZADO: Igualar com actor
...
class TradingEnv(gym.Env):
    ...
    def __init__(self, df, window_size=20, is_training=True, initial_balance=None, trading_params=None):
        ...
        # 🏆 V3 ELEGANCE REWARD SYSTEM - NOTA 10
        v3_config = V3EleganceConfig()
        self.reward_system = ElegantDayTradingRewardV3(v3_config)
        self.reward_system.reset_episode(initial_balance)
    ...
    def reset(self, **kwargs):
        ...
        #  CORREÇÃO CRÍTICA: Reset do sistema de recompensas V3 Elegance
        if hasattr(self, 'reward_system'):
            # V3 Elegance reset
            if hasattr(self.reward_system, 'reset_episode'):
                self.reward_system.reset_episode(self.initial_balance)
            # Fallback para sistemas antigos
            elif hasattr(self.reward_system, 'last_trade_step'):
                self.reward_system.last_trade_step = -999
    ...
    def _calculate_reward_and_info(self, action, old_state):
        ...
        if self.current_step % 100 == 0:  # Debug apenas a cada 100 steps
            ...
            reward, info = self.reward_system.calculate_final_reward(self, processed_action, old_state)
            done_from_reward = False  # V3 Elegance não termina episódios
