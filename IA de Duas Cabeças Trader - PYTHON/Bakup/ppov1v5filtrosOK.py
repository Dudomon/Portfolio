        ...
        try:
            ...
            try:
                ...
                if trades_lists and episode_steps_list:
                    ...
                    current_trades_per_day = total_trades / max(days_elapsed, 0.01) if episode_steps > 0 else 0
            except:
...
class TradingEnv(gym.Env):
    ...
    def _get_intelligent_observation_v5(self):
        ...
        # 🔍 DIAGNÓSTICO DETALHADO: Identificar fonte dos zeros extremos
        if self.current_step % 1000 == 0:  # A cada 1k steps para debug mais frequente
            zeros_extreme = np.sum(np.abs(flat_obs) < 1e-8)
            if zeros_extreme > len(flat_obs) * 0.1:  # >10% zeros extremos
                print(f"🔍 [DIAGNÓSTICO] Step {self.current_step}: {zeros_extreme/len(flat_obs):.1%} zeros extremos detectados")
                
                # Analisar componentes separadamente
                market_zeros = np.sum(np.abs(obs_market.flatten()) < 1e-8)
                positions_zeros = np.sum(np.abs(tile_positions.flatten()) < 1e-8)
                intelligent_zeros = np.sum(np.abs(tile_intelligent.flatten()) < 1e-8)
                
                print(f"   📊 Market data zeros: {market_zeros}/{len(obs_market.flatten())} ({market_zeros/len(obs_market.flatten()):.1%})")
                print(f"   📊 Positions zeros: {positions_zeros}/{len(tile_positions.flatten())} ({positions_zeros/len(tile_positions.flatten()):.1%})")
                print(f"   📊 Intelligent zeros: {intelligent_zeros}/{len(tile_intelligent.flatten())} ({intelligent_zeros/len(tile_intelligent.flatten()):.1%})")
                
                # Verificar se há padding sendo adicionado
                if flat_obs.shape[0] != self.observation_space.shape[0]:
                    print(f"   ⚠️ Obs shape mismatch: {flat_obs.shape[0]} vs {self.observation_space.shape[0]}")
                
                # 🔍 ANÁLISE MAIS DETALHADA do market data
                print(f"   🔍 Market data stats: mean={np.mean(obs_market):.4f}, std={np.std(obs_market):.4f}")
                print(f"   🔍 Positions stats: mean={np.mean(tile_positions):.4f}, std={np.std(tile_positions):.4f}")
                print(f"   🔍 Intelligent stats: mean={np.mean(tile_intelligent):.4f}, std={np.std(tile_intelligent):.4f}")
                
                # Verificar se processed_data tem zeros
                if hasattr(self, 'processed_data'):
                    processed_zeros = np.sum(np.abs(self.processed_data[self.current_step - self.window_size:self.current_step].flatten()) < 1e-8)
                    processed_total = len(self.processed_data[self.current_step - self.window_size:self.current_step].flatten())
                    print(f"   🔍 Processed data zeros: {processed_zeros}/{processed_total} ({processed_zeros/processed_total:.1%})")
                    
                    # 🔍 DEBUG POR FEATURE: Identificar quais features têm zeros extremos
                    if hasattr(self, 'feature_columns') and processed_zeros > 0:
                        print(f"   🔍 Analisando zeros por feature:")
                        current_data = self.processed_data[self.current_step - self.window_size:self.current_step]
                        
                        for i, feature_name in enumerate(self.feature_columns):
                            if i < current_data.shape[1]:
                                feature_data = current_data[:, i]
                                feature_zeros = np.sum(np.abs(feature_data) < 1e-8)
                                if feature_zeros > 0:
                                    print(f"      {feature_name}: {feature_zeros}/{len(feature_data)} ({feature_zeros/len(feature_data):.1%}) zeros")
                                    # Mostrar alguns valores para debug
                                    print(f"        Primeiros 5 valores: {feature_data[:5]}")
                                    print(f"        Stats: mean={np.mean(feature_data):.4f}, std={np.std(feature_data):.4f}")
                    
                    # Verificar se há colunas inteiras que são zeros
                    if hasattr(self, 'df'):
                        print(f"   🔍 Verificando colunas com muitos zeros no DataFrame:")
                        for col in self.df.columns:
                            if col in self.feature_columns:
                                col_data = self.df[col].iloc[self.current_step - self.window_size:self.current_step]
                                col_zeros = np.sum(np.abs(col_data) < 1e-8)
                                if col_zeros > len(col_data) * 0.1:  # >10% zeros
                                    print(f"      {col}: {col_zeros}/{len(col_data)} ({col_zeros/len(col_data):.1%}) zeros")
    ...
    def _apply_v5_intelligent_filters(self, action_type, v5_outputs):
        ...
        try:
            ...
            # 🎯 THRESHOLD CIENTÍFICO ÚNICO: 50% (acima do acaso)
            min_threshold = 0.5
            ...
            if failed_gates:
                return False, f"Gates V5 abaixo de 50%: {', '.join(failed_gates)}"
