...
class TradingAppV7:
    ...
    def __init__(self, root):
        ...
        try:
            ...
            # self.safe_after(2000, self._smart_visibility_heartbeat)  # REMOVIDO - pode causar travamento  # Use smart heartbeat
        except Exception:
            pass
    ...
    def _on_window_map(self, event=None):
        """Callback quando janela é mapeada (restaurada)"""
        try:
            if event and event.widget == self.root:
                # Janela foi restaurada da barra de tarefas
                self.root.focus_set()
                self.root.lift()
        except Exception:
            pass
    ...
    def _on_window_unmap(self, event=None):
        """Callback quando janela é desmapeada (minimizada)"""
        try:
            if event and event.widget == self.root:
                # Janela foi minimizada
                pass  # Não fazer nada especial
        except Exception:
            pass

    def _on_window_focus_in(self, event=None):
        """Callback quando janela recebe foco"""
        try:
            if event and event.widget == self.root:
                # Garantir que está no estado normal
