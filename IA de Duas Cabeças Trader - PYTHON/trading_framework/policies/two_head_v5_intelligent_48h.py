...
class SimplifiedUltraEntryHead48h(nn.Module):
    ...
    def __init__(self, base_features_dim=128, lstm_hidden_size=128):
        ...
        self.market_intelligence = nn.Sequential(
            nn.Linear(base_features_dim + 321, 256),  # 🔥 OTIMIZADO: Mais capacidade
            nn.GELU(),                                # 🔥 OTIMIZADO: Ativação mais suave
            nn.LayerNorm(256),
            nn.Dropout(0.02),                         # 🔥 OTIMIZADO: Menos dropout
            nn.Linear(256, 128),
            nn.GELU(),
            nn.LayerNorm(128),
            nn.Linear(128, 4),  # 4 scores: mtf, pattern, lookahead, quality
            nn.Sigmoid()
        )
        ...
        self.quality_confidence = nn.Sequential(
            nn.Linear(base_features_dim, 128),        # 🔥 OTIMIZADO: Mais capacidade
            nn.GELU(),                                # 🔥 OTIMIZADO: Ativação mais suave
            nn.LayerNorm(128),
            nn.Dropout(0.02),                         # 🔥 OTIMIZADO: Menos dropout
            nn.Linear(128, 64),                       # 🔥 CORRIGIDO: Era 64→32, agora 128→64
            nn.GELU(),                                # 🔥 OTIMIZADO: Ativação mais suave
            nn.Linear(64, 2),  # 🔥 CORRIGIDO: Era 32→2, agora 64→2
            nn.Sigmoid()
        )
...
class TwoHeadV5Intelligent48h(RecurrentActorCriticPolicy):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if hasattr(self, 'features_extractor'):
            ...
            print("[V5] 🔥 Features extractor RE-INICIALIZADO após super().__init__()")
    ...
    def _disable_default_mlp_extractor(self):
        """🔥 CORREÇÃO: Desabilitar mlp_extractor padrão que não é usado"""
        try:
            if hasattr(self, 'mlp_extractor'):
                # Congelar parâmetros do mlp_extractor para evitar vanishing gradients
                for param in self.mlp_extractor.parameters():
                    param.requires_grad = False
                print("[V5] 🔥 mlp_extractor padrão DESABILITADO - evita vanishing gradients")
        except Exception as e:
            print(f"[V5] ⚠️ Erro ao desabilitar mlp_extractor: {e}")
    ...
    def _force_reinitialize_action_value_nets(self):
        ...
        if hasattr(self, 'action_net') and self.action_net is not None:
            ...
            print("[V5] 🔥 action_net RE-INICIALIZADO com Xavier (gain=0.01)")
        ...
        if hasattr(self, 'value_net') and self.value_net is not None:
            for module in self.value_net.modules():
                if isinstance(module, nn.Linear):
                    ...
                    if module.bias is not None:
                        nn.init.uniform_(module.bias, -0.001, 0.001)  # Bias muito pequeno
            print("[V5] 🔥 value_net RE-INICIALIZADO com Xavier (gain=0.01)")
    ...
    def _get_latent(self, obs):
        ...
        # Apply V5 processing - nossa arquitetura completa
        processed_features = self._get_latent_from_obs(obs)
        
        # 🔥 GARANTIR SHAPE CORRETO: [batch, features_dim]
        if processed_features.dim() != 2:
            processed_features = processed_features.view(processed_features.size(0), -1)
        
        # Return latents for both policy and value
        return processed_features, processed_features
            ...
            if isinstance(module, nn.Linear):
                ...
                for name, param in module.named_parameters():
                    if 'weight_ih' in name:
                        ...
                        # 🔥 CORREÇÃO CRÍTICA: LSTM bias initialization SEM ZEROS
                        n = param.size(0)
                ...
                for name, param in module.named_parameters():
                    if 'weight_ih' in name:
                        ...
                        # 🔥 CORREÇÃO CRÍTICA: GRU bias initialization SEM ZEROS
                        n = param.size(0)
                        # Inicializar TODOS os gates com valores não-zero
