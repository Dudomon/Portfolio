...
class TradingEnv(gym.Env):
        ...
        try:
            ...
            for i in range(len(self.positions), 2):
                ...
                positions_obs[i, :] = [
                    ...
                    current_price_norm_current * 0.99,                         # [6] SL = 1% abaixo do preço
                    current_price_norm_current * 1.01,                         # [7] TP = 1% acima do preço
                    (hash(f"{self.current_step}_{i}_type") % 3 - 1) * 0.1     # [8] Type variável
                ]

            positions_flat = positions_obs.flatten()  # [18] - 2 posições × 9 features
            
            # 🚀 INTELLIGENT FEATURES: Calcular dinamicamente
            try:
                # Gerar componentes inteligentes REAIS usando as funções já existentes
                components = self._generate_intelligent_components()
                intelligent_features = self._flatten_intelligent_components(components)
            except Exception as e:
                # Fallback apenas em caso de erro
                intelligent_features = np.full(37, 0.4, dtype=np.float32)
            
            # 🚀 ORDER FLOW FEATURES: Calcular microestrutura de mercado
            order_flow_features = self._generate_order_flow_features(self.current_step)  # [4]

            # 🚀 COMBINAR TUDO: HÍBRIDO OTIMIZADO
            # Para cada barra: 45 features (16 + 18 + 7 + 4)
            temporal_sequence = np.zeros((seq_len, 45), dtype=np.float32)

            for i in range(seq_len):
                # HÍBRIDO: 45 features otimizadas (16 + 18 + 7 + 4)
                temporal_sequence[i, :] = np.concatenate([
                    market_data_batch[i],           # [16] market data (cherry.py)
                    positions_flat,                 # [18] 2 positions complete (robot style)
                    intelligent_features[:7],       # [7] intelligent core (V7 best)
                    order_flow_features             # [4] order flow (robot style)
                ])
    ...
    def _get_single_bar_features(self, step):
        ...
        # 🍒 HÍBRIDO: Combinar features otimizadas - 45 FEATURES (16+18+7+4)
        single_bar_obs = np.concatenate([
            market_data.flatten(),           # 16 features (market data - cherry.py)
            positions_obs.flatten(),         # 18 features (2 pos × 9 - robot style)
            intelligent_features[:7],        # 7 features (V7 core - cherry.py)
            order_flow_features              # 4 features (order flow - robot style)
        ])
        # TOTAL: 16 + 18 + 7 + 4 = 45 features HÍBRIDO!
