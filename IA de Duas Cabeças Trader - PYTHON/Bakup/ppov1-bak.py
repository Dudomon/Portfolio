...
class MetricsCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        try:
            ...
            # No final do episódio (quando episode_steps volta a 0 ou próximo do fim)
            elif current_episode_steps >= episode_length - 10 or current_episode_steps < 10:
                # Evitar spam - só mostrar se passou tempo suficiente desde última exibição
                if self.num_timesteps - self.last_step >= 1000:
                    show_metrics = True
                    if current_episode_steps < 10:
                        metrics_context = f"FINAL DO EPISÓDIO (Novo episódio iniciado)"
                    else:
                ...
                if env is None and hasattr(self, 'training_env'):
                    try:
                        ...
                        if portfolio_values and len(portfolio_values) > 0:
                            ...
                            # 🚀 CORREÇÃO: USAR MÁXIMO SIMPLES para acumular métricas globais
