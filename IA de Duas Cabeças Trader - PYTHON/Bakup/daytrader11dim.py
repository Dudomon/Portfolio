...
# 🎯 DISTRIBUIÇÃO DAS FASES (TOTAL = 10,320,000) - 🔥 AUMENTADO PARA RETREINO COMPLETO!
PHASE_DISTRIBUTION = {
    "phase_1": int(10320000 * 0.20),  # 20% = 2,064,000 (Fundamentals)
    "phase_2": int(10320000 * 0.25),  # 25% = 2,580,000 (Pattern Recognition)  
    "phase_3": int(10320000 * 0.25),  # 25% = 2,580,000 (Noise Handling)
    "phase_4": int(10320000 * 0.20),  # 20% = 2,064,000 (Advanced Strategies)
    "phase_5": int(10320000 * 0.10),  # 10% = 1,032,000 (Fine-tuning)
}
...
USE_VARIABLE_VOLATILITY = True  # Ativar volatilidade variável no treinamento
...
USE_COMPONENT_REWARDS = True  # Ativar sistema de reward com componentes especializados
...
BEST_PARAMS = {
    "learning_rate": 2.0e-04,                #  🔥 PPO V1 ORIGINAL: LR padrão que funcionava
    "n_steps": 2048,                         #  🔥 PPO V1 ORIGINAL: Buffer padrão PPO
    "batch_size": 64,                        #  🔥 PPO V1 ORIGINAL: Batch padrão PPO
    "n_epochs": 10,                          #  🔥 PPO V1 ORIGINAL: Epochs padrão PPO
    "gamma": 0.99,                           #  🔥 PPO V1 ORIGINAL: Discount padrão
    "gae_lambda": 0.95,                      #  🔥 PPO V1 ORIGINAL: GAE padrão
    "clip_range": 0.2,                       #  🔥 PPO V1 ORIGINAL: Clip padrão PPO
    "ent_coef": 0.01,                        #  🔥 PPO V1 ORIGINAL: Entropy padrão
    "vf_coef": 0.5,                          #  🔥 PPO V1 ORIGINAL: Value function padrão
    "max_grad_norm": 0.5,                    # 🔥 PPO V1 ORIGINAL: Gradient clipping padrão
    "policy_kwargs": {
...
class TradingEnv(gym.Env):
    ...
    def reset(self, **kwargs):
        ...
        # 🔄 VOLATILIDADE VARIÁVEL SYSTEM - V3
        if USE_VARIABLE_VOLATILITY:
            # Rotacionar volatilidade a cada X episódios
            if self.episode_count % EPISODES_PER_VOLATILITY == 0:
                self.current_volatility = VOLATILITY_SCHEDULE[self.volatility_idx % len(VOLATILITY_SCHEDULE)]
                self.volatility_idx += 1
                print(f"🔄 Volatilidade ajustada para: {self.current_volatility}x (Episódio {self.episode_count})")
    ...
    def _create_training_phases(self) -> List[TrainingPhase]:
        ...
        return [
            ...
            TrainingPhase(
                ...
                timesteps=2064000,  # 20% do total dobrado (2.06M/10.32M)
            ...
            TrainingPhase(
                ...
                timesteps=2580000,  # 25% do total dobrado (2.58M/10.32M)
            ...
            TrainingPhase(
                ...
                timesteps=1032000,  # 10% do total dobrado (1.03M/10.32M)
...
def main():
    ...
    print("\n" + "🔥" * 60)
    print("🔥 DAYTRADER INICIANDO - SISTEMA V7 INTUITION")
    print("🔥" * 60)
