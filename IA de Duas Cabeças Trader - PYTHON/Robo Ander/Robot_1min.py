...
# 🔐 SISTEMA DE LOGIN INTEGRADO ROBOTV7
...
"""
⚔️ Legion AI Trader V7 - Trading Robot Legion V1 com V11 Sigmoid Policy
🧠 CONFIGURADO LEGION V1: V11 Sigmoid Policy + LSTM/GRU Hybrid + 4D Actions
...
# Para distribuição: normalizer está na mesma pasta
...
# 🎯 PATHS PARA DISTRIBUIÇÃO - Tudo na mesma pasta
current_dir = os.path.dirname(os.path.abspath(__file__))

# 🧠 V7 IMPORTS - INTUITION VERSION
# NOTA: Para distribuição, esses imports são comentados pois usamos modelo pré-treinado
...
class TradingRobotV7(gym.Env):
    ...
    def _initialize_historical_data_v7(self):
        ...
        try:
            # 🚀 OTIMIZAÇÃO MEMÓRIA: Reduzir de 1000 para 300 bars (suficiente para indicadores)
            rates = mt5.copy_rates_from_pos(self.symbol, mt5.TIMEFRAME_M1, 0, 300)
            if rates is None or len(rates) == 0:
                self._log("[WARNING] Não foi possível carregar dados históricos, usando dados vazios")
                # 🚀 OTIMIZAÇÃO MEMÓRIA: Dataframe vazio menor e mais eficiente
                self.historical_df = pd.DataFrame()
                for col in self.feature_columns:
                    self.historical_df[col] = [0.0] * 50  # Reduzir de 100 para 50 linhas
                return
            ...
            # 🔧 CRITICAL FIX: Recriar df_5m após possível reindexação/fallback
            df_5m = df.resample('5T').agg({
                'open': 'first', 'high': 'max', 'low': 'min', 'close': 'last', 'tick_volume': 'sum'
            }).dropna()
            
            # 🔧 FIX: Verificar compatibilidade e usar fallback se necessário
            if len(df_5m) < 50 or len(df_5m) != len(self.historical_df):
                # Usar dados 1m como base para features de alta qualidade
                close_5m = df_1m['close']
                high_5m = df_1m['high'] 
                low_5m = df_1m['low']
                volume_5m = df_1m['tick_volume']
                df_5m_for_time = df_1m  # Para cálculo de time of day
            else:
                # Reindexar para compatibilidade
                df_5m_reindexed = df_5m.reindex(self.historical_df.index, method='ffill')
                close_5m = df_5m_reindexed['close']
                high_5m = df_5m_reindexed['high']
                low_5m = df_5m_reindexed['low']
                volume_5m = df_5m_reindexed['tick_volume']
                df_5m_for_time = df_5m_reindexed
            ...
            # Time of day
            hours = pd.to_datetime(df_5m_for_time.index).hour
        ...
        try:
            ...
            for i in range(min(2, len(mt5_positions))):  # Até 2 posições ativas
                ...
                positions_obs[base_idx:base_idx+9] = [
                    ...
                    float((pos.volume / self.lot_multiplier) / 1.0),    # [5] Volume DESNORMALIZADO para modelo
    ...
    def _process_legion_action(self, action):
        ...
        try:
            ...
            if cooldown_check[0]:  # Em cooldown
                entry_decision = 0.0  # FORÇA HOLD durante cooldown
                entry_type = "HOLD"
                ...
                # 🚀 FIX: Usar EXATAMENTE o mesmo mapeamento do SILUS.PY
                raw_decision = float(entry_decision)
                if raw_decision < 0.33:  # ACTION_THRESHOLD_LONG
                    entry_type = "HOLD"  # entry_decision = 0
                elif raw_decision < 0.67:  # ACTION_THRESHOLD_SHORT  
                    entry_type = "LONG"  # entry_decision = 1
                else:
                    entry_type = "SHORT"  # entry_decision = 2
            ...
            if entry_type == "HOLD":
                ...
                if entry_confidence < 0.8:  # 🏆 OTIMIZADO: Confidence ótima baseada em testes
                    return f"LOW CONFIDENCE: {entry_confidence:.2f} < 0.8 - HOLD"
        ...
        try:
                ...
                if self.log_widget:
                    ...
                    try:
                        ...
                        if threading.current_thread() == threading.main_thread():
                            ...
                            # 🚀 OTIMIZAÇÃO MEMÓRIA: Limpar log_widget quando muito grande
                            self._cleanup_legacy_log_widget()
                        else:
            ...
            try:
                if hasattr(self, 'session_log_path') and self.session_log_path:
                    ...
                    if important:
                        # 🚀 OTIMIZAÇÃO MEMÓRIA: Rotacionar log quando muito grande
                        self._rotate_session_log_if_needed()
                        with open(self.session_log_path, 'a', encoding='utf-8') as f:
                            f.write(log_message + "\n")
    ...
    def _rotate_session_log_if_needed(self):
        """🚀 OTIMIZAÇÃO MEMÓRIA: Rotacionar log de sessão quando exceder 15MB"""
        try:
            if hasattr(self, 'session_log_path') and self.session_log_path and os.path.exists(self.session_log_path):
                size = os.path.getsize(self.session_log_path)
                if size > 15_000_000:  # 15MB - limite seguro
                    # Criar backup e começar novo log
                    backup_path = f"{self.session_log_path}.backup"
                    if os.path.exists(backup_path):
                        os.remove(backup_path)  # Remover backup antigo
                    os.rename(self.session_log_path, backup_path)
                    # Novo log será criado automaticamente no próximo write
        except Exception:
            pass  # Não falhar se houver erro na rotação
    
    def _cleanup_legacy_log_widget(self):
        """🚀 OTIMIZAÇÃO MEMÓRIA: Limpar log_widget legacy quando muito grande"""
        try:
            if hasattr(self, 'log_widget') and self.log_widget:
                content = self.log_widget.get(1.0, tk.END)
                lines = content.count('\n')
                if lines > 1200:  # Limite menor para widget legacy
                    # Manter apenas as últimas 600 linhas
                    lines_to_keep = 600
                    self.log_widget.delete(1.0, f"{lines-lines_to_keep}.0")
        except Exception:
            pass  # Não falhar se houver erro na limpeza
...
# Funções de adaptação removidas - RobotV7 agora usa formato Legion V1 nativo
    ...
    def get_model_prediction(self, observation):
        ...
        try:
            ...
            try:
                # 🎯 LOG TODA PREDIÇÃO - LIVE TRADING
                action_name = "HOLD"
                if action[0] > 0.33:
                    action_name = "LONG" if action[0] < 1.5 else "SHORT"
    ...
    def _update_observation_buffer(self, observation):
        ...
        try:
            if isinstance(observation, np.ndarray):
                # 🚀 OTIMIZAÇÃO MEMÓRIA: Reduzir buffer e remover .copy() desnecessário
                self.last_observations.append(observation)
                if len(self.last_observations) > 30:
    ...
    def _start_live_trading(self):
        ...
        try:
            ...
            # 🚀 FIX: Loop com controle de parada para evitar travamento na GUI
            while not (hasattr(self, 'stop_event') and self.stop_event.is_set()):
                try:
                    ...
                    if action is not None:
                        ...
                        self._log(f"[DEBUG] Processando action: {action}")
                        action_analysis = self._process_legion_action(action)
                        self._log(f"[DEBUG] Action analysis resultado: {action_analysis}")
                        
                        # 🔥 EXECUTAR DECISÃO NO MT5
                        if self.mt5_connected:
                            self._log(f"[DEBUG] MT5 conectado, executando decisão...")
                    ...
                    if wait_seconds > 0.5:  # Só esperar se falta mais que 0.5s
                        ...
                        # 🚀 FIX: Sleep com verificação de stop_event
                    ...
                    # 🚀 FIX: Sleep com verificação de stop_event
                    for _ in range(600):  # 60s = 600 * 0.1s
                        if hasattr(self, 'stop_event') and self.stop_event.is_set():
                            return
                        time.sleep(0.1)
    ...
    def _process_dynamic_sl_tp_adjustment(self, position, sl_adjust, tp_adjust, current_price, pos_index):
        ...
        try:
            ...
            # Calcular lucro atual da posição (normalizado para modelo)
            normalized_volume = position.volume / self.lot_multiplier
            if position.type == 0:  # LONG
                current_pnl = (current_price - position.price_open) * normalized_volume
                pos_type = 'long'
            else:  # SHORT
                current_pnl = (position.price_open - current_price) * normalized_volume
                pos_type = 'short'
    ...
    def _execute_v7_decision(self, action_analysis):
        ...
        try:
            self._log(f"[DEBUG] _execute_v7_decision chamado com: {action_analysis}")
            ...
            self._log(f"[DEBUG] action_name={action_name}, entry_confidence={entry_confidence}")
    ...
    def _calculate_volume_by_risk_appetite(self, entry_confidence):
        ...
        try:
            ...
            # Aplicar limites do modelo (0.01 a 0.03)
            final_volume = max(0.01, min(final_volume, max_lot))
            
            # 🎯 APLICAR MULTIPLICADOR PARA MT5
            mt5_volume = final_volume * self.lot_multiplier
            mt5_volume = max(0.01, min(mt5_volume, 10.0))  # Limites de segurança MT5
            mt5_volume = round(mt5_volume, 3)
            
            # Verificar se não é zero ou negativo
            if mt5_volume <= 0:
                mt5_volume = self.original_base_lot * self.lot_multiplier
            ...
            if hasattr(self, 'debug_step_counter') and self.debug_step_counter % 50 == 0:
                ...
                self._log(f"[💰 POSITION SIZING] Portfolio: ${current_portfolio_value:.2f} (growth: {growth_factor:.2f}x) | Base: {adaptive_lot:.3f} | Entry Confidence: {entry_confidence:.2f} (mult: {risk_multiplier:.2f}x) | Modelo: {final_volume:.3f} | MT5: {mt5_volume:.3f}")
    ...
    def run(self):
        ...
        try:
            ...
            # 🚀 FIX: Verificar se foi parado via GUI
            if hasattr(self, 'stop_event') and self.stop_event.is_set():
                self._log("⏹️ RobotV7 finalizado via GUI")
...
class TradingAppV7:
    ...
    def __init__(self, root, user_data=None):
        ...
        # Título único por instância com informações do usuário
        try:
            import os as _os
            if user_data:
                user_info = f"{user_data['username']} ({user_data['access_level']})"
                unique_title = f"Legion AI Trader V7 - {user_info} [PID:{_os.getpid()}]"
            else:
        ...
        # Robot instance com configurações baseadas no usuário
        self.robot = TradingRobotV7()
        
        # 🔐 APLICAR CONFIGURAÇÕES BASEADAS NO USUÁRIO LOGADO
        if self.user_data:
            self.apply_user_restrictions()
        ...
        self.robot.stop_event = self.stop_event  # 🚀 FIX: Compartilhar stop_event com robot
    ...
    def apply_user_restrictions(self):
        """🔐 Aplicar restrições baseadas no nível do usuário"""
        if not self.user_data:
            return
            
        try:
            access_level = self.user_data.get('access_level', 'free')
            
            # Configurar lot size baseado no usuário
            if 'base_lot_size' in self.user_data:
                base_lot = float(self.user_data['base_lot_size'])
                self.robot.base_lot_size = base_lot
                self.robot.lot_multiplier = base_lot / self.robot.original_base_lot
                self.robot.max_lot_size = self.robot.original_max_lot * self.robot.lot_multiplier
                self.log(f"🔐 Lot Size configurado: {base_lot:.3f} (multiplicador: {self.robot.lot_multiplier:.1f}x)")
            
            # Configurar limites de trading baseado no nível de acesso
            if access_level == 'free':
                # Usuário free: máximo 0.01 lot size
                max_allowed_lot = 0.01
                if self.robot.base_lot_size > max_allowed_lot:
                    self.robot.base_lot_size = max_allowed_lot
                    self.robot.lot_multiplier = max_allowed_lot / self.robot.original_base_lot
                    self.robot.max_lot_size = max_allowed_lot
                    self.log(f"🔒 [FREE USER] Lot limitado a {max_allowed_lot:.2f}")
                    
            elif access_level == 'premium':
                # Usuário premium: máximo 0.05 lot size
                max_allowed_lot = 0.05
                if self.robot.base_lot_size > max_allowed_lot:
                    self.robot.base_lot_size = max_allowed_lot
                    self.robot.lot_multiplier = max_allowed_lot / self.robot.original_base_lot
                    self.robot.max_lot_size = max_allowed_lot
                    self.log(f"🟡 [PREMIUM USER] Lot limitado a {max_allowed_lot:.2f}")
                    
            elif access_level == 'admin':
                # Admin: sem limites
                self.log(f"👑 [ADMIN] Sem limites de lot size")
            
            # Aplicar outros limites se especificados no user_data
            if 'max_drawdown_percent' in self.user_data:
                max_drawdown = float(self.user_data['max_drawdown_percent'])
                self.log(f"🔐 Max Drawdown: {max_drawdown}%")
                
            if 'max_daily_trades' in self.user_data:
                max_trades = int(self.user_data['max_daily_trades'])
                self.log(f"🔐 Max Daily Trades: {max_trades}")
            
            self.log(f"✅ Configurações aplicadas para usuário: {self.user_data['username']} ({access_level})")
            
        except Exception as e:
            self.log(f"❌ Erro ao aplicar restrições do usuário: {e}")
        ...
        try:
            while not self.log_queue.empty():
                ...
                if hasattr(self, 'log_text'):
                    ...
                    # 🚀 OTIMIZAÇÃO MEMÓRIA: Limpar log_text quando muito grande
                    self._cleanup_log_text_if_needed()
        except queue.Empty:
            pass
        except Exception:
            pass
    ...
    def _cleanup_log_text_if_needed(self):
        """🚀 OTIMIZAÇÃO MEMÓRIA: Limpar log_text quando exceder 1500 linhas"""
        try:
            if hasattr(self, 'log_text'):
                content = self.log_text.get(1.0, tk.END)
                lines = content.count('\n')
                if lines > 1500:  # Limite seguro para não impactar performance
                    # Manter apenas as últimas 750 linhas
                    lines_to_keep = 750
                    self.log_text.delete(1.0, f"{lines-lines_to_keep}.0")
        except Exception:
            pass  # Não falhar se houver erro na limpeza
    ...
    def on_closing(self):
        ...
        try:
            ...
            if hasattr(self, 'stop_event'):
                self.stop_event.set()
            # 🚀 FIX: Não aguardar thread - forçar fechamento imediato
            if hasattr(self, 'trading_thread') and self.trading_thread and self.trading_thread.is_alive():
                try:
                    self.trading_active = False  # Sinalizar parada
                    # Não fazer join - deixar thread daemon morrer sozinha
                except:
                    pass
        except:
            pass
    ...
    def create_header(self, parent):
        ...
        subtitle = tk.Label(title_frame, text=f"{user_info}Professional Trading System V7.0 - 1MIN TIMEFRAME MODELS", 
    ...
    def run_trading_loop(self):
        ...
        try:
            # 🚀 FIX: Executar robot.run() na thread principal, ele já tem stop_event
            self.robot.run()
            
        except Exception as e:
            self.log(f"❌ Erro no robot.run(): {e}")
            self.trading_active = False
...
def main_gui():
    """Função principal para iniciar GUI - VERSÃO CORRIGIDA ANTI-LOOP"""
    global current_user_data, user_manager_instance
    
    # 🔧 CRITICAL FIX: Prevenir múltiplas GUIs
    if hasattr(main_gui, '_gui_running'):
        print("⚠️ GUI já está sendo executada!")
        return
    main_gui._gui_running = True
    
    try:
        # 🔐 SISTEMA DE LOGIN RobotV7 - OBRIGATÓRIO
        print("🔐 Iniciando sistema de autenticação...")
        ...
        if login_window.login_successful and login_window.session_data:
            ...
            print(f"✅ Login realizado: {user_data['username']} ({user_data['access_level']})")
            
            # 🔧 FIX: Usar mesma sessão Tk em vez de criar nova
            # Reutilizar a janela do login ou criar nova controlada
            login_window.root.destroy()  # Fechar login antes
            
            # Iniciar GUI principal com dados do usuário
        ...
        print(f"❌ Erro crítico no sistema de login: {e}")
    finally:
        # Reset flag para permitir reinicialização
        if hasattr(main_gui, '_gui_running'):
            delattr(main_gui, '_gui_running')

def safe_close(root):
    """🔧 Fecha janela de forma segura"""
    try:
...
def main_gui_with_user(user_data):
    """🎨 Inicia GUI com dados do usuário logado - VERSÃO CORRIGIDA"""
    
    print(f"👤 Usuário ativo: {user_data['username']} ({user_data['access_level']})")
    
    root = tk.Tk()
    
    # 🔧 FIX: Garantir que apenas uma instância rode
    root.protocol("WM_DELETE_WINDOW", lambda: safe_close(root))
    
    app = TradingAppV7(root, user_data)  # Passar dados do usuário
