...
CONVERGENCE_OPTIMIZATION_CONFIG = {
    ...
    "max_grad_norm": 10.0,    # 🔥 FIX: 0.1→10.0 PERMITIR gradientes altos para SAC!
...
class MetricsCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        #  🚀 EXECUTAR AVALIAR_V8.PY A CADA 1M STEPS (OTIMIZADO)
        if self.num_timesteps % 1000000 == 0 and self.num_timesteps > 0:
        ...
        # 🔍 CONVERGENCE LOGGER: Log otimizado a cada 10 steps
...
class AdvancedTrainingSystem:
    ...
    def _create_model(self, env):
        ...
        model_config = {
            ...
            "env": env,
            "learning_rate": 5e-3,                         # 🔥 ULTRA HIGH LR: Force weights out of zeros (5x normal)
            "buffer_size": 1_000_000,                      # 🔥 REVERT: Standard 1M buffer size
            "batch_size": 256,                             # 🚀 REVERT: Stable Baselines3 default for stability
            "tau": 0.005,                                  # 🚀 STABILITY: Smoother target updates
            "gamma": 0.99,                                 # 🎯 Discount factor
            "train_freq": 1,                               # 🚀 REVERT: SB3 default for optimal performance
            "gradient_steps": 1,                           # 🚀 REVERT: SB3 default prevents overfitting
            "ent_coef": "auto_1.0",                       # 🔥 AUTO: Learning entropy with higher init  
            "target_update_interval": 1,                   # 🎯 Target network update frequency
            "verbose": 1,                                  # 🔍 Debug logs
            "device": device_policy,
            "seed": 42,
            "policy_kwargs": {                             # 🚀 PERFORMANCE: Use fast MlpPolicy with V11 init
                # "features_extractor_class": SACTradingTransformerExtractor,  # TOO SLOW: 90→30 it/s
                # "features_extractor_kwargs": {"features_dim": 256},
                "net_arch": dict(pi=[512, 256, 128], qf=[512, 256, 128]),  # 🚀 PERFORMANCE: Bigger nets, no transformer
                "activation_fn": torch.nn.LeakyReLU,           # 🔥 GRADIENT FLOW: Better than SiLU for gradients
                "log_std_init": -0.5,  # 🔥 GRADIENT FIX: Less aggressive init to prevent zeros
            }
        }
        ...
        # 🚨 FIX: FORCE clip observations to prevent inf values - ALWAYS APPLY!
        print(f"🔧 [OBS DEBUG] env type: {type(env)}")
        print("🔧 [OBS CLIPPING] FORCING observation clipping to prevent inf values")
        
        # Wrapper SEMPRE aplicado - independente do tipo de env
        class ObsClipWrapper:
            def __init__(self, env):
                self._env = env
                # 🚨 CRÍTICO: Fix observation_space bounds também!
                import gym
                if hasattr(env, 'observation_space'):
                    obs_space = env.observation_space
                    if hasattr(obs_space, 'low') and hasattr(obs_space, 'high'):
                        # Replace inf bounds with finite bounds
                        low = np.where(obs_space.low == -np.inf, -1e6, obs_space.low)
                        high = np.where(obs_space.high == np.inf, 1e6, obs_space.high)
                        # 🔥 AGRESSIVO: Force range [-1, 1] 
                        self.observation_space = gym.spaces.Box(low=-1.0, high=1.0, shape=obs_space.shape, dtype=obs_space.dtype)
                        print(f"✅ [OBS SPACE] AGRESSIVO: Fixed bounds from [-inf,inf] to [-1.0, 1.0]")
                    else:
            ...
            def reset(self):
                ...
                if hasattr(obs, 'shape'):
                    # 🔥 GRADIENT-FRIENDLY: softer normalization
                    obs = np.clip(obs, -1e6, 1e6)
                    obs = obs / (1000.0 + np.abs(obs))  # Softer than tanh, better gradients
                return obs
        ...
        env = ObsClipWrapper(env)  # 🚨 SEMPRE ATIVO: Force clip inf values
        print("✅ [OBS CLIPPING] ObsClipWrapper FORÇADAMENTE ATIVO - clipping ALL inf values")
        
        model = SAC(**model_config)
        
        # 🔥 GRADIENT ZEROS FIX: Address vanishing gradients problem
        print("🔥 [GRADIENT FIX] Applying fixes for 60% zero gradients issue")
        
        # 🎯 SOLUÇÃO HÍBRIDA: LR alto (resolveu SHORT actions) + Initialization targeted
        print("🔥 [ANTI-ZEROS] LR alto resolveu SHORT actions, agora aplicando init para eliminar zeros")
        
        # Aplicar initialization APENAS na primeira camada que tem 64.4% zeros
        try:
            if hasattr(model.policy, 'actor') and hasattr(model.policy.actor, 'latent_pi'):
                ...
                if isinstance(first_layer, torch.nn.Linear):
                    ...
                    print(f"🔍 [ACTOR] Zeros antes: {zeros_before:.1f}%")
                    
                    if zeros_before > 60:  # Só se crítico
                        # Initialization mais agressiva - Kaiming He para LeakyReLU
                        with torch.no_grad():
                        ...
                        print(f"✅ [ACTOR] Zeros depois: {zeros_after:.1f}% (Kaiming init aplicada)")
            
            # Aplicar também nos critics que têm 58.5% e 60.6% zeros
                    ...
                    if hasattr(critic_net, '0'):
                        ...
                        if isinstance(critic_layer, torch.nn.Linear):
                            ...
                            if zeros_before > 50:
                                ...
                                print(f"✅ [CRITIC {critic_name.upper()}] Zeros: {zeros_before:.1f}% → {zeros_after:.1f}%")
                                
        except Exception as e:
            print(f"⚠️ [INIT] Erro: {e}")
        
        # 🚀 GRADIENT CLIPPING: Add manual gradient clipping after model setup
        if hasattr(model, 'policy') and hasattr(model.policy, 'optimizer') and model.policy.optimizer is not None:
            original_step = model.policy.optimizer.step
            
            def clipped_step():
                torch.nn.utils.clip_grad_norm_(model.policy.parameters(), max_norm=20.0)  # 🔥 INCREASED: Allow high gradients for SAC
                original_step()
            
            model.policy.optimizer.step = clipped_step
            print("✅ [GRADIENT CLIPPING] Applied max_norm=20.0 to SAC policy")
        else:
            print("⚠️ [GRADIENT CLIPPING] Optimizer not ready yet - will be applied during training")
        ...
        print("✅ [GRADIENTS] Sistema de gradientes otimizado - vanishing gradients corrigido")
