...
class TradingAppV7:
    ...
    def __init__(self, root):
        ...
        # 📊 MULTIPLE SYMBOLS CONFIGURATION
        self.symbol_configs = {
            "GOLD (XAU/USD)": {
                "symbol_variants": [
                ...
                "pip_value": 100,
                "default_sl_points": 5,
                "default_tp_points": 10,
                "market_hours": "Sunday 19:00 - Friday 21:00 BRT"
            },
            "EUR/USD": {
                "symbol_variants": [
                    "EURUSD",
                ...
                "default_tp_points": 25,
                "market_hours": "Sunday 22:00 - Friday 22:00 GMT"
            },
            "GBP/USD": {
                "symbol_variants": [
                    "GBPUSD",
    ...
    def create_control_panel(self, parent):
        ...
        symbol_section_frame = tk.Frame(button_container, bg='#2d2d2d')
        symbol_section_frame.pack(fill=tk.X, pady=(10, 5))
        
        symbol_label = tk.Label(symbol_section_frame, text="🎯 Trading Symbol:",
                               font=('Segoe UI', 9, 'bold'),
                               fg='#00d4ff', bg='#2d2d2d')
        symbol_label.pack(anchor='w')
        
        # Symbol dropdown
        self.symbol_var = tk.StringVar(value="GOLD (XAU/USD)")
        self.symbol_combobox = ttk.Combobox(symbol_section_frame, 
                                           textvariable=self.symbol_var,
                                           values=["GOLD (XAU/USD)", "EUR/USD", "GBP/USD", "USD/JPY"],
                                           state="readonly",
                                           font=('Segoe UI', 9),
                                           width=25)
        self.symbol_combobox.pack(fill=tk.X, pady=(3, 0))
    ...
    def on_symbol_change(self, event=None):
        ...
        try:
            ...
            self.log(f"📊 [SYMBOL] Mudança para: {selected_symbol}")
            
            if selected_symbol in self.symbol_configs:
                symbol_config = self.symbol_configs[selected_symbol]
                
                # Update robot's symbol detection system with new variants
                self.robot.symbol_variants = symbol_config["symbol_variants"]
                
                # Update RobotV7.py Config if it exists
                try:
                    from RobotV7 import Config
                    Config.GOLD_SYMBOLS = symbol_config["symbol_variants"]
                    self.log(f"✅ [SYMBOL] RobotV7 atualizado para {selected_symbol}")
                except:
                    pass
                
                # Force symbol re-detection when trading starts
                if hasattr(self.robot, '_detect_broker_gold_symbol'):
                    # Reset detected symbol to force new detection
                    self.robot.symbol = "GOLD"  # Reset to trigger detection
                
                # Display symbol info
                self.log(f"📊 [SYMBOL INFO] {selected_symbol}")
                self.log(f"  ├─ Variantes: {len(symbol_config['symbol_variants'])} símbolos")
                self.log(f"  ├─ SL padrão: {symbol_config['default_sl_points']} pontos")
                self.log(f"  ├─ TP padrão: {symbol_config['default_tp_points']} pontos")
                self.log(f"  └─ Horário: {symbol_config['market_hours']}")
                
            else:
                self.log(f"⚠️ [SYMBOL] Configuração não encontrada para: {selected_symbol}")
                
        except Exception as e:
            self.log(f"❌ [SYMBOL] Erro ao mudar símbolo: {e}")
...
def start_with_login():
    """🔐 Inicia sistema com login obrigatório"""
    ...
    if login_window.login_successful and login_window.session_data:
        ...
        # 🔧 FIX: Fechar janela de login antes de abrir GUI principal
        try:
