...
try:
    # Importar do arquivo local (Robo Ander)
    ...
    # Fallback para o arquivo da raiz e Modelo PPO Trader
    sys.path.append('..')
    sys.path.append('../Modelo PPO Trader')  # Adicionar caminho do Modelo PPO Trader
...
class TradingRobotV7(gym.Env):
    ...
    def __init__(self, log_widget=None):
        ...
        self.cooldown_minutes = 35  # 35 minutos após fechamento de posição
        self.last_position_closed_timestamp = 0  # Timestamp do último fechamento
        self._log(f"[🔧 INIT] Sistema de cooldown inicializado - {self.cooldown_minutes} minutos após fechamento de posição")
    ...
    def _log(self, message):
        """Log filtrado - apenas ordens e cooldown dos slots"""
        # 🔥 FILTRAR LOGS - apenas ordens e cooldown
        important_keywords = [
            'ORDEM', 'ORDER', 'POSIÇÃO FECHADA', 'POSITION', 'TRADE', 'COOLDOWN-SLOT',
            'ENTRY', 'EXIT', 'SL', 'TP', 'FECHADA', 'ABERTA', 'SLOT', 'DEBUG',
            'EXECUTION', 'MODIFY', 'TRAILING'
        ]
        
        # Verificar se mensagem contém palavra-chave importante
        if not any(keyword in message.upper() for keyword in important_keywords):
            return  # 🚫 IGNORAR mensagens irrelevantes
...
# Funções de adaptação removidas - RobotV7 agora usa formato Legion V1 nativo
            ...
            for ticket in closed_tickets:
                if ticket in self.position_stats:
                    ...
                    if history and len(history) >= 2:  # Open + Close
                        ...
                        # 🔥 ATIVAR COOLDOWN DE 35 MINUTOS APÓS FECHAMENTO
                        ...
                        try:
                            ...
                            self._log(f"🔍 [DEBUG SLOT] Ticket {ticket} mapeado para slot: {slot}")
                            if slot is None:
                                ...
                                self._log(f"🔍 [DEBUG COMMENT] Comentário: '{cmt}' → Slot extraído: {slot}")
                            if slot is not None:
                                ...
                                self._log(f"😨 [COOLDOWN-SLOT] Slot {int(slot)} em cooldown por {self.cooldown_minutes} min até {datetime.fromtimestamp(cooldown_until).strftime('%H:%M:%S')}")
                            else:
                                self._log(f"❌ [DEBUG ERROR] Não foi possível determinar slot para ticket {ticket}")
                        except Exception as e:
                            self._log(f"❌ [DEBUG EXCEPTION] Erro no cooldown: {e}")
    ...
    def _allocate_entry_slot(self):
        ...
        try:
            ...
            # 🔥 DEBUG FORÇADO: Mostrar status de TODOS os slots
            slots_debug = []
            for s in range(self.max_positions):
                ...
                slots_debug.append(f"S{s}:{'USED' if is_used else 'FREE' if is_free else f'CD{remain/60:.1f}m'}")
            
            self._log(f"🔍 [SLOTS DEBUG] {' | '.join(slots_debug)}")
            ...
            for s in range(self.max_positions):
                ...
                allow_time = self.position_slot_cooldowns.get(s, 0.0)
                if now >= allow_time:
                    self._log(f"✅ [SLOT ALOCADO] Slot {s} livre - cooldown expirou")
                    return s, 0.0
                else:
            ...
            self._log(f"❌ [TODOS SLOTS BLOQUEADOS] Menor espera: {(min_remain or 0)/60:.1f} min")
            return None, (min_remain or 0.0)
        except Exception as e:
            self._log(f"❌ [SLOT ERROR] {e}")
            return None, 0.0
    ...
    def _execute_order_v7(self, order_type: int, volume: float, sl_price: float = None, tp_price: float = None, slot_id: int = None) -> str:
        ...
        try:
            ...
            if result and result.retcode == mt5.TRADE_RETCODE_DONE:
                ...
                # 🔥 MAPEAR SLOT IMEDIATAMENTE - DIRETO E GARANTIDO
                if slot_id is not None:
                    # Aguardar para MT5 processar
                    time.sleep(0.1)
                    
                    # Obter posições recém-abertas
                    positions = self._get_robot_positions() or []
                    for pos in positions:
                        if pos.ticket == result.order:  # Ticket da ordem executada
                            self.position_slot_map[pos.ticket] = int(slot_id)
                            self._log(f"✅ [SLOT MAPEADO] Ticket #{pos.ticket} → Slot {slot_id}")
                            break
                    else:
                        self._log(f"⚠️ [SLOT WARNING] Ticket #{result.order} não encontrado para mapear slot {slot_id}")
                
                # Reconciliar mapeamento adicional
                try:
...
class TradingAppV7:
    ...
    def apply_lot_size(self):
        ...
        self.log("🔧 [LOT SIZE] Aplicando configuração...")
        try:
            # Garantir que campo está habilitado
            self.lot_size_entry.config(state='normal')
            
            # Validar entrada
            lot_input = self.lot_size_var.get().strip()
            if not lot_input:
                raise ValueError("Campo vazio")
                
            new_lot_value = float(lot_input)
            self.log(f"🔧 [LOT SIZE] Valor lido: {new_lot_value}")
            ...
            if hasattr(self, 'robot') and self.robot:
                ...
                self.log(f"🤖 [ROBOT] Valores atualizados")
            else:
                self.log("⚠️ [ROBOT] Robot não disponível")
            
            # Atualizar display
            multiplier_text = f"{lot_multiplier:.1f}x"
            max_lot_display = 0.03 * lot_multiplier
            if hasattr(self, 'multiplier_label'):
                self.multiplier_label.config(text=f"Multiplier: {multiplier_text} | Max Lot: {max_lot_display:.3f}")
            
            # Log de sucesso
            self.log(f"✅ [LOT SIZE] Aplicado: Base={new_lot_value:.3f} | Max={max_lot_display:.3f} | Multiplier={multiplier_text}")
            
        except ValueError as e:
            self.log(f"❌ [LOT SIZE] Erro: {e}")
            self.log("💡 [LOT SIZE] Use números decimais (ex: 0.04)")
            self.lot_size_var.set("0.02")  # Restaurar valor padrão
        except Exception as e:
            self.log(f"❌ [LOT SIZE] Erro inesperado: {e}")
            self.lot_size_var.set("0.02")  # Restaurar valor padrão
    ...
    def select_model_file(self):
        ...
        try:
            # Abrir dialog para selecionar arquivo .zip ou .secure
    ...
    def load_secure_model(self):
        """🔒 Load protected .secure model"""
        try:
            if ModelSecurityManager is None:
                self.log("❌ Secure model system not available")
                return False
            
            self.log(f"🔒 Loading protected model: {self.selected_model_path}")
            
            # Master key for protected models
            ...
            if not model_info:
                self.log("❌ Failed to load secure model")
                return False
            
            self.log("✅ Secure model loaded successfully")
            self.log(f"   Model Type: {model_info.get('model_type', 'Unknown')}")
            
            # Try to load protected normalizers
            self.load_secure_normalizers()
            
            # Mark as loaded
            self.robot.model_loaded = True
            self.robot.model = "SECURE_MODEL_LOADED"  # Placeholder
            
            self.log("🔒 Protected model system active")
            return True
            
        except Exception as e:
            self.log(f"❌ Error loading secure model: {e}")
            return False
    
    def load_secure_normalizers(self):
        """🔒 Load protected normalizer files"""
        try:
            master_key = "cherry_trading_secret_2025_v1"
            model_dir = os.path.dirname(self.selected_model_path)
            ...
            # Try to load protected normalizers
            normalizer_files = [
                "enhanced_normalizer_final.secure_pkl",
                "enhanced_normalizer_final_enhanced.secure_pkl"
            ]
            
            for norm_file in normalizer_files:
                norm_path = os.path.join(model_dir, norm_file)
                if os.path.exists(norm_path):
                    try:
                        normalizer = load_protected_normalizer(norm_path, master_key)
                        self.robot.normalizer = normalizer
                        self.log(f"🔒 Protected normalizer loaded: {norm_file}")
                        return True
                    except Exception as e:
                        self.log(f"⚠️ Failed to load protected normalizer {norm_file}: {e}")
            
            self.log("⚠️ No protected normalizers found, using fallback")
            return False
            
        except Exception as e:
            self.log(f"❌ Error loading secure normalizers: {e}")
            return False
...
def main_gui():
    ...
    try:
        root = tk.Tk()
        root.title("RobotV7 - Trading System")
        ...
        # Se há usuário logado, configurar logout no fechamento
        if current_user_data and user_manager_instance:
            def on_closing():
                """Handle application closing with logout"""
                try:
        ...
        print("🚀 GUI iniciada com segurança")
        root.mainloop()
        
    except KeyboardInterrupt:
        print("\n[🛑] GUI interrompida pelo usuário")
        # Fazer logout se usuário estava logado
        if current_user_data and user_manager_instance:
            try:
                user_manager_instance.logout_user(current_user_data['username'])
            except:
...
def start_with_login():
    """🔐 Inicia sistema com login obrigatório - VERSÃO CORRIGIDA ANTI-LOOP"""
    print("🔐 RobotV7 Login - Sistema de Autenticação")
    print("=" * 50)
    
    # 🔧 CRITICAL FIX: Prevenir múltiplas execuções
    if hasattr(start_with_login, '_already_running'):
        print("⚠️ Sistema já está sendo executado!")
        return
    start_with_login._already_running = True
    
    try:
        # Criar janela de login RobotV7
        ...
        if login_window.login_successful and login_window.session_data:
            ...
            print(f"   Nível: {user_data['access_level']}")
            print(f"   Sistema: {user_data.get('system', 'robotv7')}")
            
            # 🔧 FIX: Garantir que janela de login seja fechada
            try:
                if login_window.root.winfo_exists():
                    login_window.root.destroy()
                print("🔐 Janela de login fechada")
            except Exception as e:
                print(f"⚠️ Erro ao fechar login: {e}")
            
            # Aguardar um momento para evitar conflitos
            import time
            time.sleep(0.5)
            
            # Iniciar robô com dados do usuário
            print("\n🚀 Iniciando RobotV7 Profissional...")
        ...
        # Reset flag para permitir reinicialização
        if hasattr(start_with_login, '_already_running'):
            delattr(start_with_login, '_already_running')


def main_gui_with_user(user_data):
    """🎨 Inicia GUI com dados do usuário logado"""
    
    print(f"👤 Usuário ativo: {user_data['username']} ({user_data['access_level']})")
    
    # Sistema de login apenas autentica - sem controle de lotes
    user_session = {
        'username': user_data['username'],
        'access_level': user_data['access_level'],
        'max_daily_trades': user_data.get('max_daily_trades', 25),
        'max_drawdown_percent': user_data.get('max_drawdown_percent', 12.0)
    }
    
    print(f"📊 Usuário autenticado:")
    print(f"   Max trades/dia: {user_session['max_daily_trades']}")
    print(f"   Max drawdown: {user_session['max_drawdown_percent']}%")
    print(f"   🎯 LOTES: Controlados diretamente na interface do robô")
    
    # Armazenar dados do usuário globalmente para logout
    global current_user_data, user_manager_instance
    current_user_data = user_data
    user_manager_instance = RobotV7UserManager()
    
    # Iniciar GUI normal - sem modificações que possam quebrar interface
    main_gui()
...
if __name__ == "__main__":
    ...
    # 🔧 CRITICAL FIX: Prevenir múltiplas execuções do script principal
    LOCK_FILE = "robotv7_running.lock"
    
    try:
        # Verificar se já está executando
        if os.path.exists(LOCK_FILE):
            print("⚠️ RobotV7 já está executando! Fechando...")
            sys.exit(1)
        ...
        elif len(sys.argv) > 1 and sys.argv[1] == "--no-login":
            # Modo de desenvolvimento - pular login
            print("🎨 Iniciando RobotV7 SEM LOGIN (modo dev)...")
            main_gui()
            
        else:
            # Modo padrão - com login
            print("🚀 Iniciando RobotV7 com login seguro...")
            start_with_login()
