...
from trading_framework.policies.two_head_v10_pure import TwoHeadV10Pure, get_v10_pure_kwargs, validate_v10_pure_policy
...
def calculate_v6_observation_space():
    ...
    market_features_real = 16  # V9 Optimus: market structure features
    positions_count = 3      # máximo de posições
    features_per_position = 9 # active, entry_price, current_price, etc.
    position_features = positions_count * features_per_position  # 27
    ...
    # 🚀 V8_4D OBSERVATION SPACE OTIMIZADO: 450D (45 features × 10 barras)
    seq_len = 10             # Reduzido: 10 barras históricas (50% menos)
    
    # V9 Features compactas (45 total):
    # Prioridade 1 - Price Action Core: 8 features (OHLC, Volume, Velocity, Spread, ATR)
    # Prioridade 2 - Technical Essentials: 12 features (RSI, MACD, BB, EMA, Stoch)  
    # Prioridade 3 - Market Structure: 10 features (S/R, Trend, Regime, Session)
    # Prioridade 4 - Multi-timeframe: 8 features (HTF trend, momentum, volatility)
    # Prioridade 5 - Positions & Portfolio: 7 features (positions info, PnL, drawdown)
    features_per_bar = 45    # V9 Optimus: features essenciais apenas
        ...
        if JSONL_AVAILABLE:
            try:
                self.jsonl_logger = create_real_time_logger(
                    base_path="D:/Projeto/avaliacoes/elegance",
                    buffer_size=2000,
                    flush_interval=5.0,
...
class MetricsCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if self.num_timesteps % 500000 == 0 and self.num_timesteps > 0:
            print(f"\n🚀 [AVALIAR_V8] Executando avaliação automática aos {self.num_timesteps:,} steps")
...
USE_COMPONENT_REWARDS = False  # DESATIVADO - V10 usa reward simples
...
BEST_PARAMS = {
    "learning_rate": 2.0e-05,                # 🔧 AJUSTE: Aumento para estabilizar KL
...
class TradingEnv(gym.Env):
    ...
    def __init__(self, df, window_size=20, is_training=True, initial_balance=None, trading_params=None):
        ...
        self.max_positions = 2  # V9 Optimus: apenas 2 posições
        self.current_positions = 0
        
        # 🚀 V8_4D ACTION SPACE 4D - ULTRA COMPACTO E EFICIENTE
        # 
        # ENTRY HEAD (2D):
        # [0] entry_decision: [0,2] Discrete (0=hold, 1=long, 2=short)
        # [1] entry_confidence: [0,1] Quality/risk fusion
        # 
        # MANAGEMENT HEAD (2D) - CONTROLE DE 2 POSIÇÕES:
        # [2] pos1_management: [-1,1] Posição 1: negativo=ajusta SL, positivo=ajusta TP
        # [3] pos2_management: [-1,1] Posição 2: negativo=ajusta SL, positivo=ajusta TP
        #
        # LÓGICA DE MAPEAMENTO:
        # - action < 0: ajustar SL (proteção) em ±0.5 pontos
        # - action > 0: ajustar TP (target) em ±0.5 pontos
        # - action ≈ 0: HOLD (nenhum ajuste)
    ...
    def step(self, action):
        ...
        """
        # 🚀 V8_4D: Action deve ser array de 4 dimensões - 4D ACTION SPACE
    ...
    def _calculate_reward_and_info(self, action, old_state):
        ...
        if len(action) >= 8:
            ...
            # [2-3] Management Head: Controle bidirecional de 2 posições (8D)
            ...
            # 🚀 V8_4D 4D ACTION SPACE REVOLUTION
            # [0] entry_decision: [0,2] Discrete (0=hold, 1=long, 2=short)
            raw_decision = float(action[0])
            if raw_decision < ACTION_THRESHOLD_LONG:
                entry_decision = 0  # HOLD
            ...
            # 🚀 FUNÇÃO V9 BIDIRECIONAL: Converter management em ajustes SL/TP
            def convert_management_to_sltp_adjustments(mgmt_value):
                ...
                if mgmt_value < 0:
                    ...
                    if mgmt_value < -0.5:
                        return (+0.5, 0)  # Afrouxar SL (mais risco)
                    else:
                        return (-0.5, 0)  # Apertar SL (menos risco)
                elif mgmt_value > 0:
            ...
            # Fallback para action spaces menores
            raw_decision = float(action[0]) if len(action) > 0 else 0.0
            if raw_decision < ACTION_THRESHOLD_LONG:
                entry_decision = 0  # HOLD
            elif raw_decision < ACTION_THRESHOLD_SHORT:
                entry_decision = 1  # LONG
            ...
            pos1_management = float(action[2]) if len(action) > 2 else 0.0
            pos2_management = float(action[3]) if len(action) > 3 else 0.0
        ...
        if entry_decision > 0 and len(self.positions) < self.max_positions:
            ...
            if entry_confidence < MIN_CONFIDENCE_THRESHOLD:
                ...
                # 🚀 V8_4D SISTEMA SL/TP BIDIRECIONAL
                pos_index = len(self.positions)  # Índice da nova posição
                
                # Escolher ajustes V9 baseado na posição (máximo 2 posições)
                if pos_index == 0:  # Primeira posição - usar pos1_management
                    sl_adjust = pos1_sl_adjust
                    tp_adjust = pos1_tp_adjust
                elif pos_index == 1:  # Segunda posição - usar pos2_management  
                    sl_adjust = pos2_sl_adjust
                    tp_adjust = pos2_tp_adjust
                else:  # Terceira posição não permitida em V9 (só 2 posições)
                    # Use pos2 como fallback (não deveria acontecer com max_positions=2)
                    sl_adjust = pos2_sl_adjust
                    tp_adjust = pos2_tp_adjust
                
                # V9: SL/TP base + ajustes bidirecionais
                BASE_SL_POINTS = 6.0  # SL base em pontos
                BASE_TP_POINTS = 9.0  # TP base em pontos
                
                # Aplicar ajustes bidirecionais (±0.5 pontos)
                final_sl_points = BASE_SL_POINTS + sl_adjust  # Ex: 6.0 + 0.5 = 6.5
                final_tp_points = BASE_TP_POINTS + tp_adjust  # Ex: 9.0 - 0.5 = 8.5
                
                # Converter pontos para diferença de preço
                sl_price_diff = final_sl_points * 1.0
                tp_price_diff = final_tp_points * 1.0
        ...
        if entry_decision > 0 and len(self.positions) < self.max_positions and entry_confidence < MIN_CONFIDENCE_THRESHOLD:
            ...
            for i, pos in enumerate(self.positions):
                if i < 2:  # V9 Optimus máximo 2 posições
                    # Mapear ajustes para posições específicas
                    if i == 0:
                        sl_adjust = pos1_sl_adjust
                        tp_adjust = pos1_tp_adjust
                    elif i == 1:
...
class AdvancedTrainingSystem:
    ...
    def _fix_lstm_initialization(self, model):
        ...
        try:
            ...
            # 🚨 PROTEÇÃO V9: NÃO afetar componentes já corrigidos pela V9
            policy_class_name = model.policy.__class__.__name__
            if "V9" in policy_class_name:
                print(f"🔧 V9 DETECTADA ({policy_class_name}) - PULANDO _fix_lstm_initialization para preservar correções V9")
                return
        ...
        try:
            ...
            if checkpoint_path_found and os.path.exists(checkpoint_path_found):
                ...
                try:
                    ...
                    # 🛑 VALIDAÇÃO CRÍTICA: Garantir TwoHeadV10Pure após resume
                    validate_v10_pure_policy(self.current_model.policy)
    ...
    def _create_model(self, env):
        ...
        if checkpoint_path:
            ...
            try:
                ...
                # 🛑 VALIDAÇÃO CRÍTICA: Garantir TwoHeadV10Pure após carregar checkpoint
                validate_v10_pure_policy(model.policy)
        ...
        # 🚀 CONFIGURAÇÕES ESPECIALIZADAS PARA TWOHEADV10PURE - V8 FUNCIONANDO PARA 4D
        model_config = {
            ...
            "seed": 42,
            "use_sde": False,         #  SDE DESABILITADO PARA V9
            "policy_kwargs": {
                **get_v10_pure_kwargs(),
            }
        ...
        self.logger.info(f"🚀 TwoHeadV10Pure: V8 Elegance FUNCIONAL para 4D Action Space")
        self.logger.info(f"✅ V10 Features: V8 Architecture + Single Action Head + 4D Actions")
        self.logger.info(f"✅ V10 Philosophy: USAR O QUE FUNCIONA - V8 transformer + arquitetura simplificada")
        self.logger.info("=" * 60)
        ...
        # 🛑 VALIDAÇÃO CRÍTICA: Garantir TwoHeadV10Pure
        validate_v10_pure_policy(model.policy)
