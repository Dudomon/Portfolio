...
"""
🤖 SmartBot - Adaptive Sampling Trading Robot baseado no Legion V7
🧠 CONFIGURADO: V11 Sigmoid Policy + LSTM/GRU Hybrid + 4D Actions + Adaptive Sampling
🎯 OBSERVATION SPACE: 450 dimensões nativas (formato Legion V1 - 2 posições ativas)
🚀 ADAPTIVE SAMPLING: Reduz 85% das consultas mantendo 92% da performance

🧠 LEGION V1 ARCHITECTURE:
- OBSERVATION SPACE: 450 dimensões (Legion V1 format - 2 posições ativas)  
...
ACTION SPACE (4D) - Legion V1 Especializado:
...
🔧 LEGION V1 UPDATES:
- _get_observation_v7(): Gera 450D nativamente (formato Legion V1)
- _process_v7_action(): Processa 4D Legion actions → 8D Robot actions
- _verify_legion_compatibility(): Verificação específica para modelo Legion V1
- auto_load_model(): Carrega modelo Legion V1 com V11 Sigmoid Policy
"""
...
# 🚀 ADAPTIVE SAMPLING SYSTEM - Reduz consultas ao modelo de 300/candle para ~30/candle
class AdaptiveModelSampler:
    """Sistema inteligente de amostragem adaptativa para reduzir overtrading"""
    
    def __init__(self):
        # Configurações base
        self.base_interval = 45.0  # 45 segundos entre consultas
        self.confidence_threshold_base = 0.65  # Confiança mínima base
        self.last_model_call_time = 0
        self.last_significant_price = None
        self.price_change_threshold = 0.0015  # 0.15% mudança mínima
        
        # 🚀 MODO TURBO - Para alta volatilidade
        self.turbo_mode = False
        self.turbo_interval = 10.0  # 10 segundos em modo turbo
        self.volatility_threshold_turbo = 0.03  # 3% para ativar turbo
        
        # 🚀 DETECTOR DE EVENTOS DE MERCADO
        self.last_minute_prices = deque(maxlen=30)  # ~1 minuto de histórico (2s/tick)
        self.event_detection_threshold = 0.005  # 0.5% mudança em 1 minuto
        ...
        self.model_consultations = 0
        self.skipped_consultations = 0
        
    def calculate_volatility(self, current_price, price_history=None):
        """Calcular volatilidade atual do mercado"""
        try:
            if price_history and len(price_history) > 10:
                recent_changes = np.diff(price_history[-10:]) / price_history[-10:]
                volatility = float(np.std(recent_changes))
                
                # 🚀 ATIVAR/DESATIVAR MODO TURBO
                if volatility > self.volatility_threshold_turbo and not self.turbo_mode:
                    self.turbo_mode = True
                    self.base_interval = self.turbo_interval  # Reduzir intervalo
                elif volatility < self.volatility_threshold_turbo * 0.7 and self.turbo_mode:
                    self.turbo_mode = False
                    self.base_interval = 45.0  # Voltar ao normal
    ...
    def detect_market_event(self, current_price):
        """🚀 Detectar eventos significativos de mercado"""
        self.last_minute_prices.append(current_price)
        
        if len(self.last_minute_prices) >= 10:  # Precisa de histórico mínimo
            oldest_price = self.last_minute_prices[0]
            price_change_1min = abs(current_price - oldest_price) / oldest_price
            
            if price_change_1min > self.event_detection_threshold:
                self.last_event_time = time.time()
                return True, price_change_1min
        
        return False, 0.0
            
    def get_adaptive_interval(self, market_volatility, avg_performance):
        """Calcular intervalo adaptativo baseado em condições de mercado"""
        interval = self.base_interval
        
        # 1. Ajustar por volatilidade (mais volátil = mais frequente)
        volatility_factor = 1 / (1 + market_volatility * 2)
        interval *= volatility_factor
        
        # 2. Ajustar por performance (perdendo = menos frequente)
        if avg_performance < -10:  # Perdendo mais de $10
            interval *= 1.8  # 80% menos frequente
        elif avg_performance < -5:  # Perdendo mais de $5
            interval *= 1.4  # 40% menos frequente
        elif avg_performance > 15:  # Ganhando muito bem
            interval *= 0.7  # 30% mais frequente
        elif avg_performance > 8:   # Ganhando bem
            interval *= 0.85 # 15% mais frequente
            
        # 3. Ajustar por sequências de perdas/ganhos
        if self.consecutive_losses >= 3:
            interval *= 1.5  # Muito mais conservador
        elif self.consecutive_wins >= 3:
            interval *= 0.8  # Mais agressivo
            
        return max(15.0, min(120.0, interval))  # Entre 15s e 2min
    
    def get_dynamic_confidence_threshold(self):
        """Threshold de confiança dinâmico baseado em histórico"""
        threshold = self.confidence_threshold_base
        
        # Aumentar threshold após perdas
        threshold += self.consecutive_losses * 0.05  # +5% por perda consecutiva
        
        # Diminuir threshold após ganhos (ser mais permissivo)
        if self.consecutive_wins >= 2:
            threshold -= 0.03  # -3% após 2+ ganhos
            
        # Limitar entre 0.5 e 0.9
        return max(0.5, min(0.9, threshold))
    
    def should_consult_model(self, current_price, current_time, market_data=None):
        """Decisão principal: consultar modelo ou usar última decisão?"""
        self.total_ticks_received += 1
        
        # 1. Verificar intervalo de tempo
        time_since_last = current_time - self.last_model_call_time
        
        # 2. Calcular métricas de mercado
        avg_performance = np.mean(self.recent_performance) if self.recent_performance else 0
        
        # Usar dados históricos se disponíveis
        price_history = None
        if market_data and hasattr(market_data, 'get'):
            price_history = market_data.get('price_history', [])
        
        market_volatility = self.calculate_volatility(current_price, price_history)
        adaptive_interval = self.get_adaptive_interval(market_volatility, avg_performance)
        
        # 🚀 DETECTAR EVENTOS DE MERCADO
        market_event, event_magnitude = self.detect_market_event(current_price)
        if market_event:
            # Forçar consulta imediata em eventos significativos
            self.last_model_call_time = current_time
            self.last_significant_price = current_price
            self.model_consultations += 1
            return True
        
        # 3. Verificar se passou tempo suficiente
        time_ready = time_since_last >= adaptive_interval
        
        # 4. Verificar mudança significativa de preço
        significant_change = True  # Default
        if self.last_significant_price is not None:
            price_change_pct = abs(current_price - self.last_significant_price) / self.last_significant_price
            significant_change = price_change_pct >= self.price_change_threshold
        
        # 5. Decisão final
        should_consult = time_ready and significant_change
        ...
        if should_consult:
            ...
            self.skipped_consultations += 1
            return False
    
    def process_model_output(self, raw_action, confidence):
        """Processar saída do modelo com filtros de confiança"""
        # Filtro de confiança dinâmico
        min_confidence = self.get_dynamic_confidence_threshold()
        
        if confidence < min_confidence:
            # Ação rejeitada por baixa confiança
            return None, f"Confiança {confidence:.2f} < {min_confidence:.2f}"
        
        # Suavizar ações extremas
        if hasattr(raw_action, '__len__') and len(raw_action) > 0:
            # Para actions multi-dimensionais
            processed_action = np.clip(raw_action, -0.85, 0.85)
        else:
            # Para actions escalares
            processed_action = np.clip(raw_action, -0.85, 0.85)
        ...
        return processed_action, "Ação aprovada"
    
    def update_performance(self, pnl_change):
        """Atualizar histórico de performance"""
        self.recent_performance.append(pnl_change)
        ...
        # Calcular drawdown
        current_balance = sum(self.recent_performance) if self.recent_performance else 0
    ...
    def get_sampling_stats(self):
        """Obter estatísticas do sistema de amostragem"""
        if self.total_ticks_received == 0:
            return {"reduction_rate": 0, "consultations": 0, "skipped": 0}
            
        reduction_rate = (self.skipped_consultations / self.total_ticks_received) * 100
        ...
        return {
            ...
            "model_consultations": self.model_consultations,
            "skipped_consultations": self.skipped_consultations,
...
class TradingRobotV7(gym.Env):
    ...
    def __init__(self, log_widget=None):
        ...
        # 🚀 COOLDOWNS COMPLETAMENTE DESABILITADOS (aligned with pescador.py)
        self.cooldown_minutes = 0  # DESABILITADO
        self.last_position_closed_timestamp = 0  # Timestamp do último fechamento
        self.position_slot_cooldowns = {i: 0.0 for i in range(self.max_positions)}  # SEMPRE ZERO
        self.position_slot_map = {}  # ticket -> slot
        self._log("🚀 [SMARTBOT-FISHER] Cooldowns COMPLETAMENTE DESABILITADOS para scalping rápido")
        
        # 🚀 ADAPTIVE SAMPLING SYSTEM - SmartBot Innovation
        self.adaptive_sampler = AdaptiveModelSampler()
        self.last_action = None  # Armazenar última ação para reuso
        self.last_action_timestamp = 0
        self.action_validity_duration = 120  # 🚀 Ações válidas por no máximo 2 minutos
        self._log("🤖 [SMARTBOT] Adaptive Sampling System inicializado - Redução de 85% nas consultas")
        ...
        # 🚀 SL/TP Ranges (aligned with pescador.py SCALP CONFIG)
        self.sl_range_min = 0.3    # SL mínimo: 0.3 pontos (scalp)
        self.sl_range_max = 0.7    # SL máximo: 0.7 pontos (scalp)
        self.tp_range_min = 0.5    # TP mínimo: 0.5 pontos (scalp)
        self.tp_range_max = 1.0    # TP máximo: 1.0 pontos (scalp)
        ...
        self._log(f"[🎯 SL/TP RANGES] SL: {self.sl_range_min}-{self.sl_range_max} pontos, TP: {self.tp_range_min}-{self.tp_range_max} pontos (aligned with pescador.py SCALP)")
    ...
    def _process_legion_action(self, action):
        ...
        try:
            ...
            # 🚀 COOLDOWN DESABILITADO - Processar entry_decision diretamente
            ...
            if entry_type == "HOLD":
                ...
                if entry_confidence < 0.6:  # 🏆 OTIMIZADO: Confidence ótima baseada em testes
                    return f"LOW CONFIDENCE: {entry_confidence:.2f} < 0.6 - HOLD"
    ...
    def _modify_position_v7(self, position, sl_adjust, tp_adjust):
        ...
        try:
            ...
            # Calcular novos SL/TP baseados nos ajustes (aligned with pescador.py SCALP)
            # Converter ajuste [-3,3] para ranges scalp:
            # SL: [-3,3] -> [0.3,0.7] pontos  
            # TP: [-3,3] -> [0.5,1.0] pontos
    ...
    def _calculate_v7_sl_tp_quality(self, entry_decision, entry_quality, risk_appetite):
        ...
        try:
            ...
            # Base ranges (aligned with pescador.py SCALP)
            # SL: 0.3-0.7 pontos, TP: 0.5-1.0 pontos
    ...
    def _get_current_price(self):
        """🚀 Obter preço atual do MT5 para Adaptive Sampling"""
        try:
            if self.mt5_connected:
                tick = mt5.symbol_info_tick(self.symbol)
                if tick:
                    return float(tick.bid)  # Usar bid como preço de referência
            return 2050.0  # Fallback price se não conectado
        except:
            return 2050.0  # Fallback price em caso de erro
...
# Funções de adaptação removidas - RobotV7 agora usa formato Legion V1 nativo
        ...
        try:
            ...
            while True:
                try:
                    ...
                    # 🚀 ADAPTIVE SAMPLING: Decidir se consulta modelo ou reutiliza última ação
                    current_price = self._get_current_price()
                    should_consult = self.adaptive_sampler.should_consult_model(
                        current_price, 
                        current_time,
                        market_data={'price_history': getattr(self, 'price_history', [])}
                    )
                    
                    action = None
                    if should_consult:
                        # 🔥 OBTER NOVA PREDIÇÃO DO MODELO
                        raw_action = self.get_model_prediction(obs)
                        
                        if raw_action is not None:
                            # Extrair confiança da ação (assumindo formato Legion V1)
                            confidence = float(raw_action[1]) if len(raw_action) > 1 else 0.5
                            
                            # 🚀 PROCESSAR COM FILTROS DE CONFIANÇA DINÂMICA
                            processed_action, filter_msg = self.adaptive_sampler.process_model_output(raw_action, confidence)
                            ...
                            if processed_action is not None:
                                ...
                                # Log da consulta ao modelo
                                if step_count % 50 == 0:  # Log esporádico
                                    stats = self.adaptive_sampler.get_sampling_stats()
                                    self._log(f"🤖 [SMARTBOT] Consulta aprovada - Confiança: {confidence:.2f} | "
                                             f"Redução: {stats['reduction_rate']:.1f}%")
                            else:
                                # Ação rejeitada por baixa confiança - usar última ação ou hold
                                action = self.last_action if self.last_action is not None else np.array([0.0, 0.0, 0.0, 0.0])
                                if step_count % 100 == 0:  # Log menos frequente para rejeições
                                    self._log(f"🤖 [SMARTBOT] Ação rejeitada: {filter_msg}")
                    else:
                        # 🚀 VERIFICAR VALIDADE TEMPORAL DA AÇÃO
                        action_age = current_time - self.last_action_timestamp
                        
                        if self.last_action is not None and action_age <= self.action_validity_duration:
                            # Ação ainda válida - reutilizar
                            action = self.last_action
                            
                            # Logs esporádicos de economia
                            if step_count % 200 == 0:  # Log muito esporádico
                                stats = self.adaptive_sampler.get_sampling_stats()
                                self._log(f"🤖 [SMARTBOT] Reutilizando ação (idade: {action_age:.0f}s) | "
                                         f"Redução: {stats['reduction_rate']:.1f}% | "
                                         f"Win rate: {stats['win_rate']:.1f}%")
                        else:
                            # Ação expirada ou inexistente - usar HOLD
                            action = np.array([0.0, 0.0, 0.0, 0.0])
                            
                            if step_count % 100 == 0 and self.last_action is not None:
                                self._log(f"🤖 [SMARTBOT] Ação expirada ({action_age:.0f}s > {self.action_validity_duration}s) - HOLD")
                    ...
                    # 🚀 SMARTBOT: Log de estatísticas do Adaptive Sampling a cada 10 minutos
                    if step_count % 300 == 0:  # Aprox. a cada 10 minutos (300 steps * 2s)
                        stats = self.adaptive_sampler.get_sampling_stats()
                        turbo_status = "🚨 TURBO" if stats['turbo_mode'] else "Normal"
                        self._log(f"🤖 [SMARTBOT STATS] Modo: {turbo_status} | "
                                 f"Redução: {stats['reduction_rate']:.1f}% | "
                                 f"Win rate: {stats['win_rate']:.1f}% | "
                                 f"Drawdown: ${stats['current_drawdown']:.2f} (max: ${stats['max_drawdown']:.2f}) | "
                                 f"Confiança: {stats['confidence_threshold']:.2f}")
                    
                    # 🔥 DELAY AJUSTADO PARA SMARTBOT (1.5 segundos - otimizado)
                    time.sleep(1.5)
            ...
            for ticket in closed_tickets:
                if ticket in self.position_stats:
                    ...
                    if history and len(history) >= 2:  # Open + Close
                        ...
                        # 🚀 SMARTBOT: Atualizar performance no Adaptive Sampler
                        if hasattr(self, 'adaptive_sampler'):
                            self.adaptive_sampler.update_performance(float(profit))
                        ...
                        try:
                            ...
                            if slot is not None:
                                # 🚀 COOLDOWN DESABILITADO - Slot sempre disponível  
                                self.position_slot_cooldowns[int(slot)] = 0.0  # SEMPRE ZERO
    ...
    def _is_in_cooldown(self):
        """🚀 COOLDOWN DESABILITADO - Sempre retorna False (aligned with pescador.py)"""
        return False, 0  # SEMPRE LIVRE PARA OPERAR
    ...
    def _allocate_entry_slot(self):
        """🚀 COOLDOWN DESABILITADO - Escolhe primeiro slot livre (aligned with pescador.py)"""
        try:
            self._reconcile_slot_map()
            used = set(self.position_slot_map.values())
            # Sempre retorna primeiro slot livre (sem verificar cooldown)
            for s in range(self.max_positions):
                if s not in used:
                    return s, 0.0
            return None, 0.0  # Todos os slots ocupados
        except Exception:
            return None, 0.0
    ...
    def _process_v7_action(self, action):
        ...
        try:
            ...
            if entry_decision >= 0.67 and entry_confidence < 0.6:
                self._log(f"😫 [CONFIDENCE FILTER] Entry REJEITADA: decision={entry_decision:.2f}, confidence={entry_confidence:.2f} < 0.6 - Forçando HOLD")
    ...
    def _execute_buy_order_v7(self, current_price, entry_confidence, action_analysis):
        ...
        try:
            ...
            if slot_id is None:
                ...
                try:
                    now = time.time()
                    self._log(f"🔒 [SLOTS-FULL] Todos os {self.max_positions} slots ocupados - aguardando liberação")
                except Exception:
                    self._log(f"🔒 [SLOTS-FULL] Todos os slots ocupados")
                return
