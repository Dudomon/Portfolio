...
class TradingRobotV7(gym.Env):
    ...
    def _initialize_historical_data_v7(self):
        ...
        try:
            ...
            # 3. breakout_strength - CONSTANTE (cherry.py linha 4371-4376)
            # Cherry usa: base_val = (hash(feature_name) % 100) / 200.0 + 0.2
            breakout_strength_val = (hash('breakout_strength') % 100) / 200.0 + 0.2
            self.historical_df['breakout_strength'] = breakout_strength_val

            # 4. trend_consistency - CONSTANTE (cherry.py linha 4371-4376)
            trend_consistency_val = (hash('trend_consistency') % 100) / 200.0 + 0.2
            self.historical_df['trend_consistency'] = trend_consistency_val

            # 5. support_resistance - CONSTANTE (cherry.py linha 4371-4376)
            support_resistance_val = (hash('support_resistance') % 100) / 200.0 + 0.2
            self.historical_df['support_resistance'] = support_resistance_val

            # 6. volatility_regime (cherry.py linha 4340-4348)
            ...
            # Mapear para regimes específicos
            ...
            # 7. market_structure - CONSTANTE (cherry.py linha 4371-4376)
            market_structure_val = (hash('market_structure') % 100) / 200.0 + 0.2
            self.historical_df['market_structure'] = market_structure_val
    ...
    def _generate_basic_v7_components_robot(self, current_price):
        """🍒 V7 components REDUZIDOS (só 2 features essenciais) - Robot version"""
        try:
            # 1. Market Regime (volatility-based) usando dados históricos
            if len(self.historical_df) >= 20:
                recent_closes = self.historical_df[self.feature_columns[0]].tail(20)  # Usar primeira feature como proxy para close
                volatility = recent_closes.pct_change().std() if len(recent_closes) > 1 else 0.01
            ...
            if len(self.historical_df) >= 10:
                old_close = self.historical_df[self.feature_columns[0]].iloc[-10] if len(self.historical_df) >= 10 else current_price
                momentum = (current_price - old_close) / old_close if old_close > 0 else 0.0
    ...
    def _get_observation_v7(self):
        ...
        try:
            ...
            # TODO: Implementar V7 embeddings completos - por ora usar básicos expandidos
            basic_intelligent = self._generate_basic_v7_components_robot(current_price)  # 2 features
            intelligent_features = np.concatenate([
                basic_intelligent,           # [0-1] market_regime, trend_strength
                np.full(5, 0.4)             # [2-6] Placeholders para V7 embeddings (horizon, risk, regime, lookahead)
            ])  # Total: 7 features
                ...
                if position.sl > 0:
                    ...
                    if position.type == mt5.POSITION_TYPE_BUY:  # LONG
                        ...
                        if proposed_sl > position.sl and proposed_sl < current_price - 5.0:
                            new_sl = proposed_sl
                            sl_changed = True
                            self._log(f"🛡️ [SL UP] #{position.ticket}: ${position.sl:.2f} → ${new_sl:.2f} (+{sl_movement_points:.1f}pts)")
                        else:
                        ...
                        if proposed_sl < position.sl and proposed_sl > current_price + 5.0:
                            new_sl = proposed_sl
                            sl_changed = True
                            self._log(f"🛡️ [SL DOWN] #{position.ticket}: ${position.sl:.2f} → ${new_sl:.2f} (-{sl_movement_points:.1f}pts)")
                        else:
                    ...
                    if position.type == mt5.POSITION_TYPE_BUY:  # LONG
                        ...
                        if proposed_tp > current_price + 3.0:
                            ...
                            if potential_pnl <= 100:  # Respect $100 cap
                                new_tp = proposed_tp
                                tp_changed = True
                                self._log(f"🎯 [TP UP] #{position.ticket}: ${position.tp:.2f} → ${new_tp:.2f} (PnL: ${potential_pnl:.2f})")
                            else:
                        ...
                        if proposed_tp < current_price - 3.0:
                            ...
                            if potential_pnl <= 100:  # Respect $100 cap
                                new_tp = proposed_tp
                                tp_changed = True
                                self._log(f"🎯 [TP DOWN] #{position.ticket}: ${position.tp:.2f} → ${new_tp:.2f} (PnL: ${potential_pnl:.2f})")
                            else:
                                self._log(f"🚫 [TP BLOCKED] #{position.ticket}: Would exceed $100 cap (${potential_pnl:.2f})")
                        else:
                            self._log(f"🚫 [TP BLOCKED] #{position.ticket}: TP too close to current price")
...
# Funções de adaptação removidas - RobotV7 agora usa formato Legion V1 nativo
    ...
    def _process_v7_action(self, action):
        ...
        try:
            ...
            # 🚀 FIX: Usar EXATAMENTE o mesmo mapeamento do SILUS.PY (SEM int()!)
