...
from trading_framework.policies.two_head_v11_sigmoid import TwoHeadV11Sigmoid, get_v11_sigmoid_kwargs, validate_v11_sigmoid_policy
...
BEST_PARAMS = {
    ...
    "clip_range": 0.2,                       # 🔧 SIMPLES: PPO padrão
    ...
    "target_kl": 0.01,                       # 🔧 SIMPLES: KL divergence controlado
    "policy_kwargs": {
...
class TradingEnv(gym.Env):
    ...
    def __init__(self, df, window_size=20, is_training=True, initial_balance=None, trading_params=None):
        ...
        # 🎯 ACTIVITY ENHANCEMENT SYSTEM - REATIVADO para timeout de posições
        if is_training:
            self.activity_system = create_activity_enhancement_system(position_timeout=300)  # 🔧 FIX: 300 candles = 5h em timeframe 1min
            print(f"[ACTIVITY SYSTEM] ✅ Activity Enhancement REATIVADO - Timeout 5h para posições")
            print(f"[PHILOSOPHY] 🎯 Qualidade + timeout automático para evitar posições eternas")
        else:
    ...
    def _calculate_missing_features(self, missing_features):
        ...
        close_1m = self.df['close_1m'].values  # .values para velocidade máxima
        high_1m = self.df.get('high_1m', close_1m).values
        low_1m = self.df.get('low_1m', close_1m).values
        volume_1m = self.df.get('tick_volume_1m', self.df.get('real_volume_1m', np.full(len(self.df), 1000)))
        ...
        if 'volume_momentum' in missing_features:
            ...
            volume_momentum = np.where(volume_sma_20 > 0, (volume_1m - volume_sma_20) / volume_sma_20, 0.001)  # 🔧 Valor pequeno ao invés de zero
            ...
            if feature_name in missing_features or feature_name not in self.df.columns:
                ...
                if feature_name == 'volume_momentum':
                    ...
                    if 'volume_momentum' not in self.df.columns:
                        volume_sma_20 = pd.Series(volume_1m).rolling(window=20).mean().fillna(volume_1m[0]).values
                        volume_momentum = np.where(volume_sma_20 > 0, (volume_1m - volume_sma_20) / volume_sma_20, 0.25)
                        self.df.loc[:, 'volume_momentum'] = volume_momentum
                    ...
                    # Se a feature não pode ser calculada com dados reais, usar valor variável
                    # Gerar valores baseados no hash do nome da feature para evitar constantes
                    base_val = (hash(feature_name) % 100) / 200.0 + 0.2  # Range 0.2-0.7
    ...
    def _generate_fast_enhanced_features(self, step):
        """🍒 CHERRY ENHANCED FEATURES - Reduzidas para 8 features essenciais"""
        try:
            if step >= len(self.df):
            ...
            # 🍒 CHERRY: Apenas 8 features mais importantes
            return np.array([
                # Pattern recognition (2 features)
                range_pattern, volume_pattern,

                # Regime detection (2 features)
                volatility_regime, volume_regime,

                # Risk metrics (2 features)
                range_risk, volume_risk,

                # Temporal context (2 features)
                session_context, trend_context
            ], dtype=np.float32)
        except:
            return np.full(8, 0.3, dtype=np.float32)

    # 🍒 CHERRY ENHANCED FEATURES - ORDER FLOW + TIME CONTEXT + RISK MANAGEMENT
    ...
    def _get_single_bar_features(self, step):
        ...
        for i in range(len(self.positions), self.max_positions):
            ...
            positions_obs[i, :] = [
                ...
                current_price_norm * 0.99,                        # [6] SL baseado no preço atual
                current_price_norm * 1.01,                        # [7] TP baseado no preço atual
                (hash(f"{step}_{i}_type") % 3 - 1) * 0.1          # [8] Type variável (-0.1, 0, 0.1)
            ]
        ...
        # 🧠 COMPONENTES INTELIGENTES V7: FEATURES REAIS AO INVÉS DE LIXO ESTÁTICO
        try:
            # Usar V7 components REAIS para este step específico
            intelligent_features = self._generate_intelligent_components_for_step(step)
        except:
            # Fallback: V7 components básicos se der erro
            intelligent_features = self._generate_basic_v7_components(step)
        
        # 🍒 CHERRY ENHANCED: ORDER FLOW + TIME CONTEXT + RISK MANAGEMENT
        # Substituindo features genéricas por análises específicas de mercado
        order_flow_features = self._generate_order_flow_features(step)         # 4 features (substitui microstructure)
        volatility_features = self._generate_fast_volatility_features(step)    # 5 features (mantido)
        time_context_features = self._generate_time_context_features(step)     # 3 features (substitui correlation)
        risk_management_features = self._generate_risk_management_features(step) # 3 features (substitui momentum)
        # enhanced_features removidas para ajustar total para 45


        # 🍒 CHERRY: Combinar features enhanced - AJUSTAR PARA 45 FEATURES
        single_bar_obs = np.concatenate([
            market_data.flatten(),           # 16 features (market data)
            positions_obs.flatten(),         # 18 features (2 pos × 9) - REVERTIDO
            intelligent_features,            # 2 features (V7 components essenciais)
            order_flow_features,             # 4 features (ORDER FLOW)
            volatility_features[:5]          # 5 features (mantido)
            # time_context_features removido para ajustar total
            # risk_management_features removido para ajustar total
        ])
        # TOTAL: 16 + 18 + 2 + 4 + 5 = 45 features EXATO!
    ...
    def _generate_intelligent_components_for_step(self, step):
        """
        🧠 GERAÇÃO DE V7 COMPONENTS REAIS PARA UM STEP ESPECÍFICO
        Retorna 37 features de inteligência real ao invés de valores estáticos
        """
        # Temporariamente ajustar current_step para calcular components do step específico
        original_step = self.current_step
        self.current_step = step

        try:
            # Gerar components V7 reais
            components = self._generate_intelligent_components()

            # Converter para array flat de 37 features
            flat_components = self._flatten_intelligent_components(components)
    ...
    def _generate_basic_v7_components(self, step):
        """
        🍒 CHERRY: V7 components REDUZIDOS (só 2 features essenciais)
        Mantém apenas market_regime e volatility_context
        """
        try:
            if step < 20 or step >= len(self.df):
                # Valores seguros para início/fim de episódio
                return np.full(2, 0.4, dtype=np.float32)

            # Calcular apenas 2 features essenciais
            close_price = self.df['close_1m'].iloc[step] if 'close_1m' in self.df.columns else 2000.0

            # 1. Market Regime (volatility-based)
            if step >= 20:
                recent_closes = self.df['close_1m'].iloc[step-20:step] if 'close_1m' in self.df.columns else [2000.0] * 20
                volatility = np.std(recent_closes) / np.mean(recent_closes) if len(recent_closes) > 0 else 0.01
                market_regime = np.clip(volatility / 0.02, 0.1, 0.9)  # Normalize volatility
            else:
                market_regime = 0.4

            # 2. Trend Strength (momentum-based)
            if step >= 10:
                momentum = (close_price - self.df['close_1m'].iloc[step-10]) / self.df['close_1m'].iloc[step-10] if 'close_1m' in self.df.columns else 0.0
                trend_strength = np.clip(abs(momentum) * 100, 0.1, 0.9)  # Absolute momentum
            else:
                trend_strength = 0.4

            return np.array([market_regime, trend_strength], dtype=np.float32)

        except Exception as e:
            # Last resort: valores seguros mas não estáticos
            return np.full(2, 0.3, dtype=np.float32)
    
    # FUNÇÕES DUPLICADAS REMOVIDAS - USAR APENAS _get_single_bar_features()

    
    def _process_unified_sltp_management(self, pos, sl_adjust, tp_adjust, current_price, pos_index):
        """
        🎯 SISTEMA UNIFICADO DE GESTÃO SL/TP - SUBSTITUI TRAILING STOP COMPLEXO

        INTERPRETAÇÃO CLARA E CONSISTENTE:
        - sl_adjust [-0.5, +0.5]: Ajuste DIRETO do SL em ±1 ponto
          - Negativo: Apertar SL (mais proteção, SL sobe para LONG)
          - Positivo: Afrouxar SL (menos proteção, SL desce para LONG)
        - tp_adjust [-0.5, +0.5]: Ajuste DIRETO do TP em ±1 ponto
          - Negativo: TP conservador (TP mais próximo)
          - Positivo: TP ambicioso (TP mais distante)

        DISCIPLINA: Nunca violar limites de risco iniciais
        """
        result = {
            'sl_adjusted': False,
            'tp_adjusted': False,
        ...
        # 🛑 PnL CAP - Force close em $100 (manter funcionalidade existente)
        if current_pnl >= 100:
            print(f"🛑 [PnL CAP] Force closing position at ${current_pnl:.2f}")
            self._close_position(pos, self.current_step)
            result['pnl_cap_reached'] = True
            result['action_taken'] = True
            return result

        # 🛡️ INICIALIZAR DISCIPLINA SL/TP
        discipline = SLTPDiscipline(pos)

        # 🎯 CONVERTER SINAIS EM MOVIMENTOS (±0.5 → ±1.0 ponto)
        sl_movement_points, tp_movement_points = convert_unified_signals_to_movements(sl_adjust, tp_adjust)

        # 🔧 PROCESSAR AJUSTE DE SL SE SINAL SIGNIFICATIVO
        if abs(sl_adjust) >= UNIFIED_SLTP_CONFIG['signal_threshold']:
            current_sl = pos.get('sl', 0)
            if current_sl > 0:
                # Calcular novo SL baseado no tipo de posição
                if pos['type'] == 'long':
                    # LONG: movimento negativo = apertar (subir SL), positivo = afrouxar (baixar SL)
                    new_sl = current_sl - sl_movement_points
                else:  # short
                    # SHORT: movimento negativo = apertar (baixar SL), positivo = afrouxar (subir SL)
                    new_sl = current_sl + sl_movement_points

                # 🛡️ VALIDAR COM DISCIPLINA
                if discipline.validate_sl_movement(new_sl):
                    result['position_updates']['sl'] = new_sl
                    result['sl_adjusted'] = True
                    result['action_taken'] = True
                    result['discipline_info']['sl_change_points'] = sl_movement_points
                    result['discipline_info']['sl_change_usd'] = sl_movement_points * UNIFIED_SLTP_CONFIG['point_value']
                else:
                    result['sl_blocked'] = True
                    result['discipline_info']['sl_blocked_reason'] = f"Violaria disciplina: new_sl={new_sl:.4f}, limit={discipline.worst_allowed_sl:.4f}"

        # 🎯 PROCESSAR AJUSTE DE TP SE SINAL SIGNIFICATIVO
        if abs(tp_adjust) >= UNIFIED_SLTP_CONFIG['signal_threshold']:
            current_tp = pos.get('tp', 0)
            if current_tp > 0:
                # Calcular novo TP baseado no tipo de posição
                if pos['type'] == 'long':
                    # LONG: movimento negativo = TP próximo (baixar), positivo = TP distante (subir)
                    new_tp = current_tp + tp_movement_points
                else:  # short
                    # SHORT: movimento negativo = TP próximo (subir), positivo = TP distante (baixar)
                    new_tp = current_tp - tp_movement_points

                # 🛡️ VALIDAR COM DISCIPLINA
                if discipline.validate_tp_movement(new_tp, current_price):
                    result['position_updates']['tp'] = new_tp
                    result['tp_adjusted'] = True
                    result['action_taken'] = True
                    result['discipline_info']['tp_change_points'] = tp_movement_points
                    result['discipline_info']['tp_change_usd'] = tp_movement_points * UNIFIED_SLTP_CONFIG['point_value']
                else:
                    result['tp_blocked'] = True
                    result['discipline_info']['tp_blocked_reason'] = f"TP inválido: new_tp={new_tp:.4f}, current_price={current_price:.4f}"

        # 📊 INFORMAÇÕES PARA REWARD SYSTEM (compatibilidade)
        if result['sl_adjusted'] or result['tp_adjusted']:
            result['discipline_info']['unified_system'] = True
            result['discipline_info']['sl_signal'] = sl_adjust
            result['discipline_info']['tp_signal'] = tp_adjust
        ...
        try:
            ...
            if isinstance(market_regime, dict):
                market_regime = np.array([
                    ...
                    1.0 if market_regime.get('regime', 'unknown') == 'trending' else (0.2 + (current_idx % 30) / 100.0)  # Dinâmico 0.2-0.5
                ], dtype=np.float32)
            elif not isinstance(market_regime, np.ndarray):
                market_regime = np.array([0.3 + (current_idx % 50) / 100.0, 0.0, 0.2 + (current_idx % 30) / 100.0], dtype=np.float32)
    ...
    def _calculate_reward_and_info(self, action, old_state):
        ...
        if entry_decision > 0 and len(self.positions) < self.max_positions and available_slot is not None:
            ...
            if entry_confidence < MIN_CONFIDENCE_THRESHOLD:
                ...
                # 🎯 SISTEMA UNIFICADO DE CRIAÇÃO: Modelo escolhe SL/TP baseado em contexto
                initial_sl_points, initial_tp_points = convert_creation_signals_to_initial_sltp(sl_adjust, tp_adjust)

                # Converter pontos para diferença de preço (1 ponto = $1.00)
                sl_price_diff = initial_sl_points * UNIFIED_SLTP_CONFIG['point_value']
                tp_price_diff = initial_tp_points * UNIFIED_SLTP_CONFIG['point_value']
                ...
                # 🛡️ ARMAZENAR SL/TP POINTS PARA SL DISCIPLINE
                position['sl_points'] = initial_sl_points  # Armazenar pontos iniciais para discipline
                position['tp_points'] = initial_tp_points  # Armazenar pontos iniciais para discipline
        ...
        if entry_decision > 0 and len(self.positions) < self.max_positions and entry_confidence < 0.2:  # Usar mesmo threshold
            ...
            for i, pos in enumerate(self.positions):
                if i < 2:  # 🔬 CIRÚRGICO: Máximo 2 posições
                    ...
                    # 🎯 SISTEMA UNIFICADO SL/TP - Substitui trailing stop complexo
                    sltp_result = self._process_unified_sltp_management(
                        pos, sl_adjust, tp_adjust, current_price, i
                    )

                    # Aplicar mudanças validadas pela disciplina
                    if sltp_result['action_taken']:
                        pos.update(sltp_result['position_updates'])

                        # Marcar informações para reward system (compatibilidade)
                        if sltp_result['sl_adjusted']:
                            pos['sl_adjusted'] = True
                            pos['sl_adjustment_step'] = self.current_step
                            pos['sl_adjusted_this_step'] = True

                        if sltp_result['tp_adjusted']:
                            pos['tp_adjusted'] = True
                            pos['tp_adjustment_step'] = self.current_step
                            pos['tp_adjusted_this_step'] = True

                        # Armazenar info de disciplina para debugging
                        if sltp_result['discipline_info']:
                            pos['last_discipline_info'] = sltp_result['discipline_info']

                    # Registrar bloqueios de disciplina (para monitoring)
                    if sltp_result['sl_blocked'] or sltp_result['tp_blocked']:
                        pos.setdefault('discipline_blocks', 0)
                        pos['discipline_blocks'] += 1
...
# 🎯 CONFIGURAÇÃO SISTEMA UNIFICADO SL/TP - SUBSTITUIRÁ O SISTEMA ANTIGO
UNIFIED_SLTP_CONFIG = {
    'point_value': 1.00,        # 1 ponto = $1.00 USD (com 0.01 lotes)
    'movement_per_signal': 1.0, # ±0.5 signal → ±1.0 ponto movimento
    'min_movement': 1.0,        # Movimento mínimo: 1 ponto = $1.00
    'max_adjustment': 1.0,      # Ajuste máximo: 1 ponto = $1.00 por vez
    'sl_min_distance': 2.0,     # SL mínimo: 2 pontos = $2.00 da entrada
    'tp_min_distance': 3.0,     # TP mínimo: 3 pontos = $3.00 da entrada
    'signal_threshold': 0.1,    # Threshold mínimo para processar sinal
}

class SLTPDiscipline:
    """
    🛡️ DISCIPLINA UNIFICADA: Regras consistentes para todos os movimentos SL/TP

    FUNÇÃO: Garantir que o modelo nunca possa violar os limites de risco iniciais
    - SL nunca pode ser movido para uma posição PIOR que a inicial
    - TP sempre deve manter distância mínima válida do preço atual
    """

    def __init__(self, position):
        self.entry_price = position['entry_price']
        self.initial_sl_points = position.get('sl_points', 8)  # Pontos SL iniciais
        self.initial_tp_points = position.get('tp_points', 10)  # Pontos TP iniciais
        self.position_type = position['type']

        # 🛡️ LIMITES DISCIPLINARES ABSOLUTOS
        if self.position_type == 'long':
            # Para LONG: SL nunca pode ser MENOR que o inicial
            self.worst_allowed_sl = self.entry_price - self.initial_sl_points
            self.min_allowed_tp = self.entry_price + UNIFIED_SLTP_CONFIG['tp_min_distance']
        else:  # short
            # Para SHORT: SL nunca pode ser MAIOR que o inicial
            self.worst_allowed_sl = self.entry_price + self.initial_sl_points
            self.max_allowed_tp = self.entry_price - UNIFIED_SLTP_CONFIG['tp_min_distance']

    def validate_sl_movement(self, new_sl_price):
        """
        Validar se movimento SL é permitido pela disciplina

        Returns: True se movimento é permitido, False se viola disciplina
        """
        if self.position_type == 'long':
            # LONG: SL não pode ser menor (mais distante) que o inicial
            return new_sl_price >= self.worst_allowed_sl
        else:  # short
            # SHORT: SL não pode ser maior (mais distante) que o inicial
            return new_sl_price <= self.worst_allowed_sl

    def validate_tp_movement(self, new_tp_price, current_price):
        """
        Validar se movimento TP é permitido

        Returns: True se movimento é permitido, False se inválido
        """
        if self.position_type == 'long':
            # LONG: TP deve estar acima do preço atual + distância mínima
            return (new_tp_price >= self.min_allowed_tp and
                    new_tp_price > current_price + UNIFIED_SLTP_CONFIG['tp_min_distance'])
        else:  # short
            # SHORT: TP deve estar abaixo do preço atual - distância mínima
            return (new_tp_price <= self.max_allowed_tp and
                    new_tp_price < current_price - UNIFIED_SLTP_CONFIG['tp_min_distance'])

def convert_unified_signals_to_movements(sl_adjust, tp_adjust):
    """
    🎯 CONVERSÃO UNIFICADA: sl_adjust/tp_adjust → movimentos em pontos

    ESPECIFICAÇÃO:
    - Input: sl_adjust, tp_adjust [-0.5, +0.5]
    - Output: movimento de ±1.0 ponto (conforme solicitado)
    - ±0.5 signal → ±1.0 ponto movimento

    INTERPRETAÇÃO:
    - sl_adjust negativo = apertar SL (mais proteção)
    - sl_adjust positivo = afrouxar SL (menos proteção)
    - tp_adjust negativo = TP próximo (menos ganância)
    - tp_adjust positivo = TP distante (mais ganância)
    """
    # Conversão: ±0.5 → ±1.0 ponto
    movement_multiplier = UNIFIED_SLTP_CONFIG['movement_per_signal'] / 0.5  # 1.0 / 0.5 = 2.0

    sl_movement_points = sl_adjust * movement_multiplier  # ±0.5 → ±1.0 pontos
    tp_movement_points = tp_adjust * movement_multiplier  # ±0.5 → ±1.0 pontos

    # Aplicar limites máximos
    sl_movement_points = np.clip(sl_movement_points, -UNIFIED_SLTP_CONFIG['max_adjustment'],
                                                     UNIFIED_SLTP_CONFIG['max_adjustment'])
    tp_movement_points = np.clip(tp_movement_points, -UNIFIED_SLTP_CONFIG['max_adjustment'],
                                                     UNIFIED_SLTP_CONFIG['max_adjustment'])

    return sl_movement_points, tp_movement_points

def convert_creation_signals_to_initial_sltp(sl_adjust, tp_adjust):
    """
    🎯 CRIAÇÃO INTELIGENTE: Modelo escolhe configuração inicial SL/TP

    INTERPRETAÇÃO PARA CRIAÇÃO:
    - sl_adjust [-0.5, +0.5]: Define nível de proteção inicial
      - Negativo: Mais proteção (SL apertado = menos risco)
      - Positivo: Menos proteção (SL relaxado = mais risco)
    - tp_adjust [-0.5, +0.5]: Define ambição inicial
      - Negativo: Conservador (TP próximo = lucro rápido)
      - Positivo: Ambicioso (TP distante = lucro maior)

    Returns:
        tuple: (initial_sl_points, initial_tp_points) sempre positivos
    """

    # 🛡️ SL: 3-7 pontos baseado no sinal (centro = 5 pontos)
    base_sl = 5.0       # Centro: 5 pontos
    sl_range = 2.0      # Variação: ±2 pontos
    initial_sl_points = base_sl + (sl_adjust * sl_range)  # [-0.5,+0.5] → [3,7] pontos

    # 🎯 TP: 8-16 pontos baseado no sinal (centro = 12 pontos)
    base_tp = 12.0      # Centro: 12 pontos
    tp_range = 4.0      # Variação: ±4 pontos
    initial_tp_points = base_tp + (tp_adjust * tp_range)  # [-0.5,+0.5] → [8,16] pontos

    # Aplicar limites de segurança
    initial_sl_points = max(2.0, min(initial_sl_points, 8.0))  # Forçar entre 2-8 pontos
    initial_tp_points = max(5.0, min(initial_tp_points, 20.0))  # Forçar entre 5-20 pontos

    # Arredondar para múltiplos de 0.5
    initial_sl_points = round(initial_sl_points * 2) / 2
    initial_tp_points = round(initial_tp_points * 2) / 2

    return initial_sl_points, initial_tp_points

# 🗑️ REMOVIDO: convert_model_adjustments_to_points - SUBSTITUÍDO PELO SISTEMA UNIFICADO
# Esta função foi substituída por:
# - convert_creation_signals_to_initial_sltp() para criação de posições
# - convert_unified_signals_to_movements() para ajustes de posições
# O sistema unificado é mais claro, consistente e disciplinado

# 🗑️ REMOVIDO: convert_action_to_realistic_sltp - FUNÇÃO DEPRECADA
# Esta função antiga foi completamente substituída pelo sistema unificado
        ...
        try:
            ...
            for phase_idx in range(self.current_phase_idx, total_phases):
                ...
                base_callbacks = [
                    ...
                    performance_early_stop,     # 🔥 NOVO: Performance-based early stopping ATIVO
