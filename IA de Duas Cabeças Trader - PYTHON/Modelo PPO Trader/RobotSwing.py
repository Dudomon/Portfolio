...
class TradingRobotV7(gym.Env):
    ...
    def __init__(self, log_widget=None):
        ...
        self.sl_range_min = 8.0    # SL mínimo: 8 pontos (swing aligned)
        self.sl_range_max = 25.0   # SL máximo: 25 pontos (swing aligned)
        self.tp_range_min = 12.0   # TP mínimo: 12 pontos (swing aligned)
        self.tp_range_max = 40.0   # TP máximo: 40 pontos (swing aligned)
        ...
        self._log(f"[🎯 SL/TP RANGES] SL: {self.sl_range_min}-{self.sl_range_max} pontos, TP: {self.tp_range_min}-{self.tp_range_max} pontos (swing aligned)")
    ...
    def _calculate_v7_volume_fixed(self, risk_appetite, current_positions):
        ...
        try:
            ...
            # Volume debug apenas em casos específicos
            if final_volume > self.max_lot_size or final_volume < 0.01:
...
# Funções de adaptação removidas - RobotV7 agora usa formato Legion V1 nativo
    ...
    def get_model_prediction(self, observation):
        ...
        try:
            ...
            # Debug de predições apenas a cada 100 steps para reduzir spam
            ...
            # Debug de observações anômalas apenas a cada 500 steps
            if self.debug_step_counter % 500 == 0:
                try:
                ...
                if ticket in self.position_stats:
                    ...
                    if history and len(history) >= 2:  # Open + Close
                        ...
                        try:
                            ...
                            # Debug slot apenas se não encontrar o mapeamento
                            if slot is None:
                            ...
                            if slot is not None:
                                ...
                                self._log(f"❌ [SLOT ERROR] Não foi possível determinar slot para ticket {ticket}")
                        except Exception as e:
                            self._log(f"❌ [COOLDOWN ERROR] Erro no cooldown: {e}")
    ...
    def _allocate_entry_slot(self):
        ...
        try:
            ...
            # Debug de slots apenas quando necessário (sem spam)
            if min([self.position_slot_cooldowns.get(s, 0.0) for s in range(self.max_positions)]) > time.time():
                slots_debug = []
        ...
        try:
            ...
            for position in positions:
                try:
                    ...
                    # Log status apenas quando posição muda significativamente
                    if has_sl or has_tp:
                        # Log apenas se P&L mudou mais de 5% ou a cada 60 segundos
                        current_time = time.time()
                        last_log_key = f"monitor_{position.ticket}"
                        last_log_time = getattr(self, '_last_monitor_logs', {}).get(last_log_key, 0)
                        
                        if current_time - last_log_time > 60:  # Log a cada 60 segundos por posição
                            self._log(f"📊 [MONITOR] Ticket #{position.ticket} | P&L: ${pnl:.2f} | SL: ${position.sl:.2f} | TP: ${position.tp:.2f}")
                            if not hasattr(self, '_last_monitor_logs'):
                                self._last_monitor_logs = {}
                            self._last_monitor_logs[last_log_key] = current_time
    ...
    def run(self):
        ...
        try:
            ...
            if model_info['loaded']:
                ...
                # Configurar debug com intervalos reduzidos
                self.debug_composite_interval = 100   # Debug a cada 100 steps
                self.debug_anomaly_interval = 500     # Debug de anomalias a cada 500 steps
...
class TradingAppV7:
    ...
    def apply_lot_size(self):
        ...
        print("DEBUG: apply_lot_size foi chamada!")  # Debug simples
        try:
            # Validar entrada
