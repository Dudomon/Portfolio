...
# 🎯 FIX SHORT BIAS V2: EXTREMOS=AÇÕES, CENTRO=HOLD
...
from trading_framework.policies.two_head_v11_sigmoid import TwoHeadV11Sigmoid, get_v11_sigmoid_kwargs, get_v11_sigmoid_no_compression_kwargs, validate_v11_sigmoid_policy
...
EXPERIMENT_TAG = "Eighteen"  # 🎯 ENTRY TIMING V2: Multi-Signal Confluence (3 layers) + Behavioral Controls + PESO DOBRADO (12%)
...
# 🎯 GOLD TRADING CONFIGURATION - OTIMIZADO 5M STEPS 1MIN TIMEFRAME
TRAINING_CONFIG = {
    "max_dataset_bars": 1080000,    # Máximo de barras de 1m no dataset (1.08M)
    "total_timesteps": 5000000,     # 🎯 OTIMIZADO: 5M steps para evitar overfitting após 3.5M
    "training_multiplier": 4.6      # Multiplicador atualizado (5M / 1.08M)
}

# 🎯 OPTIMIZED: 3 PHASES FOCUSED TRAINING (TOTAL = 5,000,000)
PHASE_DISTRIBUTION = {
    "Phase_1_Fundamentals_Extended": int(5000000 * 0.3),     # 30% = 1.5M steps 
    "Phase_2_Risk_Management": int(5000000 * 0.25),          # 25% = 1.25M steps
    "Phase_3_Noise_Handling_Fixed": int(5000000 * 0.2),     # 20% = 1M steps
    "Phase_4_Integration": int(5000000 * 0.15),              # 15% = 750k steps
    "Phase_5_Stress_Testing": int(5000000 * 0.1),           # 10% = 500k steps
}

# 🏆 PHASE CONFIGURATIONS - DETAILED SPEC IMPLEMENTATION
PHASE_CONFIGS = {
    "Phase_1_Fundamentals_Extended": {
        "name": "Fundamentals Extended",
        "description": "Aprender mecânica básica + risk management",
        "dataset_type": "comprehensive_foundation", 
        "data_mix": {"normal": 0.6, "volatile": 0.4},
        "reward_weights": {"pnl": 0.5, "risk": 0.4, "consistency": 0.1},
        "success_criteria": {"win_rate": 0.47, "max_drawdown": 0.18},
        "focus": "Entry/Exit timing, SL/TP, position sizing, risk basics"
    },
    "Phase_2_Risk_Management": {
        "name": "Risk Management",
        "description": "Dominar gestão de risco em múltiplos ciclos",
        "dataset_type": "risk_management_focused",
        "data_mix": {"reversal": 0.4, "trending": 0.3, "volatile": 0.3},
        "reward_weights": {"risk": 0.5, "pnl": 0.3, "consistency": 0.2},
        "success_criteria": {"max_drawdown": 0.15, "calmar_ratio": 0.8},
        "focus": "SL/TP otimização, gestão de drawdown, proteção de capital"
    },
    "Phase_3_Noise_Handling_Fixed": {
        "name": "Noise Handling",
        "description": "Lidar com ruído e dados complexos",
        "dataset_type": "noisy_challenging_data",
        "data_mix": {"noisy": 0.5, "mixed": 0.5},
        "reward_weights": {"consistency": 0.4, "robustness": 0.3, "pnl": 0.3},
        "success_criteria": {"stability": 0.8, "noise_tolerance": 0.7},
        "focus": "Robustez, filtragem de ruído, estabilidade"
    },
    "Phase_4_Integration": {
        "name": "Integration",
        "description": "Integração e otimização final",
        "dataset_type": "integrated_optimization",
        "data_mix": {"diverse": 0.6, "challenging": 0.4},
        "reward_weights": {"sharpe": 0.4, "pnl": 0.3, "consistency": 0.3},
        "success_criteria": {"sharpe_ratio": 1.1, "overall_performance": 0.8},
        "focus": "Integração de habilidades, otimização global"
    },
    "Phase_5_Stress_Testing": {
        "name": "Stress Testing",
        "description": "Teste final em condições extremas",
        "dataset_type": "stress_test_scenarios",
        "data_mix": {"extreme": 0.7, "adverse": 0.3},
        "reward_weights": {"robustness": 0.5, "consistency": 0.3, "recovery": 0.2},
        "success_criteria": {"stress_tolerance": 0.9, "recovery_ability": 0.8},
        "focus": "Resistência a condições extremas, recuperação rápida"
    }
}
...
CONVERGENCE_OPTIMIZATION_CONFIG = {
    ...
    "base_lr": 3.0e-5,  # 🎯 SINCRONIZADO: Com BEST_PARAMS 3e-05
...
def get_entropy_coefficient(current_steps: int, total_steps: int = 5_000_000) -> float:
    """
    ⚡ ENTROPY ANNEALING - Decaimento linear de exploracao

    Reduz exploracao progressivamente ao longo do treinamento:
    - 0 steps: ent_coef = 0.15 (alta exploracao)
    - 5M steps: ent_coef = 0.06 (baixa exploracao, exploitation)

    Impacto esperado: +20-30% convergencia e estabilidade
    """
    start_ent = 0.15
    end_ent = 0.06

    # Progresso de 0.0 a 1.0
    progress = min(current_steps / total_steps, 1.0)

    # Decaimento linear
    current_ent = start_ent - (start_ent - end_ent) * progress

    return current_ent

def get_current_phase_config(current_steps: int) -> dict:
    """🎯 CONFIGURAÇÃO SIMPLES - Sempre retorna a mesma fase direta"""
    # Com treinamento simplificado, sempre usar mesma configuração
    return {
        "name": "Simple_Direct_Training",
        "learning_rate": 6.0e-05,  # 🚀 V3 BRUTAL: Alinhado com BEST_PARAMS
        "batch_size": 1024,
        "n_epochs": 10,
        "reward_weights": {
            "episode_return": 1.0,      # Foco principal no retorno
            "win_rate": 0.8,            # Win rate importante mas secundário
            "consistency": 0.5,         # Consistência moderada
            "drawdown": -0.3            # Penalizar drawdowns
        }
    }
...
def get_gold_trading_params_for_phase(current_steps: int) -> dict:
    """🎯 PARÂMETROS SIMPLES - Sempre retorna os mesmos parâmetros otimizados"""
    # Com treinamento simplificado, sempre usar os mesmos parâmetros otimizados
    return GOLD_TRADING_PARAMS.copy()
...
def calculate_v6_observation_space():
    """🍒 CHERRY - Observation Space V11 ENHANCED com Order Flow + Time Context + Risk Management"""
    print("=" * 60)
    print(f"🍒 CALCULANDO CHERRY ENHANCED OBSERVATION SPACE V11 ({EXPERIMENT_TAG})")
    print("=" * 60)
    ...
    timeframes = 1           # 1m apenas
    high_quality_count = 9   # volume_momentum, price_position, etc.  
    positions_count = 2      # 🔬 CIRÚRGICO: máximo de posições (otimizado)
    ...
    print(f"📊 BASE FEATURES: {base_features_count} x {timeframes} timeframe = {base_features_count * timeframes}")
    print(f"📊 HIGH QUALITY: {high_quality_count} features")
    print(f"🔥 MARKET REAL: {market_real_count} features")
    print(f"📊 POSITIONS: {positions_count} pos x {features_per_position} features = {positions_count * features_per_position} (🔬 OTIMIZADO)")
...
class ConvergenceLogger:
    ...
    def _extract_model_metrics(self, model, info_dict):
        """Extrair métricas do modelo - com cache para performance"""
        # Cache agressivo: só recalcular a cada 50 steps
        current_step = getattr(self, 'num_timesteps', 0)
        cache_key = f"model_metrics_{current_step // 50}"
        
        if hasattr(self, '_metrics_cache') and self._metrics_cache.get('key') == cache_key:
            cached_metrics = self._metrics_cache.get('data', {}).copy()
            # Adicionar info_dict atual ao cache
            if info_dict:
                for key, value in info_dict.items():
                    if isinstance(value, (int, float, np.number)):
                        cached_metrics[key] = float(value)
        ...
        self._metrics_cache = {'key': cache_key, 'data': metrics.copy()}
...
def load_1m_dataset():
    """🚀 Carregar dataset 1m MASSIVO MT5 - 1 milhão de barras reais"""
    import os

    # Usar dataset MT5 massivo diretamente
    dataset_path = "data/GOLD_1M_MT5_MASSIVE_1061264bars_20250924_092923.pkl"

    if not os.path.exists(dataset_path):
        raise FileNotFoundError(f"❌ Dataset MT5 massivo não encontrado: {dataset_path}")

    print(f"📊 [MT5-1M] Carregando dataset massivo: {dataset_path}")

    df = pd.read_pickle(dataset_path)
    print(f"✅ [MT5-1M] Dataset carregado: {len(df):,} barras")
    print(f"📈 [MT5-1M] Período: {df['timestamp'].min()} até {df['timestamp'].max()}")
    print(f"🔧 [MT5-1M] Features: {len(df.columns)} colunas")
    ...
    column_mapping = {
        ...
        'sma_20_1m': 'sma_20_1m',
        'ema_9_1m': 'ema_9_1m',
        'bb_upper_1m': 'bb_upper_1m',
        'bb_lower_1m': 'bb_lower_1m',
        'bb_position_1m': 'bb_position_1m',
        'volatility_10_1m': 'volatility_20_1m',
        'trend_strength_1m': 'trend_strength_1m',
        'momentum_5_1m': 'momentum_5_1m'
    }
    ...
    if 'volume_1m' in df.columns:
        volume_zeros = (df['volume_1m'] == 0).sum()
        if volume_zeros > len(df) * 0.5:  # >50% zeros
            ...
            if 'close_1m' in df.columns:
                price_changes = df['close_1m'].pct_change().abs()
                base_volume = 1000  # Volume base
                # 🔥 VOLUME ORGÂNICO: Usar dados reais do Yahoo, sem síntese artificial
                df.loc[df['volume_1m'] == 0, 'volume_1m'] = 1.0  # Mínimo orgânico
                print(f"🔥 VOLUME ORGÂNICO: Dados reais do Yahoo (range: {df['volume_1m'].min():.0f}-{df['volume_1m'].max():.0f})")
...
class ImprovedEarlyStopping:
    """🎯 Early stopping baseado em Episode Return + Win Rate"""
    
    def __init__(self, patience=50000, min_improvement=0.03):
    ...
    def calculate_composite_metric(self, episode_buffer):
        """Métrica composta robusta: Return + Win Rate + Consistência"""
        if len(episode_buffer) < 30:
            return -np.inf
            
        recent = episode_buffer[-50:]  # Últimos 50 episódios
        
        # 1. Performance básica
        returns = [e['r'] for e in recent if 'r' in e]
        if not returns:
            return -np.inf
        ...
        # 2. Estabilidade
        return_std = np.std(returns) if len(returns) > 1 else 1.0
        consistency = 1.0 / (1.0 + return_std)  # Menor std = melhor
        
        # 3. Métrica final ponderada
        metric = (
            avg_return * 0.5 +           # 50% performance
            win_rate * 1000 * 0.3 +      # 30% win rate (scaled)
            consistency * 100 * 0.2       # 20% consistência (scaled)
        )
        
        return metric
        
    def should_stop(self, current_step, episode_buffer):
        """Verificar se deve parar o treinamento baseado em episódios"""
        if len(episode_buffer) < 30:
            return False
            
        current_metric = self.calculate_composite_metric(episode_buffer)
        self.metric_history.append((current_step, current_metric))
        
        if current_metric > self.best_metric * (1 + self.min_improvement):
            self.best_metric = current_metric
            self.best_step = current_step
            
            # Calcular componentes para log
            recent = episode_buffer[-50:]
            returns = [e['r'] for e in recent if 'r' in e]
            avg_return = np.mean(returns) if returns else 0
            win_rate = np.mean([r > 0 for r in returns]) if returns else 0
            
            print(f"🎯 NEW BEST: Step {current_step}, Metric {current_metric:.2f}")
            print(f"   📊 Return: {avg_return:.2f}, Win Rate: {win_rate:.1%}, Episodes: {len(returns)}")
            return False
            
        if current_step - self.best_step > self.patience:
            print(f"⚠️ EARLY STOP: Peak foi em {self.best_step} steps (Metric: {self.best_metric:.2f})")
            print(f"⚠️ Sem melhoria por {self.patience:,} steps - parando treinamento")
            return True
        return False

def load_optimized_data_original():
    """
    🚀 CARREGAMENTO DATASET MASSIVO MT5 - 1 MILHÃO DE BARRAS REAIS
    """
    dataset_path = 'data/GOLD_1M_MT5_MASSIVE_1061264bars_20250924_092923.pkl'  # 🚀 DATASET MASSIVO MT5 - 1M+ barras 100% reais
    print(f"[MT5-MASSIVE] Carregando dataset massivo MT5: {dataset_path}")
    start_time = time.time()

    # Carregar do pickle (mais eficiente para dataset grande)
    df = pd.read_pickle(dataset_path)

    # Configurar index com timestamp
    df.set_index('timestamp', inplace=True)

    load_time = time.time() - start_time

    print(f"[MT5-MASSIVE] Dataset carregado: {len(df):,} barras")
    print(f"[MT5-MASSIVE] Período: {df.index.min()} até {df.index.max()}")
    print(f"[MT5-MASSIVE] Duração: {(df.index.max() - df.index.min()).days} dias")
    print(f"[MT5-MASSIVE] Tempo: {load_time:.3f}s")
    print(f"[MT5-MASSIVE] Features: {len(df.columns)} colunas")
    print(f"[MT5-MASSIVE] Colunas: {list(df.columns[:10])}...")  # Mostrar primeiras 10

    return df

def get_latest_processed_file_fallback():
    """
    🚀 FALLBACK PARA DATASET MASSIVO MT5 - 1 MILHÃO DE BARRAS REAIS
    """
    dataset_path = 'data/GOLD_1M_MT5_MASSIVE_1061264bars_20250924_092923.pkl'  # 🚀 DATASET MASSIVO MT5 - 1M+ barras 100% reais
    print(f"[MT5-FALLBACK] Carregando dataset massivo MT5 (fallback): {dataset_path}")
    ...
    print(f"[MT5-FALLBACK] Carregado: {len(df):,} barras")
    print(f"[MT5-FALLBACK] Features: {len(df.columns)} colunas")
    print(f"[MT5-FALLBACK] Colunas: {list(df.columns[:8])}...")  # Mostrar primeiras 8
    return df
...
class EntropyAnnealingCallback(BaseCallback):
    """⚡ ENTROPY ANNEALING - Reduz exploracao progressivamente"""

    def __init__(self, total_steps: int = 5_000_000, verbose: int = 0):
        super().__init__(verbose)
        self.total_steps = total_steps
        self.last_log_step = 0

    def _on_step(self) -> bool:
        """Atualizar ent_coef dinamicamente"""
        current_ent = get_entropy_coefficient(self.num_timesteps, self.total_steps)

        # Atualizar o modelo
        self.model.ent_coef = current_ent

        # Log a cada 100k steps
        if self.num_timesteps - self.last_log_step >= 100_000:
            print(f"⚡ Entropy Annealing: {current_ent:.4f} @ {self.num_timesteps:,} steps")
...
class HighFrequencyCheckpointCallback(BaseCallback):
    """
    🎯 HIGH FREQUENCY CHECKPOINT - Salva a cada 5K steps em range específico

    Usado para isolar o melhor modelo em uma janela de convergência conhecida.
    """
    def __init__(self,
                 start_step=1500000,      # Início do range (1.5M)
                 end_step=1600000,        # Fim do range (1.6M)
                 save_freq=5000,          # Salvar a cada 5K steps
                 save_path=None,
                 name_prefix="Fourteen",
        ...
        print(f"🎯 HIGH FREQUENCY CHECKPOINT ativado:")
        print(f"   Range: {start_step:,} - {end_step:,} steps")
        print(f"   Frequência: a cada {save_freq:,} steps")
        print(f"   Total de checkpoints: {(end_step - start_step) // save_freq}")

    def _on_step(self) -> bool:
        real_timesteps = self.num_timesteps + self.total_steps_offset

        # Verificar se estamos dentro do range
        if self.start_step <= real_timesteps <= self.end_step:
            # 🔧 FIX: Verificar se é momento de salvar usando módulo correto
            # Salvar nos múltiplos de save_freq dentro do range
            if real_timesteps % self.save_freq == 0:
                try:
                    from datetime import datetime
                    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")

                    # Criar diretório específico para high-frequency checkpoints
                    hf_dir = f"{self.save_path}/high_frequency_1.5M-1.6M"
                    os.makedirs(hf_dir, exist_ok=True)

                    checkpoint_path = f"{hf_dir}/checkpoint_{real_timesteps}_steps_{timestamp}.zip"

                    if self.verbose:
                        checkpoint_num = (real_timesteps - self.start_step) // self.save_freq + 1
                        total_checkpoints = (self.end_step - self.start_step) // self.save_freq
                        print(f"\n🎯 HIGH-FREQ CHECKPOINT - Step {real_timesteps:,} ({checkpoint_num}/{total_checkpoints})")
                    ...
                    if os.path.exists(checkpoint_path):
                        ...
                        print(f"   ❌ ERRO: Checkpoint não foi salvo!")

                except Exception as e:
                    print(f"❌ Erro ao salvar high-frequency checkpoint: {e}")
                    import traceback
                    traceback.print_exc()
...
class EarlyStoppingCallback(BaseCallback):
    ...
    def __init__(self, 
                 ...
                 patience_steps=250000,      # Steps de tolerância (reduzido para 250K)
                 min_steps=1000000,          # Mínimo de 1M steps antes de poder parar
                 check_freq=50000,           # Verificação a cada 50K steps
                 verbose=1):
        super().__init__(verbose)
    ...
    def _on_step(self) -> bool:
        # ✅ EARLY STOPPING ATIVADO - Monitorando métricas de treinamento
        if self.num_timesteps < self.min_steps:
            return True  # Continuar até atingir steps mínimos
            
        # Verificar apenas a cada check_freq steps
        ...
        try:
            ...
            if hasattr(self.model, 'logger') and hasattr(self.model.logger, 'name_to_value'):
                ...
                if entropy_loss is not None and policy_loss is not None:
                    ...
                    # Capturar reward médio se disponível
                    episode_reward = metrics.get('rollout/ep_rew_mean', None)
                    ...
                    if episode_reward is not None:
                        ...
                        # Manter apenas últimos 10 valores para média móvel
                        if len(self.reward_history) > 10:
                            self.reward_history.pop(0)
                        
                        current_avg_reward = np.mean(self.reward_history)
                        improvement = (current_avg_reward - self.best_reward) / (abs(self.best_reward) + 1e-8)
                        
                        if improvement > 0.02:  # 2% de melhoria
                            self.best_reward = current_avg_reward
                            self.no_improvement_steps = 0
                            if self.verbose:
                                print(f"✅ Nova melhor performance: {current_avg_reward:.3f} (step {self.num_timesteps:,})")
                        else:
                            self.no_improvement_steps += self.check_freq
                            
                        # Parar se sem melhoria há muito tempo
                        if self.no_improvement_steps >= self.patience_steps:
                            print(f"\n🚨 EARLY STOPPING ATIVADO aos {self.num_timesteps:,} steps!")
                            print(f"   Razão: Sem melhoria há {self.no_improvement_steps:,} steps")
                            print(f"   Melhor performance: {self.best_reward:.3f}")
                            print(f"   Performance atual: {current_avg_reward:.3f}")
                            return False  # Parar treinamento
                    
                    # Fallback: parar por gradientes mortos apenas em casos extremos
                    elif (self.bad_entropy_count >= self.patience_steps * 2 or 
                          self.bad_policy_count >= self.patience_steps * 2):
                        
                        print(f"\n🚨 EARLY STOPPING ATIVADO aos {self.num_timesteps:,} steps!")
                        print(f"   Razão: {'Entropy collapse' if self.bad_entropy_count >= self.patience_steps * 2 else 'Policy gradients mortos'}")
...
class MetricsCallback(BaseCallback):
        ...
        try:
            ...
            try:
                ...
                if trades_lists and episode_steps_list:
                    ...
                    current_trades_per_day = (total_trades / max(1, episode_steps)) * 1440 if episode_steps > 0 else 0
            except:
    ...
    def _on_step(self) -> bool:
        ...
        # 🔍 CONVERGENCE LOGGER: Log detalhado a cada 100 steps (otimização performance)
        ...
        if show_metrics:
            try:
                        ...
                        if portfolio_values and len(portfolio_values) > 0:
                            ...
                            try:
                                ...
                                if current_prices and current_steps:
                                    current_price = current_prices[0]['close_1m'].iloc[current_steps[0]]
                            ...
                            try:
                                ...
                                if episode_steps > 0:
                                    episode_days_elapsed = episode_steps / 1440.0  # 1440 steps = 1 dia (1min bars)
                            ...
                            global_days_elapsed = self.num_timesteps / 1440.0  # 1440 steps = 1 dia (1min bars)
                ...
                #  MÉTRICA PRINCIPAL: Lucro/dia baseado em 1440 barras = 1 dia (1min bars)
                days_elapsed_1440 = self.num_timesteps / 1440.0  # 1440 barras de 1min = 1 dia
                lucro_por_dia = total_pnl / max(days_elapsed_1440, 0.001)  # Evitar divisão por zero
    ...
    def _run_avaliar_v8_evaluation(self):
        """🚀 Executa avaliar_1min.py usando checkpoints existentes (não cria novos)"""
        import subprocess
        import os
        import threading
        import glob
        ...
        def run_evaluation_async():
            try:
                # 🔧 Usar o checkpoint mais recente já salvo (50k intervals)
                ...
                if not existing_checkpoints:
                    print("⚠️ Nenhum checkpoint encontrado para avaliação")
                    return
                
                # Usar o checkpoint mais recente
                latest_checkpoint = max(existing_checkpoints, key=os.path.getctime)
                print(f"📊 Avaliando checkpoint mais recente: {os.path.basename(latest_checkpoint)}")
                
                # Atualizar avaliar_1min.py com checkpoint existente
                avaliar_path = "D:/Projeto/avaliacao/avaliar_1min.py"
                ...
                for i, line in enumerate(lines):
                    if line.startswith('CHECKPOINT_PATH = '):
                        lines[i] = f'CHECKPOINT_PATH = "{latest_checkpoint}"'
                        break
                ...
                print(f"✅ avaliar_1min.py atualizado com checkpoint existente")
                
                # Executar avaliar_1min.py
                print(f"🚀 Executando avaliação com checkpoint existente...")
                result = subprocess.run([
                    'python', 'avaliacao/avaliar_1min.py'
                ], 
                cwd='D:/Projeto',
                capture_output=True, 
                text=True, 
                ...
                if result.returncode == 0:
                    ...
                    print(f"📊 Últimas linhas da avaliação:")
                    for line in stdout_lines[-5:]:  # Últimas 5 linhas
                        print(f"  {line}")
                else:
        ...
        eval_thread.start()
        print(f"🔄 Avaliação iniciada em background usando checkpoint existente")
...
CONTINUATION_PARAMS = {
    "learning_rate": 6.0e-05,        # 🚀 V3 BRUTAL: Alinhado com BEST_PARAMS
    "critic_learning_rate": 4.0e-05,  # 🚀 V3 BRUTAL: Alinhado com BEST_PARAMS
    ...
    "ent_coef": 0.08,                 # 🚀 V4 INNO: Exploração adequada para rewards amplificados
    "batch_size": 32,                 # Batches menores: 64 → 32
    "target_kl": 0.03,                # KL divergence mais tolerante (0.01→0.03)
}

BEST_PARAMS = {
    "learning_rate": 6.0e-05,                # 🚀 V3 BRUTAL: Dobrado de 3e-05 para 6e-05 
    "critic_learning_rate": 4.0e-05,        # 🚀 V3 BRUTAL: Dobrado de 2e-05 para 4e-05
    "n_steps": 2048,                         # 🏆 COMPROVADO: Trajetória adequada
    "batch_size": 1024,                      # 🚀 OTIMIZADO: Batch 8x maior para MÁXIMA estabilidade
    "n_epochs": 10,                          # 🔥 RESTAURADO: Valor original para aprendizado completo
    "gamma": 0.99,                           # 🏆 PADRÃO: Long-term thinking
    "gae_lambda": 0.95,                      # 🏆 PADRÃO: GAE padrão
    "clip_range": 0.12,                      # 🔧 OTIMIZADO: Mais conservador para estabilidade
    "ent_coef": 0.08,                        # 🚀 V4 INNO: Exploração aumentada para rewards amplificados
    "vf_coef": 0.5,                          # 🔧 SIMPLES: Value function padrão
    "max_grad_norm": 0.5,                    # 🔧 SIMPLES: Gradient clipping moderado
    "target_kl": 0.03,                       # 🔧 AJUSTADO: KL divergence menos restritivo
    "policy_kwargs": {
...
class TradingEnv(gym.Env):
    MAX_STEPS = 14400  # 🔧 AJUSTE 1MIN: 14400 steps = 10 dias úteis (1440 min/dia × 10 dias)
    ...
    def __init__(self, df, window_size=20, is_training=True, initial_balance=None, trading_params=None):
        ...
        self.max_positions = 2  # 🔬 CIRÚRGICO: Apenas 2 posições ativas (otimização intelligent features)
        ...
        # [0] entry_decision: [-1,1] Continuous (< -0.33=hold, < 0.33=long, >= 0.33=short)
        ...
        # Adicionar apenas 1m (mais granular, remove redundância multi-timeframe)
        for tf in ['1m']:
            self.feature_columns.extend([f"{f}_{tf}" for f in base_features_1m_only])
        ...
        self.INACTIVITY_THRESHOLD = 120  # ~2h em 1m
        self.last_action = None
        self.hold_count = 0
        
        # 🚨 SISTEMA DE COOLDOWN ANTI-OVERTRADING ADAPTATIVO (por slot)
        self.cooldown_base = 35         # 🔧 MANTIDO: 35 steps = 35 minutos (correto para 1min)
        self.cooldown_after_trade = 35  # 🔧 MANTIDO: Será ajustado dinamicamente baseado no resultado
        # Cooldown independente por slot (pos1 e pos2)
        self.slot_cooldowns = {i: 0 for i in range(self.max_positions)}  # slot_index -> steps restantes
        ...
        # 💰 SISTEMA V3-BRUTAL: Reward system focado em dinheiro com SL/TP dinâmico
        # ATUALIZADO: Usar V3 Brutal com trailing stops e SL/TP rewards
        self.reward_system = create_reward_system("v3_brutal", initial_balance)
        print(f"  🎯 V3-BRUTAL REWARD: PnL 85% + Shaping 15% (inclui SL/TP+Trailing)")
        print(f"  🛡️ PAIN AMPLIFICATION: Perdas >8% doem 1.5x mais")
        print(f"  🎯 TRAILING STOPS: Rewards para ativação, proteção e movimentos")
        print(f"  🎯 SL/TP DINÂMICO: Rewards para defesa de portfolio e expansão de alvos")
        
        # 🎯 ACTIVITY ENHANCEMENT SYSTEM - DESABILITADO para re-treino sem timeout
        if is_training:
            self.activity_system = None  # 🔧 DESABILITADO: Timeout artificial estava causando comportamento burro
            print(f"[ACTIVITY SYSTEM] ❌ Activity Enhancement DESABILITADO - SEM TIMEOUT de posições")
            print(f"[PHILOSOPHY] 🎯 Modelo aprenderá gestão natural de posições (sem muleta de timeout)")
        else:
    ...
    def step(self, action):
        ...
        if done:
            ...
            for pos in self.positions[:]:
                ...
                # 🔧 CONSOLIDADO: Usar método centralizado
                self._add_trade(trade_info)
            self.positions = []
    ...
    def _apply_volatility_multiplier(self, multiplier):
        ...
        volatility_cols = ['high_1m', 'low_1m', 'close_1m', 'open_1m']
        modified_cols = 0
    ...
    def _prepare_data(self):
        ...
        expected_features_1m = [f"{f}_{tf}" for tf in ['1m'] 
        ...
        expected_features = expected_features_1m + expected_high_quality
        
        # 🔧 CORREÇÃO: Mapear features do dataset para nomes esperados
        feature_mapping = {
            'returns_1m': 'returns',
            'volatility_20_1m': 'volatility_20', 
            'sma_20_1m': 'sma_20',
            'sma_50_1m': 'sma_50',
            'rsi_14_1m': 'rsi_14',
            'stoch_k_1m': 'stoch_k',
            'bb_position_1m': 'bb_position',
            'trend_strength_1m': 'trend_strength',
            'atr_14_1m': 'atr_14',
        ...
        # 🔥 FORÇAR CÁLCULO DAS 7 HIGH QUALITY FEATURES SEMPRE
        required_hq_features = ['volume_momentum', 'price_position', 'breakout_strength',
                              'trend_consistency', 'support_resistance', 'volatility_regime', 'market_structure']

        # Calcular missing features incluindo as 7 high quality
        missing_features = [col for col in expected_features + required_hq_features if col not in self.df.columns]

        if len(missing_features) == 0:
            print(f"[PREPARE DATA] OK Features já pré-calculadas, usando dados otimizados")
        else:
            print(f"[PREPARE DATA] AVISO Calculando {len(missing_features)} features ausentes (incluindo 7 high quality)...")
            self._calculate_missing_features(missing_features)
        
        # 🔥 FORÇAR RECÁLCULO: NUNCA usar cache (features foram corrigidas!)
        if hasattr(self.df, 'processed_data_cache'):
            print(f"[PREPARE DATA] ⚠️ Cache detectado - IGNORANDO (features corrigidas)")
            delattr(self.df, 'processed_data_cache')  # Remover cache antigo

        # SEMPRE recalcular processed_data
        # 🧠 CALCULAR FEATURES AUSENTES COM DADOS REAIS AO INVÉS DE FALLBACKS
        for col in self.feature_columns:
            if col not in self.df.columns:
                print(f"🧠 [FEATURE] Calculando '{col}' com dados reais ao invés de fallback")

                # Calcular features técnicas reais baseadas em dados disponíveis
                if col == 'rsi_14_1m' and 'close_1m' in self.df.columns:
                    # RSI real baseado em close
                    delta = self.df['close_1m'].diff()
                    ...
                    self.df.loc[:, col] = 100 - (100 / (1 + rs))

                elif col == 'stoch_k_1m' and all(c in self.df.columns for c in ['high_1m', 'low_1m', 'close_1m']):
                    # Stochastic real
                    low_14 = self.df['low_1m'].rolling(14, min_periods=1).min()
                    high_14 = self.df['high_1m'].rolling(14, min_periods=1).max()
                    self.df.loc[:, col] = 100 * (self.df['close_1m'] - low_14) / (high_14 - low_14 + 1e-8)

                elif col == 'bb_position_1m' and 'close_1m' in self.df.columns:
                    # Bollinger Bands position real
                    sma_20 = self.df['close_1m'].rolling(20, min_periods=1).mean()
                    std_20 = self.df['close_1m'].rolling(20, min_periods=1).std()
                    upper_bb = sma_20 + (2 * std_20)
                    lower_bb = sma_20 - (2 * std_20)
                    self.df.loc[:, col] = (self.df['close_1m'] - lower_bb) / (upper_bb - lower_bb + 1e-8)

                elif col == 'atr_14_1m' and all(c in self.df.columns for c in ['high_1m', 'low_1m', 'close_1m']):
                    # ATR real
                    high_low = self.df['high_1m'] - self.df['low_1m']
                    high_close = np.abs(self.df['high_1m'] - self.df['close_1m'].shift(1))
                    low_close = np.abs(self.df['low_1m'] - self.df['close_1m'].shift(1))
                    true_range = np.maximum(high_low, np.maximum(high_close, low_close))
                    self.df.loc[:, col] = true_range.rolling(14, min_periods=1).mean()

                else:
                    # Fallback apenas se não conseguir calcular
                    print(f"⚠️ [FALLBACK] Usando valor padrão para '{col}' (dados insuficientes)")
                    self.df.loc[:, col] = 0.001
        ...
        if 'opportunity' not in self.df.columns:
            ...
            if 'sma_cross_1m' in self.df.columns:
                cross = self.df['sma_cross_1m']
    ...
    def _calculate_missing_features(self, missing_features):
        ...
        #  OTIMIZAÇÃO 1: Detectar colunas disponíveis (com ou sem sufixo _1m)
        # Tentar com sufixo _1m primeiro, depois sem
        if 'close_1m' in self.df.columns:
            close_1m = self.df['close_1m'].values
            high_1m = self.df.get('high_1m', close_1m).values
            low_1m = self.df.get('low_1m', close_1m).values
            volume_1m = self.df.get('volume_1m', self.df.get('tick_volume_1m', np.full(len(self.df), 100))).values
        else:
            # Dataset sem sufixo (ex: GC=F_3YEARS)
            close_1m = self.df['close'].values
            high_1m = self.df.get('high', close_1m).values
            low_1m = self.df.get('low', close_1m).values
            volume_1m = self.df.get('tick_volume', self.df.get('real_volume', np.full(len(self.df), 100)))
            if hasattr(volume_1m, 'values'):
                volume_1m = volume_1m.values
        
        #  OTIMIZAÇÃO 2: Calcular todas as features de uma vez com vetorização
        features_to_calc = []
        
        # Features básicas 5m (mais importantes)
        if 'returns_1m' in missing_features:
            returns_1m = np.full_like(close_1m, 0.0001)  # 🔧 Valor pequeno ao invés de zeros
            returns_1m[1:] = np.diff(close_1m) / close_1m[:-1]
            # Garantir que o primeiro valor não seja zero
            if abs(returns_1m[0]) < 1e-8:
                returns_1m[0] = 0.0001
            self.df.loc[:, 'returns_1m'] = returns_1m
            features_to_calc.append('returns_1m')
        
        if 'volatility_20_1m' in missing_features:
            vol_20 = pd.Series(close_1m).rolling(window=20).std().fillna(0.001).values  # 🔧 Valor pequeno ao invés de zero
            self.df.loc[:, 'volatility_20_1m'] = vol_20
            features_to_calc.append('volatility_20_1m')
        
        if 'sma_20_1m' in missing_features:
            sma_20 = pd.Series(close_1m).rolling(window=20).mean().fillna(method='bfill').fillna(close_1m[0]).values  # 🔧 Usar primeiro valor ao invés de zero
            self.df.loc[:, 'sma_20_1m'] = sma_20
            features_to_calc.append('sma_20_1m')
        
        if 'sma_50_1m' in missing_features:
            sma_50 = pd.Series(close_1m).rolling(window=50).mean().fillna(method='bfill').fillna(close_1m[0]).values  # 🔧 Usar primeiro valor ao invés de zero
            self.df.loc[:, 'sma_50_1m'] = sma_50
            features_to_calc.append('sma_50_1m')
        
        if 'trend_strength_1m' in missing_features:
            # Usar como fallback para trend_strength
            sma_20 = pd.Series(close_1m).rolling(window=20).mean().fillna(close_1m[0]).values
            atr_14 = pd.Series(high_1m - low_1m).rolling(window=14).mean().fillna(1).values
            trend_strength = np.where(atr_14 > 0, np.abs(close_1m - sma_20) / atr_14, 0.5)
            ...
            self.df.loc[:, 'trend_strength_1m'] = trend_strength
            features_to_calc.append('trend_strength_1m')
        
        #  OTIMIZAÇÃO 3: Features de alta qualidade vetorizadas
        print(f"[HIGH QUALITY] Calculando features de alta qualidade...")

        # 🔥 REMOVIDO: volume_momentum e price_position agora são calculadas
        # APENAS no bloco required_hq_features abaixo (linhas 4350+)
        # para evitar duplicação e valores estáticos
        
        if 'volatility_ratio' in missing_features:
            vol_20 = pd.Series(close_1m).rolling(window=20).std().fillna(0.001).values  # 🔧 Valor pequeno ao invés de zero
            vol_50 = pd.Series(close_1m).rolling(window=50).std().fillna(0.001).values  # 🔧 Valor pequeno ao invés de zero
        ...
        if 'intraday_range' in missing_features:
            intraday_range = np.where(close_1m > 0, (high_1m - low_1m) / close_1m, 0.001)  # 🔧 Valor pequeno ao invés de zero
        ...
        if 'market_regime' in missing_features:
            sma_20 = pd.Series(close_1m).rolling(window=20).mean().fillna(close_1m[0]).values
            atr_14 = pd.Series(high_1m - low_1m).rolling(window=14).mean().fillna(1).values
            market_regime = np.where(atr_14 > 0, np.abs(close_1m - sma_20) / atr_14, 0.5)
            # 🔧 CORREÇÃO EXTRA: Substituir zeros extremos por valores pequenos
        ...
        if 'session_momentum' in missing_features:
            session_momentum = np.full_like(close_1m, 0.0001)  # 🔧 Valor pequeno ao invés de zeros
            session_momentum[240:] = (close_1m[240:] - close_1m[:-240]) / close_1m[:-240]
            # Garantir que valores iniciais não sejam zero
            session_momentum[:240] = 0.0001
            # 🔧 CORREÇÃO EXTRA: Substituir zeros extremos por valores pequenos
        ...
        # 🔥 REMOVIDO: volatility_regime agora é calculada
        # APENAS no bloco required_hq_features abaixo (linhas 4350+)
        # para evitar duplicação e valores estáticos
        ...
        for feature_name in required_hq_features:
            if feature_name in missing_features or feature_name not in self.df.columns:
                ...
                if feature_name == 'volume_momentum':
                    # Volume momentum SEMPRE recalcular (REAL, não constante)
                    volume_sma_20 = pd.Series(volume_1m).rolling(window=20).mean().fillna(volume_1m[0]).values
                    ...
                    # Price position SEMPRE recalcular (REAL, não constante)
                    high_20 = pd.Series(high_1m).rolling(window=20).max().fillna(high_1m[0]).values
                    low_20 = pd.Series(low_1m).rolling(window=20).min().fillna(low_1m[0]).values
                    ...
                    # Volatility regime SEMPRE recalcular (REAL, não constante)
                    # 🔥 CONTINUOUS: Ratio contínuo ao invés de 3 valores discretos
                    vol_20 = pd.Series(close_1m).rolling(window=20).std().fillna(0.001).values
                    vol_50 = pd.Series(close_1m).rolling(window=50).std().fillna(0.001).values
                    ...
                    # 🎯 TP TARGET ZONES: Identifica zonas REALISTAS para TP (VETORIZADO)
                    # Versão rápida usando rolling max/min ao invés de loops
                    ...
                    rolling_resistance = high_series.rolling(window=lookback, min_periods=1).max()
                    # Suporte próximo = low mínimo recente
                    rolling_support = low_series.rolling(window=lookback, min_periods=1).min()

                    # Distância para resistência (alvo TP LONG)
                    dist_to_resistance = rolling_resistance - close_series
                    # Distância para suporte (alvo TP SHORT)
                    dist_to_support = close_series - rolling_support

                    # Combinar (menor = alvo mais próximo)
                    combined_distance = np.minimum(dist_to_resistance.values, dist_to_support.values)

                    # Normalizar pela ATR
                    atr_14 = (high_series - low_series).rolling(window=14).mean().fillna(1).values
                ...
                elif feature_name == 'trend_consistency':
                    # 🔥 TREND CONSISTENCY: Consistência do trend via series de closes
                    # Mede quantos closes consecutivos estão na mesma direção
                    ...
                    self.df.loc[:, 'trend_consistency'] = trend_consistency.fillna(0.5).values
                elif feature_name == 'support_resistance':
                    # 🎯 SL ZONE QUALITY: Identifica ZONAS seguras para colocar SL (VETORIZADO)
                    # Versão rápida usando rolling min/max ao invés de loops
                    ...
                    combined_distance = np.minimum(dist_to_support.values, dist_to_resistance.values)
                    ...
                    # Valor ALTO = volatilidade em spike (SL deve ser mais largo)
                    # Valor BAIXO = mercado calmo (SL pode ser mais apertado)

                    # ATR atual vs ATR médio de longo prazo
                    ...
                    # 🔥 FALLBACK: Se feature desconhecida, criar constante mas avisar
                    print(f"⚠️ [WARNING] Feature '{feature_name}' desconhecida, usando valor constante")
                    base_val = (hash(feature_name) % 100) / 200.0 + 0.2
                    self.df.loc[:, feature_name] = base_val
                
                features_to_calc.append(feature_name)
        
        #  OTIMIZAÇÃO 4: Features simples sem TA (evitar overhead)
        if 'sma_cross_1m' in missing_features and 'sma_20_1m' in self.df.columns and 'sma_50_1m' in self.df.columns:
            sma_cross = np.where(self.df['sma_20_1m'].values > self.df['sma_50_1m'].values, 1.0, -1.0)
            self.df.loc[:, 'sma_cross_1m'] = sma_cross
            features_to_calc.append('sma_cross_1m')
        
        if 'momentum_5_1m' in missing_features:
            momentum_5 = np.full_like(close_1m, 0.0001)  # 🔧 Valor pequeno ao invés de zeros
            momentum_5[5:] = (close_1m[5:] - close_1m[:-5]) / close_1m[:-5]
            # Garantir que valores iniciais não sejam zero
            momentum_5[:5] = 0.0001
            self.df.loc[:, 'momentum_5_1m'] = momentum_5
            features_to_calc.append('momentum_5_1m')
    ...
    def _generate_fast_microstructure_features(self, step):
        ...
        try:
            ...
            close = self.df['close_1m'].iloc[step]
            volume = self.df['volume_1m'].iloc[step]
            high = self.df['high_1m'].iloc[step]
            low = self.df['low_1m'].iloc[step]
            ...
            # 🧠 FEATURES DINÂMICAS BASEADAS EM DADOS REAIS
            # Substituir valores estáticos por cálculos reais
            time_factor = np.sin(step * 0.01) * 0.1 + 0.5  # Componente temporal
            momentum = (close - self.df['close_1m'].iloc[max(0, step-1)]) / close if step > 0 and close > 0 else 0
            volume_ratio = volume / (self.df['volume_1m'].iloc[max(0, step-10):step+1].mean() + 1) if step >= 10 else 1.0

            return np.array([
                range_pct,                              # 0: Range real
                volume_norm,                            # 1: Volume real
                price_position,                         # 2: Price position real
                time_factor,                            # 3: Time component dinâmico
                momentum,                               # 4: Momentum real
                volume_ratio,                           # 5: Volume ratio real
                range_pct * volume_norm,                # 6: Range*Volume interaction
                price_position * momentum,              # 7: Position*Momentum interaction
                np.clip(momentum + 0.5, 0.1, 0.9),     # 8: Adjusted momentum
                volume_ratio * time_factor,             # 9: Volume*Time interaction
                abs(momentum) * price_position,         # 10: Momentum magnitude
                np.sin(volume_norm * np.pi) * 0.2 + 0.5, # 11: Volume-based oscillator
                np.clip(range_pct * 5, 0.1, 0.9),      # 12: Scaled range
                time_factor * price_position            # 13: Time*Position interaction
            ], dtype=np.float32)
        except:
            return np.full(14, 0.3, dtype=np.float32)
    ...
    def _generate_fast_volatility_features(self, step):
        ...
        try:
            ...
            window_data = self.df['close_1m'].iloc[max(0, step-5):step+1]
    ...
    def _generate_fast_correlation_features(self, step):
        ...
        try:
            ...
            timestamp = step % 1440  # Steps em um dia (1min intervals)
            session_factor = np.sin(2 * np.pi * timestamp / 1440)
            
            # 🧠 CORRELAÇÕES BASEADAS EM DADOS REAIS
            price = self.df['close_1m'].iloc[step]
            volume = self.df['volume_1m'].iloc[step]

            # Correlações calculadas dinamicamente
            spy_corr = np.clip(session_factor * 0.3 + 0.4, 0.1, 0.9)  # Evitar exatamente 0.5
            volume_corr = np.clip(np.log(volume + 1) / 15.0, 0.1, 0.9)
            price_corr = np.clip((price % 100) / 100.0, 0.1, 0.9)
            time_corr = np.clip(np.sin(step * 0.03) * 0.3 + 0.4, 0.1, 0.9)

            return np.array([spy_corr, volume_corr, price_corr, time_corr], dtype=np.float32)
        except:
            return np.full(4, 0.3, dtype=np.float32)
    ...
    def _generate_fast_momentum_features(self, step):
        ...
        try:
            ...
            # Momentum básico
            current = self.df['close_1m'].iloc[step]
            prev1 = self.df['close_1m'].iloc[step-1]
            prev3 = self.df['close_1m'].iloc[step-3]
            
            mom1 = (current - prev1) / prev1 if prev1 > 0 else 0
            mom3 = (current - prev3) / prev3 if prev3 > 0 else 0
            
            # 🧠 MOMENTUM FEATURES COMPLETAMENTE DINÂMICAS
            volume = self.df['volume_1m'].iloc[step]

            # Diferentes timeframes de momentum
            mom_acceleration = mom1 - mom3  # Aceleração do momentum
            volume_momentum = np.log(volume + 1) / 10.0
            momentum_strength = abs(mom1) + abs(mom3)
            momentum_consistency = 1.0 - abs(mom1 - mom3) * 10  # Consistência entre timeframes

            return np.array([
                np.clip(mom1 * 100, -0.9, 0.9),           # Momentum 1 período
                np.clip(mom3 * 100, -0.9, 0.9),           # Momentum 3 períodos
                np.clip(mom_acceleration * 50, 0.1, 0.9), # Aceleração
                np.clip(volume_momentum, 0.1, 0.9),       # Volume momentum
                np.clip(momentum_strength * 20, 0.1, 0.9), # Força do momentum
                np.clip(momentum_consistency, 0.1, 0.9)    # Consistência
            ], dtype=np.float32)
        except:
            return np.full(6, 0.3, dtype=np.float32)
    ...
    def _generate_fast_enhanced_features(self, step):
        ...
        try:
            ...
            # 🧠 ENHANCED FEATURES COMPLETAMENTE DINÂMICAS
            # Calcular features avançadas baseadas em dados reais
            high = self.df['high_1m'].iloc[step]
            low = self.df['low_1m'].iloc[step]

            # Pattern recognition features (baseadas em dados reais)
            range_pattern = (high - low) / close if close > 0 else 0.01
            volume_pattern = np.sin(np.log(volume + 1)) * 0.3 + 0.4
            price_pattern = (close % 10) / 10.0
            time_pattern = np.sin(step * 0.05) * 0.2 + 0.4

            # Regime detection features
            volatility_regime = np.clip(range_pattern * 5, 0.1, 0.9)
            volume_regime = np.clip(volume / 5000.0, 0.1, 0.9)
            price_regime = np.clip((close % 50) / 50.0, 0.1, 0.9)
            temporal_regime = np.clip(np.cos(step * 0.02) * 0.3 + 0.4, 0.1, 0.9)

            # Risk metrics
            range_risk = np.clip(range_pattern * 2, 0.1, 0.9)
            volume_risk = np.clip(1.0 - vol_norm, 0.1, 0.9)
            momentum_risk = np.clip(abs(close - 2000) / 100.0, 0.1, 0.9)
            time_risk = np.clip(np.sin(step * 0.1) * 0.2 + 0.3, 0.1, 0.9)

            # Temporal context
            session_context = np.clip((step % 1440) / 1440.0, 0.1, 0.9)
            trend_context = np.clip(np.tan(step * 0.001) * 0.1 + 0.4, 0.1, 0.9)
            cycle_context = np.clip(np.sin(step * 0.007) * 0.2 + 0.4, 0.1, 0.9)
            flow_context = np.clip(vol_norm * range_pattern, 0.1, 0.9)

            return np.array([
                # Pattern recognition (4 features)
                range_pattern, volume_pattern, price_pattern, time_pattern,

                # Regime detection (4 features)
                volatility_regime, volume_regime, price_regime, temporal_regime,

                # Risk metrics (4 features)
                range_risk, volume_risk, momentum_risk, time_risk,

                # Temporal context (4 features)
                session_context, trend_context, cycle_context, flow_context,

                # Additional enhanced features (4 features)
                np.clip(pattern_strength * 2, 0.1, 0.9),
                np.clip(vol_norm * 0.8, 0.1, 0.9),
                np.clip(range_pattern + vol_norm, 0.1, 0.9),
                np.clip(np.sin(volume) * 0.1 + 0.3, 0.1, 0.9)
            ], dtype=np.float32)
        except:
            return np.full(20, 0.3, dtype=np.float32)
    ...
    def _get_vectorized_temporal_features(self, seq_len):
        ...
        try:
            ...
            # 🚀 POSITIONS VECTORIZADO: 2 POSIÇÕES COMPLETAS (HÍBRIDO)
            positions_obs = np.full((2, 9), 0.001, dtype=np.float32)

            # Atualizar posições ativas (usando dados do step atual)
            for i in range(min(len(self.positions), 2)):
                pos = self.positions[i]
                current_price = self.df['close_1m'].iloc[self.current_step] if self.current_step < len(self.df) else pos.get('entry_price', 2000.0)
                
                # ⚡ REMOVIDO: Divisão por 10000 - formato XXXX.XX já correto
                entry_price = max(pos.get('entry_price', 0.01), 0.01) / 1000.0  # Normalização simples
                current_price_norm = max(current_price, 0.01) / 1000.0
                ...
                sl = max(pos.get('sl', 0.01), 0.01) / 1000.0 if pos.get('sl') else 0.01  # Normalização simples
                tp = max(pos.get('tp', 0.01), 0.01) / 1000.0 if pos.get('tp') else 0.01
            ...
            # 🔥 POSIÇÕES VAZIAS VARIÁVEIS - OLD-CHERRY STYLE (linha 5077-5092)
            # Usar variação baseada no step atual para evitar valores estáticos
            current_price_current = self.df['close_1m'].iloc[self.current_step] if self.current_step < len(self.df) else 2000.0
            current_price_norm_current = current_price_current / 1000.0
            ...
            # 🎯 USAR FUNÇÃO MASTER: SEM CONFUSÃO, SEM CONCATENAÇÃO
            # Cada step tem suas 45 features completas
            temporal_sequence = np.zeros((seq_len, 45), dtype=np.float32)

            for i in range(seq_len):
                step_idx = start_step + i
                # Função MASTER retorna TODAS as 45 features de uma vez!
                temporal_sequence[i, :] = self._get_complete_observation_45_features(step_idx)
    ...
    def _generate_vectorized_microstructure(self, start_step, seq_len):
        ...
        try:
            ...
            close_batch = self.df['close_1m'].iloc[start_step:start_step+seq_len].values
            volume_batch = self.df['volume_1m'].iloc[start_step:start_step+seq_len].values
            high_batch = self.df['high_1m'].iloc[start_step:start_step+seq_len].values
            low_batch = self.df['low_1m'].iloc[start_step:start_step+seq_len].values
    ...
    def _generate_vectorized_volatility(self, start_step, seq_len):
        ...
        try:
            ...
            close_data = self.df['close_1m'].iloc[window_start:start_step+seq_len].values
    ...
    def _generate_vectorized_momentum(self, start_step, seq_len):
        ...
        try:
            ...
            close_data = self.df['close_1m'].iloc[max(0, start_step-3):start_step+seq_len].values
    ...
    def _generate_order_flow_features(self, step):
        """
        🍒 ORDER FLOW ANALYSIS - HÍBRIDO
        Análise de microestrutura de mercado (4 features)
        """
        try:
            if step < 2 or step >= len(self.df):
                return np.full(4, 0.3, dtype=np.float32)

            # Obter dados OHLCV
            close = self.df['close_1m'].iloc[step]
            prev_close = self.df['close_1m'].iloc[step-1]
            high = self.df['high_1m'].iloc[step] if 'high_1m' in self.df.columns else close * 1.001
            low = self.df['low_1m'].iloc[step] if 'low_1m' in self.df.columns else close * 0.999
            volume = self.df['volume_1m'].iloc[step] if 'volume_1m' in self.df.columns else 100.0
            ...
            # 2. VOLUME IMBALANCE (direção do volume)
    ...
    def _get_single_bar_features(self, step):
        ...
        # 🧠 POSIÇÕES VAZIAS COM VALORES REALISTAS AO INVÉS DE FALLBACKS ESTÁTICOS
        current_price = self.df[f'close_{self.base_tf}'].iloc[step] if step < len(self.df) else 2000.0
        current_price_norm = current_price / 1000.0  # Normalização consistente

        for i in range(len(self.positions), self.max_positions):
            # Usar variação baseada no preço atual para evitar valores estáticos
            price_variation = (hash(f"{step}_{i}") % 100) / 10000.0  # Pequena variação determinística
            volume_variation = (hash(f"{step}_{i}_vol") % 50) / 100000.0

            # 🎯 FIX VIÉS: Randomizar direção de SL/TP para posições inativas
            # Metade terá padrão LONG, metade padrão SHORT
            random_direction = (hash(f"{step}_{i}_dir") % 2) * 2 - 1  # -1 ou +1
            if random_direction > 0:  # Padrão LONG
                sl = current_price_norm * 0.99   # SL abaixo
                tp = current_price_norm * 1.01   # TP acima
                position_type_inactive = 0.1     # Leve tendência LONG
            else:  # Padrão SHORT
                sl = current_price_norm * 1.01   # SL acima
                tp = current_price_norm * 0.99   # TP abaixo
                position_type_inactive = -0.1    # Leve tendência SHORT

            positions_obs[i, :] = [
                0.001 + price_variation,                           # [0] Inativa (com variação)
                current_price_norm + price_variation,              # [1] Entry price baseado no preço atual
                current_price_norm,                                # [2] Current price real
                -0.001 - price_variation,                          # [3] PnL pequeno negativo (inativo)
                0.1 + (hash(f"{step}_{i}_dur") % 100) / 1000.0,   # [4] Duration variável (não constante)
                0.001 + volume_variation,                          # [5] Volume pequeno variável
                sl,                                                # [6] SL balanceado (LONG ou SHORT)
                tp,                                                # [7] TP balanceado (LONG ou SHORT)
                position_type_inactive                             # [8] Type balanceado (-0.1 ou +0.1)
            ]
        ...
        for i in range(self.max_positions):
            if abs(positions_obs[i, 4]) < 1e-6:  # Duration no índice 4
                positions_obs[i, 4] = 0.1 + (hash(f"{step}_{i}_dur_fix") % 100) / 1000.0  # Forçar valor variável
        ...
        # 🎯 USAR FUNÇÃO MASTER: TODAS AS 45 FEATURES DE UMA VEZ, SEM CONFUSÃO!
        single_bar_obs = self._get_complete_observation_45_features(step)
    ...
    # =====================================================================
    # ⚠️ DEPRECATED FUNCTIONS - NÃO USAR! MANTER APENAS PARA COMPATIBILIDADE
    # Use _get_complete_observation_45_features() ao invés!
    # =====================================================================

    def _generate_intelligent_components_for_step(self, step):
        """⚠️ DEPRECATED - Use _get_complete_observation_45_features()"""
        # Mantido para compatibilidade, mas não é mais usado
        return np.full(37, 0.4, dtype=np.float32)

    def _generate_basic_v7_components(self, step):
        """⚠️ DEPRECATED - Use _get_complete_observation_45_features()"""
        # Mantido para compatibilidade, mas não é mais usado
        return np.full(2, 0.4, dtype=np.float32)

    def _get_complete_observation_45_features(self, step):
        """
        🎯 FUNÇÃO MASTER: RETORNA TODAS AS 45 FEATURES DE UMA VEZ

        SEM concatenações escondidas, SEM confusão!

        ESTRUTURA CLARA:
        ================
        [0-15]   = 16 MARKET FEATURES (OHLCV + indicators)
        [16-33]  = 18 POSITION FEATURES (2 positions × 9 features cada)
        [34-40]  = 7 INTELLIGENT FEATURES (volume_momentum, price_position, etc)
        [41-44]  = 4 ORDER FLOW FEATURES (spread, imbalance, impact, mm_signal)
        ...
        if step < 0 or step >= len(self.df):
            return np.full(45, 0.4, dtype=np.float32)

        # ============================================================
        # BLOCO 1: MARKET FEATURES [0-15] = 16 features
        # ============================================================
        try:
            if step < len(self.processed_data):
                obs[0:16] = self.processed_data[step, :16]
            else:
                obs[0:16] = 0.3
        except:
            obs[0:16] = 0.3
        ...
        # ============================================================
        # BLOCO 2: POSITION FEATURES [16-33] = 18 features (2 pos × 9)
        # ============================================================
        try:
            positions_obs = self._get_positions_observation(step)
            obs[16:34] = positions_obs.flatten()[:18]
        except Exception as e:
            # Fallback: valores pequenos, NÃO zero!
            obs[16:34] = 0.001

        # ============================================================
        # BLOCO 3: INTELLIGENT FEATURES [34-40] = 7 features
        # ============================================================
        try:
            # Feature 34: volume_momentum
            obs[34] = self.df['volume_momentum'].iloc[step] if 'volume_momentum' in self.df.columns else 0.4

            # Feature 35: price_position
            obs[35] = self.df['price_position'].iloc[step] if 'price_position' in self.df.columns else 0.4

            # Feature 36: breakout_strength (TP target zones)
            obs[36] = self.df['breakout_strength'].iloc[step] if 'breakout_strength' in self.df.columns else 0.4

            # Feature 37: trend_consistency
            obs[37] = self.df['trend_consistency'].iloc[step] if 'trend_consistency' in self.df.columns else 0.4

            # Feature 38: support_resistance (SL zone quality)
            obs[38] = self.df['support_resistance'].iloc[step] if 'support_resistance' in self.df.columns else 0.4

            # Feature 39: volatility_regime
            obs[39] = self.df['volatility_regime'].iloc[step] if 'volatility_regime' in self.df.columns else 0.4

            # Feature 40: market_structure (volatility spike)
            obs[40] = self.df['market_structure'].iloc[step] if 'market_structure' in self.df.columns else 0.4
        except:
            obs[34:41] = 0.4

        # ============================================================
        # BLOCO 4: ORDER FLOW FEATURES [41-44] = 4 features
        # ============================================================
        try:
            order_flow = self._generate_order_flow_features(step)
            obs[41:45] = order_flow[:4]
        except:
            obs[41:45] = 0.3

        return obs

    def _get_positions_observation(self, step):
        """
        🎯 GERAR OBSERVATION DE POSIÇÕES - ALINHADO COM OLD-CHERRY

        Retorna 2 positions × 9 features = 18 features
        Posições inativas têm valores PEQUENOS (0.001), não zero!
        """
        positions_obs = np.full((self.max_positions, 9), 0.001, dtype=np.float32)

        # Se não há posições, retornar array com valores pequenos
        if len(self.positions) == 0:
            return positions_obs

        # Preencher posições ativas
        for i in range(min(len(self.positions), self.max_positions)):
            pos = self.positions[i]

            # Normalizar valores
            current_price = self.df['close_1m'].iloc[step] if 'close_1m' in self.df.columns else self.df['close'].iloc[step]
            entry_price = pos.get('entry_price', current_price)
            sl_price = pos.get('sl', entry_price * 0.99)
            tp_price = pos.get('tp', entry_price * 1.01)

            # Normalizar preços por preço atual
            entry_norm = entry_price / current_price if current_price > 0 else 1.0
            sl_norm = sl_price / current_price if current_price > 0 else 0.99
            tp_norm = tp_price / current_price if current_price > 0 else 1.01

            # PnL normalizado
            pnl = pos.get('pnl', 0.0)
            pnl_norm = np.tanh(pnl / 100.0)  # Normalizar entre -1 e 1

            # Type: 1.0=LONG, -1.0=SHORT
            pos_type = 1.0 if pos.get('type') == 'long' else -1.0

            positions_obs[i, :] = [
                1.0,                                    # [0] Active flag
                entry_norm,                             # [1] Entry price (normalizado)
                1.0,                                    # [2] Current price (sempre 1.0 quando normalizado)
                pnl_norm,                               # [3] PnL normalizado
                min(pos.get('duration', 0) / 100.0, 1.0),  # [4] Duration normalizada
                min(pos.get('volume', 0.01), 1.0),      # [5] Volume normalizado
                sl_norm,                                # [6] SL price (normalizado)
                tp_norm,                                # [7] TP price (normalizado)
                pos_type                                # [8] Type (1.0 ou -1.0)
            ]

        return positions_obs

    def _get_intelligent_features_from_df(self, step):
        """
        🔥 EXTRAIR AS 7 INTELLIGENT FEATURES DE ALTA QUALIDADE DO DATAFRAME

        Estas features JÁ estão calculadas no df (linhas 4336-4451):
        1. volume_momentum - Momentum de volume vs média
        2. price_position - Posição do preço no range 20-bar
        3. breakout_strength - TP target zones (distância para S/R)
        4. trend_consistency - Consistência da tendência
        5. support_resistance - SL zone quality (distância normalizada)
        6. volatility_regime - Volatilidade 20 vs 50
        7. market_structure - Recent volatility spike
        ...
        try:
            ...
            # 1. VOLUME_MOMENTUM
            if 'volume_momentum' in self.df.columns:
                features.append(self.df['volume_momentum'].iloc[step])
            else:
                features.append(0.4)

            # 2. PRICE_POSITION
            if 'price_position' in self.df.columns:
                features.append(self.df['price_position'].iloc[step])
            else:
                features.append(0.4)

            # 3. BREAKOUT_STRENGTH (TP target zones)
            if 'breakout_strength' in self.df.columns:
                features.append(self.df['breakout_strength'].iloc[step])
            else:
                features.append(0.4)

            # 4. TREND_CONSISTENCY
            if 'trend_consistency' in self.df.columns:
                features.append(self.df['trend_consistency'].iloc[step])
            else:
                features.append(0.4)

            # 5. SUPPORT_RESISTANCE (SL zone quality)
            if 'support_resistance' in self.df.columns:
                features.append(self.df['support_resistance'].iloc[step])
            else:
                features.append(0.4)

            # 6. VOLATILITY_REGIME
            if 'volatility_regime' in self.df.columns:
                features.append(self.df['volatility_regime'].iloc[step])
            else:
                features.append(0.4)

            # 7. MARKET_STRUCTURE (volatility spike)
            if 'market_structure' in self.df.columns:
                features.append(self.df['market_structure'].iloc[step])
            else:
                features.append(0.4)
    ...
    def _generate_order_flow_features(self, step):
        """🍒 ORDER FLOW ANALYSIS - Substituição de microstructure genérica"""
        try:
            if step >= len(self.df) or step < 1:
                return np.full(4, 0.3, dtype=np.float32)
            ...
            # 2. VOLUME IMBALANCE (simulado via price action + volume)
            price_change = close - prev_close
            volume_intensity = np.clip(volume / 1000, 0.1, 10.0)
            # Simular imbalance: subida forte + volume alto = compra institucional
            volume_imbalance = np.tanh(price_change * volume_intensity * 0.001)
            volume_imbalance = np.clip(volume_imbalance, -0.5, 0.5) + 0.5  # [0,1]

            # 3. PRICE IMPACT ESTIMATE
            # Estimar impacto baseado em volume vs range
            if range_price > 0:
                price_impact = volume_intensity / (range_price + 1)
                price_impact = np.clip(price_impact, 0.1, 0.9)
            else:
            ...
            # 4. MARKET MAKER SIGNAL
            # MM presente quando: baixo volume + range pequeno = consolidação
            # MM ausente quando: alto volume + range grande = breakout
            mm_signal = 1 / (1 + volume_intensity * (range_price / close + 0.001))
            mm_signal = np.clip(mm_signal, 0.1, 0.9)
    ...
    def _generate_time_context_features(self, step):
        """🍒 TIME CONTEXT - Substituição de correlation genérica"""
        try:
            if step >= len(self.df):
                return np.full(3, 0.3, dtype=np.float32)

            # Obter timestamp se disponível, senão simular
            try:
                if hasattr(self.df, 'index') and hasattr(self.df.index, 'hour'):
                    hour = self.df.index[step].hour
                    minute = self.df.index[step].minute
                    weekday = self.df.index[step].weekday()
                else:
                    # Simular baseado em step
                    total_minutes = step % (7 * 24 * 60)  # Ciclo semanal
                    hour = (total_minutes // 60) % 24
                    minute = total_minutes % 60
                    weekday = total_minutes // (24 * 60)
            except:
                # Fallback: simular baseado em step
                total_minutes = step % (7 * 24 * 60)
                hour = (total_minutes // 60) % 24
                minute = total_minutes % 60
                weekday = total_minutes // (24 * 60)

            # 1. SESSION TYPE (Asia/London/NY/Overlap)
            # NY: 14-22 UTC, London: 8-17 UTC, Asia: 23-8 UTC
            if 8 <= hour < 14:
                session_type = 0.25  # London only
            elif 14 <= hour < 17:
                session_type = 0.75  # London-NY overlap (alta liquidez)
            elif 17 <= hour < 22:
                session_type = 0.5   # NY only
            else:
                session_type = 0.1   # Asia/low activity

            # 2. HOUR OF DAY (encoded as sin/cos composite)
            hour_angle = hour * 2 * np.pi / 24
            hour_encoded = (np.sin(hour_angle) + 1) / 2  # [0,1]

            # 3. MARKET AGE (time since market open - 9:30 NY = 14:30 UTC)
            market_open_hour = 14.5  # 14:30 UTC
            if hour >= market_open_hour:
                hours_since_open = hour - market_open_hour + minute/60
            else:
                hours_since_open = (24 - market_open_hour) + hour + minute/60

            market_age = np.clip(hours_since_open / 8.0, 0.1, 0.9)  # Normalize to 8h session

            return np.array([session_type, hour_encoded, market_age], dtype=np.float32)

        except:
            return np.full(3, 0.3, dtype=np.float32)

    def _generate_risk_management_features(self, step):
        """🍒 RISK MANAGEMENT - Substituição de momentum genérico"""
        try:
            if step >= len(self.df):
                return np.full(3, 0.3, dtype=np.float32)

            # 1. PORTFOLIO HEAT (% do capital em risco)
            total_risk = 0
            for pos in self.positions:
                if pos.get('active', False):
                    entry_price = pos.get('entry_price', 2000)
                    volume = pos.get('volume', 0.01)
                    sl = pos.get('sl', 0)
                    if sl > 0:
                        risk_per_position = abs(entry_price - sl) * volume
                        total_risk += risk_per_position

            portfolio_heat = np.clip(total_risk / self.portfolio_value, 0.0, 0.5)

            # 2. VOLATILITY REGIME (current vs historical)
            if step >= 20:
                try:
                    recent_returns = []
                    for i in range(max(0, step-20), step):
                        if i < len(self.df)-1:
                            ret = (self.df['close_1m'].iloc[i+1] - self.df['close_1m'].iloc[i]) / self.df['close_1m'].iloc[i]
                            recent_returns.append(ret)

                    if recent_returns:
                        current_vol = np.std(recent_returns) * np.sqrt(1440)  # Annualized
                        # Regime: 0.1=low vol, 0.5=normal, 0.9=high vol
                        vol_regime = np.clip(current_vol / 0.2, 0.1, 0.9)
                    else:
                        vol_regime = 0.5
                except:
                    vol_regime = 0.5
            ...
            # 3. LIQUIDITY SCORE (baseado em volume recente)
            if step >= 5:
                try:
                    recent_volumes = self.df['volume_1m'].iloc[max(0, step-5):step+1].values
                    avg_volume = np.mean(recent_volumes)
                    current_volume = self.df['volume_1m'].iloc[step]
                    liquidity_score = np.clip(current_volume / (avg_volume + 1), 0.1, 0.9)
                except:
                    liquidity_score = 0.5
            else:
                liquidity_score = 0.5

            return np.array([portfolio_heat, vol_regime, liquidity_score], dtype=np.float32)
    ...
    def _process_dynamic_trailing_stop(self, pos, sl_adjust, tp_adjust, current_price, pos_index):
        ...
        if current_pnl >= 100:
            print(f"🛑 [PnL CAP $100] Auto-closing position at ${current_pnl:.2f}")
            self._close_position(pos, self.current_step)
            result['pnl_cap_reached'] = True
            result['action_taken'] = True
            return result

        # 🎯 SL TRAILING ONLY (A FAVOR DO TRADE)
            ...
            if current_tp > 0:
                ...
                if pos['type'] == 'long':
                    ...
                    if new_tp > current_price + 3.0:
                        ...
                        potential_pnl = (new_tp - pos['entry_price']) * pos.get('volume', 0.01)
                    ...
                    if new_tp < current_price - 3.0:
                        # Check if new TP would exceed $100 cap
                        potential_pnl = (pos['entry_price'] - new_tp) * pos.get('volume', 0.01)
                        if potential_pnl <= 100:  # Respect $100 cap
                            result['position_updates']['tp'] = new_tp
                            result['tp_adjusted'] = True
                            result['action_taken'] = True
    ...
    def test_quadruple_selectors_silus(self):
        """🧪 Teste dos Seletores Quadruplos no silus.py"""
        print("🧪 [SILUS TEST] Testando Seletores Quadruplos...")
        ...
        # Teste 1-4: Mesmos testes do RobotV7
        test_values = [(-0.8, "SL Afrouxar"), (-0.3, "SL Apertar"), (0.3, "TP Próximo"), (1.0, "TP Distante")]
        
        for mgmt_val, desc in test_values:
            sl_adj, tp_adj = convert_management_to_sltp_adjustments(mgmt_val)
            print(f"🔍 pos1_mgmt = {mgmt_val} → sl_adjust={sl_adj}, tp_adjust={tp_adj} ({desc})")
        
        print("✅ [SILUS TEST] Seletores Quadruplos mapeados corretamente!")
        print("💡 [SILUS INFO] silus.py agora processa tp_adjust para modificar TP dinamicamente!")
    ...
    def _classify_market_regime(self, current_idx):
        ...
        try:
            ...
            # 🔥 FIFTEEN FIX 1: Trend strength mais sensível com slope da SMA
            if 'sma_20_1m' in self.df.columns:
                sma_20 = self.df['sma_20_1m'].iloc[current_idx-lookback:current_idx].values
                price = self.df['close_1m'].iloc[current_idx-lookback:current_idx].values

                # Calcular slope da SMA para detectar tendência
                sma_diff = np.diff(sma_20[-20:]) if len(sma_20) >= 20 else np.diff(sma_20)
                trend_slope = np.mean(sma_diff) / np.mean(sma_20[-20:]) if np.mean(sma_20[-20:]) > 0 else 0.0

                # Distance from SMA (normalizado)
                price_vs_sma = np.mean(price - sma_20) / np.mean(sma_20) if np.mean(sma_20) > 0 else 0.0

                # Combined trend strength
                trend_strength = trend_slope + price_vs_sma

                # 🎯 SIXTEEN FIX: THRESHOLDS REALISTAS (4x menos sensíveis): 0.002 → 0.0005
                direction = 1.0 if trend_strength > 0.0005 else (-1.0 if trend_strength < -0.0005 else 0.0)

                # 🎯 Classificar regime com direção
                abs_strength = abs(trend_strength)

                if abs_strength > 0.005:  # Trending (ajustado de 1.0)
                    if trend_strength > 0:
                        regime = 'trending_up'
                    else:
                        regime = 'trending_down'  # CRÍTICO para evitar compras
                elif abs_strength < 0.001:  # Ranging (ajustado de 0.3)
                    regime = 'ranging'
                else:
                    regime = 'volatile'

                # 🚨 SIXTEEN FIX: DETECTOR DE CRASH MAIS CONSERVADOR: -3% → -1.5%
                price_change_pct = (price[-1] - price[0]) / price[0] if price[0] > 0 else 0.0
                if price_change_pct < -0.015:  # Queda > 1.5% em 50min (~61 pontos no gold)
                    regime = 'crash'  # Modelo NUNCA deve comprar em crash
            else:
                # Fallback usando preços
                prices = self.df['close_1m'].iloc[current_idx-lookback:current_idx].values
    ...
    def _analyze_volatility_context(self, current_idx):
        ...
        try:
            ...
            if 'atr_14_1m' in self.df.columns:
                atr_values = self.df['atr_14_1m'].iloc[current_idx-lookback:current_idx].values
    ...
    def _calculate_momentum_confluence(self, current_idx):
        """🔥 SIXTEEN FIX 3: Calcular confluência de momentum COM CONTEXTO DE TREND (sem recursive call)"""
        try:
            lookback = min(14, current_idx)
            if lookback < 5:
                return {'score': 0.25, 'direction': 0.1, 'strength': 0.25}  # 🔧 Valores não-zero

            # 🎯 SIXTEEN FIX: Calcular direção localmente (sem chamar _classify_market_regime)
            trend_direction = 0.0
            if 'sma_20_1m' in self.df.columns and 'close_1m' in self.df.columns:
                current_price = self.df['close_1m'].iloc[current_idx]
                current_sma = self.df['sma_20_1m'].iloc[current_idx]
                trend_direction = 1.0 if current_price > current_sma else -1.0

            confluence_score = 0.0
            direction_sum = 0.0
            indicators_count = 0

            # 🔥 RSI CONTEXTUALIZADO COM TREND
            if 'rsi_14_1m' in self.df.columns:
                rsi = self.df['rsi_14_1m'].iloc[current_idx]

                if rsi < 30:  # Oversold
                    if trend_direction > 0:  # Trend bullish
                        confluence_score += 0.5  # Oversold em alta = buy opportunity
                        direction_sum += 1.0
                    else:  # Trend bearish ou neutro
                        confluence_score += 0.2  # Oversold em queda = stay out!
                        direction_sum += 0.0  # NÃO dá sinal de compra
                elif rsi > 70:  # Overbought
                    if trend_direction < 0:  # Trend bearish
                        confluence_score += 0.5  # Overbought em queda = short opportunity
                        direction_sum -= 1.0
                    else:  # Trend bullish ou neutro
            ...
            # MACD
            if 'macd_12_26_9_1m' in self.df.columns and 'macd_signal_12_26_9_1m' in self.df.columns:
                macd = self.df['macd_12_26_9_1m'].iloc[current_idx]
                macd_signal = self.df['macd_signal_12_26_9_1m'].iloc[current_idx]
            ...
            if 'sma_10_1m' in self.df.columns and 'sma_20_1m' in self.df.columns:
                sma_10 = self.df['sma_10_1m'].iloc[current_idx]
                sma_20 = self.df['sma_20_1m'].iloc[current_idx]
    ...
    def _detect_liquidity_zones(self, current_idx):
        ...
        try:
            ...
            highs = self.df['high_1m'].iloc[current_idx-lookback:current_idx].values
            lows = self.df['low_1m'].iloc[current_idx-lookback:current_idx].values
            current_price = self.df['close_1m'].iloc[current_idx]
    ...
    def _calculate_risk_metrics_simplified(self, current_idx):
        ...
        try:
            ...
            # 2. Volatility Risk
            if 'atr_14_1m' in self.df.columns:
                atr = self.df['atr_14_1m'].iloc[current_idx]
    ...
    def _generate_v7_basic_components(self, current_idx, market_regime, volatility_context, momentum_confluence, risk_assessment):
        ...
        """
        try:
            # 🔥 FIFTEEN FIX 4: Codificação preserva DIREÇÃO do regime
            if isinstance(market_regime, dict):
                # Codificar regime COM DIREÇÃO preservada
                regime_encoding = {
                    'trending_up': 1.0,
                    'trending_down': -1.0,  # CRÍTICO! Modelo precisa saber direção
                    'crash': -2.0,  # SUPER CRÍTICO! Nunca compre em crash
                    'ranging': 0.0,
                    'volatile': 0.5,
                    'trending': 0.5,  # Fallback para regime genérico (não deveria acontecer)
                    'unknown': 0.0
                }
                regime_str = market_regime.get('regime', 'unknown')
                regime_encoded = regime_encoding.get(regime_str, 0.0)

                market_regime = np.array([
                    market_regime.get('strength', 0.3 + (current_idx % 50) / 100.0),  # Dinâmico 0.3-0.8
                    market_regime.get('direction', 0.0),
                    regime_encoded  # 🔥 PRESERVA DIREÇÃO! -2.0=crash, -1.0=down, 0.0=neutral, 1.0=up
                ], dtype=np.float32)
            elif not isinstance(market_regime, np.ndarray):
                market_regime = np.array([0.3 + (current_idx % 50) / 100.0, 0.0, 0.0], dtype=np.float32)  # Neutral default
                
            if isinstance(volatility_context, dict):
                volatility_context = np.array([
                    volatility_context.get('percentile', 0.3 + (current_idx % 40) / 100.0),  # Dinâmico
                    1.0 if volatility_context.get('expanding', False) else 0.0,
                    1.0 if volatility_context.get('level', 'normal') == 'high' else (0.2 + (current_idx % 35) / 100.0)  # Dinâmico
                ], dtype=np.float32)
            elif not isinstance(volatility_context, np.ndarray):
                volatility_context = np.array([0.3 + (current_idx % 40) / 100.0, 0.0, 0.2 + (current_idx % 35) / 100.0], dtype=np.float32)
                
            if isinstance(momentum_confluence, dict):
                momentum_confluence = np.array([
                    momentum_confluence.get('score', 0.3 + (current_idx % 45) / 100.0),  # Dinâmico
                    momentum_confluence.get('direction', 0.0),
                    momentum_confluence.get('strength', 0.2 + (current_idx % 60) / 100.0)  # Dinâmico
                ], dtype=np.float32)
            elif not isinstance(momentum_confluence, np.ndarray):
                momentum_confluence = np.array([0.3 + (current_idx % 45) / 100.0, 0.0, 0.2 + (current_idx % 60) / 100.0], dtype=np.float32)
                
            if isinstance(risk_assessment, dict):
                risk_assessment = np.array([
                    risk_assessment.get('drawdown_risk', 0.2 + (current_idx % 55) / 100.0),  # Dinâmico
                    risk_assessment.get('volatility_risk', 0.25 + (current_idx % 25) / 100.0),  # Dinâmico
                    risk_assessment.get('position_risk', 0.3 + (current_idx % 70) / 100.0)   # Dinâmico
                ], dtype=np.float32)
            elif not isinstance(risk_assessment, np.ndarray):
                risk_assessment = np.array([0.2 + (current_idx % 55) / 100.0, 0.25 + (current_idx % 25) / 100.0, 0.3 + (current_idx % 70) / 100.0], dtype=np.float32)
    ...
    def _calculate_reward_and_info(self, action, old_state):
        ...
        if len(action) >= 4:
            ...
            # 🔬 LISTAS CIRÚRGICAS: apenas 2 posições ativas (otimizado)
            sl_adjusts = [pos1_sl_adjust, pos2_sl_adjust]
            tp_adjusts = [pos1_tp_adjust, pos2_tp_adjust]
            
            # 🚨 SISTEMA DE COOLDOWN ANTI-OVERTRADING (por slot)
            ...
            # Se houver espaço para nova posição, encontrar primeiro slot disponível
            available_slot = None
            if entry_decision > 0 and len(self.positions) < self.max_positions:
                ...
                # Se não encontrou slot disponível, bloquear entrada
        ...
        if entry_decision > 0 and len(self.positions) < self.max_positions and available_slot is not None:
            # 🎯 FILTRO DE CONFIANÇA MÍNIMA - DRASTICAMENTE REDUZIDO 
            ...
            if entry_confidence < MIN_CONFIDENCE_THRESHOLD:
                ...
                position = {
                    ...
                    'position_id': available_slot  # Usar slot específico encontrado
                }
                ...
                # 🔬 ESCOLHA CIRÚRGICA: SL/TP baseado na posição (2 posições apenas)
                ...
                else:  # ⚠️ ERRO: Não deveria haver terceira posição
                    raise ValueError(f"🔬 CIRÚRGICO: Tentativa de criar posição {pos_index} > max_positions=2")
                
                # 🎯 UNIFICADO: Usar função unificada para criação
                creation_result = convert_model_adjustments_to_points(sl_adjust, tp_adjust, "creation")
                sl_points = creation_result['sl_points']  # Já positivo e validado
                tp_points = creation_result['tp_points']  # Já positivo e validado
        ...
        if entry_decision > 0 and len(self.positions) < self.max_positions and entry_confidence < 0.2:  # Usar mesmo threshold
            ...
            for i, pos in enumerate(self.positions):
                if i < 2:  # 🔬 CIRÚRGICO: Máximo 2 posições
                    ...
                    if trailing_result['action_taken']:
                        ...
                        # Marcar informações para reward system (NEW SYSTEM doesn't use trailing_activated)
            ...
            for pos in self.positions[:]:
                ...
                if duration > 2880:  # 48 HORAS máximo conforme especificação da política
                    self._close_position(pos, self.current_step)
                    action_taken = True
        ...
        # JSONL: log detalhado de reward (quando disponível), com frequência moderada
        try:
            if 'jsonl_logger' in globals() or True:
                # Acessar ConvergenceLogger via variável local se existir
                # (self.current_model treina dentro de AdvancedTrainingSystem; usamos referência global abaixo)
                pass
        except Exception:
            pass
        ...
        # JSONL: logar reward info detalhado quando logger disponível
        try:
            # Buscar logger no escopo mais alto
            global convergence_logger
            if 'convergence_logger' in globals() and convergence_logger and convergence_logger.jsonl_logger:
                if self.current_step % 200 == 0:  # frequência moderada
                    detailed = {
                        'base_reward': float(reward),
                        'has_components': bool(info and 'reward_components' in info),
                        'quality_score': float(info.get('quality_score', 0)) if isinstance(info, dict) else 0.0,
                        'market_quality': info.get('market_quality', '') if isinstance(info, dict) else '',
                        'entry_confidence': float(info.get('entry_confidence', 0)) if isinstance(info, dict) else 0.0,
                        'ma_cross_long': bool(info.get('ma_cross_long', False)) if isinstance(info, dict) else False,
                        'ma_cross_short': bool(info.get('ma_cross_short', False)) if isinstance(info, dict) else False,
                        'double_bottom': bool(info.get('double_bottom', False)) if isinstance(info, dict) else False,
                        'double_top': bool(info.get('double_top', False)) if isinstance(info, dict) else False,
                        'position_time_ratio': float(info.get('position_time_ratio', 0)) if isinstance(info, dict) else 0.0
                    }
                    convergence_logger.jsonl_logger.log_reward_info(self.current_step, detailed)
        except Exception:
            pass
    ...
    def _get_trades_today(self):
        ...
        try:
            ...
            # 🔧 FIX: Trades por dia baseado em steps (1440 steps = 1 dia em 1min)
            steps_per_day = 1440  # CORRIGIDO: 288 (5min) → 1440 (1min)
    ...
    def _close_position(self, position, exit_step):
        ...
        # 🎯 TRACKING PARA COOLDOWN ADAPTATIVO
        # Atualizar status do último trade para o sistema de cooldown
        if pnl > 0:
        ...
        trade_info = {
            ...
            'tp_adjusted': position.get('tp_adjusted', False),  # TP ajustado dinamicamente
        }

        # 🧠 DADOS PARA REWARD INTELIGENTE: Detectar movimento de preço durante trailing
        if position.get('trailing_activated', False):
            # Calcular direção do preço durante trailing
            entry_step = position.get('entry_step', self.current_step - 100)
            trailing_activation_step = position.get('trailing_activation_step', entry_step)

            if trailing_activation_step < self.current_step:
                # Preço no início vs fim do trailing
                price_start = self.df[f'close_{self.base_tf}'].iloc[trailing_activation_step]
                price_end = current_price
                price_movement = 1 if price_end > price_start else -1

                trade_info['price_direction_during_trail'] = price_movement
                trade_info['final_sl'] = position.get('sl', 0)
        ...
        # 🚨 ATIVAR COOLDOWN ADAPTATIVO POR SLOT após fechar trade
        # Ajustar cooldown baseado no resultado do trade
        if hasattr(self.reward_system, 'get_suggested_cooldown'):
            self.cooldown_after_trade = self.reward_system.get_suggested_cooldown()
        else:
            # Fallback para cooldown adaptativo manual
            if self.last_trade_was_loss:
                if self.consecutive_losses >= 3:
                    self.cooldown_after_trade = 60  # 🔧 CORRETO: 60 steps = 60min em timeframe 1min
                else:
                    self.cooldown_after_trade = 45  # 🔧 CORRETO: 45 steps = 45min em timeframe 1min
            elif self.last_trade_was_win:
                if self.consecutive_wins >= 3:
                    self.cooldown_after_trade = 30  # 🔧 CORRETO: 30 steps = 30min em timeframe 1min
                else:
                    self.cooldown_after_trade = 25  # 🔧 CORRETO: 25 steps = 25min em timeframe 1min
            else:
                self.cooldown_after_trade = self.cooldown_base  # Default

        # Aplicar cooldown ao slot associado à posição fechada (position_id definido na criação)
        try:
            slot_idx = int(position.get('position_id', 0))
            if slot_idx in self.slot_cooldowns:
                self.slot_cooldowns[slot_idx] = int(self.cooldown_after_trade)
                if self.current_step % 50 == 0:  # Log esporádico
                    result_str = "WIN" if self.last_trade_was_win else "LOSS" if self.last_trade_was_loss else "NEUTRAL"
                    print(f"[COOLDOWN-SLOT] Fechou {result_str} no slot {slot_idx} → cooldown {self.cooldown_after_trade} steps")
        except Exception:
            pass

    def _add_trade(self, trade_info):
        """
        🔧 MÉTODO CENTRALIZADO - Adiciona trade evitando duplicações
        Ponto único para registrar todos os trades
        """
    ...
    def _get_position_pnl(self, pos, current_price):
        #  ESCALA REAL DO OURO: XXXX.XX 
        # 1 ponto = $1 USD com 0.01 lot
        # Fórmula correta: price_diff * lot_size * 100
        price_diff = 0
        if pos['type'] == 'long':
        ...
        # ESCALA CORRETA: 100 (padrão ouro)
        # 10 pontos × 0.02 lot × 100 = $20 USD
...
REALISTIC_SLTP_CONFIG = {
    # 🎯 RANGES BASEADOS EM ANÁLISE REAL DO OURO 1MIN
    # Análise: ATR médio 0.96pt, SL 10pt = 28.9% hit rate, TP 12-18pt = 25-37% hit rate
    'sl_min_points': 10,    # SL mínimo: 10 pontos ($10 risk com 0.01 lot) - SWEET SPOT
    'sl_max_points': 15,    # SL máximo: 15 pontos ($15 risk) - Ajustado de 25pt (hit rate <17%)
    'tp_min_points': 12,    # TP mínimo: 12 pontos ($12 reward) - 36.6% hit rate em ~3.5h
    'tp_max_points': 18,    # TP máximo: 18 pontos ($18 reward) - Ajustado de 25pt (hit rate <15%)
...
def convert_model_adjustments_to_points(sl_adjust, tp_adjust, context="adjustment"):
    ...
    SUBSTITUI TODAS AS DUPLICAÇÕES:
    - convert_action_to_realistic_sltp() [DEPRECADO]
    - tp_adjust * 5.0 [BUGADO]
    ...
    # Usar configuração do próprio silus.py
    ...
    def perform_immediate_evaluation(self, eval_request):
        ...
        def evaluate():
            ...
            try:
                ...
                total_days = total_steps / 1440  # 1440 steps = 1 dia (1min bars)
...
class AdvancedTrainingSystem:
    ...
    def _create_training_phases(self) -> List[TrainingPhase]:
        """🎯 TREINO SIMPLES E DIRETO - SEM CURRICULUM LEARNING"""
        return [
            TrainingPhase(
                name="Simple_Direct_Training",
                phase_type=PhaseType.FUNDAMENTALS,
                timesteps=5000000,  # 5M steps total - sweet spot comprovado
                description="Treinamento direto no dataset completo - sem filtros, sem fases",
                data_filter=None,  # 🔥 SEM FILTROS - dataset completo sempre
                success_criteria={
                    "episode_return": 0.05,  # 5% retorno médio por episódio
                    "win_rate": 0.40,        # 40% win rate realista
                },
                reset_criteria={
                    "episode_return": -0.50,  # Reset só se performance terrível
                    "win_rate": 0.10          # Reset só se win rate absurdo
                }
            )
        ]
        ...
        try:
            ...
            if checkpoint_path_found and os.path.exists(checkpoint_path_found):
                ...
                try:
                    ...
                    validate_v11_sigmoid_policy(self.current_model.policy)
            ...
            class RobustSaveCallback(BaseCallback):
                def __init__(self, save_freq=25000, save_path=DIFF_MODEL_DIR, name_prefix=EXPERIMENT_TAG, total_steps_offset=0, training_env=None):
                    super().__init__()
                    self.save_freq = save_freq
                    self.save_path = save_path
                    self.name_prefix = name_prefix
            ...
            for phase_idx in range(self.current_phase_idx, total_phases):
                ...
                # ⚠️ CONFIGURAR EARLY STOPPING BASEADO EM PERFORMANCE  
                early_stopping = ImprovedEarlyStopping(
                    patience=250000,     # 250k steps = ~17 dias (tempo realista para confirmar degradação)
                    min_improvement=0.015 # 1.5% melhoria (mercado é ruidoso, valor realista)
                )
                
                # 🔥 CALLBACK QUE USA O EARLY STOPPING
                class PerformanceEarlyStoppingCallback(BaseCallback):
                    def __init__(self, early_stopping_system):
                        super().__init__()
                        self.early_stopping = early_stopping_system
                        self.episode_buffer = []
                        
                    def _on_step(self):
                        # Capturar episódios completos
                        if hasattr(self.training_env, 'get_episode_buffer'):
                            episode_buffer = self.training_env.get_episode_buffer()
                            if len(episode_buffer) > len(self.episode_buffer):
                                self.episode_buffer = episode_buffer
                                
                        # Verificar se deve parar
                        if len(self.episode_buffer) > 30:  # Mínimo para análise
                            should_stop = self.early_stopping.should_stop(
                                self.num_timesteps, 
                                self.episode_buffer
                            )
                            if should_stop:
                                print(f"🛡️ PERFORMANCE EARLY STOPPING ATIVADO!")
                                return False  # Para o treinamento
                        return True
                
                performance_early_stop = PerformanceEarlyStoppingCallback(early_stopping)
                
                # ⚡ ENTROPY ANNEALING CALLBACK - EXPLORAÇÃO PROGRESSIVA
                entropy_annealing_callback = EntropyAnnealingCallback(
                    total_steps=5_000_000,
                    verbose=1
                )

                # 🎯 HIGH FREQUENCY CHECKPOINT CALLBACK - ISOLAR MELHOR MODELO
                high_freq_checkpoint = HighFrequencyCheckpointCallback(
                    start_step=1_500_000,    # 1.5M steps
                    end_step=1_600_000,      # 1.6M steps
                    save_freq=5_000,         # A cada 5K steps
                    save_path=DIFF_MODEL_DIR,
                    name_prefix=EXPERIMENT_TAG,
                    total_steps_offset=self.total_steps_completed,  # Offset correto
                    training_env=env,
                    verbose=1
                )
                ...
                base_callbacks = [
                    ...
                    metrics_callback,
                    progress_callback,
                    entropy_annealing_callback,  # ⚡ NOVO: Entropy annealing para convergência
                    high_freq_checkpoint,        # 🎯 NOVO: Checkpoints a cada 5K no range 1.5M-1.6M
                    # early_stopping_callback,  # 🚫 DESABILITADO: Para aos 1.2M por falta melhoria 2%
                    # performance_early_stop,   # 🚫 DESABILITADO: Causava parada prematura do treino
                ...
                # 🔍 DEBUG: Mostrar valores antes de iniciar treinamento
                print(f"\n🔍 DEBUG PRE-TRAINING:")
                print(f"   self.total_steps_completed = {self.total_steps_completed:,}")
                print(f"   current_phase.timesteps = {current_phase.timesteps:,}")
                print(f"   remaining_steps = {remaining_steps:,}")
                print(f"   phase_idx = {phase_idx}")
                print(f"   self.current_phase_idx = {self.current_phase_idx}")
                print(f"   model.num_timesteps = {self.current_model.num_timesteps:,}")
                print(f"   reset_num_timesteps = False")
                print(f"🔍 O modelo vai treinar por: {remaining_steps:,} steps\n")

                # 🚀 EXECUTAR TREINAMENTO SIMPLES
                # 🔍 WRAPPER CALLBACK PARA DETECTAR QUAL CALLBACK ESTÁ PARANDO
                class CallbackDebugWrapper(BaseCallback):
                    def __init__(self, callback, name):
                        super().__init__()
                        self.wrapped_callback = callback
                        self.name = name

                    def _on_training_start(self):
                        # Propagar model e outras propriedades para o callback interno
                        self.wrapped_callback.model = self.model
                        self.wrapped_callback.training_env = self.training_env
                        self.wrapped_callback.n_calls = self.n_calls
                        self.wrapped_callback.num_timesteps = self.num_timesteps
                        if hasattr(self.wrapped_callback, '_on_training_start'):
                            self.wrapped_callback._on_training_start()

                    def _on_step(self) -> bool:
                        # Sincronizar TODAS as propriedades necessárias antes de chamar _on_step
                        self.wrapped_callback.model = self.model
                        self.wrapped_callback.training_env = self.training_env
                        self.wrapped_callback.n_calls = self.n_calls
                        self.wrapped_callback.num_timesteps = self.num_timesteps
                        self.wrapped_callback.locals = self.locals  # 🔥 PROPAGAR LOCALS para captura de ações
                        self.wrapped_callback.globals = self.globals  # 🔥 PROPAGAR GLOBALS
                        self.wrapped_callback.parent = self.parent if hasattr(self, 'parent') else None

                        result = self.wrapped_callback._on_step() if hasattr(self.wrapped_callback, '_on_step') else True
                        if not result:
                            print(f"\n🚨 CALLBACK '{self.name}' RETORNOU FALSE! Parando treinamento aos {self.num_timesteps:,} steps")
                            import traceback
                            traceback.print_stack()
                        return result

                # Wrap all callbacks with debug wrapper
                wrapped_callbacks = []
                for i, cb in enumerate(all_callbacks):
                    cb_name = cb.__class__.__name__
                    wrapped = CallbackDebugWrapper(cb, cb_name)
                    wrapped_callbacks.append(wrapped)

                debug_combined_callback = CallbackList(wrapped_callbacks)

                # Treinar toda a fase de uma vez
                self.current_model.learn(
                    total_timesteps=remaining_steps,
                    callback=debug_combined_callback,
        ...
        for checkpoint_dir in checkpoint_dirs:
            if os.path.exists(checkpoint_dir):
                for file in os.listdir(checkpoint_dir):
                    if file.endswith('.zip') and (EXPERIMENT_TAG.lower() in file.lower() or False):
    ...
    def _create_model(self, env):
        ...
        if checkpoint_path:
            ...
            try:
                ...
                validate_v11_sigmoid_policy(model.policy)
        ...
        def lr_schedule_lstm_warmup(progress):
            ...
            base_lr = BEST_PARAMS["learning_rate"]  # 🎯 Usar BEST_PARAMS (3e-05)
        ...
        model_config = {
            ...
            "env": env,
            "learning_rate": BEST_PARAMS["learning_rate"],  # 🎯 USANDO BEST_PARAMS
            "n_steps": BEST_PARAMS["n_steps"],              # 🔥 CORRIGIDO: 2048→1024 para updates mais frequentes
            "batch_size": BEST_PARAMS["batch_size"],        # 🚀 OTIMIZADO: 1024 para MÁXIMA estabilidade
            ...
            "vf_coef": BEST_PARAMS["vf_coef"],            # 🎯 USANDO BEST_PARAMS: 0.5
            ...
            "policy_kwargs": {
                **get_v11_sigmoid_kwargs(),  # 🔥 BYPASS: Compressão 450D→64D + 7 features bypass
                # 🚨 CRITIC LR: usar BEST_PARAMS
        ...
        self.logger.info(f"🚀 Critic Learning Rate: {BEST_PARAMS['critic_learning_rate']}")
        self.logger.info(f"📈 Batch Size: {model_config['batch_size']}")
        self.logger.info(f"⚡ Device: {model_config['device']}")
        self.logger.info(f"🚀 TwoHeadV11Sigmoid: LSTM Única + Heads Específicos + SILU Activation (V11)")
        self.logger.info(f"✅ V11 Sigmoid Features: Entry Head Específico + Management Head Específico + Memory Elegante")
        self.logger.info(f"✅ V11 Sigmoid Philosophy: Simplicidade Focada + Uma LSTM + 4D Actions + SILU Activation")
        self.logger.info("=" * 60)
    ...
    def _calculate_detailed_metrics(self, episode_rewards, portfolio_values, drawdowns, trades, total_steps, eval_name):
        ...
        try:
            ...
            return {
                ...
                'trades_per_day': float(total_trades / max(total_steps / 1440, 1)),  # 🔧 FIX: 1440 steps = 1 day (1min timeframe)
...
def print_gold_spec_banner():
    """🎯 Exibe banner do sistema SILUS simplificado"""
    print("\n" + "🎯" * 60)
    print("🚀 SILUS SIMPLE - TREINAMENTO DIRETO E EFICAZ")
    print("🎯" * 60)
    print("🎯 TARGET: Trader excepcional para GOLD (GC_YAHOO)")
    print(f"📊 TRAINING: {TRAINING_CONFIG['total_timesteps']/1000000:.1f}M steps - treinamento direto sem fases")
    print("🏅 GOALS: Win Rate >55%, Profit Factor >1.5, Sharpe >1.2")
    print("⚡ ARCHITECTURE: Simplicidade e Eficiência")
    print("🎯" * 60)
    
    print("\n🎯 TREINAMENTO SIMPLIFICADO:")
    print("  ✅ UMA FASE: Treinamento direto no dataset completo")
    print("  ✅ SEM FILTROS: Todo o histórico disponível")
    print("  ✅ SEM CURRICULUM: Aprendizado natural e orgânico")
    print("  ✅ EARLY STOPPING: Performance-based inteligente")
    
    print("\n🔧 HYPERPARAMETERS COMPROVADOS:")
    ...
    print(f"  Cooldown: {7} steps após trade")
    print("🎯" * 60)
