...
import pandas as pd
import time
# import tkinter as tk  # Removido - duplicado
# from tkinter import scrolledtext, ttk  # Removido - duplicado
from threading import Thread, Event
...
class TradingAppV7:
    ...
    def __init__(self, root):
        ...
        # 🔧 FIX: Thread-safe GUI management
        import queue
        import threading
        ...
        self.update_stats()
        # Garantir visibilidade da janela ao iniciar e manter um heartbeat de foco/visibilidade
        try:
            self._force_show_window(initial=True)
            self._bind_visibility_events()
            self.safe_after(2000, self._smart_visibility_heartbeat)  # Use smart heartbeat
        except Exception:
            pass
    ...
    def _force_show_window(self, initial=False):
        ...
        try:
            ...
            # Pulsar topmost para trazer à frente
            self.root.lift()
            ...
            try:
                ...
                pass
            # Windows: usar Win32 API para restaurar a janela do taskbar de forma robusta
            try:
                import ctypes
            ...
            if initial:
                # Em algumas máquinas, uma segunda pulsação ajuda
                self.root.after(600, lambda: (self.root.lift(), self.root.attributes('-topmost', True), self.root.after(200, lambda: self.root.attributes('-topmost', False))))
        except Exception:
            pass

    def _ensure_within_screen(self):
        """Recoloca a janela dentro dos limites visíveis da tela se necessário."""
        try:
            self.root.update_idletasks()
            sw = self.root.winfo_screenwidth()
            sh = self.root.winfo_screenheight()
            w = max(400, self.root.winfo_width() or 1200)
            h = max(300, self.root.winfo_height() or 800)
            x = self.root.winfo_x()
            y = self.root.winfo_y()
            # Se estiver muito fora, reposiciona para um ponto visível
            offscreen = (x < -w//2) or (y < -h//2) or (x > sw - 50) or (y > sh - 50)
            if offscreen:
                nx = min(max(50, (sw - w)//2), max(50, sw - w - 50))
                ny = min(max(50, (sh - h)//2), max(50, sh - h - 50))
                self.root.geometry(f"{w}x{h}+{nx}+{ny}")
        except Exception:
            pass

    # 🔧 FIX: Removed old _visibility_heartbeat - replaced with _smart_visibility_heartbeat

    def _bind_visibility_events(self):
        """Garante que ao ser clicada/mostrada pelo Windows a GUI apareça imediatamente."""
        try:
            # Quando a janela é mapeada (desminimizada) ou recebe foco, forçar exibir
            self.root.bind("<Map>", lambda e: self._gentle_restore_window())
            self.root.bind("<FocusIn>", lambda e: self._gentle_restore_window())
        except Exception:
            pass
    ...
    def _smart_visibility_heartbeat(self):
        """🔧 FIX: Smarter visibility check with debouncing"""
        try:
            current_time = time.time()
        ...
        self.safe_after(5000, self._smart_visibility_heartbeat)
    
    def _gentle_restore_window(self):
        """🔧 FIX: Gentle window restoration without aggressive focus stealing"""
        try:
            # Só restaurar, não forçar foco agressivamente
    ...
    def update_stats(self):
        """🔧 FIX: Update statistics with responsiveness check"""
        try:
            current_time = time.time()
            ...
            # 🔧 FIX: Schedule next update with adaptive interval and responsiveness check
