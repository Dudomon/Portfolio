...
from trading_framework.policies.two_head_v5_intelligent_48h import TwoHeadV5Intelligent48h, get_intelligent_v5_kwargs
...
def create_enhanced_normalizer_wrapper(env, obs_size=None, normalizer_file=None):
    ...
    if obs_size is None:
        if hasattr(env, 'observation_space'):
            ...
            obs_size = 1320  # Fallback baseado no cálculo real (27 features + 27 positions + 12 intelligent) × 20 window
...
def monitor_enhanced_normalizer_health(enhanced_env, obs):
    ...
    try:
        ...
        if near_zero > 0.7:
            print(f"AVISO ALERTA Enhanced Normalizer: {near_zero*100:.1f}% das observações próximas de zero!")
...
class MetricsCallback(BaseCallback):
                ...
                if env is None and hasattr(self, 'training_env'):
                    try:
                        ...
                        if portfolio_values and len(portfolio_values) > 0:
                            ...
                            # 🚀 MÉTRICAS DO ENVIRONMENT ATUAL (sem fallback)
                            current_trades = self.training_env.get_attr('trades')[0]
                            current_positions = self.training_env.get_attr('positions')[0]
                            current_portfolio = self.training_env.get_attr('portfolio_value')[0]
                            
                            # Métricas atualizadas
                            total_trades = len(current_trades)
                            profitable_trades = len([t for t in current_trades if t.get('pnl_usd', 0) > 0])
                            win_rate = (profitable_trades / total_trades * 100) if total_trades > 0 else 0
                            total_pnl = sum(t.get('pnl_usd', 0) for t in current_trades)
                            portfolio = current_portfolio
                            
                            # 🚀 CORREÇÃO: ACUMULAR MÉTRICAS GLOBAIS (não usar max)
                            self.global_metrics['total_trades'] = self.global_metrics.get('total_trades', 0) + max(0, total_trades - self.global_metrics.get('last_episode_trades', 0))
                            self.global_metrics['profitable_trades'] = self.global_metrics.get('profitable_trades', 0) + max(0, profitable_trades - self.global_metrics.get('last_episode_profitable', 0))
                            self.global_metrics['total_pnl'] = self.global_metrics.get('total_pnl', 0) + max(0, total_pnl - self.global_metrics.get('last_episode_pnl', 0))
                            self.global_metrics['peak_drawdown'] = max(self.global_metrics.get('peak_drawdown', 0), episode_drawdown)
                            
                            # Armazenar valores do episódio atual para próxima comparação
                            self.global_metrics['last_episode_trades'] = total_trades
                            self.global_metrics['last_episode_profitable'] = profitable_trades
                            self.global_metrics['last_episode_pnl'] = total_pnl
    ...
    def _update_global_metrics(self, env):
        ...
        try:
            ...
            # 🚀 ATUALIZAR MÉTRICAS GLOBAIS COM TRAINING_ENV (sem environment antigo)
            current_trades = self.training_env.get_attr('trades')[0]
            current_trades_count = len(current_trades)
            
            if current_trades_count > 0:
                episode_total_trades = current_trades_count
                episode_pnl = sum(t.get('pnl_usd', 0) for t in current_trades)
                episode_profitable_trades = len([t for t in current_trades if t.get('pnl_usd', 0) > 0])
...
class TradingEnv(gym.Env):
    MAX_STEPS = 1500  #  OTIMIZADO: 1500 steps por episódio para convergência mais rápida
    ...
    def __init__(self, df, window_size=20, is_training=True, initial_balance=500, trading_params=None):
        ...
        self._prepare_data()
        n_features = len(self.feature_columns) + self.max_positions * 9  #  CORRIGIDO: 9 features por posição (compatibilidade com 12D action space)
        # 🧠 V5 ENHANCEMENT: Adicionar 12 componentes inteligentes (CORRIGIDO)
        n_intelligent_features = 12  # Market regime (3) + Volatility (3) + Momentum (3) + Risk (3) = 12 features
        total_features = n_features + n_intelligent_features
        self.observation_space = spaces.Box(
            low=-np.inf, high=np.inf, shape=(window_size * total_features,), dtype=np.float32
        )
        self.win_streak = 0
        ...
        self.tp_range_min = self.trading_params.get('tp_range_min', 16)  # OK MANTIDO: 16 pontos
        self.tp_range_max = self.trading_params.get('tp_range_max', 82)  # OK MANTIDO: 82 pontos
    ...
    def reset(self, **kwargs):
        ...
        max_step = len(self.df) - self.MAX_STEPS - 1  # Considera MAX_STEPS=1500
        ...
        obs = np.clip(obs, -5.0, 5.0)  # Limitar features entre -5 e +5 (mais agressivo para estabilidade)
        return obs
    ...
    def step(self, action):
        ...
        #  OTIMIZADO: Episódios médios para melhor convergência
        # Equilibrio entre aprendizado de longo prazo e estabilidade de gradientes
        if self.episode_steps >= 1500:  # Otimizado para 1500 steps para convergência mais rápida
            done = True
        
        #  SOLUÇÃO: Controle preciso de duração para cálculo correto de gradientes
        
        # 🚀 V5: CAPTURAR OUTPUTS DA ENTRY HEAD DURANTE TREINAMENTO
        current_obs = self._get_observation()
        self.last_v5_outputs = self._capture_v5_entry_outputs(current_obs)
        
        # Debug V5 (apenas primeiros 10 steps)
        if self.episode_steps < 10 and self.last_v5_outputs:
            gates = self.last_v5_outputs.get('gates', {})
            gate_values = {}
            ...
            print(f"🚀 [V5 TRAINING] Step {self.episode_steps}: Gates = {gate_values}")
        ...
        # 🚨 PROTEÇÃO CRÍTICA CONTRA BANKRUPTCY: Limitar portfolio mínimo
        if self.portfolio_value < 1.0:  # Se portfolio < $1, forçar reset
            self.portfolio_value = 1.0
            self.realized_balance = 1.0
            done = True  # Forçar fim do episódio
            
        #  CORREÇÃO CRÍTICA: Calcular drawdown CORRETAMENTE - MÁXIMO 100%
        if self.peak_portfolio_value > 0 and self.portfolio_value >= 0:
            ...
            # 🚨 CORREÇÃO: Drawdown NUNCA pode passar de 100%
            self.current_drawdown = min(max(dd_ratio * 100, 0), 100.0)  # MÁXIMO 100%
    ...
    def _calculate_missing_features(self, missing_features):
        ...
        if 'returns_5m' in missing_features:
            returns_5m = np.zeros_like(close_5m)
        ...
        if 'volatility_20_5m' in missing_features:
            vol_20 = pd.Series(close_5m).rolling(window=20).std().fillna(0).values
            self.df.loc[:, 'volatility_20_5m'] = vol_20
            features_to_calc.append('volatility_20_5m')
        
        if 'sma_20_5m' in missing_features:
            sma_20 = pd.Series(close_5m).rolling(window=20).mean().fillna(method='bfill').fillna(0).values
            self.df.loc[:, 'sma_20_5m'] = sma_20
            features_to_calc.append('sma_20_5m')
        
        if 'sma_50_5m' in missing_features:
            sma_50 = pd.Series(close_5m).rolling(window=50).mean().fillna(method='bfill').fillna(0).values
        ...
        if 'volume_momentum' in missing_features:
            ...
            volume_momentum = np.where(volume_sma_20 > 0, (volume_5m - volume_sma_20) / volume_sma_20, 0)
        ...
        if 'volatility_ratio' in missing_features:
            vol_20 = pd.Series(close_5m).rolling(window=20).std().fillna(0).values
            vol_50 = pd.Series(close_5m).rolling(window=50).std().fillna(0).values
        ...
        if 'intraday_range' in missing_features:
            intraday_range = np.where(close_5m > 0, (high_5m - low_5m) / close_5m, 0)
        ...
        if 'session_momentum' in missing_features:
            session_momentum = np.zeros_like(close_5m)
        ...
        if 'momentum_5_5m' in missing_features:
            momentum_5 = np.zeros_like(close_5m)
    ...
    def _get_observation(self):
        ...
        #  V5 ENHANCEMENT: Gerar observation space inteligente
        return self._get_intelligent_observation_v5()
    ...
    def _get_intelligent_observation_v5(self):
        ...
        positions_obs = np.zeros((self.max_positions, 9))  # CORRIGIDO: 9 features por posição
        ...
        if flat_obs.shape[0] != self.observation_space.shape[0]:
            if flat_obs.shape[0] > self.observation_space.shape[0]:
                ...
                padding = np.zeros(self.observation_space.shape[0] - flat_obs.shape[0], dtype=np.float32)
    ...
    def _classify_market_regime(self, current_idx):
        ...
        try:
            ...
            if 'sma_20_5m' in self.df.columns:
                ...
                trend_strength = np.mean(price - sma_20) / np.std(price - sma_20) if np.std(price - sma_20) > 0 else 0.0
    ...
    def _generate_v5_specialized_components(self, current_idx, market_regime, volatility_context, momentum_confluence, risk_assessment):
        ...
        try:
            ...
            timeframe_fusion = np.zeros(128, dtype=np.float32)
            ...
            pattern_memory = np.zeros(192, dtype=np.float32)
            ...
            # 🚀 CORREÇÃO V5: Retornar todas as 352 dimensões necessárias
            return {
                'horizon_embedding': np.zeros(8, dtype=np.float32),
                'timeframe_fusion': np.zeros(128, dtype=np.float32),
                'risk_embedding': np.zeros(8, dtype=np.float32),
                'regime_embedding': np.zeros(8, dtype=np.float32),
                'pattern_memory': np.zeros(192, dtype=np.float32),
                'market_features': np.zeros(8, dtype=np.float32),  # NOVO: Componente faltante
                'lookahead': np.zeros(1, dtype=np.float32)
            }
    ...
    def _flatten_intelligent_components(self, components):
        ...
        try:
            ...
            # 🔧 CORREÇÃO: Remover print que causa spam
            # print(f"[V5-ERROR] Erro ao achatar componentes: {e}")
    ...
    def _log_v5_decisions_intelligently(self, v5_analysis: Dict, action_taken: str):
        ...
        try:
            ...
            # Logar apenas se houver decisões importantes
            if important_decisions:
                print(f"\n🧠 V5 DECISÕES IMPORTANTES (Step {current_step}):")
        ...
        if entry_decision > 0 and len(self.positions) < self.max_positions:
            ...
            if entry_allowed:
                ...
                if pos_index < 3:  # Garantir que não exceda max_positions
                    ...
                    # Converter ajustes [-3,3] para pontos de preço
    ...
    def _check_entry_filters(self, action_type):
        """
        🚀 FILTROS V5 INTELIGENTES: Integra Entry Head especializada da TwoHeadV5
        """
        try:
            # 🎯 REATIVADO: Verificar filtros V5 se disponíveis
            if hasattr(self, 'last_v5_outputs') and self.last_v5_outputs:
                ...
                if not v5_passed:
                    return False, v5_reason
            
            current_step = min(self.current_step, len(self.df) - 1)
            
            # 🛡️ REATIVADO: Aplicar filtros V5 com configuração moderada
            fatigue_detected = self._check_market_fatigue_v5()
            if fatigue_detected:
                return False, "Market Fatigue V5: Detectada fadiga de mercado"
            
            # Quality filters com configuração mais permissiva
            quality_passed = self._check_v5_quality_filters(action_type, current_step)
            if not quality_passed:
                return False, "Quality Filters V5: Condições de mercado inadequadas"
            
            # ✅ FALLBACK: Filtros básicos permissivos
            basic_passed = self._check_basic_entry_filters(action_type, current_step)
            
            return basic_passed, "Entry Filters V5: Aprovado"
            
        except Exception as e:
            # Fallback para filtros básicos em caso de erro
            basic_passed = self._check_basic_entry_filters(action_type, current_step)
            return basic_passed, f"Entry Filters: Fallback usado devido a erro: {str(e)[:50]}"

    def _apply_v5_intelligent_filters(self, action_type, v5_outputs):
        """🚀 Aplicar filtros especializados da Entry Head V5"""
        try:
            if 'gates' in v5_outputs:
                gates = v5_outputs['gates']
                
                # Verificar todos os 6 gates especializados
                temporal_pass = gates.get('temporal', 0) > 0.5
                validation_pass = gates.get('validation', 0) > 0.7  
                risk_pass = gates.get('risk', 0) > 0.6
                market_pass = gates.get('market', 0) > 0.5
                quality_pass = gates.get('quality', 0) > 0.7
                confidence_pass = gates.get('confidence', 0) > 0.7
    ...
    def _check_market_fatigue_v5(self):
        """🛡️ Verificar fadiga usando detector V5 especializado"""
        try:
            # Se temos acesso à Entry Head V5
            if hasattr(self, 'model') and hasattr(self.model, 'policy'):
                policy = self.model.policy
                if hasattr(policy, 'entry_head') and hasattr(policy.entry_head, 'recent_trades_memory'):
                    # Analisar memória de trades recentes da V5
                    memory = policy.entry_head.recent_trades_memory
                    if memory.numel() > 0:
                        recent_performance = memory.mean().item()
                        fatigue_threshold = 0.3  # Limiar de fadiga
                        return recent_performance < fatigue_threshold
            
            # Fallback: verificar fatiga básica por número de trades recentes
            trades_copy = list(self.trades)
            recent_trades = len([t for t in trades_copy[-20:] if t.get('entry_step', 0) > self.current_step - 20])
            return recent_trades > 12  # Mais de 12 trades em 20 steps = fadiga
            
        except Exception as e:
            return False  # Em caso de erro, não bloquear por fadiga
    
    def _check_v5_quality_filters(self, action_type, current_step):
        """📊 Quality Filters V5 adaptativos"""
        try:
            # Momentum Filter
            ...
            if action_type == 1:  # Long
                momentum_ok = momentum_5m > 0.001 and momentum_15m > 0.001
            else:  # Short
                momentum_ok = momentum_5m < -0.001 and momentum_15m < -0.001
            ...
            volatility_ok = 0.005 < vol_ratio < 0.05  # 0.5% a 5% volatilidade
            
            # Volume Filter
            volume_5m = self.df.get('volume_5m', pd.Series([1])).iloc[current_step]
            volume_sma = self.df.get('volume_sma_20_5m', pd.Series([1])).iloc[current_step]
            volume_ok = volume_5m > volume_sma * 0.8  # Volume mínimo 80% da média
            
            # Trend Strength Filter
            trend_5m = self.df.get('ema_trend_5m', pd.Series([0])).iloc[current_step]
            trend_ok = abs(trend_5m) > 0.1  # Trend mínimo
            
            # Combinar filtros (pelo menos 3 de 4 devem passar)
            filters_passed = sum([momentum_ok, volatility_ok, volume_ok, trend_ok])
            return filters_passed >= 3
            
        except Exception as e:
            return True  # Em caso de erro, não bloquear
    
    def _check_v5_adaptive_thresholds(self, action_type):
        """🎯 Thresholds adaptativos V5 baseados em regime de mercado"""
        try:
            # Detectar regime de mercado
            current_step = min(self.current_step, len(self.df) - 1)
            volatility = self.df.get('volatility_20_5m', pd.Series([0.01])).iloc[current_step]
            price = self.df['close_5m'].iloc[current_step]
            vol_ratio = volatility / price if price > 0 else 0.01
            
            # Determinar regime e thresholds
            if vol_ratio > 0.03:  # Alta volatilidade - Conservative
                confidence_threshold = 0.8
                quality_threshold = 0.85
            elif vol_ratio > 0.015:  # Volatilidade normal - Moderate  
                confidence_threshold = 0.7
                quality_threshold = 0.75
            else:  # Baixa volatilidade - Aggressive
                confidence_threshold = 0.6
                quality_threshold = 0.65
            
            # Verificar se último output da V5 atende aos thresholds
            if hasattr(self, 'last_v5_outputs') and 'scores' in self.last_v5_outputs:
                scores = self.last_v5_outputs['scores']
                confidence = scores.get('confidence', 0)
                quality = scores.get('quality', 0)
                
                return confidence > confidence_threshold and quality > quality_threshold
            
            return True  # Se não temos scores V5, aprovar
            
        except Exception as e:
            return True  # Em caso de erro, não bloquear
    
    def _check_basic_entry_filters(self, action_type, current_step):
        """⚡ Filtros básicos de fallback"""
        try:
            # Anti-microtrading básico
            trades_copy = list(self.trades)
            recent_trades = len([t for t in trades_copy[-10:] if t.get('entry_step', 0) > self.current_step - 10])
            
            # Permitir entrada se menos de 8 trades em 10 steps
            return recent_trades < 8
            
        except Exception as e:
            return True  # Em caso de erro, permitir entrada
    
    def _capture_v5_entry_outputs(self, obs):
        """🚀 Capturar outputs da Entry Head V5 durante treinamento"""
        try:
            # Verificar se temos modelo com Entry Head V5
            model = None
            ...
            # Verificar se Entry Head está ativa
            if not getattr(policy, 'enable_ultra_specialized_entry', False):
                return None
            ...
            print(f"⚠️ [V5] Erro ao capturar outputs Entry Head: {e}")
            return None
    
    def set_model(self, model):
        """🚀 Definir modelo atual para captura V5"""
...
class AdvancedTrainingSystem:
    ...
    def _validate_v5_policy(self, model):
        """🛡️ GARANTIR que o modelo usa TwoHeadV5Intelligent48h"""
        try:
            policy_name = model.policy.__class__.__name__
            
            # Verificação 1: Policy é TwoHeadV5Intelligent48h
            if policy_name != "TwoHeadV5Intelligent48h":
                raise ValueError(f"❌ CRÍTICO: Policy {policy_name} não é TwoHeadV5Intelligent48h!")
            
            # Verificação 2: Entry Head existe
            if not hasattr(model.policy, 'entry_head'):
                raise ValueError("❌ CRÍTICO: TwoHeadV5 não possui Entry Head!")
            
            # Verificação 3: Entry Head é SimplifiedUltraEntryHead48h
            entry_head_name = model.policy.entry_head.__class__.__name__
            if entry_head_name != "SimplifiedUltraEntryHead48h":
                raise ValueError(f"❌ CRÍTICO: Entry Head é {entry_head_name}, deveria ser SimplifiedUltraEntryHead48h!")
            
            # Verificação 4: Componentes V5 habilitados
            if not getattr(model.policy, 'enable_ultra_specialized_entry', False):
                raise ValueError("❌ CRÍTICO: enable_ultra_specialized_entry está False!")
            
            # Logs de confirmação
            self.logger.info("🛡️ VALIDAÇÃO V5 COMPLETA:")
            self.logger.info(f"   ✅ Policy: {policy_name}")
            self.logger.info(f"   ✅ Entry Head: {entry_head_name} (SIMPLIFICADA)")
            self.logger.info(f"   ✅ Ultra Specialized: {model.policy.enable_ultra_specialized_entry}")
            self.logger.info(f"   ✅ V5 Gates: {'6 Gates' if hasattr(model.policy.entry_head, 'adaptive_threshold_main') else 'Não encontrado'}")
        ...
        try:
            ...
            if checkpoint_path_found and os.path.exists(checkpoint_path_found):
                ...
                try:
                    ...
                    # 🛑 VALIDAÇÃO CRÍTICA: Garantir TwoHeadV5 após resume
                    self._validate_v5_policy(self.current_model)
    ...
    def _find_latest_checkpoint(self):
        ...
        for checkpoint_dir in checkpoint_dirs:
            if os.path.exists(checkpoint_dir):
                for file in os.listdir(checkpoint_dir):
                    if file.endswith('.zip') and ('diff' in file.lower()):
        ...
        if not available_models:
            self.logger.info("❌ NENHUM CHECKPOINT 'DIFF' ENCONTRADO - Iniciando treinamento do zero")
        ...
        if available_models:
            ...
            print("\n" + "="*80)
            print("🔄 RESUME TRAINING - CHECKPOINT 'DIFF' DETECTADO")
            print("="*80)
            ...
            print("❌ NENHUM CHECKPOINT 'DIFF' ENCONTRADO - Iniciando do zero")
            return None, None, None
    ...
    def _create_model(self, env):
        ...
        if checkpoint_path:
            ...
            try:
                ...
                # 🛑 VALIDAÇÃO CRÍTICA: Garantir TwoHeadV5 após carregar checkpoint
                self._validate_v5_policy(model)
        ...
        #  CONFIGURAÇÕES ESPECIALIZADAS PARA TWOHEADV5 - CORRIGIDAS APÓS ANÁLISE DE LOGS
        model_config = {
            "policy": TwoHeadV5Intelligent48h,
            "env": env,
            "learning_rate": BEST_PARAMS["learning_rate"],  #  5e-5: CRÍTICO para arquitetura complexa
            "n_steps": BEST_PARAMS["n_steps"],              #  4096: Estabilidade para attention
            "batch_size": BEST_PARAMS["batch_size"],        #  256: MÍNIMO para multi-head attention
            "n_epochs": BEST_PARAMS["n_epochs"],            #  3: Evitar overfitting
            "gamma": BEST_PARAMS["gamma"],                  #  0.99: Padrão
            "gae_lambda": BEST_PARAMS["gae_lambda"],        #  0.95: Padrão
            "clip_range": BEST_PARAMS["clip_range"],        #  0.12: Equilibrio TwoHeadV5
            "ent_coef": BEST_PARAMS["ent_coef"],            #  0.005: CRÍTICO - era -16 entropy!
            "vf_coef": BEST_PARAMS["vf_coef"],              #  0.5: Padronizado
            "max_grad_norm": BEST_PARAMS["max_grad_norm"],  #  0.3: Clipping rigoroso
            "verbose": 1,             #  VERBOSE ATIVADO para debug
            "device": device_policy,
            "seed": 42,
            "use_sde": False,         #  SDE DESABILITADO PARA TWOHEADV5
            "policy_kwargs": get_intelligent_v5_kwargs()  #  CONFIGURAÇÕES ESPECIALIZADAS V5
        ...
        self.logger.info(f" TwoHeadV5Intelligent48h: 2-LSTM + 1-GRU + 4-Head Attention (SIMPLIFICADA)")
        ...
        self.logger.info(f" Melhorias V5: Entry Head SIMPLIFICADA, Weighted Gates, 9 Scores, Market Fatigue, Quality Filters")
        self.logger.info("=" * 60)
        
        model = RecurrentPPO(**model_config)
        
        # 🛑 VALIDAÇÃO CRÍTICA: Garantir TwoHeadV5
        self._validate_v5_policy(model)
