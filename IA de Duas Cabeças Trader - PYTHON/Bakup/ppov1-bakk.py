...
class MetricsCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        try:
            ...
            episode_length = getattr(env, 'MAX_STEPS', 1500)  # Duração padrão do episódio
            
            # Determinar se deve mostrar métricas
            show_metrics = False
            metrics_context = ""
            
            # No meio do episódio (750 steps ou 50% da duração)
            if current_episode_steps == 750 or current_episode_steps == episode_length // 2:
                show_metrics = True
                metrics_context = f"MEIO DO EPISÓDIO (Step {current_episode_steps}/{episode_length})"
            
            # No final do episódio - condições mais amplas
            elif (current_episode_steps >= episode_length - 50 or  # Próximo do fim
                  current_episode_steps < 50 or  # Início do novo episódio 
                  current_episode_steps == episode_length):  # Exatamente no final
                # Mostrar se passou tempo suficiente desde última exibição
                if self.num_timesteps - self.last_step >= 500:
                    show_metrics = True
                    if current_episode_steps < 50:
                        metrics_context = f"FINAL DO EPISÓDIO (Novo episódio iniciado - Step {current_episode_steps})"
                    else:
            ...
            # Fallback para sistema antigo a cada 4000 steps
            show_metrics = (self.num_timesteps - self.last_step >= 4000)
        ...
        if show_metrics:
            try:
                    ...
                    try:
                        ...
                        if portfolio_values and len(portfolio_values) > 0:
                            ...
                            try:
                                ...
                                # Fallback: usar episode_steps como proxy para atividade
                                if total_trades == 0:
                                    episode_steps = getattr(env, 'episode_steps', 0)
                                    # Estimar trades baseado na atividade (conservador)
                                    total_trades = max(0, episode_steps // 100)  # 1 trade a cada 100 steps aprox
                            ...
                            # 🚀 CORREÇÃO: Acumular métricas globais corretamente
                            # Total trades: usar máximo (pico de trades em um episódio)
                            self.global_metrics['total_trades'] = max(self.global_metrics.get('total_trades', 0), total_trades)
                            # Profitable trades: ACUMULAR não usar máximo
                            self.global_metrics['profitable_trades'] += profitable_trades
                            # PnL: acumular também (total PnL de todos os episódios)
                            self.global_metrics['total_pnl'] += total_pnl
                            # Drawdown: usar máximo (pior drawdown já visto)
                ...
                try:
                    ...
                    if total_trades == 0:
                        episode_steps = getattr(env, 'episode_steps', 0)
                        total_trades = max(0, episode_steps // 100)
    ...
    def _update_global_metrics(self, env):
        ...
        try:
            ...
            if current_trades_count > 0:
                ...
                # Atualizar métricas globais corretamente
                self.global_metrics['total_trades'] = max(self.global_metrics['total_trades'], episode_total_trades)
                self.global_metrics['total_pnl'] += episode_pnl  # Acumular PnL total
                self.global_metrics['profitable_trades'] += episode_profitable_trades  # Acumular trades lucrativos
