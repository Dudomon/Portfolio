...
EXPERIMENT_TAG = "EXPERTGAIN_V2"  # 🔥 V2: Sistema melhorado com LR dinâmico
...
# 🎯 CONFIGURAÇÕES DE TREINAMENTO: Baseado no dataset multitimeframe
TRAINING_CONFIG = {
    "max_dataset_bars": 1290000,    # Máximo de barras de 5m no dataset
    "total_timesteps": 10320000,    # 8x o dataset máximo (1.29M × 8) TOTAL
    "training_multiplier": 8        # Multiplicador para timesteps totais
}

# 🎯 FASES ESPECIALIZADAS EXPERTGAIN V2 - FOCO EM ENTRY QUALITY
PHASE_DISTRIBUTION = {
    "phase_1_unlock": 500000,      # 🔓 Desbloquear gates travadas (Entry Quality > 0.10)
    "phase_2_calibrate": 750000,   # 📊 Calibrar quality (Entry Quality > 0.30)
    "phase_3_optimize": 750000,    # 🎯 Otimizar trading (Entry Quality > 0.50)
}

# 🔥 OBJETIVOS POR FASE - EXPERTGAIN V2
PHASE_OBJECTIVES = {
    "phase_1_unlock": {
        "target_entry_quality": 0.15,
...
CONVERGENCE_OPTIMIZATION_CONFIG = {
    ...
    "max_grad_norm": 10.0,    # 🚨 CRITICAL FIX: 2.0→10.0 (transformers need high gradient norms!)
    "adaptive_accumulation": True,
    
    # Advanced LR Scheduler - DESABILITADO (usar LR fixo)
    "base_lr": 3.0e-4,  # 🔥 TESTE DIAGNÓSTICO: Sincronizado com BEST_PARAMS
...
def calculate_v6_observation_space():
    """Calcula e valida o observation space para TwoHeadV7Intuition com SEQUÊNCIA TEMPORAL REAL"""
    print("=" * 60)
    print(f"CALCULANDO OBSERVATION SPACE DAYTRADER V7 TEMPORAL ({EXPERIMENT_TAG})")
    print("=" * 60)
    ...
    intelligent_v7_count = 37  # V7 otimizado: removidas 12 básicas + 180 pattern memory + 116 timeframe fusion + 12 embeddings
    microstructure_count = 14  # 8 order flow + 6 tick analytics
    volatility_advanced_count = 5  # GARCH + breakout + clustering + realized_vs_implied + skew
    correlation_count = 4  # SPY + sector + VIX divergence + dollar impact
    momentum_multi_count = 6  # 1m-5m + 5m-15m + 15m-1h confluences + divergence + acceleration + sustainability
    
    # 🎯 FEATURES ADICIONAIS PARA ATINGIR META DE 2580 DIMENSÕES
    additional_features_count = 20  # Features extras para completar arquitetura
    
    # BREAKDOWN das features adicionais:
    ...
    seq_len = 20             # 🔥 CORRIGIDO: 20 barras históricas reais (mantém tamanho original)
    
    # 🔥 OTIMIZADO: Market features reduzidas de 27 para 16 (9 base_5m + 7 high_quality)
    market_features_real = 16  # 9 features 5m + 7 high quality otimizadas
    position_features = positions_count * features_per_position  # 3 * 9 = 27
    features_per_bar = market_features_real + position_features + intelligent_v7_count + microstructure_count + volatility_advanced_count + correlation_count + momentum_multi_count + additional_features_count  # 16 + 27 + 37 + 14 + 5 + 4 + 6 + 20 = 129
    observation_space_size = features_per_bar * seq_len  # 129 * 20 = 2580
    ...
    print(f"🔥 MARKET REAL: {market_features_real} features")
    print(f"📊 POSITIONS: {positions_count} pos x {features_per_position} features = {position_features}")
    print(f"📊 INTELLIGENT V7: {intelligent_v7_count} features (V7 Intuition usa Unified Backbone)")
    print(f"📊 FEATURES PER BAR: {features_per_bar} features")
    print(f"🔥 SEQUENCE LENGTH: {seq_len} barras históricas (TEMPORAL REAL)")
    print(f"🎯 OBSERVATION SPACE: {features_per_bar} x {seq_len} = {observation_space_size} dimensões")
    print("=" * 60)
    print(f"✅ DAYTRADER V7 TEMPORAL CONFIGURADO: {observation_space_size} DIMENSÕES")
    print("=" * 60)
...
EXPECTED_OBS_SIZE = 2580  # Sistema otimizado: 129 features × 20 barras
...
class ConvergenceLogger:
    ...
    def log_training_step(self, step, model, env, info_dict=None):
        ...
        try:
            ...
            if self.jsonl_logger and step % 10 == 0:  # Log apenas a cada 10 steps
                try:
                    ...
                    # Gradient data - só a cada 20 steps e se válido
                    if step % 20 == 0:
                    ...
                    # Performance data - só a cada 100 steps
                    if step % 100 == 0:  
                        performance_data = {
                            'episode_reward': metrics.get('episode_reward', 0),
                    ...
                    # Convergence metrics - só a cada 50 steps
                    if step % 50 == 0:
                        convergence_data = {
                            'convergence_score': convergence_score,
                            'loss_trend': 'stable',  
            ...
            # Log análise de convergência a cada 200 steps
            if step % 200 == 0:
                analysis = self.analyze_convergence_trends()
                if analysis:
                    self.log_convergence_analysis(step, analysis)
            
            # Log análise de gradientes a cada 100 steps
            if step % 100 == 0:
    ...
    def _extract_model_metrics(self, model, info_dict):
        ...
        try:
            ...
            # Manual gradient norm calculation - OTIMIZADO
            # Só calcular a cada 20 steps para economizar performance
            if hasattr(model, 'policy') and hasattr(self, '_last_step'):
                current_step = getattr(self, '_current_step', 0)
                if current_step % 20 == 0:  # Só calcular ocasionalmente
                    total_norm = 0.0
                    param_count = 0
                    ...
                    # Usar valor cached para economizar performance
    ...
    if 'volume_5m' in df.columns:
        ...
        if volume_zeros > len(df) * 0.5:  # >50% zeros
            ...
            if 'close_5m' in df.columns:
                ...
                synthetic_volume = base_volume * (1 + price_changes * 10)  # Volume proporcional à volatilidade
                synthetic_volume = synthetic_volume.fillna(base_volume)
                df.loc[df['volume_5m'] == 0, 'volume_5m'] = synthetic_volume[df['volume_5m'] == 0]
                print(f"🔧 VOLUME SINTÉTICO: Gerado volume realista (base: {base_volume}, range: {df['volume_5m'].min():.0f}-{df['volume_5m'].max():.0f})")
...
def load_optimized_data_original():
    ...
    """
    dataset_path = 'data/GC_YAHOO_ENHANCED_V3_BALANCED_20250804_192226.csv'
...
def create_enhanced_normalizer_wrapper(env, obs_size=None, normalizer_file=None):
    ...
    enhanced_env = create_enhanced_normalizer(
        ...
        clip_obs=3.0,      # 🔥 CORRIGIDO: Menos clipping agressivo para reduzir zeros extremos  
        clip_reward=5.0,   # ✅ MANTIDO: Funciona bem
    ...
    try:
        ...
        if hasattr(enhanced_env, 'normalizer'):
            ...
            dummy_normalizer = EnhancedVecNormalize(
                ...
                norm_obs=True,
                norm_reward=True,
                clip_obs=3.0,  # 🔥 CORRIGIDO: Consistente com otimizações
                clip_reward=5.0
            )
...
# 🚀 EXPERTGAIN V2 - HIPERPARÂMETROS OTIMIZADOS PARA FINE-TUNING
# 🔥 SISTEMA PROGRESSIVO COM LR DINÂMICO E REWARD SHAPING
EXPERTGAIN_PARAMS = {
    "learning_rate": 5e-05,                  # 🎯 V2: LR BAIXO para fine-tuning (era 3.5e-04)
    "n_steps": 2048,                         # ✅ V2: Buffer mantido
    "batch_size": 128,                       # 🔥 V2: Batch MAIOR para estabilidade
    "n_epochs": 8,                           # 🔥 V2: MAIS epochs para exploração
    "gamma": 0.99,                           # ✅ V2: Visão de longo prazo
    "gae_lambda": 0.95,                      # ✅ V2: GAE padrão
    "clip_range": 0.25,                      # 🔥 V2: MAIS liberdade para explorar
    "ent_coef": 0.02,                        # 🔥 V2: MAIS entropia inicial (decay programado)
    "vf_coef": 0.5,                          # ✅ V2: VF balanceado
    "max_grad_norm": 1.0,                    # 🔥 V2: Permite gradientes maiores
    "policy_kwargs": {
        "lstm_hidden_size": 128,        # 🎯 EXPERTGAIN: Mantido
        "n_lstm_layers": 2,             # 🎯 EXPERTGAIN: Mantido
        "attention_heads": 4,           # 🎯 EXPERTGAIN: Mantido
        "shared_lstm": False,
        "enable_critic_lstm": True,
        "lstm_kwargs": None,
        "net_arch": [128, 64],          # 🎯 EXPERTGAIN: Mantido
        "activation_fn": torch.nn.ReLU,
        "ortho_init": True,
        "log_std_init": -0.3,           # 🎯 EXPERTGAIN: Maior variabilidade para exploração
        "full_std": True,
        "use_expln": False,
        "squash_output": False
    },
...
BEST_PARAMS = EXPERTGAIN_PARAMS
# --- FIM HIPERPARÂMETROS FIXOS OTIMIZADOS ---

# ====================================================================
# 🎯 EXPERTGAIN V2 - SISTEMAS ESPECIALIZADOS
# ====================================================================
...
class ExpertGainRewardShaper:
    ...
    def shape_reward(self, original_reward, action, info):
        ...
        if len(self.quality_history) > 10:
            recent_avg = sum(list(self.quality_history)[-10:]) / 10
            old_avg = sum(list(self.quality_history)[:10]) / min(10, len(self.quality_history))
        ...
        if info.get("episode_done"):
            ...
            self.trade_count = 0  # Reset para próximo episódio
...
class DynamicLRAdjuster:
    ...
    """
    def __init__(self, model, base_lr=3.5e-04):
        self.model = model
    ...
    def update(self, current_quality, current_step):
        """
        Atualiza LR baseado em Entry Quality
        """
        self.quality_history.append(current_quality)
        
        # Só ajustar a cada 10000 steps
        if current_step - self.last_adjustment_step < 10000:
            return
        ...
        if current_quality > 0.5 and len(self.quality_history) > 20:
            recent_std = np.std(list(self.quality_history)[-20:])
...
class SmartEarlyStopping:
    ...
    def should_stop(self, current_quality, current_trades, current_step):
        """
        Decide se deve parar
        """
        # Se não está tradeando, não vale a pena continuar
        if current_trades == 0 and current_step > 50000:
        ...
        if current_quality > self.best_quality + self.min_improvement:
            ...
            self.steps_without_improvement += 1000  # Incrementa por check interval
...
class TradingEnv(gym.Env):
    MAX_STEPS = 2000  # 🚨 FIX CRÍTICO: 2000 steps para sequências longas (weight_hh_l0 precisa de continuidade temporal)
    ...
    def __init__(self, df, window_size=20, is_training=True, initial_balance=None, trading_params=None):
        ...
        market_features = 16  # Otimizado: 9 features 5m + 7 high quality
        position_features = 3 * 9        # max_positions (3) * features_per_position (9) = 27  
        intelligent_features = 37         # V7 otimizado: removidas 12 básicas + 180 pattern memory + 116 timeframe fusion + 12 embeddings
        microstructure_features = 14      # 8 order flow + 6 tick analytics
        volatility_features = 5           # GARCH + breakout + clustering + realized_vs_implied + skew
        correlation_features = 4          # SPY + sector + VIX divergence + dollar impact
        momentum_features = 6             # 1m-5m + 5m-15m + 15m-1h confluences + divergence + acceleration + sustainability
        enhanced_features = 20            # Pattern recognition + regime detection + risk metrics + temporal context
        
        features_per_bar = market_features + position_features + intelligent_features + microstructure_features + volatility_features + correlation_features + momentum_features + enhanced_features  # 16 + 27 + 37 + 14 + 5 + 4 + 6 + 20 = 129
        
        # 🎯 TEMPORAL SEQUENCE OTIMIZADO: 20 barras históricas × 129 features = 2580
        seq_len = 20  # 20 barras históricas  
        calculated_obs_size = seq_len * features_per_bar  # 20 × 129 = 2580
        ...
        print(f"✅ V7 TEMPORAL OBSERVATION SPACE: {calculated_obs_size} dimensões (seq_len={seq_len} × features_per_bar={features_per_bar})")
        print(f"   🔥 MANTÉM 1480 ORIGINAL: Sequência REAL ao invés de artificial")
        ...
        # 🎯 SISTEMA BALANCEADO V2.0: Reward system reformulado para day trading
        ...
        print(f"[REWARD SYSTEM] Sistema balanceado V2.0 ativado (risk-adjusted + anti-gaming + fases progressivas)")
    ...
    def step(self, action):
        ...
        """
        # Action deve ser array de 11 dimensões - SEM FALLBACKS
        if not isinstance(action, np.ndarray) or action.shape != (11,):
            raise ValueError(f"Action deve ser numpy array (11,), recebido: {type(action)} shape={getattr(action, 'shape', 'N/A')}")
        ...
        # 🚀 OTIMIZAÇÃO: Reutilizar observação cacheada
        obs = self._cached_current_obs
        # Limpar cache para próximo step
        if hasattr(self, '_cached_current_obs'):
            delattr(self, '_cached_current_obs')
    ...
    def _prepare_data(self):
        ...
        if hasattr(self.df, 'processed_data_cache'):
            ...
            if np.any(zero_mask):
                ...
                self.processed_data[zero_mask] = np.random.normal(0.001, 0.0005, zeros_count)
        ...
        for feature_name in required_hq_features:
            if feature_name in missing_features or feature_name not in self.df.columns:
                ...
                if feature_name == 'volume_momentum':
                    ...
                    # Para outras features, usar valores sintéticos estáveis não-zero
                    feature_values = 0.4 + 0.1 * np.sin(np.arange(len(close_5m)) * 0.05) + 0.05 * np.random.randn(len(close_5m))
                    feature_values = np.clip(feature_values, 0.1, 0.9)  # Manter no range [0.1, 0.9]
                    self.df.loc[:, feature_name] = feature_values
    ...
    def _get_vectorized_temporal_features(self, seq_len):
        """
        🚀 VECTORIZAÇÃO ULTRA-RÁPIDA: Gerar 20 barras em operação única
        
        Substitui loop de 20 chamadas por operações vectorizadas numpy
        """
        try:
            ...
            if end_step <= len(self.processed_data):
                market_data_batch = self.processed_data[start_step:end_step, :16]  # [20, 16]
            else:
                # Fallback para casos extremos
                market_data_batch = np.full((seq_len, 16), 0.3, dtype=np.float32)
            
            # 🚀 POSITIONS VECTORIZADO: Mesmo estado para todas as 20 barras
            ...
            # 🚀 COMBINAR TUDO: Vectorizado
            # Para cada barra: market[16] + positions[27] + intelligent[37] + advanced[49] = 129
            temporal_sequence = np.zeros((seq_len, 129), dtype=np.float32)
            
            for i in range(seq_len):
                temporal_sequence[i, :] = np.concatenate([
                    market_data_batch[i],          # [16]
                    positions_flat,                # [27] - mesmo para todas as barras
                    intelligent_features,          # [37] - mesmo para todas as barras
                    microstructure_batch[i],       # [14] 
                    volatility_batch[i],           # [5]
                    correlation_batch[i],          # [4]
                    momentum_batch[i],             # [6]
                    enhanced_batch[i]              # [20]
                ])
    ...
    def _get_temporal_observation_v7(self):
        ...
        Gera histórico real das últimas 20 barras (mantém tamanho 1480, mas dados REAIS)
        """
        # Parâmetros para sequência temporal real
        seq_len = 20  # 20 barras históricas (manter compatibilidade com observation space original)
        ...
        if np.any(zeros_mask):
            flat_obs[zeros_mask] = np.random.normal(0.01, 0.005, size=np.sum(zeros_mask))
    ...
    def _get_single_bar_features(self, step):
        ...
        for i in range(min(len(self.positions), self.max_positions)):
            ...
            # 🔍 DEBUG CRÍTICO: Log detalhado quando há posições ativas
            if step == real_current_step and len(self.positions) > 0 and real_current_step % 200 == 0:  # Log a cada 200 steps quando há posições
                print(f"✅ [POSITIONS ACTIVE] Pos {i}/{len(self.positions)}: entry_step={pos.get('entry_step', 'N/A')}, real_current_step={real_current_step}, raw_steps={raw_duration_steps}, final_duration={duration:.6f}")
        ...
        # 🔍 DEBUG FINAL: Verificar durations no array final + FORÇA BRUTA SEMPRE
        # CORRIGIDO: Indices para função vectorizada [23, 32, 41] não [20, 29, 38]
        global_duration_indices = [23, 32, 41]  # Posições globais das durations (função vectorizada)
        final_durations = [single_bar_obs[i] if i < len(single_bar_obs) else 0.0 for i in global_duration_indices]
        
        # 🚨 FORÇA BRUTA FINAL: SEMPRE aplicar valores não-zero para durations
        for i, idx in enumerate(global_duration_indices):
            if idx < len(single_bar_obs) and abs(single_bar_obs[idx]) < 1e-6:
                single_bar_obs[idx] = 0.42  # Valor fixo não-zero
                if step == real_current_step and step % 100 == 0:  # Log mais frequente
                    print(f"🔧 [FORÇA BRUTA VECTORIZED] Step {step}: Índice {idx} corrigido de {final_durations[i]:.6f} para 0.42")
            elif step == real_current_step and step % 500 == 0:  # Log valores OK ocasionalmente
                print(f"✅ [DURATION OK VECTORIZED] Step {step}: Índice {idx} = {single_bar_obs[idx]:.6f}")
        
        # Log final apenas quando necessário
        if step == real_current_step and any(abs(d) < 1e-6 for d in final_durations) and step % 500 == 0:
            print(f"🚨 [DEBUG FINAL] Step {step}: Durations antes: {final_durations} | Array shape: {single_bar_obs.shape}")
        
        # 🔧 VALIDAÇÃO: Voltando para 9*3=27 position features (formato original)
        expected_features = 16 + 27 + 37 + 14 + 5 + 4 + 6 + 20  # market (16) + positions (27) + intelligent (37) + microstructure (14) + volatility (5) + correlation (4) + momentum (6) + enhanced (20) = 129
        if single_bar_obs.shape[0] != expected_features:
            # Ajustar para 129 features
            if single_bar_obs.shape[0] < expected_features:
    ...
    def _calculate_reward_and_info(self, action, old_state):
        ...
        if self.current_step % 100 == 0:  # Debug apenas a cada 100 steps
            action_start_time = time.time()
        else:
            action_start_time = None  # Não medir para máxima velocidade
        ...
        if len(action) >= 11:
            ...
            # 🔥 FIX SHORT THRESHOLD: Usar threshold mais baixo para SHORT
            raw_decision = float(action[0])
            if raw_decision < 0.5:
                entry_decision = 0  # HOLD
            elif raw_decision < 1.5:
                ...
                entry_decision = 2  # SHORT (qualquer valor >= 1.5)
            entry_quality = float(action[1])  # [0,1] Qualidade da entrada
        ...
        if entry_decision > 0 and len(self.positions) < self.max_positions:
            # 🎯 FILTRO DE QUALIDADE MÍNIMA (alinhado com RobotV7)
            ...
            if entry_quality < MIN_QUALITY_THRESHOLD:
                ...
                if self.current_step % 1000 == 0:  # Log só a cada 1000 steps
                    print(f"[QUALITY FILTER] Entry rejected: quality={entry_quality:.2f} < {MIN_QUALITY_THRESHOLD}")
            else:
                # 🚀 PASSOU NO FILTRO DE QUALIDADE - V7 Intuition decide
                entry_allowed = True
                # 🎯 Position size baseado em risk_appetite (NÃO em entry_quality)
                lot_size = self._calculate_adaptive_position_size_quality(risk_appetite)
                ...
                if pos_index < 3:  # Garantir que não exceda max_positions
                    ...
                    # 🎯 SL/TP ajustados por Entry Quality Score (alinhado com RobotV7)
                    base_sl_points = 5.0  # Base: 5 pontos (range 2-8)
                    base_tp_points = 9.0  # Base: 9 pontos (range 3-15)
                    
                    # Ajuste por qualidade: quality alta = SL mais próximo + TP mais longe
                    quality_factor = (entry_quality - 0.5) * 0.4  # [-0.2, +0.2]
                    
                    # SL: quality alta = SL mais próximo (mais confiante)
                    sl_points = base_sl_points * (1.0 - quality_factor * 0.5)  # -10% se quality=1.0
                    
                    # TP: quality alta = TP mais longe (mais ambicioso)
                    tp_points = base_tp_points * (1.0 + quality_factor)  # +20% se quality=1.0
                    
                    # Garantir limites (alinhado com daytrader ranges)
                    sl_points = max(2.0, min(sl_points, 8.0))
                    tp_points = max(3.0, min(tp_points, 15.0))
                    
                    if position['type'] == 'long':
                        position['sl'] = current_price - sl_points
                        position['tp'] = current_price + tp_points
                    else:
                        position['sl'] = current_price + sl_points
                        position['tp'] = current_price - tp_points
        ...
        # Se não passou no filtro de qualidade dentro do bloco anterior, também é HOLD
        if entry_decision > 0 and len(self.positions) < self.max_positions and entry_quality < MIN_QUALITY_THRESHOLD:
            action_taken = False
        ...
        if self.current_step % 100 == 0:  # Debug apenas a cada 100 steps
            ...
            reward_time = (reward_end_time - reward_start_time) * 1000
            # 🎯 CONVERGENCE: Store reward performance (no print)
            if reward_time > 0.5:
                if not hasattr(self, '_reward_performance'):
                    self._reward_performance = []
                self._reward_performance.append({
                    'step': self.current_step,
                    'reward_time_ms': reward_time
                })
        else:
            # 🚀 FAST PATH: Sem medição de tempo para máxima velocidade  
            reward, info, done_from_reward = self.reward_system.calculate_reward_and_info(self, processed_action, old_state)
    ...
    def _process_v5_specialized_action(self, action):
        ...
        # ACTION SPACE: [entry_decision, entry_quality, temporal_signal, risk_appetite, market_regime_bias, sl1, sl2, sl3, tp1, tp2, tp3]
        
        # 🔥 FIX SHORT THRESHOLD: Aplicar mesma lógica
        if len(action) > 0:
            raw_decision = float(action[0])
            if raw_decision < 0.5:
                entry_decision = 0  # HOLD
            ...
            entry_decision = 0
        entry_quality = float(action[1]) if len(action) > 1 else 0.5  # Mantém compatibilidade V5
        ...
        processed_action = np.array([
            ...
            entry_quality,  # [1] quality (0-1)
            entry_quality,  # [2] position size (manter compatibilidade V5)
        ...
        v5_analysis = {
            ...
            "quality_score": self._calculate_v5_quality_score(entry_quality, temporal_signal, risk_appetite, market_regime_bias)
        }
        
        # Log inteligente das decisões V5
        self._log_v5_decisions_intelligently(v5_analysis, f"Entry: {entry_decision}, Quality: {entry_quality:.2f}")
    ...
    def _close_position(self, position, exit_step):
        ...
        # 🎯 DETERMINAÇÃO INTELIGENTE DA RAZÃO DO FECHAMENTO
        close_reason = "manual"
        ...
        if position['type'] == 'long':
            if current_price <= position.get('sl', 0):
                ...
                if position.get('trailing_activated', False):
                    ...
                    close_reason = "SL hit"
            elif current_price >= position.get('tp', float('inf')):
                close_reason = "TP hit"
        else:  # short
            if current_price >= position.get('sl', float('inf')):
                # Verificar se foi trailing stop
                if position.get('trailing_activated', False):
                    close_reason = "trailing_stop"
                    is_trailing_stop = True
...
def make_wrapped_env(df, window_size, is_training, initial_portfolio=None):
    # 🎯 USAR CONFIGURAÇÃO UNIFICADA se não especificado
...
class AdvancedTrainingSystem:
    def __init__(self, base_dir: str = DIFF_MODEL_DIR, experiment_tag: str = EXPERIMENT_TAG):
        ...
        # 🔥 EXPERTGAIN V2 - SISTEMAS ESPECIALIZADOS
        self.reward_shaper = None  # Será inicializado após criar o env
        self.lr_adjuster = None    # Será inicializado após criar o modelo
        self.early_stopping = SmartEarlyStopping(patience=100000, min_improvement=0.01)
    ...
    def _load_custom_checkpoint(self, checkpoint_path, env):
        ...
        try:
            ...
            if isinstance(checkpoint_data, dict) and 'policy' in checkpoint_data:
                ...
                self.current_phase_idx = checkpoint_data.get('current_phase_idx', 0)
                self.total_steps_completed = checkpoint_data.get('step_count', 0)
    ...
    def _create_training_phases(self) -> List[TrainingPhase]:
        """🎯 EXPERTGAIN V2: Fases especializadas com objetivos progressivos"""
        phases = []
        
        # 🔥 FASE 1: UNLOCK GATES (500k steps)
        if "phase_1_unlock" in PHASE_DISTRIBUTION:
            objectives = PHASE_OBJECTIVES["phase_1_unlock"]
            phases.append(TrainingPhase(
                name="ExpertGain_V2_Phase_1_Unlock",
                phase_type=PhaseType.FUNDAMENTALS,
                timesteps=PHASE_DISTRIBUTION["phase_1_unlock"],
                description=f"🔓 Desbloquear gates travadas - Target EQ: {objectives['target_entry_quality']}",
                data_filter="mixed",
                success_criteria={
                    "entry_quality": objectives['target_entry_quality'],
                    "trades_per_episode": objectives['min_trades_per_episode']
                },
                reset_criteria={
                    "win_rate": 0.15,  # Muito tolerante inicialmente
                    "max_drawdown": 0.50  # Permite exploração agressiva
                }
            ))
        
        # 🔥 FASE 2: CALIBRATE QUALITY (750k steps)
        if "phase_2_calibrate" in PHASE_DISTRIBUTION:
            objectives = PHASE_OBJECTIVES["phase_2_calibrate"]
            phases.append(TrainingPhase(
                name="ExpertGain_V2_Phase_2_Calibrate",
                phase_type=PhaseType.RISK_MANAGEMENT,
                timesteps=PHASE_DISTRIBUTION["phase_2_calibrate"],
                description=f"📊 Calibrar Entry Quality - Target EQ: {objectives['target_entry_quality']}",
                data_filter="mixed",
                success_criteria={
                    "entry_quality": objectives['target_entry_quality'],
                    "trades_per_episode": objectives['min_trades_per_episode']
                ...
                reset_criteria={
                    "win_rate": 0.25,  # Um pouco mais exigente
                    "max_drawdown": 0.40  # Menos tolerante que fase 1
                }
            ))
        
        # 🔥 FASE 3: OPTIMIZE TRADING (750k steps)
        if "phase_3_optimize" in PHASE_DISTRIBUTION:
            objectives = PHASE_OBJECTIVES["phase_3_optimize"]
            phases.append(TrainingPhase(
                name="ExpertGain_V2_Phase_3_Optimize",
                phase_type=PhaseType.INTEGRATION,
                timesteps=PHASE_DISTRIBUTION["phase_3_optimize"],
                description=f"🎯 Otimizar Trading - Target EQ: {objectives['target_entry_quality']}",
                data_filter="mixed",
                success_criteria={
                    "entry_quality": objectives['target_entry_quality'],
                    "trades_per_episode": objectives['min_trades_per_episode'],
                    "win_rate": 0.50  # Exigir win rate decente
                },
                reset_criteria={
                    "win_rate": 0.35,  # Mais exigente na fase final
                    "max_drawdown": 0.30  # Menos tolerante a drawdowns
                }
            ))
    ...
    def train(self):
        ...
        try:
            ...
            #  🎯 EXPERTGAIN: RESUME AUTOMÁTICO DO DAYTRADER, MAS FASES DO ZERO
            checkpoint_path_found, resume_phase_idx, resume_steps = self._find_latest_checkpoint()
            
            # 🎯 EXPERTGAIN: SEMPRE COMEÇAR FASES DO ZERO (mesmo com checkpoint carregado)
            if checkpoint_path_found:
                print("🎯 EXPERTGAIN: Checkpoint encontrado, mas iniciando fases especializadas do zero")
                resume_phase_idx = 0  # Força início das fases ExpertGain
            ...
            if checkpoint_path_found and os.path.exists(checkpoint_path_found):
                ...
                try:
                    ...
                    # 🛑 VALIDAÇÃO CRÍTICA: Garantir TwoHeadV7Intuition após resume
                    _validate_v7_policy(self.current_model.policy)
                    ...
                    if "different number of parameter groups" in error_msg:
                        ...
                        try:
                            ...
                            if 'policy' in checkpoint:
                                ...
                                # MANTER INFORMAÇÕES DE RESUME - CRÍTICO!
                                self.current_phase_idx = resume_phase_idx
                                self.total_steps_completed = resume_steps
                                self.current_model.num_timesteps = resume_steps
                ...
                # Tentar carregar do checkpoint DayTrader 8M como base
                daytrader_8m_path = "D:/Projeto/Otimizacao/treino_principal/models/DAYTRADER/DAYTRADER_phase4stresstesting_8000000_steps_20250808_173027.zip"
                if os.path.exists(daytrader_8m_path):
                    print(f"\n🎯 CARREGANDO BASE DO DAYTRADER 8M STEPS...")
                    print(f"📂 Arquivo: {os.path.basename(daytrader_8m_path)}")
                    try:
                        device = "cuda" if torch.cuda.is_available() else "cpu"
                        self.current_model = RecurrentPPO.load(daytrader_8m_path, env=env, device=device)
                        
                        # 🎯 AJUSTAR LR PARA FINE-TUNING - FORÇAR EM TODOS OS LUGARES
                        expertgain_lr = EXPERTGAIN_PARAMS["learning_rate"]  # 5e-05
                        
                        # Método 1: Policy optimizer
                        if hasattr(self.current_model.policy, 'optimizer'):
                            for param_group in self.current_model.policy.optimizer.param_groups:
                                param_group['lr'] = expertgain_lr
                                print(f"   🔧 Policy optimizer LR: {param_group['lr']:.1e}")
                        
                        # Método 2: Model learning_rate attribute
                        if hasattr(self.current_model, 'learning_rate'):
                            self.current_model.learning_rate = expertgain_lr
                            print(f"   🔧 Model learning_rate: {expertgain_lr:.1e}")
                            
                        # Método 3: Force via lr_schedule
                        if hasattr(self.current_model, 'lr_schedule'):
                            self.current_model.lr_schedule = lambda _: expertgain_lr
                            print(f"   🔧 LR schedule forçado: {expertgain_lr:.1e}")
                        
                        self.current_phase_idx = 0  # Começar do início do ExpertGain
                        self.total_steps_completed = 0  # Reset steps para o ExpertGain
                        print(f"✅ MODELO BASE CARREGADO COM SUCESSO - LR ajustado para {EXPERTGAIN_PARAMS['learning_rate']:.1e}")
                        print("🎯 Iniciando fine-tuning ExpertGain!")
                    except Exception as e:
                        print(f"❌ Erro ao carregar DayTrader 8M: {e}")
                        print("🔄 Criando modelo do zero como fallback...")
                        self.current_model = self._create_model(env)
                        self.current_phase_idx = 0
                        self.total_steps_completed = 0
                else:
                    print(f"\n📝 Iniciando treinamento do zero (DayTrader 8M não encontrado)...")
                    self.current_model = self._create_model(env)
                    self.current_phase_idx = 0
                    self.total_steps_completed = 0
    ...
    def _create_phase_environment(self, df: pd.DataFrame, phase: TrainingPhase):
        ...
        if USE_ENHANCED_NORMALIZER:
            #  🎯 EXPERTGAIN: CARREGAR ENHANCED NORMALIZER DO DAYTRADER
            normalizer_paths = [
                f'{DIFF_MODEL_DIR}/enhanced_normalizer.pkl',  # ExpertGain primeiro
                'Otimizacao/treino_principal/models/DAYTRADER/enhanced_normalizer.pkl',  # DayTrader
                'trading_framework/training/checkpoints/DAYTRADER/enhanced_normalizer.pkl'  # Alternativo
            ]
            
            normalizer_file = None
            for path in normalizer_paths:
                if os.path.exists(path):
                    normalizer_file = path
                    print(f"🎯 EXPERTGAIN: Usando Enhanced Normalizer: {path}")
                    break
            
            if normalizer_file is None:
                self.logger.info("🆕 Criando novo Enhanced VecNormalize...")
            else:
                self.logger.info(f"🔄 Carregando Enhanced VecNormalize do DayTrader: {normalizer_file}")
    ...
    def _create_model(self, env):
        ...
        if checkpoint_path:
            ...
            try:
                ...
                # 🛑 VALIDAÇÃO CRÍTICA: Garantir TwoHeadV7Intuition após carregar checkpoint
        ...
        def lr_schedule_lstm_warmup(progress):
            ...
            base_lr = 2.0e-04  # 🔥 PPO V1 ORIGINAL: LR padrão que funcionava
        ...
        #  CONFIGURAÇÕES ESPECIALIZADAS PARA TWOHEADV7UNIFIED - POLÍTICA LIMPA E FUNCIONAL
        model_config = {
            "policy": TwoHeadV7Intuition,
            "env": env,
            "learning_rate": lr_schedule_lstm_warmup,  # 🔧 FIX: Usar lr_schedule ao invés de valor fixo
            ...
            "vf_coef": 0.8,                                # 🔥 FIX: 1.2 → 0.8 (reduzido para evitar gradient domination)
            "max_grad_norm": 5.0,                          # 🔥 CORRIGIDO: 10.0 → 5.0 (compatível com LR maior)
            "verbose": 1,             #  VERBOSE ATIVADO para debug
            "device": device_policy,
            "seed": 42,
            "use_sde": False,         #  SDE DESABILITADO PARA V7
            "policy_kwargs": get_v7_intuition_kwargs()  #  CONFIGURAÇÕES ESPECIALIZADAS V7 INTUITION
        ...
        self.logger.info(f"🎯 TwoHeadV7Unified: Duas Cabeças + Gates Especializados + Inicialização Adequada (POLÍTICA LIMPA)")
        self.logger.info(f"✅ Unified Features: Entry Head + Management Head + 8 Gates Especializados")
        self.logger.info(f"✅ Unified Philosophy: Código Limpo + Sem Herança + Funcionalidade Completa")
        self.logger.info("=" * 60)
        ...
        # 🛑 VALIDAÇÃO CRÍTICA: Garantir TwoHeadV7Intuition
        _validate_v7_policy(model.policy)
    ...
    def _comprehensive_evaluation(self, df: pd.DataFrame, is_training: bool, eval_name: str) -> Dict:
        ...
        try:
            ...
            MAX_STEPS = 2000  # 🚨 FIX CRÍTICO: Consistente com treinamento (sequências longas)
            max_episodes = 10
    ...
    def _determine_phase_from_steps(self, steps):
        """🎓 CURRICULUM LEARNING: Determinar fase baseado nos steps - CORRIGIDO PARA 10.32M TOTAL"""
        # 🔥 FIX CRÍTICO: Usar thresholds corretos baseados em PHASE_DISTRIBUTION
        # Total: 10.320.000 steps divididos em 5 fases
        # Phase_1_Fundamentals: 0 - 2.064M (20%)
        # Phase_2_Pattern_Recognition: 2.064M - 4.644M (25%)  
        # Phase_3_Noise_Handling: 4.644M - 7.224M (25%)
        # Phase_4_Advanced_Strategies: 7.224M - 9.288M (20%)
        # Phase_5_Fine_Tuning: 9.288M - 10.32M (10%)
        
        # Usar os valores de PHASE_DISTRIBUTION corretamente
        phase_thresholds = [
            PHASE_DISTRIBUTION["phase_1"],                                                    # 2.064.000
            PHASE_DISTRIBUTION["phase_1"] + PHASE_DISTRIBUTION["phase_2"],                   # 4.644.000
            PHASE_DISTRIBUTION["phase_1"] + PHASE_DISTRIBUTION["phase_2"] + PHASE_DISTRIBUTION["phase_3"],  # 7.224.000
            PHASE_DISTRIBUTION["phase_1"] + PHASE_DISTRIBUTION["phase_2"] + PHASE_DISTRIBUTION["phase_3"] + PHASE_DISTRIBUTION["phase_4"],  # 9.288.000
            TRAINING_CONFIG["total_timesteps"]                                               # 10.320.000
        ]
        ...
        for i, threshold in enumerate(phase_thresholds):
            ...
            if steps < threshold:
                print(f"   → Fase atual: {i}")
                return i
        
        # Se passou de todas as fases, está na última (mas isso não deveria acontecer)
        print(f"   → Fase atual: {len(phase_thresholds) - 1} (ÚLTIMA FASE)")
...
def main():
    """🎯 EXPERTGAIN V2 - Fine-tuning inteligente com fases especializadas"""
    
    # 🎯 EXPERTGAIN V2 HEADER
    print("\n" + "🔥" * 60)
    print("🔥 EXPERTGAIN V2 - SISTEMA INTELIGENTE DE FINE-TUNING")
    print("🎯 OBJETIVO: Entry Quality 0.488 → 0.55+ (Progressivo)")
    print("📊 FASES: Unlock (500k) → Calibrate (750k) → Optimize (750k)")
    print("🚀 FEATURES: LR Dinâmico + Reward Shaping + Early Stopping")
    print("🔥" * 60)
