# 🏗️ SAC PROFILING VERSION - PROFILING EM TEMPO REAL
import sys
import os
import codecs
import cProfile
...
from trading_framework.policies.sac_trading_transformer import SACTradingTransformerExtractor, get_sac_trading_transformer_kwargs, validate_sac_trading_transformer
...
# 🎯 GOLD TRADING CONFIGURATION - OTIMIZADO 5M STEPS (ANTI-OVERFITTING)
TRAINING_CONFIG = {
    ...
    "training_multiplier": 3.9      # Multiplicador atualizado (5M / 1.29M)
}
...
CONVERGENCE_OPTIMIZATION_CONFIG = {
    ...
    "max_grad_norm": 0.1,     # 🆘 CRÍTICO: 0.3→0.1 (gradient explosion continuando!)
...
def get_current_phase_config(current_steps: int) -> dict:
    ...
    return {
        ...
        "learning_rate": 8.0e-05,  # 🚀 V4 INNO: LR aumentado 2.7x para rewards amplificados
        "batch_size": 1024,
        "n_epochs": 10,
        "reward_weights": {
...
class ConvergenceLogger:
    ...
    def _extract_model_metrics(self, model, info_dict):
        ...
        # Skip metrics if SAC not fully initialized (optimizers None during early training)
        if hasattr(model, 'policy') and model.policy is None:
            return metrics
        ...
        try:
            ...
            if hasattr(model, 'policy'):
                ...
                elif hasattr(model.policy, 'optimizer') and model.policy.optimizer is not None:
                    # PPO style - single optimizer
                    try:
                        metrics['learning_rate'] = model.policy.optimizer.param_groups[0]['lr']
                    except (AttributeError, IndexError):
                        pass
                elif hasattr(model, 'policy') and hasattr(model.policy, 'actor_optimizer') and model.policy.actor_optimizer is not None:
                    # SAC style - optimizers in policy
                    try:
                        metrics['actor_learning_rate'] = model.policy.actor_optimizer.param_groups[0]['lr']
                        metrics['learning_rate'] = model.policy.actor_optimizer.param_groups[0]['lr']  # Use actor LR as main
                        if hasattr(model.policy, 'critic_optimizer') and model.policy.critic_optimizer is not None:
                            metrics['critic_learning_rate'] = model.policy.critic_optimizer.param_groups[0]['lr']
                    except (AttributeError, IndexError):
                        pass
                elif hasattr(model, 'actor') and hasattr(model.actor, 'optimizer') and model.actor.optimizer is not None:
                    # SAC style - direct actor/critic optimizers
                    try:
                        metrics['actor_learning_rate'] = model.actor.optimizer.param_groups[0]['lr']
                        metrics['learning_rate'] = model.actor.optimizer.param_groups[0]['lr']  # Use actor LR as main
                        if hasattr(model, 'critic') and hasattr(model.critic, 'optimizer') and model.critic.optimizer is not None:
                            metrics['critic_learning_rate'] = model.critic.optimizer.param_groups[0]['lr']
                    except (AttributeError, IndexError):
                        pass
...
class DynamicLearningRateScheduler:
    ...
    def _set_learning_rate(self, model, new_lr):
        ...
        try:
            if hasattr(model, 'policy') and hasattr(model.policy, 'optimizer') and model.policy.optimizer is not None:
                for param_group in model.policy.optimizer.param_groups:
                    param_group['lr'] = new_lr
            elif hasattr(model, 'optimizer') and model.optimizer is not None:
                for param_group in model.optimizer.param_groups:
                    param_group['lr'] = new_lr
            # SAC: policy optimizers
            elif hasattr(model, 'policy') and hasattr(model.policy, 'actor_optimizer') and model.policy.actor_optimizer is not None:
                for param_group in model.policy.actor_optimizer.param_groups:
                    param_group['lr'] = new_lr
                if hasattr(model.policy, 'critic_optimizer') and model.policy.critic_optimizer is not None:
                    for param_group in model.policy.critic_optimizer.param_groups:
                        param_group['lr'] = new_lr
            # SAC: direct optimizers
            elif hasattr(model, 'actor') and hasattr(model.actor, 'optimizer') and model.actor.optimizer is not None:
            ...
            try:
                ...
                if hasattr(model, 'policy') and hasattr(model.policy, 'optimizer') and model.policy.optimizer is not None:
                    try:
                        ...
                        pass
                # Método SAC: policy optimizers
                elif hasattr(model, 'policy') and hasattr(model.policy, 'actor_optimizer') and model.policy.actor_optimizer is not None:
                    try:
                        lr = model.policy.actor_optimizer.param_groups[0]['lr']
                        if lr > 0:
                            self.learning_rates.append(lr)
                            captured_something = True
                            lr_captured = True
...
class ProfilingCallback(BaseCallback):
    """
    🔍 Callback para PROFILING EM TEMPO REAL durante treinamento
    
    Monitora performance de CADA STEP do treinamento:
    - Tempo por step
    - Memory usage
    - Function profiling
    ...
    def __init__(self, profiler, verbose: int = 0):
        super().__init__(verbose)
    ...
    def _on_step(self) -> bool:
        ...
        # Log previous step time
        if self.step_start_time is not None:
            step_duration = time.time() - self.step_start_time
            self.profiler.log_step_time(step_duration)
        
        # Start timing current step
        self.step_start_time = time.time()
        self.step_count += 1
        
        # Generate profiling report periodically
        if self.profiler.should_report():
            self.profiler.generate_realtime_report(self.step_count)
        
        return True
    
    def _on_training_start(self) -> None:
        """Called when training starts"""
        print("🔍 PROFILING CALLBACK ATIVO - Monitorando performance em tempo real")
        self.step_start_time = time.time()
    
    def _on_training_end(self) -> None:
        """Called when training ends"""
        print("🔍 GERANDO RELATÓRIO FINAL DE PROFILING...")
        self.profiler.generate_realtime_report(self.step_count)
...
class MetricsCallback(BaseCallback):
    ...
    def _run_avaliar_v8_evaluation(self):
        """🚀 Executa avaliar_v11.py usando checkpoints existentes (não cria novos)"""
        import subprocess
        import os
        import threading
        import glob
        ...
        def run_evaluation_async():
            try:
                ...
                # Atualizar avaliar_v11.py com checkpoint existente
                ...
                print(f"✅ avaliar_v11.py atualizado com checkpoint existente")
                
                # Executar avaliar_v11.py
                print(f"🚀 Executando avaliação com checkpoint existente...")
                result = subprocess.run([
                    'python', 'avaliacao/avaliar_v11.py'
                ], 
                cwd='D:/Projeto',
                capture_output=True, 
                text=True, 
...
CONTINUATION_PARAMS = {
    "learning_rate": 4.0e-05,        # 🚀 V4 INNO: Continuation também aumentado para V4
    "critic_learning_rate": 6.0e-05,  # 🚀 V4 INNO: Critic continuation aumentado
...
BEST_PARAMS = {
    "learning_rate": 3.0e-05,                # 🔧 CLIP FIX: Reduzido de 8e-05 para estabilizar clipping
    "critic_learning_rate": 2.0e-05,        # 🔧 CLIP FIX: Reduzido de 4e-05 proporcionalmente
...
class TradingEnv(gym.Env):
    ...
    def __init__(self, df, window_size=20, is_training=True, initial_balance=None, trading_params=None):
        ...
        self.cooldown_after_trade = 7   # 🏆 OTIMIZADO: 7 steps - configuração cientificamente perfeita
        ...
        # 💰 SISTEMA V5 SHARPE: Reward system otimizado para Sharpe ratio
        self.reward_system = create_reward_system("v5_sharpe", initial_balance)
        print(f"  🎯 V5 SHARPE REWARD: Sharpe 60% + Risk 25% + Volatility 15%")
        ...
        if is_training:
            self.activity_system = create_activity_enhancement_system(
                ...
                progressive_timeout=True,   # 🎯 Sistema progressivo de timeout
                training_steps_total=TRAINING_CONFIG["total_timesteps"]  # 🎯 AUTOMÁTICO: Detecta steps do config
            )
            print(f"[ACTIVITY SYSTEM] 🎯 Activity Enhancement ativado (timeout PROGRESSIVO: 60→90→∞ candles adaptado para {TRAINING_CONFIG['total_timesteps']/1000000:.1f}M steps)")
        else:
    ...
    def _get_vectorized_temporal_features(self, seq_len):
        """
        🚀 VECTORIZAÇÃO ULTRA-RÁPIDA + CACHE: Gerar 10 barras V10Pure com cache
        
        Substitui loop por operações vectorizadas numpy + cache para performance
        """
        # 🚀 CACHE: Evitar recálculos se step não mudou
        cache_key = (self.current_step, seq_len)
        ...
        try:
            ...
            # 🚀 CACHE: Armazenar resultado para reutilização
            # Limitar cache size para evitar memory leak
            if len(self._features_cache) > 100:
                # Remove oldest entries (simple FIFO)
                oldest_keys = list(self._features_cache.keys())[:50]
                for key in oldest_keys:
                    del self._features_cache[key]
            
            self._features_cache[cache_key] = temporal_sequence
            return temporal_sequence
            
        except Exception as e:
            # Fallback seguro
            fallback = np.full((seq_len, 129), 0.3, dtype=np.float32)
            self._features_cache[cache_key] = fallback
            return fallback
    ...
    def _calculate_reward_and_info(self, action, old_state):
        ...
        if entry_decision > 0 and len(self.positions) < self.max_positions:
            # 🎯 FILTRO DE CONFIANÇA MÍNIMA (fusão quality + risk) - AJUSTADO PARA 0.6
            MIN_CONFIDENCE_THRESHOLD = 0.6  # Só entrar se confiança > 60% (MAIS PERMISSIVO)
    ...
    def _save_custom_checkpoint(self, model, path, step_count):
        ...
        try:
            ...
            try:
                ...
                # SAC: múltiplos optimizers
                elif hasattr(model, 'actor') and hasattr(model.actor, 'optimizer') and model.actor.optimizer is not None:
                    checkpoint_data['optimizer_state'] = {
                        'actor': model.actor.optimizer.state_dict(),
                        'critic': model.critic.optimizer.state_dict() if hasattr(model, 'critic') and hasattr(model.critic, 'optimizer') and model.critic.optimizer is not None else None
                    }
        ...
        try:
            ...
            if checkpoint_path_found and os.path.exists(checkpoint_path_found):
                ...
                try:
                    ...
                    # 🛑 VALIDAÇÃO SAC: Verificar modelo SAC após resume
                    self.logger.info(f"✅ SAC model resumed with {self.current_model.policy.__class__.__name__}")
                                    ...
                                    if path_name == "Framework":
                                        try:
                                            ...
                                            if path.endswith('.zip'):
                                                ...
                                                test_model = SAC.load(path, env=None)
                    ...
                    try:
                        ...
                        try:
                            ...
                            # SAC: múltiplos optimizers
                                ...
                                print(f"   📝 SAC optimizer states saved")
            ...
            for phase_idx in range(self.current_phase_idx, total_phases):
                ...
                # 🔍 PROFILING CALLBACK - NOVO
                profiling_callback = ProfilingCallback(PROFILER, verbose=1)
                
                # Lista base de callbacks
                base_callbacks = [
                    # 🔍 PROFILING EM TEMPO REAL - PRIMEIRO CALLBACK!
                    profiling_callback,     # 🔥 NOVO: Profiling completo em tempo real
    ...
    def _create_model(self, env):
        ...
        if checkpoint_path:
            ...
            try:
                ...
                model = SAC.load(checkpoint_path, env=env, device=device_policy)
                
                # 🛑 VALIDAÇÃO: SAC model loaded successfully
                self.logger.info(f"✅ SAC model loaded: {type(model.policy).__name__}")
        ...
        # 🔥 SAC v2 CONFIGURAÇÕES - MLP POLICY WITH SAC TRANSFORMER FEATURES
        model_config = {
            "policy": "MlpPolicy",                         # 🔥 Use standard MlpPolicy 
            "env": env,
            "learning_rate": 1e-5,                         # 🆘 CRÍTICO: 3e-5→1e-5 (explosion continuando!)
            "buffer_size": 1_000_000,                      # 🔥 Experience replay buffer
            "batch_size": 512,                             # 🚀 OPTIMIZED: Larger batch for better GPU utilization
            "tau": 0.01,                                   # 🚀 FIXED: Faster target update for stability
            "gamma": 0.99,                                 # 🎯 Discount factor
            "train_freq": 4,                               # 🚀 FIXED: Less frequent training for stability
            "gradient_steps": 2,                           # 🚀 FIXED: More gradient steps per train call
            "ent_coef": 'auto',                            # 🔥 AUTO-TUNING entropy coefficient  
            "target_update_interval": 1,                   # 🎯 Target network update frequency
            "verbose": 1,                                  # 🔍 Debug logs
            "device": device_policy,
            "seed": 42,
            "policy_kwargs": {                             # 🔥 SAC Transformer Features Extractor
        ...
        #  CONFIRMAÇÕES DE CONFIGURAÇÃO SAC
        self.logger.info("=" * 60)
        self.logger.info("🔥 CONFIGURAÇÕES DO SAC v2:")
        self.logger.info("=" * 60)
        self.logger.info(f"🧠 Policy: {model_config['policy']} (MLP Neural Networks)")
        self.logger.info(f"📊 Algorithm: SAC v2 (Soft Actor-Critic with Auto-tuning)")
        self.logger.info(f"🎯 Learning Rate: {model_config['learning_rate']}")
        self.logger.info(f"💾 Buffer Size: {model_config['buffer_size']:,} transitions")
        self.logger.info(f"📈 Batch Size: {model_config['batch_size']}")
        self.logger.info(f"🔧 Tau (Target Update): {model_config['tau']}")
        self.logger.info(f"🎲 Entropy Coef: {model_config['ent_coef']} (auto-tuning)")
        self.logger.info(f"⚡ Train Frequency: Every {model_config['train_freq']} step(s)")
        self.logger.info(f"⚡ Device: {model_config['device']}")
        self.logger.info(f"🔥 SAC Features: Off-Policy + Experience Replay + Twin Critics + Auto Entropy")
        self.logger.info(f"✅ SAC Philosophy: Maximum Entropy RL + Sample Efficiency + Stability")
        self.logger.info("=" * 60)
        
        # 🔍 DEBUG: Check environment spaces before creating SAC model
        self.logger.info(f"🔍 Environment observation_space: {env.observation_space}")
        self.logger.info(f"🔍 Environment action_space: {env.action_space}")
        
        print(f"🎮 Creating SAC model on device: {device_policy}")
        model = SAC(**model_config)
        
        # 🚀 FORCE GPU UTILIZATION: Move all model components to GPU explicitly
        if device_policy == "cuda" and torch.cuda.is_available():
            print("🚀 FORCING ALL MODEL COMPONENTS TO GPU...")
            
            # Force policy to GPU
            model.policy = model.policy.to(device_policy)
            print("  ✅ Policy moved to GPU")
            
            # Force actor/critic networks to GPU if they exist
            if hasattr(model.policy, 'actor'):
                model.policy.actor = model.policy.actor.to(device_policy)
                print("  ✅ Actor moved to GPU")
            if hasattr(model.policy, 'critic'):
                model.policy.critic = model.policy.critic.to(device_policy)
                print("  ✅ Critic moved to GPU")
            if hasattr(model.policy, 'critic_target'):
                model.policy.critic_target = model.policy.critic_target.to(device_policy)
                print("  ✅ Critic target moved to GPU")
            
            # Verificar utilização de GPU após mover modelo
            gpu_memory_after = torch.cuda.memory_allocated() / 1024 / 1024
            print(f"🎮 GPU Memory after model creation: {gpu_memory_after:.1f}MB")
            
            # Force a forward pass to ensure GPU utilization
            print("🚀 WARMING UP GPU WITH DUMMY FORWARD PASS...")
            with torch.no_grad():
                dummy_obs = torch.randn(1, 450, device=device_policy)
                if model.policy.features_extractor is not None:
                    _ = model.policy.features_extractor(dummy_obs)
                else:
                    print("⚠️ WARNING: features_extractor is None, skipping warmup")
                final_gpu_memory = torch.cuda.memory_allocated() / 1024 / 1024
                print(f"🎮 GPU Memory after warmup: {final_gpu_memory:.1f}MB")
        
        # 🔥 CRITICAL: SAC Transformer Policy already has optimized initialization
        self.logger.info("✅ SAC Transformer Policy includes automatic zero-gradient fixes")
        ...
        if CONVERGENCE_OPTIMIZATION_AVAILABLE and CONVERGENCE_OPTIMIZATION_CONFIG["enabled"]:
            ...
            # SAC: múltiplos optimizers
                ...
                if hasattr(model, 'critic') and hasattr(model.critic, 'optimizer') and model.critic.optimizer is not None:
                    for param_group in model.critic.optimizer.param_groups:
                        param_group['lr'] = CONVERGENCE_OPTIMIZATION_CONFIG['base_lr']
                print(f"📈 SAC Learning Rates inicial: {CONVERGENCE_OPTIMIZATION_CONFIG['base_lr']:.2e}")
        
        # 🚀 SAC USES MLP: No LSTM initialization needed
        self.logger.info("✅ SAC v2 uses MLP networks - no LSTM initialization required")
        
        # 🛑 VALIDAÇÃO SAC: Verificar MlpPolicy com SAC Transformer Features  
        self.logger.info(f"✅ SAC v2 with MlpPolicy created: {model.policy.__class__.__name__}")
        if hasattr(model.policy, 'features_extractor') and model.policy.features_extractor is not None:
            self.logger.info(f"✅ Features Extractor: {model.policy.features_extractor.__class__.__name__}")
        else:
            self.logger.warning("⚠️ No features extractor found in policy")
...
class RealTimeProfiler:
    """
    🔥 PROFILER EM TEMPO REAL COMPLETO
    
    Monitora TUDO que pode afetar performance:
    - CPU usage por função
    ...
    def __init__(self):
        ...
        print("🔍 PROFILER EM TEMPO REAL INICIADO!")
    ...
    def log_step_time(self, step_duration):
        """Log time taken for a training step"""
        self.step_times.append(step_duration)
    
    def should_report(self):
        """Check if it's time to generate a report"""
        return time.time() - self.last_report > 30  # Report every 30s
    
    def generate_realtime_report(self, step_count):
        """Generate comprehensive real-time performance report"""
        current_time = time.time()
        
        print("\n" + "📊" * 80)
        print(f"📊 PROFILING REPORT - Step {step_count} - {datetime.now().strftime('%H:%M:%S')}")
        print("📊" * 80)
        
        # Training speed analysis
        if len(self.step_times) > 10:
            recent_steps = list(self.step_times)[-100:]  # Last 100 steps
            avg_step_time = np.mean(recent_steps)
            steps_per_sec = 1.0 / avg_step_time if avg_step_time > 0 else 0
            
            print(f"🚀 VELOCIDADE DE TREINAMENTO:")
            print(f"   Average step time: {avg_step_time:.4f}s")
            print(f"   Steps per second: {steps_per_sec:.2f}")
            print(f"   Estimated steps/hour: {int(steps_per_sec * 3600)}")
            
            # Identify bottlenecks
            slowest_steps = sorted(recent_steps, reverse=True)[:5]
            fastest_steps = sorted(recent_steps)[:5]
            print(f"   Slowest 5 steps: {[f'{t:.4f}s' for t in slowest_steps]}")
            print(f"   Fastest 5 steps: {[f'{t:.4f}s' for t in fastest_steps]}")
        
        # System resource analysis
        if len(self.cpu_usage) > 10:
            recent_cpu = list(self.cpu_usage)[-300:]  # Last 30 seconds
            avg_cpu = np.mean(recent_cpu)
            max_cpu = np.max(recent_cpu)
            
            print(f"\n💻 RECURSOS DO SISTEMA:")
            print(f"   CPU Usage: {avg_cpu:.1f}% (avg), {max_cpu:.1f}% (peak)")
            
            recent_memory = list(self.memory_usage)[-300:]
        ...
        try:
            ...
            print(f"   Python Memory: {current / 1024 / 1024:.1f}MB current, {peak / 1024 / 1024:.1f}MB peak")
        except:
            pass
        
        # Function profiling (top bottlenecks)
        if self.is_profiling:
            self.stop_profiling()
        ...
        steps_per_hour = step_count / total_hours if total_hours > 0 else 0
        
        print(f"\n⏱️ EFICIÊNCIA GERAL:")
        print(f"   Total runtime: {total_hours:.2f} hours")
        print(f"   Average steps/hour: {steps_per_hour:.0f}")
        ...
        try:
            ...
            if torch.cuda.is_available():
                ...
                print(f"\n🎮 GPU USAGE:")
                print(f"   GPU Memory: {gpu_memory:.1f}MB allocated, {gpu_memory_cached:.1f}MB cached")
        except:
            pass
...
def main():
    """Main function com sistema de treinamento e PROFILING EM TEMPO REAL"""
    global PROFILER
    
    # 🔍 INICIALIZAR PROFILER EM TEMPO REAL
    ...
    PROFILER.log_step_time(time.time() - step_start_time)
    ...
    try:
        ...
        # 🔍 CLEANUP PROFILER
        if PROFILER:
            print("\n🔍 FINALIZANDO PROFILER...")
            PROFILER.cleanup()

if __name__ == "__main__":
    # 🔍 SACPROFILING - PROFILING EM TEMPO REAL COMPLETO
    main()
