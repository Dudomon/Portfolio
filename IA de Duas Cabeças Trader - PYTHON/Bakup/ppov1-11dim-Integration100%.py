...
def create_enhanced_normalizer_wrapper(env, obs_size=None, normalizer_file=None):
    ...
    #  CORREÇÃO CRÍTICA: Detectar tamanho real da observação se não fornecido
    if obs_size is None:
        if hasattr(env, 'observation_space'):
            obs_size = env.observation_space.shape[0]
        else:
            obs_size = 1320  # Fallback baseado no cálculo real corrigido
    ...
    enhanced_env = create_enhanced_normalizer(
        ...
        obs_size=obs_size,
        training=True,
        norm_obs=True,   # OK ATIVADO - Enhanced Normalizer principal
        norm_reward=True,  # OK ATIVADO - Enhanced Normalizer principal 
        clip_obs=2.0,      #  OTIMIZADO: Ideal para dados financeiros (papers recomendam 2-3σ)
        clip_reward=5.0,   #  OTIMIZADO: Baixo clipping melhora estabilidade (research)
        ...
        warmup_steps=2000, #  OTIMIZADO: Calibração mais robusta (papers: 1000-5000)
...
BEST_PARAMS = {
    "learning_rate": 2.678385767462569e-05,  #  ALINHADO COM MEU2.PY: Learning rate que converge
    ...
    "ent_coef": 0.01709320402078782,         #  ALINHADO COM MEU2.PY: Entropy que converge
    "vf_coef": 0.6017559963200034,           #  ALINHADO COM MEU2.PY: VF coefficient que converge
...
class TradingEnv(gym.Env):
    MAX_STEPS = 3000  #  CORREÇÃO CRÍTICA: 3000 steps por episódio para melhor convergência
    ...
    def __init__(self, df, window_size=20, is_training=True, initial_balance=500, trading_params=None):
        ...
        # 🧠 V5 ENHANCEMENT: Adicionar 12 componentes inteligentes
    ...
    def step(self, action):
        ...
        if self.episode_steps >= 3000:  # Aumentado para 3000 steps para melhor convergência
            done = True
...
class AdvancedTrainingSystem:
    ...
    def _create_phase_environment(self, df: pd.DataFrame, phase: TrainingPhase):
        ...
        if USE_ENHANCED_NORMALIZER:
            ...
            env = create_enhanced_normalizer_wrapper(env, obs_size=None, normalizer_file=normalizer_file)
    ...
    def _create_model(self, env):
        ...
        #  CONFIGURAÇÕES ESPECIALIZADAS PARA TWOHEADV5
        model_config = {
            "policy": TwoHeadV5Intelligent48h,
            "env": env,
            "learning_rate": 2.5e-4,  #  LR OTIMIZADO PARA TWOHEADV5
            "n_steps": 2048,          #  STEPS OTIMIZADO PARA TWOHEADV5
            "batch_size": 128,        #  BATCH OTIMIZADO PARA TWOHEADV5
            "n_epochs": 4,            #  ÉPOCAS OTIMIZADO PARA TWOHEADV5
            "gamma": 0.99,            #  GAMMA PADRÃO
            "gae_lambda": 0.95,       #  GAE PADRÃO
            "clip_range": 0.2,        #  CLIP OTIMIZADO PARA TWOHEADV5
            "ent_coef": 0.01,         #  ENTROPY OTIMIZADO PARA TWOHEADV5
            "vf_coef": 0.5,           #  VF OTIMIZADO PARA TWOHEADV5
        ...
        self.logger.info(f" TwoHeadV5Intelligent48h: 2-LSTM + 1-GRU + 8-Head Attention")
        ...
        self.logger.info(f" Melhorias V5: Entry Head Ultra-Especializada, 6 Gates, 10 Scores, Market Fatigue, Quality Filters")
        self.logger.info("=" * 60)
