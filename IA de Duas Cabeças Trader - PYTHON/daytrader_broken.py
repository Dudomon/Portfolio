...
#  SISTEMA DE REWARDS V3 BRUTAL - FOCADO 100% EM FAZER DINHEIRO
...
BEST_PARAMS = {
    "learning_rate": 3.5e-05,                # 🔥 FIX STAGNATION: Aumento 2e-05→3.5e-05 para sair da estagnação
    "critic_learning_rate": 2.0e-05,        # 🔧 FIX POST-SIGMOID: Reduzir LR para bounded output estável
    ...
    "ent_coef": 0.08,                        # 🔥 FIX ENTROPY COLLAPSE: Aumento 0.02→0.08 para restaurar exploração
    "vf_coef": 1.0,                          # 🔥 FIX EXPLAINED VARIANCE: Aumentar peso do critic 0.5→1.0
    "max_grad_norm": 0.05,                   # 🔧 FIX POST-SIGMOID: Clipping muito conservador para bounded output
...
class TradingEnv(gym.Env):
    ...
    def __init__(self, df, window_size=20, is_training=True, initial_balance=None, trading_params=None):
        ...
        # [2] sl_adjust_global: [-3,3] SL global para todas posições
        # [3] tp_adjust_global: [-3,3] TP global para todas posições
        ...
        # 🎯 SISTEMA V3 BRUTAL: Reward system focado 100% em fazer dinheiro
    ...
    def _calculate_reward_and_info(self, action, old_state):
        ...
        if len(action) >= 8:
            ...
            # [2-7] SL/TP system: controle específico para cada posição
        ...
        if entry_decision > 0 and len(self.positions) < self.max_positions:
            ...
            if entry_confidence < MIN_CONFIDENCE_THRESHOLD:
                ...
                # 🔧 SISTEMA SL/TP 8D: Controle específico para cada uma das 3 posições
                pos_index = len(self.positions)  # Índice da nova posição
                
                # Escolher SL/TP baseado na posição
                if pos_index == 0:  # Primeira posição
                    sl_adjust = sl_pos1
                    tp_adjust = tp_pos1
                elif pos_index == 1:  # Segunda posição  
                    sl_adjust = sl_pos2
                    tp_adjust = tp_pos2
                else:  # Terceira posição (pos_index == 2)
                    sl_adjust = sl_pos3
                    tp_adjust = tp_pos3
    ...
    def _force_close_positions_by_timeout(self):
        ...
        for pos in positions_to_close:
            try:
                # Calcular PnL final
                pnl = self._get_position_pnl(pos, close_price)
