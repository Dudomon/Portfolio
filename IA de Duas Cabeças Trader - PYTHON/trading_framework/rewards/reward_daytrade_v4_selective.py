...
📊 DISTRIBUIÇÃO DO REWARD (REBALANCEADA):
...
- Activity = ~2-4% (mantido para quebrar inércia)
"""
...
class SelectiveTradingReward:
    ...
    def __init__(self, initial_balance: float = 1000.0):
        ...
        # 🔥 CONFIGURAÇÕES V4-SELECTIVE RIGOROSAS
        self.pain_multiplier = 2.5  # 🔧 AUMENTADO: 1.5 → 2.5 (punição mais severa)
        self.risk_penalty_threshold = 0.08  # 🔧 REDUZIDO: 0.15 → 0.08 (corta perdas mais cedo)
        self.max_reward = 2.0
        
        # 🎯 NOVOS PARÂMETROS ANTI-OVERTRADING (SUAVIZADOS)
        self.max_position_time_ratio = 0.65  # 🔧 SUAVIZADO: 50% → 65% do tempo posicionado
        self.optimal_position_time_ratio = 0.30  # 🔧 SUAVIZADO: 25% → 30% do tempo
        self.min_position_time_ratio = 0.08  # 🔧 SUAVIZADO: 5% → 8% do tempo (evitar inatividade total)
    ...
    def calculate_reward_and_info(self, env, action: np.ndarray, old_state: Dict) -> Tuple[float, Dict, bool]:
        ...
        # 6. REWARD FINAL V4-SELECTIVE REBALANCEADO (correção crítica)
        pure_pnl_component = pnl_reward * 0.70          # 🔧 AUMENTADO: 60% → 70% (fortalecer PnL signal)
        shaping_component = shaping_reward * 0.08       # 🔧 REDUZIDO: 15% → 8% (dar espaço para shaping ser mais ativo)
        selectivity_component = selectivity_reward * 0.08  # 🔧 CRÍTICO: 15% → 8% (dividir por 2 - estava dominando)
        quality_component = quality_reward * 0.07       # 🔧 REDUZIDO: 10% → 7% (ainda importante mas balanceado)
    ...
    def _calculate_selectivity_reward(self, env, action: np.ndarray, has_position: bool, is_in_cooldown: bool) -> Tuple[float, Dict]:
        ...
        try:
            ...
            if not is_in_cooldown:
                # 1. PENALIZAÇÃO POR OVERTRADING SUAVIZADA (só quando não está em cooldown)
                if position_time_ratio > self.max_position_time_ratio:
                    # Penalização progressiva SUAVIZADA por estar muito tempo posicionado
                    excess = position_time_ratio - self.max_position_time_ratio
                    penalty = -0.3 * excess * 1.0  # 🔧 SUAVIZADO: -1.0*2.0 → -0.3*1.0 (6x mais leve)
                    reward += penalty
                    info['overtrading_penalty'] = penalty
                    self.overtrading_penalties_applied += 1
                    
                # 2. REWARD POR RATIO ÓTIMO (SUAVIZADO)
                elif self.optimal_position_time_ratio * 0.7 <= position_time_ratio <= self.optimal_position_time_ratio * 1.3:
                    # Bonus SUAVIZADO por estar no range ótimo (21-39% do tempo)
                    bonus = 0.15  # 🔧 SUAVIZADO: 0.3 → 0.15 (menos impactante)
                    reward += bonus
            ...
            if not has_position:
                ...
                if volatility < self.min_volatility_for_entry or trend_strength < self.min_trend_strength:
                    ...
                    if self.patience_counter > 15:  # 🔧 SUAVIZADO: 10 → 15 steps de paciência
                        patience_reward = 0.05  # 🔧 SUAVIZADO: 0.1 → 0.05 (menos impactante)
            ...
            if has_position:
                ...
                for pos in positions:
                    ...
                    # 🔥 INCENTIVO FORTE para corte rápido de perdas
                    if unrealized_pnl < -0.005 * self.initial_balance and position_age < 30:  # 🔧 Threshold: -1% → -0.5%, tempo: 20 → 30
                        if action[0] < 0.33:  # Ação de fechar (HOLD/CLOSE)
                            cut_loss_bonus = 0.25  # 🔧 AUMENTADO: 0.05 → 0.25 (5x mais incentivo)
        ...
        try:
            ...
            if opened_new_position and entry_decision > 0:  # Ação de entrada (LONG ou SHORT)
                ...
                if self.min_volatility_for_entry <= volatility <= self.min_volatility_for_entry * 10:
                    quality_score += 0.3  # 🔧 FORTALECIDO: 0.2 → 0.3 (50% maior)
                    info['good_volatility'] = True
                    market_good = True
                elif volatility > self.min_volatility_for_entry * 10:
                    quality_score -= 0.15  # 🔧 FORTALECIDO: -0.1 → -0.15 (penalidade mais clara)
                    info['excessive_volatility'] = True
                else:
                    quality_score -= 0.25  # 🔧 FORTALECIDO: -0.2 → -0.25 (penalidade mais clara)
                    info['low_volatility'] = True
                
                # 2. Tendência clara - FORTALECIDO
                if trend_strength > self.min_trend_strength:
                    quality_score += 0.25  # 🔧 FORTALECIDO: 0.2 → 0.25 (25% maior)
                    info['clear_trend'] = True
                    market_good = market_good and True
                else:
                    quality_score -= 0.12  # 🔧 FORTALECIDO: -0.1 → -0.12 (penalidade mais clara)
                ...
                # 🔥 TIMING RIGOROSO baseado em perdas consecutivas
                if self.consecutive_losses > 1:  # 🔧 MUDANÇA: 2 → 1 (penaliza mais cedo)
                    # Penalização crescente por revenge trading
                    revenge_penalty = -0.2 * self.consecutive_losses  # 🔧 ESCALANTE: -0.2, -0.4, -0.6...
                ...
                reward = quality_score * 0.8  # 🔧 FORTALECIDO: 0.5 → 0.8 (60% maior scaling)
            ...
            for idx, pos in enumerate(positions[:2]):  # Máximo 2 posições
                ...
                if pnl_percent < -0.01:  # Perdendo >1%
                    if mgmt_value < -0.5:  # Foco em SL (proteção)
                        ...
                        protection_bonus = 0.12  # 🔧 FORTALECIDO: 0.05 → 0.12 (mais que dobrou)
                        ...
                        greed_penalty = -0.08  # 🔧 FORTALECIDO: -0.05 → -0.08 (penalidade mais clara)
                ...
                # 2. GESTÃO DE POSIÇÃO VENCEDORA
                    ...
                    if mgmt_value > 0.3:  # Foco em TP
                        ...
                        runner_bonus = 0.08  # 🔧 FORTALECIDO: 0.03 → 0.08 (quase 3x)
                        reward += runner_bonus
                        info[f'pos{idx}_runner'] = True
                    elif mgmt_value < -0.7:  # Muito foco em SL
                        # QUESTIONÁVEL: Muito conservador com lucro
                        conservative_penalty = -0.04  # 🔧 FORTALECIDO: -0.02 → -0.04 (sinal mais claro)
                ...
                if position_age > 100:  # Posição antiga
                    if abs(mgmt_value) > 0.5:  # Ação decisiva
                        ...
                        decisive_bonus = 0.06  # 🔧 FORTALECIDO: 0.02 → 0.06 (3x mais forte)
    ...
    def _calculate_pure_pnl_reward(self, env) -> Tuple[float, Dict]:
        ...
        try:
            ...
            # 🔥 PAIN MULTIPLICATION RIGOROSA - Ativa mais cedo
            if pnl_percent_clipped < -0.01:  # 🔧 MUDANÇA: -3% → -1% (ativa mais cedo)
                pain_factor = 1.0 + (self.pain_multiplier - 1.0) * np.tanh(abs(pnl_percent_clipped) * 30)  # 🔧 INTENSIDADE: 20 → 30
    ...
    def _calculate_activity_bonus(self, env, action: np.ndarray) -> Tuple[float, Dict]:
        ...
        try:
            ...
            # 1. BONUS BASE POR QUALQUER AÇÃO DECISIVA (DOBRADO)
            if action_magnitude > 0.5:
                decisive_bonus = (0.02 / 3) * 2  # 🔧 DOBRADO: volta para 2/3 do original
                reward += decisive_bonus
                info['decisive_action_bonus'] = decisive_bonus
            elif action_magnitude > 0.2:
                moderate_bonus = (0.01 / 3) * 2  # 🔧 DOBRADO: volta para 2/3 do original
                reward += moderate_bonus  
                info['moderate_action_bonus'] = moderate_bonus
            
            # 2. BONUS POR ENTRADA (quebra inércia de HOLD) - DOBRADO
            if entry_decision > 0:  # LONG ou SHORT
                entry_bonus = ((0.03 / 3) * 2) * entry_confidence  # 🔧 DOBRADO: volta para 2/3 do original
                reward += entry_bonus
            ...
            if len(action) > 2:
                ...
                if mgmt_magnitude > 0.3:
                    mgmt_bonus = (0.015 / 3) * 2  # 🔧 DOBRADO: volta para 2/3 do original
                    reward += mgmt_bonus
                    info['management_action_bonus'] = mgmt_bonus
            
            # 4. ANTI-INÉRCIA: Bonus por quebrar período de inatividade - DOBRADO
            if hasattr(self, 'last_significant_action_step'):
                ...
                if steps_since_action > 20 and action_magnitude > 0.3:
                    inertia_break_bonus = min((0.05 / 3) * 2, (steps_since_action * 0.001 / 3) * 2)  # 🔧 DOBRADO: volta para 2/3 do original
    ...
    def _calculate_intelligent_shaping(self, env, action: np.ndarray) -> Tuple[float, Dict]:
        ...
        try:
            ...
            if abs(progress) > 0.001:
                progress_reward = progress * 0.2 * 25.0  # 🔧 FORTALECIDO: 0.1*20 → 0.2*25 (2.5x mais forte)
            ...
            if len(self.recent_pnl_trend) >= 3:
                ...
                if momentum > 0.01:
                    momentum_reward = 0.08 * momentum * 30.0  # 🔧 FORTALECIDO: 0.05*25 → 0.08*30 (quase 2x)
                    shaping_reward += momentum_reward
                elif momentum < -0.01:
                    momentum_reward = 0.08 * momentum * 15.0  # 🔧 FORTALECIDO: 0.05*10 → 0.08*15 (2.4x)
                    shaping_reward += momentum_reward
                else:
                    momentum_reward = 0.0
