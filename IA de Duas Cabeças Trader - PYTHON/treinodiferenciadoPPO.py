...
def load_optimized_data():
    ...
    yahoo_cache = "data_cache/GC=F_YAHOO_DAILY_CACHE_20250704_142845.pkl"
...
def create_enhanced_normalizer_wrapper(env, obs_size=None, normalizer_file=None):
    ...
    enhanced_env = create_enhanced_normalizer(
        ...
        obs_size=obs_size,
        training=True,
        norm_obs=True,   # ✅ ATIVADO - Enhanced Normalizer principal
        norm_reward=True,  # ✅ ATIVADO - Enhanced Normalizer principal 
    ...
    try:
        ...
        if hasattr(enhanced_env, 'normalizer'):
            ...
            if hasattr(normalizer, 'save'):
                ...
                print(f"⚠️ Enhanced Normalizer não tem método save(): {filepath}")
                return False
            ...
            print(f"⚠️ Ambiente não tem Enhanced Normalizer - criando compatibilidade: {filepath}")
            ...
            if success:
                print(f"✅ Enhanced Normalizer de compatibilidade salvo: {filepath}")
                return True
            else:
...
def monitor_enhanced_normalizer_health(enhanced_env, obs):
    ...
    try:
        ...
        if near_zero > 0.7:
            print(f"⚠️ ALERTA Enhanced Normalizer: {near_zero*100:.1f}% das observações próximas de zero!")
            print(f"   📊 Mean: {obs_mean:.4f}, Std: {obs_std:.4f}, Range: [{obs_min:.4f}, {obs_max:.4f}]")
            return False
        
        if extreme_values > 0.15:
            print(f"⚠️ ALERTA Enhanced Normalizer: {extreme_values*100:.1f}% valores extremos!")
            return False
...
class TradingEnv(gym.Env):
    ...
    def __init__(self, df, window_size=20, is_training=True, initial_balance=500, trading_params=None):
        ...
        # 🎯 ACTION SPACE SIMPLIFICADO - ACTION HEAD + MANAGER HEAD (7 dimensões)
        # Estrutura otimizada para gestão de até 3 posições simultâneas
        # 
        ...
        #     - Decisão de entrada de nova posição
        #     - 0: Não fazer nada (hold)
        #     - 1: Abrir posição long (compra)
        #     - 2: Abrir posição short (venda)
        # 
        # MANAGER HEAD (6 dimensões - SL/TP para 3 posições):
        ...
        # 
        # CONVERSÃO DE PONTOS:
        # - Valores [-3,3] são multiplicados por fator de conversão
        # - Para OURO: 1 ponto = $1.00 diferença de preço
        # - Exemplo: action[1] = 2.5 → SL = entry_price ± (2.5 * multiplicador)
        # 
        # GESTÃO DE POSIÇÕES:
        # - Se posição N não existe, sl_posN e tp_posN são ignorados
        # - Se posição N existe, sl_posN e tp_posN ajustam seus níveis
        # - Position sizing é calculado automaticamente via _calculate_adaptive_position_size()
        # 
        # BENEFÍCIOS DA NOVA ESTRUTURA:
        # - 30% menos dimensões (10→7)
        # - Foco claro: Action entra, Manager gere
        # - Elimina redundância das táticas individuais
        # - Mantém controle total sobre SL/TP de cada posição
    ...
    def _calculate_missing_features(self, missing_features):
        ...
        print(f"[HIGH QUALITY] ✅ Features calculadas em {calc_time:.3f}s: {len(features_to_calc)} features")
    ...
    def _calculate_reward_and_info(self, action, old_state):
        ...
        if len(action) >= 7:
            # ACTION HEAD - Decisão de entrada
            entry_decision = int(action[0])  # 0=hold, 1=long, 2=short
            
            # MANAGER HEAD - SL/TP para as 3 posições
            ...
            if entry_decision > 0 and len(self.positions) < self.max_positions:
                # Calcular tamanho da posição (sem confidence, usando padrão)
                lot_size = self._calculate_adaptive_position_size(1.0)
...
class AdvancedTrainingSystem:
    ...
    def _create_training_phases(self) -> List[TrainingPhase]:
        """🚀 FASES OTIMIZADAS PARA DATASET MASSIVO YAHOO (1.1M BARRAS, 15+ ANOS) - EXATAMENTE 2X BARRAS"""
        return [
            TrainingPhase(
                name="Phase_1_Fundamentals",
                phase_type=PhaseType.FUNDAMENTALS,
                timesteps=440000,  # 🚀 EXATO 2X: ~0.40 passos/barra (440k/1.1M)
            ...
            TrainingPhase(
                ...
                timesteps=550000,  # 🚀 EXATO 2X: ~0.50 passos/barra (550k/1.1M)
            ...
            TrainingPhase(
                ...
                timesteps=220000,  # 🚀 EXATO 2X: ~0.20 passos/barra (220k/1.1M)
    ...
    def train(self):
        ...
        try:
            ...
            if checkpoint_path_found and os.path.exists(checkpoint_path_found):
                ...
                print("✅ Sistema de estados do ambiente DESABILITADO - evitando métricas congeladas")
                ...
                def _on_step(self) -> bool:
                    ...
                    if real_timesteps % self.save_freq == 0:
                        try:
                            ...
                            print("✅ Salvamento de estados do ambiente DESABILITADO - evitando métricas congeladas")
                            ...
                            try:
                                ...
                                for normalizer_path in normalizer_paths:
                                    try:
                                        ...
                                        print(f"✅ Enhanced Normalizer pronto para produção salvo: {normalizer_path}")
                        ...
                        for final_normalizer_path in final_normalizer_paths:
                            try:
                                ...
                                if hasattr(env, 'save'):
                                    ...
                                    print(f"✅ Enhanced Normalizer FINAL salvo: {final_normalizer_path}")
    ...
    def _create_phase_environment(self, df: pd.DataFrame, phase: TrainingPhase):
        ...
        if USE_ENHANCED_NORMALIZER:
            ...
            self.logger.info(f"✅ Normalização de Rewards: {norm_reward}")
            ...
            self.logger.info("⚠️ Enhanced Normalizer DESABILITADO")
    ...
    def _determine_phase_from_steps(self, steps):
        """🚀 ATUALIZADO: Determinar índice da fase baseado no dataset massivo (2.2M total) - EXATAMENTE 2X BARRAS"""
        # Fases atualizadas para dataset massivo: 440k, 550k, 550k, 440k, 220k (total acumulado = 2.2M)
        phase_thresholds = [
            440000,   # Fase 1: 0 - 440k (~0.40 passos/barra)
            990000,   # Fase 2: 440k - 990k (~0.50 passos/barra)
            1540000,  # Fase 3: 990k - 1.54M (~0.50 passos/barra)
            1980000,  # Fase 4: 1.54M - 1.98M (~0.40 passos/barra)
            2200000   # Fase 5: 1.98M - 2.2M (~0.20 passos/barra)
        ]
