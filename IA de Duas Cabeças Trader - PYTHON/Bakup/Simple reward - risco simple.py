"""
💰 SISTEMA DE RECOMPENSAS SIMPLES E MATEMATICAMENTE COERENTE - V6 ULTRA-BALANCEADO
...
🎯 PRINCÍPIOS V6:
1. PnL real dominante (90% do peso)
2. Win bonus ultra-reduzido (+1.5 vs +3.0 anterior)
3. SL/TP rewards reduzidos (+1.0 vs +2.0 anterior)
4. Controles de risco suavizados (drawdown, overtrading)
5. Zona de atividade ultra-expandida (12-24 vs 14-22 anterior)

🔥 MELHORIAS V6 - FOCO EM CONVERGÊNCIA:
- Win bonus drasticamente reduzido para evitar overfitting
- SL/TP rewards reduzidos para estabilidade
- Penalidade por drawdown suavizada (-0.5 por 5%)
- Penalidade por overtrading suavizada (-0.2 por >30 trades/dia)
- Zona alvo ultra-flexível (12-24 trades/dia)
- Sistema de qualidade mantido mas reduzido
- Todos os bônus reduzidos para estabilidade

🚨 CORREÇÃO CRÍTICA V6.1:
- ADICIONADOS PESOS DE TRAILING STOP QUE ESTAVAM FALTANDO
- Trailing stop execution: +0.5 (era +3.0)
- Trailing stop activation: +0.3 (era +2.0)  
- Trailing stop protection: +0.2 (era +1.5)
- Trailing stop timing: +0.1 (era +1.0)
- Missed trailing opportunity: 0.0 (desabilitado)
- CORREÇÃO DO KeyError que impedia convergência
"""
...
class SimpleRewardCalculator:
    ...
    def __init__(self, initial_balance: float = 1000.0):
        ...
        # 🔥 PESOS V6 ULTRA-BALANCEADOS - SISTEMA CONVERGENTE
        self.weights = {
            # 💰 CORE DOMINANTE - PnL direto (90% do peso total)
            "pnl_direct": 1.0,           # Base: $1 reward = $1 PnL
            "win_bonus": 1.5,            # 🔥 REDUZIDO: +$1.5 por trade vencedor (era 3.0)
            "loss_penalty": -0.5,        # 🔥 REDUZIDO: -$0.5 por trade perdedor (era -1.0)
            
            # 🎯 INCENTIVOS PARA FECHAMENTO (5% do peso total)
            "trade_completion_bonus": 1.0,  # 🔥 REDUZIDO: +1.0 por FECHAR trade (era 2.0)
            "position_management": 0.5,     # 🔥 REDUZIDO: +0.5 por gestão ativa (era 1.0)
            "target_zone_bonus": 0.5,       # 🔥 REDUZIDO: +0.5 por zona 12-24 trades/dia (era 1.0)
            
            # 🎯 QUALIDADE ESCALADA REDUZIDA
            "excellent_trade_bonus": 1.5,    # 🔥 REDUZIDO: +1.5 por trade >$15 lucro (era 3.0)
            "quality_trade_bonus": 0.5,      # 🔥 REDUZIDO: +0.5 por trade >$5 lucro (era 1.0)
            "win_streak_bonus": 0.2,         # 🔥 REDUZIDO: +0.2 por cada win consecutivo (era 0.5)
            "quick_profit_bonus": 0.5,       # 🔥 REDUZIDO: +0.5 por lucro <30 steps (era 1.0)
            
            # 🧠 EXPERT SL/TP REDUZIDO
            "perfect_sltp_ratio": 1.0,       # 🔥 REDUZIDO: +1.0 por ratio 1.5-1.8 (era 2.0)
            "target_sltp_ranges": 0.5,       # 🔥 REDUZIDO: +0.5 por usar ranges corretos (era 1.0)
            "adaptive_sltp": 0.2,            # 🔥 REDUZIDO: +0.2 por adaptação contextual (era 0.5)
            "smart_sltp_timing": 0.2,        # 🔥 REDUZIDO: +0.2 por timing inteligente (era 0.5)
            
            # 🛡️ CONTROLES DE RISCO SUAVIZADOS
            "flip_flop_penalty": 0.0,        # MANTIDO: Sem penalidade por flip-flop
            "micro_trade_penalty": 0.0,      # MANTIDO: Sem penalidade por micro-trades
            "poor_sltp_penalty": 0.0,        # MANTIDO: Sem penalidade por SL/TP ruins
            "excessive_drawdown": -0.5,      # 🔥 REDUZIDO: -$0.5 por cada 5% de drawdown (era -1.0)
            "overtrading_penalty": -0.2,     # 🔥 REDUZIDO: -$0.2 por trade >30/dia (era -0.5)
            
            # 🎯 BÔNUS ESPECIAIS REDUZIDOS
            "consistency_bonus": 0.2,        # 🔥 REDUZIDO: +0.2 por consistência (era 0.5)
            "risk_management_bonus": 0.2,    # 🔥 REDUZIDO: +0.2 por gestão de risco (era 0.5)
            
            # 🔥 CORREÇÃO CRÍTICA: ADICIONAR PESOS DE TRAILING STOP QUE ESTAVAM FALTANDO
            "trailing_stop_execution": 0.5,      # 🔥 REDUZIDO: +0.5 por trailing executado (era 3.0)
            "trailing_stop_activation": 0.3,     # 🔥 REDUZIDO: +0.3 por ativar trailing (era 2.0)
            "trailing_stop_protection": 0.2,     # 🔥 REDUZIDO: +0.2 por proteger lucros (era 1.5)
            "trailing_stop_timing": 0.1,         # 🔥 REDUZIDO: +0.1 por timing correto (era 1.0)
            "missed_trailing_opportunity": 0.0,  # 🔥 DESABILITADO: Sem penalidade por perder trailing
        }
    ...
    def _calculate_activity_reward(self, trades_today: int, entry_decision: int) -> float:
        ...
        # 🎯 ANÁLISE DA ZONA DE ATIVIDADE - ZONA ULTRA-EXPANDIDA (12-24)
        if self.target_zone_min <= trades_today <= self.target_zone_max:  # Zona alvo (12-24) - BÔNUS
    ...
    def _calculate_risk_management_reward(self, env) -> float:
        """
        🎯 GESTÃO DE RISCO E DRAWDOWN - CONTROLE MELHORADO
        Recompensa gestão inteligente de risco + penalidade por drawdown
        """
        risk_reward = 0.0
        
        try:
            # 🛡️ PENALIDADE POR DRAWDOWN EXCESSIVO - NOVO CONTROLE
            current_dd = getattr(env, 'current_drawdown', 0.0)
            if current_dd > 5.0:  # Acima de 5%
                # Penalidade progressiva: -1.0 por cada 5% de drawdown
