...
class UnifiedMarketBackbone(nn.Module):
    ...
    def __init__(self, 
        ...
        # 0. Input Projection: NOT NEEDED - extract_features já faz isso!
        # Remover projection desnecessária - transformer já converte 2580→128
    ...
    def forward(self, observations: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, int, Dict]:
        ...
        # 1. Shared feature extraction (já vem com dimensão correta do transformer)
...
class TwoHeadV7Intuition(TwoHeadV7Enhanced):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # 🔧 FIX: Usar features_dim REAL do transformer (256, não 128!)
        actual_features_dim = self.v7_features_dim  # 256 - o valor correto!
        self.unified_backbone = UnifiedMarketBackbone(
            input_dim=actual_features_dim,  # 256 from transformer
    ...
    def forward_actor(self, features: torch.Tensor, lstm_states, episode_starts: torch.Tensor):
        ...
        # 1. Extract features first (2580 → 256 via transformer)
        ...
        # [2] sl_adjust_global: -3 to 3 - SL global para todas posições
        actions[:, 2] = torch.tanh(raw_actions[:, 2]) * 3.0
        
        # [3] tp_adjust_global: -3 to 3 - TP global para todas posições  
    ...
    def forward_critic(self, features: torch.Tensor, lstm_states, episode_starts: torch.Tensor):
        ...
        regime_emb = self.unified_backbone.regime_embedding(
            torch.tensor(regime_id, device=features.device, dtype=torch.long)
        )
    ...
    def get_enhanced_memory_context(self, batch_size: int, regime_id: int) -> torch.Tensor:
        ...
        context_tensor = torch.tensor(context[:8], dtype=torch.float32, device='cuda:0')  # Fix device
