...
class MarketContextEncoder(nn.Module):
    """🌍 Market Context Encoder - ÚNICO detector de regime"""
    ...
    def forward(self, lstm_features: torch.Tensor) -> Tuple[torch.Tensor, int, Dict]:
        """
...
class DaytradeEntryHead(nn.Module):
    ...
    def __init__(self, input_dim: int = 320):  # LSTM(256) + context(64)
        ...
        self.entry_confidence_net = nn.Sequential(
            ...
            nn.LeakyReLU(negative_slope=0.01),
            nn.Linear(32, 1),
            nn.Sigmoid()  # Sigmoid for confidence [0,1]
        )
...
class TwoHeadV8Elegance(RecurrentActorCriticPolicy):
    """
    🚀 TwoHeadV8Elegance - "Simplicidade Focada no Daytrade"
    ...
    def __init__(
        ...
        **kwargs
        ...
        # 🚀 V8 ELEGANCE COMPONENTS
        
        # 1. UMA LSTM compartilhada (elegância)
        ...
        self.v8_critic = nn.Sequential(
            ...
            nn.LeakyReLU(negative_slope=0.01),
            nn.LayerNorm(128),
            nn.Dropout(0.1),  # Reduced regularization
            nn.Linear(128, 64),
            nn.LeakyReLU(negative_slope=0.01),
            nn.Dropout(0.05),  # Minimal dropout before output
            nn.Linear(64, 1)
        )
    ...
    def _initialize_v8_components(self):
        """🔧 Inicialização específica dos componentes V8"""
    ...
    def forward_actor(self, features: torch.Tensor, lstm_states, episode_starts: torch.Tensor):
        """🎯 Forward Actor V8 - LSTM compartilhada + heads específicos"""
        ...
        # 3. Processar features através da LSTM compartilhada
        # LSTM expects (batch, seq, features) and states (num_layers, batch, hidden)
        lstm_out, new_lstm_states = self.v8_shared_lstm(extracted_features, lstm_states)
        
        # 2. Market context (CACHED to avoid duplication with critic)
        current_step = self.training_step
        if current_step != self._cached_context_step:
            context_features, regime_id, context_info = self.market_context(lstm_out)
        ...
        # 3. Combinar LSTM + context para heads
        combined_features = torch.cat([lstm_out, context_features], dim=-1)
        
        # 4. Entry Head específico (entry + confidence) 
        # Squeeze para remover seq dimension se necessário
        lstm_features_2d = lstm_out.squeeze(1)  # [batch, seq, 256] → [batch, 256]
        context_features_2d = context_features.squeeze(1) if len(context_features.shape) == 3 else context_features
        
        raw_entry, entry_confidence, entry_gate_info = self.entry_head(lstm_features_2d, context_features_2d)
        
        # 5. Management Head específico (SL/TP por posição)
        pos1_mgmt, pos2_mgmt = self.management_head(lstm_features_2d, context_features_2d)
    ...
    def forward_critic(self, features: torch.Tensor, lstm_states, episode_starts: torch.Tensor):
        ...
        # 3. Processar features através da LSTM compartilhada (mesmo que actor)
        lstm_out, new_lstm_states = self.v8_shared_lstm(extracted_features, lstm_states)
        
        # 2. Market context (USE CACHED from actor if available)
        current_step = self.training_step  
        ...
        # 3. Squeeze para remover seq dimension
        lstm_features_2d = lstm_out.squeeze(1)  # [batch, seq, 256] → [batch, 256]
        context_features_2d = context_features.squeeze(1) if len(context_features.shape) == 3 else context_features
        
        # 4. Combinar para critic
        critic_input = torch.cat([lstm_features_2d, context_features_2d], dim=-1)
...
def get_v8_elegance_kwargs():
    """Retorna kwargs para TwoHeadV8Elegance"""
    return {
        # V8 Elegance parameters
        'v8_lstm_hidden': 256,
        'v8_features_dim': 256,
...
def validate_v8_elegance_policy(policy):
    """Valida TwoHeadV8Elegance"""
